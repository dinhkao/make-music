...
# =======================================================================
# SONG — YOU'RE MY DAYLIGHT (A major, 90 bpm, 64 bars = 2:51)
#
# Westlife mid-song feature: the instrumental SOLO section (My Love has
# one between bridge and final chorus; You Raise Me Up breaks for
# strings/whistle). Here the pluck synth takes a lead line over the verse
# loop. The ending is the Flying Without Wings close: the last chord held
# by a 4-voice stack, no key change.
#
# Harmony — Oasis "Wonderwall" (1995, recorded key A, capo-2 G shapes):
#   verse:   F#m7 A | Esus4 B7sus4          (2 beats per chord, the loop)
#   pre:     Dadd9 Esus4 | F#m7 | G G/F# | F#m7 G B7sus4
#            (Cadd9-Dsus4-Em7 then the G-G/F#-Em7-G-A7sus4 climb)
#   chorus:  Dadd9 F#m7 | A F#m7  (x4)      (Cadd9-Em7-G-Em7)
#   signature: the two-note pedal point (C#5+F#5) rings over every verse
#   chord — reproduced here as a sustained keys dyad in the verses.
# =======================================================================
BPM = 90.0
BEAT = 60.0 / BPM
BAR = 4.0
SECS = [('INTRO',4), ('V1',8), ('PRE1',4), ('CH1',8), ('SOLO',6),
        ('V2',6), ('PRE2',4), ('CH2',8), ('BRIDGE',4), ('CH3',8), ('CODA',4)]
S = {}; _b = 0.0
for _n, _c in SECS:
    S[_n] = _b; _b += _c * BAR
END_BEATS = _b
DUR = T(END_BEATS)
TOTAL = DUR + 3.5
print("YOU'RE MY DAYLIGHT  bars", int(END_BEATS / BAR), ' duration', round(DUR, 1), 's')

CH = {
 'F#m7': (42, (54,57,61,64), 42),   'A':    (45, (57,61,64,69), 45),
 'Esus4':(40, (52,57,59,64), 40),   'B7sus4':(47, (59,64,66,69), 47),
 'Dadd9':(38, (50,54,57,64), 38),   'G':   (43, (55,59,62,67), 43),
 'G/F#': (43, (55,59,62,67), 42),
}
SPLIT = {'F#m7A': ('F#m7','A'), 'Esus4B7': ('Esus4','B7sus4'),
         'Dadd9Esus4': ('Dadd9','Esus4'), 'GF#': ('G','G/F#'),
         'F#m7G': ('F#m7','G'), 'Dadd9F#m7': ('Dadd9','F#m7'),
         'AF#m7': ('A','F#m7'),
         'F#m7GB': [('F#m7',0),('G',2),('B7sus4',4)]}
V1BAR  = ['F#m7A','Esus4B7'] * 4
PREBAR = ['Dadd9Esus4','F#m7','GF#','F#m7GB']
CHBAR  = ['Dadd9F#m7','AF#m7'] * 4
SOLOBAR= ['F#m7A','Esus4B7'] * 3
V2BAR  = ['F#m7A','Esus4B7'] * 3
BRBAR  = ['GF#','F#m7G','B7sus4','B7sus4']
CH3BAR = ['Dadd9F#m7','AF#m7'] * 4
CODABAR= ['F#m7A','Esus4B7','F#m7','F#m7']

BASSL = {
 'F#m7': [(0,42,1.7,.32),(1.7,49,1.2,.24)],
 'A':    [(0,45,1.7,.32),(1.7,52,1.2,.24)],
 'Esus4':[(0,40,1.7,.30),(1.7,47,1.2,.22)],
 'B7sus4':[(0,47,1.7,.32),(1.7,54,1.2,.24)],
 'Dadd9':[(0,38,1.7,.32),(1.7,45,1.2,.24)],
 'G':    [(0,43,1.7,.30),(1.7,50,1.2,.22)],
 'G/F#': [(0,42,1.7,.30),(1.7,50,1.2,.22)],
}
BASSP = {
 'F#m7': [(0,42,1.0,.34),(1.0,49,1.8,.26)],
 'Dadd9':[(0,38,1.0,.34),(1.0,45,1.8,.26)],
 'Esus4':[(0,40,1.0,.34),(1.0,47,1.8,.26)],
 'G':    [(0,43,1.0,.34),(1.0,50,1.8,.26)],
 'G/F#': [(0,42,1.0,.34),(1.0,50,1.8,.26)],
 'B7sus4':[(0,47,1.0,.36),(1.0,54,1.8,.28)],
}
BASS8 = {
 'F#m7': [(0,42,.5,.32),(.5,49,.45,.24),(1,54,.5,.28),(1.5,49,.45,.24),
 ...
 'Esus4':[(0,40,.5,.30),(.5,47,.45,.24),(1,52,.5,.28),(1.5,47,.45,.24),
          (2,52,.5,.28),(2.5,47,.45,.24),(3,55,.5,.24),(3.5,47,.45,.24)],
 'B7sus4':[(0,47,.5,.32),(.5,54,.45,.24),(1,59,.5,.28),(1.5,54,.45,.24),
          (2,59,.5,.28),(2.5,54,.45,.24),(3,61,.5,.24),(3.5,54,.45,.24)],
 'Dadd9':[(0,38,.5,.32),(.5,45,.45,.24),(1,50,.5,.28),(1.5,45,.45,.24),
          (2,54,.5,.28),(2.5,45,.45,.24),(3,52,.5,.24),(3.5,45,.45,.24)],
 'G':    [(0,43,.5,.32),(.5,50,.45,.24),(1,55,.5,.28),(1.5,50,.45,.24),
          (2,55,.5,.28),(2.5,50,.45,.24),(3,52,.5,.24),(3.5,50,.45,.24)],
 'G/F#': [(0,42,.5,.30),(.5,50,.45,.24),(1,55,.5,.28),(1.5,50,.45,.24),
          (2,55,.5,.28),(2.5,50,.45,.24),(3,52,.5,.24),(3.5,50,.45,.24)],
}
BASSC = {
 'F#m7': [(0,42,3.4,.30)],  'A': [(0,45,3.4,.30)],
 'Esus4':[(0,40,3.4,.28)],  'B7sus4':[(0,47,3.4,.30)],
}

bus = Bus(); drm = Bus(); vx = Bus()

def pedal(b, gain=0.05):
    """the Wonderwall two-note ringing dyad (C#5 + F#5) over verse chords"""
    bus.add(T(b), keys(mhz(61), 1.9, gain), pan=-0.35)
    bus.add(T(b), keys(mhz(66), 1.9, gain), pan=-0.35)

def sololead(b, ch):
    """pluck lead over the verse loop (the instrumental break)"""
    seq = {'F#m7': [54,57,61,64,61,57,54],
           'A':    [57,61,64,69,64,61,57],
           'Esus4':[52,57,59,64,59,57,52],
           'B7sus4':[59,64,66,69,66,64,59]}
    for p, m in enumerate(seq[ch]):
        bus.add(T(b + p * 0.5), pluck(mhz(m), 0.9, 0.17),
                pan=0.25 if p % 2 else -0.25)

V1 = [
 # "i've been walking in the shadows"
 [(0,.5,'E4','ai','','v'),(.5,.5,'E4','i','b','n'),(1.0,.5,'F#4','o','w','k'),
  (1.5,.5,'G#4','i','','ng'),(2.0,.5,'A4','i','','n'),(2.5,.45,'B4','e','th',''),
  (2.95,.5,'A4','a','sh',''),(3.45,.5,'G#4','ow','d','z')],
 # "where the light forgets to fall"
 [(0,.5,'G#4','e','w','r'),(.5,.45,'A4','e','th',''),(.95,.55,'B4','ai','l','t'),
  (1.5,.5,'A4','o','f','r'),(2.0,.5,'G#4','e','g','ts'),(2.5,.45,'F#4','u','t',''),
  (2.95,.8,'E4','o','f','l')],
 # "i've been counting all the reasons"
 [(0,.5,'F#4','ai','','v'),(.5,.5,'G#4','i','b','n'),(1.0,.55,'A4','ow','k','nt'),
  (1.55,.5,'B4','i','','ng'),(2.05,.5,'A4','e','th',''),(2.55,.5,'G#4','i','r',''),
  (3.05,.8,'F#4','u','s','n')],
 # "why i ever left at all"
 [(0,.5,'F#4','ai','w',''),(.5,.45,'G#4','ai','',''),(.95,.5,'A4','e','','v'),
  (1.45,.5,'A4','e','','r'),(1.95,.5,'G#4','e','l','f'),(2.45,.45,'F#4','a','','t'),
  (2.9,.8,'E4','o','','l')],
 # "but the moon it keeps its distance"
 [(0,.5,'E4','u','b','t'),(.5,.45,'F#4','e','th',''),(.95,.55,'G#4','u','m','n'),
  (1.5,.5,'A4','i','','t'),(2.0,.5,'B4','i','k','p'),(2.5,.45,'A4','i','','ts'),
  (2.95,.5,'G#4','i','d',''),(3.45,.5,'F#4','e','st','n')],
 # "and the stars they hold their line"
 [(0,.5,'F#4','a','','n'),(.5,.45,'G#4','e','th',''),(.95,.55,'A4','a','st','rs'),
  (1.5,.5,'B4','e','th',''),(2.0,.5,'A4','o','h','l'),(2.5,.45,'G#4','e','th',''),
  (2.95,.8,'F#4','ai','l','n')],
 # "while i'm standing in the doorway"
 [(0,.5,'G#4','ai','w','l'),(.5,.5,'A4','ai','','m'),(1.0,.55,'B4','a','st',''),
  (1.55,.5,'A4','i','','ng'),(2.05,.5,'G#4','i','','n'),(2.55,.5,'F#4','e','th',''),
  (3.05,.5,'E4','o','d','r'),(3.55,.45,'F#4','ay','w','')],
 # "watching daylight find your eyes"
 [(0,.5,'G#4','o','w','ch'),(.5,.45,'A4','i','','ng'),(.95,.55,'B4','ay','d',''),
  (1.5,.5,'B4','ai','l','t'),(2.0,.5,'A4','ai','f','nd'),(2.5,.45,'G#4','o','y','r'),
  (2.95,.8,'F#4','ai','','z')],
]
PRE = [
 # "and there's so much i would say"
 [(0,.5,'B4','a','','n'),(.5,.45,'C#5','e','th','r'),(.95,.5,'C#5','o','s',''),
  (1.45,.55,'D5','u','m','ch'),(2.0,.5,'C#5','ai','',''),(2.5,.5,'B4','u','w','d'),
  (3.0,.7,'A4','ay','s','')],
 # "if the words would only come"
 [(0,.5,'B4','i','f',''),(.5,.45,'C#5','e','th',''),(.95,.55,'D5','u','w','r'),
  (1.5,.5,'C#5','u','w','d'),(2.0,.5,'B4','o','','n'),(2.5,.45,'A4','i','l',''),
  (2.95,.8,'G#4','u','k','m')],
 # "so i'm leaving all my reasons"
 [(0,.5,'C#5','o','s',''),(.5,.5,'D5','ai','','m'),(1.0,.55,'E5','i','l','v'),
  (1.55,.5,'E5','i','','ng'),(2.05,.5,'D5','o','','l'),(2.55,.5,'C#5','ai','m',''),
  (3.05,.5,'B4','i','r',''),(3.55,.45,'A4','u','s','n')],
 # "at the door for you to find"
 [(0,.55,'B4','a','','t'),(.55,.5,'C#5','e','th',''),(1.05,.55,'D5','o','d','r'),
  (1.6,.5,'E5','o','f','r'),(2.1,.5,'D5','u','y',''),(2.6,.5,'C#5','u','t',''),
  (3.1,.8,'B4','ai','f','nd')],
]
HOOK = [
 # "you're my daylight, daylight"
 [(0,.8,'E4','o','y','r'),(.8,.5,'G#4','ai','m',''),(1.3,.5,'A4','ay','d',''),
  (1.8,.5,'B4','ai','l','t'),(2.3,.5,'C#5','ay','d',''),(2.8,.8,'B4','ai','l','t')],
 # "when the night is closing in"
 [(0,.5,'C#5','e','w','n'),(.5,.45,'B4','e','th',''),(.95,.55,'A4','ai','n','t'),
  (1.5,.5,'A4','i','','z'),(2.0,.5,'G#4','o','k','l'),(2.5,.45,'F#4','i','','ng'),
  (2.95,.8,'E4','i','','n')],
 # "you're my daylight, daylight" (higher landing)
 [(0,.8,'E4','o','y','r'),(.8,.5,'G#4','ai','m',''),(1.3,.5,'A4','ay','d',''),
  (1.8,.5,'B4','ai','l','t'),(2.3,.5,'D5','ay','d',''),(2.8,.8,'C#5','ai','l','t')],
 # "the only place i've ever been"
 [(0,.5,'B4','e','th',''),(.5,.45,'C#5','o','','n'),(.95,.5,'D5','i','l',''),
  (1.45,.5,'E5','ay','p','l'),(1.95,.5,'D5','ai','','v'),(2.45,.5,'C#5','e','','v'),
  (2.95,.45,'B4','e','','r'),(3.4,.5,'A4','i','b','n')],
 # "and i don't need a compass"
 [(0,.5,'B4','a','','n'),(.5,.45,'A4','ai','',''),(.95,.55,'G#4','o','d','nt'),
  (1.5,.5,'F#4','i','n','d'),(2.0,.5,'E4','a','',''),(2.5,.5,'F#4','u','k','m'),
  (3.0,.7,'G#4','a','p','s')],
 # "or a map to guide me home"
 [(0,.5,'G#4','o','','r'),(.5,.45,'A4','a','',''),(.95,.55,'B4','a','m','p'),
  (1.5,.5,'A4','u','t',''),(2.0,.5,'G#4','ai','g','d'),(2.5,.45,'F#4','i','m',''),
  (2.95,.8,'E4','o','h','m')],
 # "cause you're my daylight"
 [(0,.55,'E4','o','k','z'),(.55,.55,'F#4','o','y','r'),(1.1,.5,'G#4','ai','m',''),
  (1.6,.5,'A4','ay','d',''),(2.1,.8,'B4','ai','l','t')],
 # "daylight"
 [(0,.6,'C#5','ay','d',''),(.6,1.4,'D5','ai','l','t')],
]
V2 = [
 # "there's a river that runs backwards"
 [(0,.5,'E4','e','th','r'),(.5,.45,'F#4','a','',''),(.95,.55,'G#4','i','r','v'),
  (1.5,.5,'A4','e','','r'),(2.0,.5,'A4','a','th','t'),(2.5,.5,'G#4','u','r','n'),
  (3.0,.5,'F#4','a','b','k'),(3.5,.45,'E4','u','w','d')],
 # "through the streets i used to know"
 [(0,.5,'F#4','u','th','r'),(.5,.45,'G#4','e','th',''),(.95,.55,'A4','i','st','t'),
  (1.5,.5,'B4','ai','',''),(2.0,.5,'A4','u','','z'),(2.5,.45,'G#4','u','t',''),
  (2.95,.8,'F#4','o','n','w')],
 # "and it carries all the letters"
 [(0,.5,'F#4','a','','n'),(.5,.45,'G#4','i','','t'),(.95,.55,'A4','a','k','r'),
  (1.5,.5,'B4','i','r',''),(2.0,.5,'A4','o','','l'),(2.5,.5,'G#4','e','th',''),
  (3.0,.5,'F#4','e','l','t'),(3.5,.45,'E4','e','t','r')],
 # "that i never learned to send"
 [(0,.5,'G#4','a','th','t'),(.5,.45,'A4','ai','',''),(.95,.55,'B4','e','n','v'),
  (1.5,.5,'A4','e','','r'),(2.0,.5,'G#4','u','l','r'),(2.5,.45,'F#4','u','t',''),
  (2.95,.8,'E4','e','s','nd')],
 # "but the current is forgiving"
 [(0,.5,'F#4','u','b','t'),(.5,.45,'G#4','e','th',''),(.95,.55,'A4','u','k','r'),
  (1.5,.5,'B4','e','r','nt'),(2.0,.5,'A4','i','','z'),(2.5,.5,'G#4','o','f','r'),
  (3.0,.5,'F#4','i','g','v'),(3.5,.45,'E4','i','','ng')],
 # "and it circles back to you"
 [(0,.5,'G#4','a','','n'),(.5,.45,'A4','i','','t'),(.95,.55,'B4','u','s','r'),
  (1.5,.5,'A4','u','k','l'),(2.0,.5,'G#4','a','b','k'),(2.5,.45,'F#4','u','t',''),
  (2.95,.8,'E4','u','y','')],
]
BR = [
 # "there are things i'd like to tell you"
 [(0,.5,'E4','e','th','r'),(.5,.45,'F#4','a','','r'),(.95,.55,'G#4','i','th','ng'),
  (1.5,.5,'A4','ai','','d'),(2.0,.5,'B4','ai','l',''),(2.5,.5,'A4','u','t',''),
  (3.0,.5,'G#4','e','t','l'),(3.5,.45,'F#4','u','y','')],
 # "that i never learned to say"
 [(0,.5,'G#4','a','th','t'),(.5,.45,'A4','ai','',''),(.95,.55,'B4','e','n','v'),
  (1.5,.5,'A4','e','','r'),(2.0,.5,'G#4','u','l','r'),(2.5,.45,'F#4','u','t',''),
  (2.95,.8,'E4','ay','s','')],
 # "so i'm writing them in daylight"
 [(0,.5,'F#4','o','s',''),(.5,.5,'G#4','ai','','m'),(1.0,.55,'A4','i','r','t'),
  (1.55,.5,'B4','i','','ng'),(2.05,.5,'A4','e','th','m'),(2.55,.5,'G#4','i','','n'),
  (3.05,.5,'F#4','ay','d',''),(3.55,.45,'E4','ai','l','t')],
 # "till the words give me away"
 [(0,.5,'F#4','i','t','l'),(.5,.45,'G#4','e','th',''),(.95,.55,'A4','u','w','r'),
  (1.5,.5,'B4','i','g','v'),(2.0,.5,'A4','i','m',''),(2.5,.45,'G#4','a','',''),
  (2.95,.8,'F#4','ay','w','')],
]
CH3_LAST = [(0,.6,'C#5','ay','d',''),(.6,1.8,'E5','ai','l','t')]   # held final "daylight"

# ---------------------------------------------------------- arrangement
# ---- INTRO: the loop with the pedal dyad ----
for i, ch in enumerate(['F#m7','A','Esus4','B7sus4']):
    b = S['INTRO'] + i * BAR
    padbar(b, ch, gain=0.10, cut=2100, atk=1.5, seed=i)
...
for i in range(8):
    b = S['V1'] + i * BAR
    sl = bar_chords(V1BAR)[i]
    for ch, sp in sl:
        padbar(b, ch, gain=0.08, cut=2000, atk=1.4, seed=100 + i)
        keysbar_split(b, [(ch, sp)], gain=0.13)
...
for i in range(4):
    b = S['PRE1'] + i * BAR
    sl = bar_chords(PREBAR)[i]
    padbar(b, chord_at(sl, 0), gain=0.12 + 0.02 * i, cut=2500 + 400 * i, atk=0.8, seed=300 + i)
    keysbar_split(b, sl, gain=0.16)
    for ch, sp in sl:
        bassbar(b, ch, BASSP, at=sp)
    vpre(b, i, vel=0.7)
...
for i in range(8):
    b = S['CH1'] + i * BAR
    sl = bar_chords(CHBAR)[i]
    padbar(b, chord_at(sl, 0), gain=0.16, cut=4200, atk=0.35, seed=500 + i)
    keysbar_split(b, sl, gain=0.15)
    for ch, sp in sl:
        bassbar(b, ch, BASS8, at=sp)
    choir(vx, b, CH[chord_at(sl, 0)][1], gain=0.09, seedbase=600 + i)
    vchorus(b, i, loud=1.0)
    hookline(vx, b, HOOK[i], gain=1.0, seedbase=700 + i * 17)

# ---- SOLO: the instrumental break (My Love device) ----
for i in range(6):
    b = S['SOLO'] + i * BAR
    sl = bar_chords(SOLOBAR)[i]
    for ch, sp in sl:
        padbar(b, ch, gain=0.12, cut=3000, atk=0.9, seed=750 + i)
        keysbar_split(b, [(ch, sp)], gain=0.14)
        bassbar(b, ch, BASS8, at=sp)
        sololead(b + sp, ch)
    vverse(b, vel=0.55)
...
for i in range(6):
    b = S['V2'] + i * BAR
    sl = bar_chords(V2BAR)[i]
    for ch, sp in sl:
        padbar(b, ch, gain=0.09, cut=2200, atk=1.2, seed=800 + i)
        keysbar_split(b, [(ch, sp)], gain=0.13)
        bassbar(b, ch, BASSL, at=sp)
    pedal(b, gain=0.05)
    vverse(b, vel=0.6)
...
for i in range(4):
    b = S['PRE2'] + i * BAR
    sl = bar_chords(PREBAR)[i]
    padbar(b, chord_at(sl, 0), gain=0.15 + 0.02 * i, cut=3100 + 500 * i, atk=0.5, seed=1000 + i)
    keysbar_split(b, sl, gain=0.18)
    for ch, sp in sl:
        bassbar(b, ch, BASSP, at=sp)
    vpre(b, i, vel=0.9)
...
for i in range(8):
    b = S['CH2'] + i * BAR
    sl = bar_chords(CHBAR)[i]
    padbar(b, chord_at(sl, 0), gain=0.18, cut=4400, atk=0.3, seed=1200 + i)
    keysbar_split(b, sl, gain=0.16)
    for ch, sp in sl:
        bassbar(b, ch, BASS8, at=sp)
    choir(vx, b, CH[chord_at(sl, 0)][1], gain=0.10, seedbase=1300 + i)
    vchorus(b, i, loud=1.0)
    hookline(vx, b, HOOK[i], gain=1.05, seedbase=1400 + i * 17, halo_g=0.14)

# ---- BRIDGE: the climb (G G/F# F#m7 G B7sus4), rim groove then build ----
for i in range(3):
    b = S['BRIDGE'] + i * BAR
    sl = bar_chords(BRBAR)[i]
    padbar(b, chord_at(sl, 0), gain=0.09, cut=2000, atk=1.4, seed=1500 + i)
    keysbar_split(b, sl, gain=0.12)
    for ch, sp in sl:
        bassbar(b, ch, BASSL, at=sp)
    drm.add(T(b + 1), rim(0.4)); drm.add(T(b + 3), rim(0.4))
    drm.add(T(b + 0), kick(0.5)); drm.add(T(b + 2.5), kick(0.4))
...
bb = S['BRIDGE'] + 3 * BAR                # build bar on B7sus4
padbar(bb, 'B7sus4', gain=0.12, cut=3200, atk=0.5, seed=44)
keysarp(bb, 'B7sus4', gain=0.14)
bassbar(bb, 'B7sus4', BASSL)
for k in range(16):
    drm.add(T(bb + k * 0.25), snare(vel=0.2 + 0.035 * k))
drm.add(T(bb + 3.2), riser(1.8, 0.09, 350, 8000))
drm.add(T(bb + 3.95), crash(1.0))

# ---- CH3: everything, final held chord with the 4-voice stack ----
for i in range(8):
    b = S['CH3'] + i * BAR
    sl = bar_chords(CHBAR)[i]
    padbar(b, chord_at(sl, 0), gain=0.20, cut=5000, atk=0.25, seed=1700 + i)
    keysbar_split(b, sl, gain=0.18)
    for ch, sp in sl:
        bassbar(b, ch, BASS8, at=sp)
    choir(vx, b, CH[chord_at(sl, 0)][1], gain=0.12, seedbase=1800 + i)
    vchorus(b, i, loud=1.08)
    drm.add(T(b), crash(0.75))
    if i % 2 == 0:
        bus.add(T(b + 0.5), stab([m + 12 for m in CH[chord_at(sl, 0)][1]], 0.8, 0.16, cut=3000),
                pan=-0.2)
    if i < 7:
        hookline(vx, b, HOOK[i], gain=1.1, seedbase=1900 + i * 17, halo_g=0.15)
b7 = S['CH3'] + 7 * BAR                   # Flying Without Wings ending
for ev in CH3_LAST:
    ...
    choir(vx, b7, CH['F#m7'][1], gain=0.10, seedbase=2900)

# ---- CODA: loop once more, quiet, fade ----
for i in range(4):
    b = S['CODA'] + i * BAR
    sl = bar_chords(CODABAR)[i]
    for ch, sp in sl:
        padbar(b, ch, gain=0.08, cut=2000, atk=1.5, seed=2100 + i)
        keysbar_split(b, [(ch, sp)], gain=0.11)
        bassbar(b, ch, BASSC, at=sp)
    if i == 0:
        drm.add(T(b + 0), kick(0.38)); drm.add(T(b + 2), kick(0.32))

master(bus.L, bus.R, vx.L, vx.R, "youre-my-daylight.wav", DUR)
master(bus.L, bus.R, None, None, "youre-my-daylight-instrumental.wav", DUR)
