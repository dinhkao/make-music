...
# =======================================================================
# SONG — CARRY THE FLAME (D minor, 84 bpm, 60 bars = 2:51)
#
# Westlife live move: the false stop. Everything drops to a bare chord
# just before the final chorus, then the full band crashes back in —
# the "stand up from the stools" moment. The chorus itself never changes
# key; the cycle just keeps rotating and getting louder (faithful to
# Zombie, whose power IS the repetition).
#
# Harmony — The Cranberries "Zombie" (1994): one cycle for the entire
# song, Em - C - G - D/F# (i - VI - III - VII), the D/F# inversion
# giving the descending bass G-F#-E. Transposed to D minor:
#   every section: Dm - Bb - F - C/E        (the C/E = VII inversion,
#   bass descends F -> E -> D at each turn of the cycle)
#   bridge: Dm Dm Bb F | C (false stop) | snare build -> re-entry
# =======================================================================
BPM = 84.0
BEAT = 60.0 / BPM
BAR = 4.0
...
DUR = T(END_BEATS)
TOTAL = DUR + 3.5
print('CARRY THE FLAME  bars', int(END_BEATS / BAR), ' duration', round(DUR, 1), 's')

CH = {
 'Dm': (38, (50,53,57,62), 38),   'Bb': (46, (58,62,65,70), 46),
 'F':  (41, (53,57,60,65), 41),   'C':  (36, (48,52,55,60), 36),
 'C/E':(36, (48,52,55,60), 40),
}
V1BAR  = ['Dm','Dm','Bb','Bb','F','F','C/E','C/E']
PREBAR = ['Dm','Bb','F','C/E']
CHBAR  = ['Dm','Dm','Bb','Bb','F','F','C/E','C/E']
V2BAR  = ['Dm','Dm','Bb','F','C/E','C/E']
BRBAR  = ['Dm','Dm','Bb','F']
CH3BAR = ['Dm','Dm','Bb','Bb','F','F','C/E','C/E']
CODABAR= ['Dm','Dm','Bb','F']
...
BASS8 = {
 ...
 'C/E':[(0,40,.5,.30),(.5,43,.45,.24),(1,48,.5,.28),(1.5,43,.45,.24),
        (2,48,.5,.28),(2.5,43,.45,.24),(3,52,.5,.24),(3.5,43,.45,.24)],
}
BASSC = {
 'Dm': [(0,38,3.4,.30)],  'Bb': [(0,46,3.4,.30)],
 'F':  [(0,41,3.4,.28)],  'C/E': [(0,40,3.4,.28)],
}

bus = Bus(); drm = Bus(); vx = Bus()

V1 = [
 # "there's a fire down in the valley"
 [(0,.5,'D4','e','th','r'),(.5,.45,'D4','a','',''),(.95,.5,'F4','ai','f','r'),
  (1.45,.55,'G4','ow','d','n'),(2.0,.5,'A4','i','','n'),(2.5,.45,'A4','e','th',''),
  (2.95,.5,'G4','a','v','l'),(3.45,.5,'F4','i','l','')],
 # "that the rain can't put out"
 [(0,.5,'F4','a','th','t'),(.5,.45,'G4','e','th',''),(.95,.55,'A4','ay','r','n'),
  (1.5,.5,'A4','a','k','nt'),(2.0,.5,'G4','u','p','t'),(2.5,.9,'F4','ow','','t')],
 # "there's a heart inside the silence"
 [(0,.5,'G4','e','th','r'),(.5,.45,'A4','a','',''),(.95,.55,'A4','a','h','r'),
  (1.5,.5,'G4','i','','n'),(2.0,.5,'F4','ai','s','d'),(2.5,.45,'F4','e','th',''),
  (2.95,.5,'G4','ai','s',''),(3.45,.5,'A4','e','l','n')],
 # "that refuses to drown"
 [(0,.5,'A4','a','th','t'),(.5,.45,'G4','i','r',''),(.95,.5,'F4','u','f',''),
  (1.45,.5,'F4','e','','z'),(1.95,.5,'G4','u','t',''),(2.45,.9,'A4','ow','dr','n')],
 # "and the years keep coming faster"
 [(0,.5,'F4','a','','n'),(.5,.45,'G4','e','th',''),(.95,.5,'A4','i','y','r'),
  (1.45,.5,'A4','i','k','p'),(1.95,.5,'G4','u','k','m'),(2.45,.45,'F4','i','','ng'),
  (2.9,.5,'G4','a','f','s'),(3.4,.55,'A4','e','t','r')],
 # "like the waves upon the shore"
 [(0,.5,'A4','ai','l',''),(.5,.45,'G4','e','th',''),(.95,.55,'A4','ay','w','v'),
  (1.5,.5,'G4','u','','p'),(2.0,.5,'F4','o','','n'),(2.5,.45,'E4','e','th',''),
  (2.95,.8,'F4','o','sh','r')],
 # "but i'm standing in the water"
 [(0,.5,'F4','u','b','t'),(.5,.5,'G4','ai','','m'),(1.0,.55,'A4','a','st',''),
  (1.55,.5,'A4','i','','ng'),(2.05,.5,'G4','i','','n'),(2.55,.5,'F4','e','th',''),
  (3.05,.5,'E4','o','w',''),(3.55,.45,'D4','e','t','r')],
 # "a thousand times before"
 [(0,.5,'F4','a','',''),(.5,.45,'G4','ow','th',''),(.95,.55,'A4','a','s','nd'),
  (1.5,.5,'A4','ai','t','m'),(2.0,.5,'G4','i','b',''),(2.5,.45,'F4','o','f','r'),
  (2.95,.8,'E4','','','')],
]
PRE = [
 # "and i lift my hands up higher"
 [(0,.5,'A4','a','','n'),(.5,.45,'C5','ai','',''),(.95,.5,'D5','i','l','f'),
  (1.45,.5,'D5','ai','m',''),(1.95,.5,'C5','a','h','nd'),(2.45,.45,'A4','u','','p'),
  (2.9,.5,'G4','ai','h',''),(3.4,.55,'A4','e','','r')],
 # "let the wind take what it will"
 [(0,.5,'A4','e','l','t'),(.5,.45,'C5','e','th',''),(.95,.55,'D5','i','w','nd'),
  (1.5,.5,'D5','ay','t','k'),(2.0,.5,'C5','u','w','t'),(2.5,.45,'A4','i','','t'),
  (2.95,.8,'G4','i','w','l')],
 # "cause the only thing that matters"
 [(0,.5,'A4','o','k','z'),(.5,.45,'C5','e','th',''),(.95,.5,'D5','o','','n'),
  (1.45,.5,'C5','i','l',''),(1.95,.5,'A4','i','th','ng'),(2.45,.45,'G4','a','th','t'),
  (2.9,.5,'F4','a','m','t'),(3.4,.55,'G4','e','t','r')],
 # "is the heart that's beating still"
 [(0,.55,'G4','i','','z'),(.55,.5,'A4','e','th',''),(1.05,.55,'C5','a','h','r'),
  (1.6,.5,'C5','a','th','ts'),(2.1,.5,'A4','i','b','t'),(2.6,.45,'G4','i','','ng'),
  (3.05,.8,'A4','i','st','l')],
]
HOOK = [
 # "carry the flame, carry the flame"
 [(0,.6,'F4','a','k','r'),(.6,.45,'F4','i','r',''),(1.05,.5,'A4','e','th',''),
  (1.55,.7,'C5','ay','f','l'),(2.25,.5,'C5','a','k','r'),(2.75,.45,'D5','i','r',''),
  (3.2,.4,'D5','e','th',''),(3.6,.4,'C5','ay','f','l')],
 # "through the endless, endless rain"
 [(0,.5,'D5','u','th','r'),(.5,.45,'C5','e','th',''),(.95,.6,'A4','e','','nd'),
  (1.55,.5,'G4','e','l','s'),(2.05,.6,'A4','e','','nd'),(2.65,.5,'G4','e','l','s'),
  (3.15,.7,'F4','ay','r','n')],
 # "carry the flame, carry the flame" (higher landing)
 [(0,.6,'F4','a','k','r'),(.6,.45,'G4','i','r',''),(1.05,.5,'A4','e','th',''),
  (1.55,.7,'D5','ay','f','l'),(2.25,.5,'D5','a','k','r'),(2.75,.45,'D5','i','r',''),
  (3.2,.4,'D5','e','th',''),(3.6,.4,'D5','ay','f','l')],
 # "till the fire comes again"
 [(0,.5,'D5','i','t','l'),(.5,.45,'C5','e','th',''),(.95,.55,'A4','ai','f',''),
  (1.5,.5,'A4','e','r',''),(2.0,.5,'G4','u','k','m'),(2.5,.45,'A4','a','',''),
  (2.95,.8,'C5','ay','g','n')],
 # "every shadow that has found me"
 [(0,.5,'A4','e','','v'),(.5,.45,'C5','i','r',''),(.95,.55,'D5','a','sh',''),
  (1.5,.5,'D5','ow','d',''),(2.0,.5,'C5','a','th','t'),(2.5,.5,'A4','a','h','z'),
  (3.0,.5,'G4','ow','f','nd'),(3.5,.45,'A4','i','m','')],
 # "every stone across my way"
 [(0,.5,'A4','e','','v'),(.5,.45,'C5','i','r',''),(.95,.55,'D5','o','st','n'),
  (1.5,.5,'C5','a','',''),(2.0,.5,'A4','o','k','r'),(2.5,.45,'G4','ai','m',''),
  (2.95,.8,'F4','ay','w','')],
 # "i will carry, i will carry"
 [(0,.5,'F4','ai','',''),(.5,.5,'G4','i','w','l'),(1.0,.5,'A4','a','k','r'),
  (1.5,.45,'C5','i','r',''),(1.95,.5,'A4','ai','',''),(2.45,.5,'G4','i','w','l'),
  (2.95,.5,'F4','a','k','r'),(3.45,.55,'G4','i','r','')],
 # "the flame"
 [(0,.6,'A4','e','th',''),(.6,1.3,'C5','ay','f','l'),(2.2,1.0,'D5','o','','')],
]
V2 = [
 # "in the morning i remember"
 [(0,.5,'D4','i','','n'),(.5,.45,'F4','e','th',''),(.95,.55,'G4','o','m','r'),
  (1.5,.5,'A4','i','','ng'),(2.0,.5,'A4','ai','',''),(2.5,.5,'G4','i','r',''),
  (3.0,.5,'F4','e','m',''),(3.5,.45,'E4','e','b','r')],
 # "what the darkness tried to steal"
 [(0,.5,'F4','u','w','t'),(.5,.45,'G4','e','th',''),(.95,.55,'A4','a','d','r'),
  (1.5,.5,'A4','e','n',''),(2.0,.5,'G4','ai','tr','d'),(2.5,.45,'F4','u','t',''),
  (2.95,.8,'E4','i','st','l')],
 # "every promise that you made me"
 [(0,.5,'G4','e','','v'),(.5,.45,'A4','i','r',''),(.95,.55,'A4','o','p','r'),
  (1.5,.5,'G4','i','','z'),(2.0,.5,'F4','a','th','t'),(2.5,.5,'G4','u','y',''),
  (3.0,.5,'A4','ay','m','d'),(3.5,.45,'A4','i','m','')],
 # "every scar that taught me to feel"
 [(0,.5,'A4','e','','v'),(.5,.45,'G4','i','r',''),(.95,.5,'F4','a','sk','r'),
  (1.45,.5,'G4','a','th','t'),(1.95,.5,'A4','o','t',''),(2.45,.5,'G4','i','m',''),
  (2.95,.45,'F4','u','t',''),(3.4,.55,'E4','i','f','l')],
 # "and i'll walk into the winter"
 [(0,.5,'F4','a','','n'),(.5,.5,'G4','ai','','l'),(1.0,.55,'A4','o','w','k'),
  (1.55,.5,'A4','i','','n'),(2.05,.5,'G4','u','t',''),(2.55,.5,'F4','e','th',''),
  (3.05,.5,'E4','i','w','n'),(3.55,.45,'D4','e','t','r')],
 # "with the summer in my chest"
 [(0,.5,'F4','i','w','th'),(.5,.45,'G4','e','th',''),(.95,.55,'A4','u','s','m'),
  (1.5,.5,'A4','e','m','r'),(2.0,.5,'G4','i','','n'),(2.5,.45,'F4','ai','m',''),
  (2.95,.8,'E4','e','ch','st')],
]
BR = [
 # "when the world goes quiet"
 [(0,.6,'D4','e','w','n'),(.6,.5,'F4','e','th',''),(1.1,.55,'G4','u','w','r'),
  (1.65,.55,'A4','o','g','z'),(2.2,.5,'A4','ai','k','w'),(2.7,.8,'G4','e','','t')],
 # "and the road gives out"
 [(0,.55,'G4','a','','n'),(.55,.5,'A4','e','th',''),(1.05,.55,'C5','o','r','d'),
  (1.6,.5,'A4','i','g','v'),(2.1,.9,'G4','ow','','t')],
 # "i will still be burning"
 [(0,.55,'G4','ai','',''),(.55,.5,'A4','i','w','l'),(1.05,.55,'C5','i','st','l'),
  (1.6,.5,'B4','i','b',''),(2.1,.5,'A4','u','b','r'),(2.6,.9,'G4','i','','ng')],
 # "till the last of me goes out"
 [(0,.5,'A4','i','t','l'),(.5,.45,'G4','e','th',''),(.95,.55,'F4','a','l','st'),
  (1.5,.5,'E4','o','','v'),(2.0,.5,'D4','i','m',''),(2.5,.45,'E4','o','g','z'),
  (2.95,.8,'F4','ow','','t')],
]
CODA_1 = [(0,.6,'F4','a','k','r'),(.6,.45,'G4','i','r',''),(1.05,.5,'A4','e','th',''),
          (1.55,.9,'C5','ay','f','l'),(2.6,1.0,'D5','o','','')]   # "carry the flame"

# ---------------------------------------------------------- arrangement
# ---- INTRO: the cycle with the descending-bass turn ----
for i, ch in enumerate(['Dm','Bb','F','C/E']):
    b = S['INTRO'] + i * BAR
...
# ---- BRIDGE: stripped, then the FALSE STOP, then the build ----
for i in range(4):
    b = S['BRIDGE'] + i * BAR
    ch = BRBAR[i]
...
b4 = S['BRIDGE'] + 4 * BAR           # FALSE STOP: one bar, keys alone
bus.add(T(b4), keys(mhz(60), 2.6, 0.16), pan=-0.1)
padat(bus, T(b4), [m + 12 for m in CH['C'][1]], 2.5, atk=1.0, cut=1800, seed=44, gain=0.10)
b5 = S['BRIDGE'] + 5 * BAR           # snare 16th build + riser
for k in range(16):
    drm.add(T(b5 + k * 0.25), snare(vel=0.2 + 0.035 * k))
drm.add(T(b5 + 3.2), riser(1.8, 0.09, 350, 8000))
drm.add(T(b5 + 3.95), crash(1.0))

# ---- CH3: the re-entry. full force, no key change, everything ----
for i in range(8):
    b = S['CH3'] + i * BAR
    ch = CHBAR[i]
...
for ev in CODA_1:
    ...
    choir(vx, b0, CH['Dm'][1], gain=0.08, seedbase=2700)

master(bus.L, bus.R, vx.L, vx.R, 'carry-the-flame.wav', DUR)
master(bus.L, bus.R, None, None, 'carry-the-flame-instrumental.wav', DUR)
