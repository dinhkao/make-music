...
# =======================================================================
# SONG — THE BEST OF DAYS (C major, 84 bpm, 60 bars = 2:51)
#
# Westlife "You Raise Me Up" finale: the last chorus arrives with an
# orchestra-style SWELL — the supersaw pad sweeps its cutoff open (the
# lpsweep in the pad synth) while the drums hold, then everything lands
# a whole step up (C -> D, the My Love modulation) for the biggest chorus
# of the song. Ends on a held chord with the full 4-voice stack.
#
# Harmony — Smashing Pumpkins "Today" (1993, D shapes / E-minor reading):
#   verse:   D - A - G (-> D - A - Bm)  =>  C - G - F (-> C - G - Am)
#   chorus:  Em - G - Bm (-> F# - C#)   =>  Dm - C - Am (-> E - B)
#   chorus2: Em - G - D - A             =>  Dm - C - G - F
#   bridge:  D - A - D - G / D - A - Bm - G  =>  C G | C F | C G | Am F
#   outro:   D - A - Bm - G             =>  D - A - Bm - G (new key)
# The borrowed major chord (B in Em; E here) is the mode-mixture signature.
# =======================================================================
BPM = 84.0
BEAT = 60.0 / BPM
BAR = 4.0
...
DUR = T(END_BEATS)
TOTAL = DUR + 3.5
print('THE BEST OF DAYS  bars', int(END_BEATS / BAR), ' duration', round(DUR, 1), 's')

CH = {
 'C':  (36, (48,52,55,60), 36),   'G':  (43, (55,59,62,67), 43),
 'F':  (41, (53,57,60,65), 41),   'Am': (45, (57,60,64,69), 45),
 'Dm': (38, (50,53,57,62), 38),   'E':  (40, (52,56,59,64), 40),
 'B':  (47, (59,63,66,70), 47),
 'D':  (38, (50,54,57,62), 38),   'A':  (45, (57,61,64,69), 45),
 'G2': (43, (55,59,62,67), 43),   'Bm': (47, (59,62,66,71), 47),
 'Em': (40, (52,55,59,64), 40),   'F#': (42, (54,58,61,66), 42),
 'C#': (37, (49,53,56,61), 37),
}
SPLIT = {'CG': ('C','G'), 'CF': ('C','F'), 'AmF': ('Am','F'),
         'F#C#': ('F#','C#')}
V1BAR  = ['C','G','F','C','G','F','C','G']
PREBAR = ['Dm','C','G','F']
CHBAR  = ['Dm','C','Am','Am'] * 2
V2BAR  = ['C','G','F','C','G','F']
BRBAR  = ['CG','CF','CG','AmF']
CH3BAR = ['Em','D','Bm','Em','D','Bm','F#C#','Em']   # whole step up
CODABAR= ['D','A','Bm','G2','D','D']

BASSL = {
 'C':  [(0,36,1.7,.32),(1.7,43,1.2,.24)],
 'G':  [(0,43,1.7,.32),(1.7,50,1.2,.24)],
 'F':  [(0,41,1.7,.30),(1.7,48,1.2,.22)],
 'Am': [(0,45,1.7,.30),(1.7,52,1.2,.22)],
 'Dm': [(0,38,1.7,.30),(1.7,45,1.2,.22)],
 'E':  [(0,40,1.5,.32),(1.5,47,1.3,.24)],
 'B':  [(0,47,1.5,.32),(1.5,54,1.3,.24)],
}
BASSP = {
...
BASS8 = {
 ...
 'Am': [(0,45,.5,.32),(.5,52,.45,.24),(1,57,.5,.28),(1.5,52,.45,.24),
...
BASSC = {
 'D':  [(0,38,3.4,.30)],  'A':  [(0,45,3.4,.30)],
 'Bm': [(0,47,3.4,.28)],  'G2': [(0,43,3.4,.28)],
}

bus = Bus(); drm = Bus(); vx = Bus()

V1 = [
 # "wake up slow, the sun is low"
 [(0,.5,'C4','ay','w','k'),(.5,.45,'E4','u','','p'),(.95,.55,'G4','o','sl',''),
  (1.5,.5,'A4','e','th',''),(2.0,.5,'G4','u','s','n'),
  (2.5,.45,'F4','i','','z'),(2.95,.8,'E4','o','l','')],
 # "the whole wide world can wait"
 [(0,.5,'E4','e','th',''),(.5,.45,'G4','o','h','l'),(.95,.55,'A4','ai','w','d'),
  (1.5,.5,'C5','u','w','r'),(2.0,.5,'A4','a','k','n'),(2.5,.45,'G4','ay','w','t')],
 # "i've been counting all the grey days"
 [(0,.5,'G4','ai','','v'),(.5,.45,'A4','i','b','n'),(.95,.55,'C5','ow','k','nt'),
  (1.5,.5,'C5','i','','ng'),(2.0,.5,'A4','o','','l'),(2.5,.5,'G4','e','th',''),
  (3.0,.5,'F4','ay','gr',''),(3.5,.45,'E4','ay','d','z')],
 # "till the color came to stay"
 [(0,.5,'E4','i','t','l'),(.5,.45,'G4','e','th',''),
  (.95,.55,'A4','o','k','l'),(1.5,.5,'A4','e','','r'),(2.0,.5,'G4','ay','k','m'),
  (2.5,.45,'F4','u','t',''),(2.95,.8,'E4','ay','st','')],
 # "and this morning feels like something"
 [(0,.5,'G4','a','','n'),(.5,.45,'A4','i','th','s'),(.95,.55,'C5','o','m','r'),
  (1.5,.5,'C5','i','','ng'),(2.0,.5,'A4','i','f','l'),(2.5,.5,'G4','ai','l',''),
  (3.0,.5,'F4','u','s','m'),(3.5,.45,'E4','i','th','ng')],
 # "that the sky forgot to take"
 [(0,.5,'E4','a','th','t'),(.5,.45,'G4','e','th',''),(.95,.55,'A4','ai','sk',''),
  (1.5,.5,'G4','o','f','r'),(2.0,.5,'F4','o','g','t'),(2.5,.45,'E4','u','t',''),
  (2.95,.8,'D4','ay','t','k')],
 # "so i'm standing in the window"
 [(0,.5,'G4','o','s',''),(.5,.5,'A4','ai','','m'),(1.0,.55,'C5','a','st',''),
  (1.55,.5,'C5','i','','ng'),(2.05,.5,'A4','i','','n'),(2.55,.5,'G4','e','th',''),
  (3.05,.5,'F4','i','w','n'),(3.55,.45,'E4','o','d','')],
 # "watching all my worries break"
 [(0,.5,'G4','o','w','ch'),(.5,.45,'A4','i','','ng'),(.95,.55,'C5','o','','l'),
  (1.5,.5,'A4','ai','m',''),(2.0,.5,'G4','u','w','r'),
  (2.5,.45,'F4','i','r',''),(2.95,.8,'E4','ay','br','k')],
]
PRE = [
 # "and if the road should turn to dust"
 [(0,.5,'A4','a','','n'),(.5,.45,'C5','i','f',''),(.95,.55,'D5','e','th',''),
  (1.5,.5,'D5','o','r','d'),(2.0,.5,'C5','u','sh','d'),(2.5,.45,'A4','u','t','r'),
  (2.95,.5,'G4','u','t',''),(3.45,.5,'A4','u','d','st')],
 # "i will still be walking on"
 [(0,.5,'A4','ai','',''),(.5,.45,'C5','i','w','l'),(.95,.55,'D5','i','st','l'),
  (1.5,.5,'D5','i','b',''),(2.0,.5,'C5','o','w','k'),(2.5,.45,'A4','i','','ng'),
  (2.95,.8,'G4','o','','n')],
 # "cause today i have the answer"
 [(0,.5,'C5','o','k','z'),(.5,.45,'D5','u','t',''),(.95,.55,'E5','ay','d',''),
  (1.5,.5,'E5','ai','',''),(2.0,.5,'D5','a','h','v'),(2.5,.5,'C5','e','th',''),
  (3.0,.5,'A4','a','','n'),(3.5,.45,'B4','e','s','r')],
 # "to the question i've been running"
 [(0,.55,'B4','u','t',''),(.55,.5,'C5','e','th',''),(1.05,.55,'D5','e','k','w'),
  (1.6,.5,'E5','e','sh','n'),(2.1,.5,'D5','ai','','v'),
  (2.6,.5,'C5','i','b','n'),(3.1,.5,'B4','u','r','n'),(3.6,.4,'A4','i','','ng')],
]
HOOK = [
 # "these are the best of days"
 [(0,.8,'F4','i','th','z'),(.8,.5,'A4','a','','r'),(1.3,.5,'C5','e','th',''),
  (1.8,.5,'D5','e','b','st'),(2.3,.45,'C5','o','','v'),
  (2.75,.8,'A4','ay','d','z')],
 # "and i'm holding every one"
 [(0,.5,'C5','a','','n'),(.5,.45,'D5','ai','','m'),(.95,.55,'E5','o','h','l'),
  (1.5,.5,'E5','i','','ng'),(2.0,.5,'D5','e','','v'),
  (2.5,.45,'C5','i','r',''),(2.95,.8,'A4','u','w','n')],
 # "these are the best of days" (higher landing)
 [(0,.8,'F4','i','th','z'),(.8,.5,'A4','a','','r'),(1.3,.5,'C5','e','th',''),
  (1.8,.5,'E5','e','b','st'),(2.3,.45,'D5','o','','v'),
  (2.75,.8,'C5','ay','d','z')],
 # "under this golden sun"
 [(0,.5,'D5','u','','n'),(.5,.45,'C5','e','d','r'),(.95,.55,'A4','i','th','s'),
  (1.5,.5,'G4','o','g','l'),(2.0,.5,'F4','e','n',''),(2.5,.8,'G4','u','s','n')],
 # "and the hours fall like petals"
 [(0,.5,'A4','a','','n'),(.5,.45,'C5','e','th',''),(.95,.55,'D5','ow','','r'),
  (1.5,.5,'E5','o','f','l'),(2.0,.5,'D5','ai','l',''),(2.5,.5,'C5','e','p','t'),
  (3.0,.5,'A4','u','','l')],
 # "but the flower's in my hand"
 [(0,.5,'A4','u','b','t'),(.5,.45,'C5','e','th',''),(.95,.55,'E5','ow','f','l'),
  (1.5,.5,'E5','e','','r'),(2.0,.5,'D5','i','','n'),(2.5,.45,'C5','ai','m',''),
  (2.95,.8,'B4','a','h','nd')],
 # "these are the best of days" (final)
 [(0,.55,'F4','i','th','z'),(.55,.55,'A4','a','','r'),(1.1,.5,'C5','e','th',''),
  (1.6,.5,'E5','e','b','st'),(2.1,.5,'D5','o','','v'),(2.6,.5,'C5','ay','d','z'),
  (3.1,.8,'D5','o','','')],
 # "the best of days"
 [(0,.6,'C5','e','th',''),(.6,.5,'D5','e','b','st'),(1.1,1.9,'C5','o','','v'),
  (3.0,.5,'A4','ay','d','z')],
]
V2 = [
 # "i have walked through broken weather"
 [(0,.5,'C4','ai','',''),(.5,.45,'E4','a','h','v'),(.95,.55,'G4','o','w','k'),
  (1.5,.5,'A4','u','th','r'),(2.0,.5,'C5','o','b','r'),(2.5,.5,'A4','e','k','n'),
  (3.0,.5,'G4','e','w','th'),(3.5,.45,'F4','e','','r')],
 # "i have stood in falling rain"
 [(0,.5,'E4','ai','',''),(.5,.45,'G4','a','h','v'),(.95,.55,'A4','u','st','d'),
  (1.5,.5,'C5','i','','n'),(2.0,.5,'A4','o','f','l'),(2.5,.45,'G4','i','','ng'),
  (2.95,.8,'F4','ay','r','n')],
 # "but the forecast has been changing"
 [(0,.5,'G4','u','b','t'),(.5,.45,'A4','e','th',''),(.95,.55,'C5','o','f','r'),
  (1.5,.5,'C5','a','k','st'),(2.0,.5,'A4','a','h','z'),(2.5,.5,'G4','i','b','n'),
  (3.0,.5,'F4','ay','ch','n'),(3.5,.45,'E4','i','','ng')],
 # "every time you speak my name"
 [(0,.5,'F4','e','','v'),(.5,.45,'G4','i','r',''),(.95,.55,'A4','ai','t','m'),
  (1.5,.5,'C5','u','y',''),(2.0,.5,'B4','i','sp','k'),(2.5,.45,'A4','ai','m',''),
  (2.95,.8,'G4','ay','n','m')],
 # "let the thunder do the talking"
 [(0,.5,'G4','e','l','t'),(.5,.45,'A4','e','th',''),(.95,.55,'C5','u','th','n'),
  (1.5,.5,'C5','e','d','r'),(2.0,.5,'A4','u','d',''),(2.5,.5,'G4','e','th',''),
  (3.0,.5,'F4','o','t','k'),(3.5,.45,'E4','i','','ng')],
 # "i'll be singing in the sound"
 [(0,.5,'F4','ai','','l'),(.5,.45,'G4','i','b',''),(.95,.55,'A4','i','s','ng'),
  (1.5,.5,'C5','i','','ng'),(2.0,.5,'B4','i','','n'),(2.5,.45,'A4','e','th',''),
  (2.95,.8,'G4','ow','s','nd')],
]
BR = [
 # "and when the night comes calling"
 [(0,.5,'G4','a','','n'),(.5,.45,'A4','e','w','n'),(.95,.55,'C5','e','th',''),
  (1.5,.5,'D5','ai','n','t'),(2.0,.5,'C5','u','k','m'),(2.5,.45,'A4','o','k','l'),
  (2.95,.8,'G4','i','','ng')],
 # "with its shadows long and grey"
 [(0,.5,'A4','i','w','th'),(.5,.45,'C5','i','','ts'),(.95,.55,'D5','a','sh',''),
  (1.5,.5,'D5','ow','d',''),(2.0,.5,'C5','o','l','ng'),(2.5,.45,'A4','a','','n'),
  (2.95,.8,'G4','ay','gr','')],
 # "i will keep this light inside me"
 [(0,.5,'A4','ai','',''),(.5,.45,'B4','i','w','l'),(.95,.55,'C5','i','k','p'),
  (1.5,.5,'D5','i','th','s'),(2.0,.5,'E5','ai','l','t'),(2.5,.5,'D5','i','','n'),
  (3.0,.5,'C5','ai','s','d'),(3.5,.45,'B4','i','m','')],
 # "till the light is mine to stay"
 [(0,.55,'C5','i','t','l'),(.55,.5,'D5','e','th',''),(1.05,.55,'E5','ai','l','t'),
  (1.6,.5,'D5','i','','z'),(2.1,.5,'C5','ai','m','n'),(2.6,.5,'B4','u','t',''),
  (3.1,.8,'A4','ay','st','')],
]
CH3 = [shift(l, 2) for l in HOOK]               # whole step up (C -> D)
CODA_1 = [(0,.8,'G4','i','th','z'),(.8,.5,'B4','a','','r'),(1.3,.5,'D5','e','th',''),
          (1.8,.5,'F#5','e','b','st'),(2.3,.45,'E5','o','','v'),(2.75,1.5,'D5','ay','d','z')]

# ---------------------------------------------------------- arrangement
# ---- INTRO: keys + pad over the verse, borrowed E color on bar 4 ----
...
# ---- BRIDGE: the borrowed E comes out, rim groove, build ----
for i in range(3):
    b = S['BRIDGE'] + i * BAR
...
bb = S['BRIDGE'] + 3 * BAR            # Am F: the SWELL begins
for ch, sp in [('Am', 0), ('F', 2)]:
    padat(bus, T(bb + sp), [m + 12 for m in CH[ch][1]], 1.2, atk=0.4,
          cut=2800, seed=40 + sp, gain=0.12, sweep=14)
    keysbar_split(bb, [(ch, sp)], gain=0.14)
    bassbar(bb, ch, BASSL, at=sp)
for k in range(16):
    drm.add(T(bb + k * 0.25), snare(vel=0.2 + 0.035 * k))
drm.add(T(bb + 3.2), riser(1.8, 0.10, 350, 8500))
drm.add(T(bb + 3.95), crash(1.0))

# ---- CH3: THE KEY CHANGE + orchestra swell, everything ----
for i in range(8):
    b = S['CH3'] + i * BAR
    sl = bar_chords(CH3BAR)[i]
    ch0 = chord_at(sl, 0)
    padbar(b, ch0, gain=0.20, cut=5200, atk=0.25, seed=1700 + i, sweep=22)
    keysbar_split(b, sl, gain=0.18)
    for ch, sp in sl:
        bassbar(b, ch, BASS8, at=sp)
    choir(vx, b, CH[ch0][1], gain=0.12, seedbase=1800 + i)
    vchorus(b, i, loud=1.08)
    drm.add(T(b), crash(0.75))
    if i % 2 == 0:
        bus.add(T(b + 0.5), stab([m + 12 for m in CH[ch0][1]], 0.8, 0.16, cut=3000), pan=-0.2)
    hookline(vx, b, CH3[i], gain=1.1, seedbase=1900 + i * 17, halo_g=0.15)

# ---- CODA: outro loop (D A Bm G), final re-sing, held stack, fade ----
for i in range(6):
    b = S['CODA'] + i * BAR
    ch = CODABAR[i]
...
b2 = S['CODA'] + 2 * BAR             # "these are the best of days" over Bm
for ev in CODA_1:
    ...
    choir(vx, b2, CH['Bm'][1], gain=0.08, seedbase=2700)

master(bus.L, bus.R, vx.L, vx.R, 'best-of-days.wav', DUR)
master(bus.L, bus.R, None, None, 'best-of-days-instrumental.wav', DUR)
