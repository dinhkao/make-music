"""03-still-becoming - mot file duy nhat, chi can numpy + scipy.
   Cam hung: "Imaginal Disk" - Magdalena Bay (2024):
   - verse i7-v7-bIII-bVII (Cry For Me), pre-chorus chromatic m7 staircase (Fear Sex)
   - chorus maj7 chain + chromatic II7 (That's My Floor + Killing Time)
   - melody dui chord (tension 87), "loose syncopation" (Hooktheory Killing Time)
   - bridge co F#maj7b5 (Watching TV), outro arp warp + noise (Fear Sex)
   - "la la la" refrain (That's My Floor), wall-of-sound vocal, tambourine indie-pop
   python3 03-still-becoming.py             -> ca hai ban wav (co giong + instrumental)
   python3 03-still-becoming.py --vocals    -> chi ban co giong
   python3 03-still-becoming.py --no-vocals -> chi ban instrumental
"""
...
# ================================================================= BAI 02
# DOUBLE VISION - 4/4, 120bpm, Db major, dreamy-pop (cam hung "Tunnel Vision" - Magdalena Bay)
#
# NGHIEN CUU TUNNEL VISION (Imaginal Disk, 2024, Lewin & Tenenbaum, mix Dave Fridmann):
#   - Key Db, 120 BPM, 4/4, 5:06. Melody range G#3-Bb4, 100% diatonic.
#   - Verse: Db - Dbmaj7 dao, them Gb; V2 mo rong Fm, Gb, Ab, Bbm7.
#   - Chorus: Db - B - Gb - Abm7  (I - bVI - IV - v7: bass xuong nua cung, mo mo)
#   - Post-chorus: Db tinh (repeated 4x) + chant "Can I get out?"
#   - Bridge: giu Db 8 o ("I burn my candle out"), roi breakdown prog-rock
#   - Outro: chromatic loan (Dbm Db ... E Gbm Ab A C Dbm), cat ngot.
#   - Trong: acoustic kit, mo dau "dull restless", chen "intense stabs", cuoi thumping.
#
# Y TUONG: doi lai "tunnel vision" (chi thay mot thu) - bai nay thay TAT CA
# gap doi: hai canh cua giong het nhau, hai loi ra, hai ban than trong guong.
# "two doors, same key" - cau noi voi 01-thirteen-doors (mot cai cua khong dong).
# Narrative: lullaby -> measured -> bung ra (chorus I-bVI-IV-v) -> chant -> fractured
# (V3 stutter) -> breakdown chroma -> "i see double" whisper -> CAT NGOT.
NAME="03-still-becoming"
BAR=4.0                                    # 4/4
SECS=[('INTRO',8),('V1',16),('PRE1',8),('CH1',12),('POST',4),
      ('V2',16),('PRE2',8),('CH2',16),('BRIDGE',8),('CH3',12),('OUT',8),('TAIL',4)]
S={}; _b=0.0
for _n,_c in SECS: S[_n]=_b; _b+=_c*BAR
END=_b
configure(120,120,END)
CUT=T(END)                                 # ket bai bang cat ngot (nhu bai goc)
...
# ============ HOA AM (F#m/A - Imaginal Disk DNA) ============
# verse i7-v7-bIII-bVII (Cry For Me), pre chromatic staircase (Fear Sex),
# chorus maj7 chain + II7 chromatic (That's My Floor + Killing Time),
# bridge F#maj7b5 (Watching TV), outro arp crawl (Fear Sex)
CH={'F#m7': (42,[54,57,61,64]), 'C#m7':(49,[61,64,68,71]),
   'A':    (45,[57,61,64]),    'E':(40,[52,56,59]),
   'Gmaj7':(43,[55,59,62,66]), 'G#m7':(44,[56,59,63,66]),
   'Amaj7':(45,[57,61,64,68]), 'Cmaj7':(48,[60,64,67,71]),
   'B7':   (47,[59,62,66,69]), 'Dmaj7':(50,[62,66,69,73]),
   'F#maj7b5':(42,[54,58,60,64]), 'G':(43,[55,59,62]), 'C':(48,[60,64,67]),
   'F#m':  (42,[54,57,61]),     'Bm7':(47,[59,62,66])}
VCH=['F#m7','C#m7','A','E']*4            # verse
PCH=['F#m7','Gmaj7','G#m7','C#m7']*2     # pre-chorus staircase
CCH=['Amaj7','Gmaj7','Cmaj7','B7']*3     # chorus
LCH=['A','G','C','B7']                   # "la la la" post-chorus
BCH=['Dmaj7','F#maj7b5','Cmaj7','G']*2   # bridge (modulation + chord la)
OCH=['F#m7','Gmaj7','G#m7','C#m7']*2     # outro arp crawl
def bar_at(sec,i): return S[sec]+i*BAR

# ---------- arp harf 8th tren hop am ----------
def harp_arp(b0,ts,g=0.05,seed=0):
    p=[0,1,2,1,0,2,1,2]
...
def stab(b0,ts,g=0.055):
    for off in (2.0,4.0):
        wurli(kbd,T(b0+off),ts[1]+12,0.9,g,det=-4)
        if len(ts)>3: horn(brs,T(b0+off+0.5),ts[-1]+12,0.5,g*0.7,rough=1.3)

# ================= TRONG: pop-dreamy 120bpm, acoustic feel =================
K=Kit(seed=73); P=Performer(K,T,SPB,TOTAL,seed=23,style='indie')
def dr_intro(b0,lvl=0.6,arc=1.0):
    P.K(b0,0,0.75*lvl,arc,tune=45); P.K(b0+3.5,14,0.55*lvl,arc,tune=45)
    ...
    P.WD(b0+1.5,6,0.4*lvl,tune=950,arc=arc); P.WD(b0+3.5,14,0.4*lvl,tune=1150,arc=arc)
def dr_verse(b0,lvl=1.0,arc=1.0,tamb=False):
    P.K(b0,0,1.0*lvl,arc,tune=46)
    P.K(b0+1.5,6,0.62*lvl,arc)
    P.K(b0+2.5,10,0.72*lvl,arc)
    P.K(b0+3.5,14,0.68*lvl,arc)
    P.S(b0+2,8,0.9*lvl,'center',arc); P.S(b0+4,0,0.9*lvl,'center',arc)
    p=0.0
    while p<4.0-1e-6:
        P.H(b0+p,int(p*4)%16,(0.5 if p%1 else 0.8)*lvl,o=0.0,art='tip',arc=arc); p+=0.5
    for gp in (0.75,2.75,3.25): P.S(b0+gp,int(gp*4)%16,0.35*lvl,'ghost',arc)
    if tamb:
        for p in (2.0,4.0): P.TB(b0+p-0.004,int(p*4)%16,0.5*lvl,arc)
def dr_instr(b0,lvl=1.05,arc=1.0):
    for k in range(4): P.K(b0+k,0,0.95*lvl,arc,tune=46)
    P.S(b0+2,8,1.0*lvl,'center',arc); P.S(b0+4,0,1.0*lvl,'center',arc)
    p=0.0
    while p<4.0-1e-6:
        P.H(b0+p,int(p*4)%16,(0.55 if p%1 else 0.85)*lvl,o=0.0,
            art='edge' if p in (0.0,2.0,4.0) else 'tip',arc=arc); p+=0.5
    P.CR(b0,0,0.8*lvl); P.CR(b0+4,0,0.7*lvl)
def dr_chorus(b0,lvl=1.0,arc=1.0,brk=False):
    for k in range(4): P.K(b0+k,0,(0.95 if k%2==0 else 0.8)*lvl,arc,tune=47)
    P.K(b0+2.75,11,0.55*lvl,arc)
    P.S(b0+2,8,1.0*lvl,'center',arc); P.S(b0+4,0,1.0*lvl,'center',arc)
    P.CL(b0+2-0.003,8,0.75*lvl,arc); P.CL(b0+4-0.003,0,0.75*lvl,arc)
    p=0.0
    while p<4.0-1e-6:
        op=(abs(p-3.5)<1e-6)
        P.H(b0+p,int(p*4)%16,(0.8 if p%1 else 0.95)*lvl,o=0.55 if op else 0.0,
    ...
    if brk:                              # pickup stutter cuoi o
        P.S(b0+3.75,15,0.8*lvl,'ghost',arc); P.S(b0+3.875,15,0.9*lvl,'center',arc)
    P.CR(b0,0,0.85*lvl,size=1.05)
def dr_post(b0,lvl=0.7,arc=1.0):
    P.K(b0,0,0.85*lvl,arc,tune=45); P.K(b0+2.5,10,0.7*lvl,arc)
    P.CL(b0+2,8,0.7*lvl,arc); P.CL(b0+4,0,0.7*lvl,arc)
def dr_bridge(b0,lvl=0.8,arc=1.0):
    P.K(b0,0,0.85*lvl,arc,tune=44); P.K(b0+3.5,14,0.7*lvl,arc,tune=44)
    P.S(b0+2,8,0.6*lvl,'rim',arc)
    p=0.0
    while p<4.0-1e-6:
        P.RD(b0+p,int(p*4)%16,0.42*lvl,bell=(p==0),arc=arc); p+=0.5
    P.TM(b0+1.0,4,0.5*lvl,tune=150,arc=arc); P.TM(b0+3.0,12,0.5*lvl,tune=112,arc=arc)
def dr_outro(b0,lvl=1.15,arc=1.0):
    for k in range(4):
        P.K(b0+k,0,0.95*lvl,arc,tune=48)
        if k%2==0: P.CR(b0+k,0,0.55*lvl,size=0.8)
    P.S(b0+1.5,6,0.9*lvl,'center',arc); P.S(b0+2,8,0.9*lvl,'center',arc)
    P.S(b0+3.5,14,0.85*lvl,'center',arc); P.S(b0+4,0,1.0*lvl,'center',arc)
    p=0.0
    while p<4.0-1e-6:
        P.H(b0+p,int(p*4)%16,0.75*lvl,o=0.3 if p%1 else 0.0,
            art='edge' if p%1==0 else 'tip',arc=arc); p+=0.5

# ================= TRONG: sap xep =================
for i in range(8):
    b=bar_at('INTRO',i); a=[1.0,0.97,1.0,1.03][i%4]
    if i>=2: dr_intro(b,0.55+0.06*i,a)
    if i>=6: P.TB(b+1.0,4,0.4,a); P.TB(b+3.0,12,0.4,a)
    if i==7: P.fill(b+2.0,2.0,'snare',0.7,next_crash_beat=S['V1'])
for i in range(16):
    b=bar_at('V1',i); a=[1.0,0.97,1.0,1.02][i%4]
    dr_verse(b,0.95,a,tamb=True)
    if i in (7,15): P.fill(b+2.5,1.5,'roll',0.85)
for i in range(8):
    b=bar_at('PRE1',i); a=[1.0,0.98,1.0,1.02][i%4]
    dr_post(b,0.85,a)
    if i%2==1: P.TB(b+0.5,2,0.35,a); P.TB(b+2.5,10,0.35,a)
    if i==7: P.fill(b,2.0,'roll',1.3,next_crash_beat=S['CH1'])
for i in range(12):
    b=bar_at('CH1',i); a=[1.0,0.97,1.01,1.03][i%4]
    dr_chorus(b,1.02,a,brk=(i%4==3))
    P.TB(b+0.5,2,0.4,a); P.TB(b+2.5,10,0.4,a)
    if i==11: P.fill(b+2.0,2.0,'tom',1.0,next_crash_beat=S['POST'])
for i in range(4):
    b=bar_at('POST',i); a=[1.0,0.98,1.0,1.02][i%4]
    dr_post(b,0.75,a)
    P.TB(b+0.5,2,0.45,a); P.TB(b+2.5,10,0.45,a)
    if i==3: P.fill(b+2.0,2.0,'snare',0.8,next_crash_beat=S['V2'])
for i in range(16):
    b=bar_at('V2',i); a=[1.0,0.97,1.0,1.02][i%4]
    dr_verse(b,0.98,a,tamb=True)
    if i in (7,15): P.fill(b+2.5,1.5,'roll',0.9)
for i in range(8):
    b=bar_at('PRE2',i); a=[1.0,0.98,1.0,1.02][i%4]
    dr_post(b,0.85,a)
    if i%2==1: P.TB(b+0.5,2,0.38,a); P.TB(b+2.5,10,0.38,a)
    if i==7: P.fill(b,2.0,'roll',1.4,next_crash_beat=S['CH2'])
for i in range(16):
    b=bar_at('CH2',i); a=[1.0,0.97,1.01,1.03][i%4]
    dr_chorus(b,1.08,a,brk=(i%4==3))
    P.TB(b+0.5,2,0.42,a); P.TB(b+2.5,10,0.42,a)
    if i==15: P.fill(b+2.0,2.0,'tom',1.1,next_crash_beat=S['BRIDGE'])
for i in range(8):
...
for i in range(4):
    b=bar_at('TAIL',i)
    P.K(b,0,0.6,1.0,tune=45); P.K(b+2.5,10,0.5,1.0,tune=45)
    P.S(b+2,8,0.4,'rim',1.0)
P.apply_chokes()
DRUMS=mix_kit(P.bus,room_amount=0.22,oh_amount=0.90,lofi=0.0,lpf=10000)
P.apply_chokes()
DRUMS=mix_kit(P.bus,room_amount=0.22,oh_amount=0.90,lofi=0.0,lpf=10000)

# ================= SAP XEP NHAC CU =================
noise_sw(fx,0,T(END),0.006,True,60,900)
for i in range(8):                     # INTRO: arp crawl + riff vao bar 4
    b=bar_at('INTRO',i); ch=VCH[i%4]; root,ts=CH[ch]
    harp_arp(b,ts,0.05,seed=i)
    if i>=4:
        riff(b,ts,0.095,seed=i)
        supersaw(pad,T(b),[root+12]+[t+12 for t in ts[:2]],2.0,0.028,atk=1.2,seed=i)
    if i%2==1: wurli(kbd,T(b+2),ts[1]+12,1.2,0.040)
for i in range(16):                    # V1: i7-v7-bIII-bVII (Cry For Me)
    ...
    if i%4 in (2,3): wurli(kbd,T(b+1),ts[1]+12,1.4,0.05)
for i in range(8):                     # PRE1: chromatic staircase (Fear Sex)
    b=bar_at('PRE1',i); ch=PCH[i%4]; root,ts=CH[ch]
    popbass(bs,T(b),root,1.0,0.24); popbass(bs,T(b+1),root+12,1.0,0.18)
    popbass(bs,T(b+2),root,1.0,0.22); popbass(bs,T(b+3),root+7,1.0,0.20)
    supersaw(pad,T(b),[root+12]+[t+12 for t in ts[:2]],2.0,0.032,atk=0.4,seed=i)
    clav(kbd,T(b),ts[1]+12,0.9,0.05,seed=i)
for i in range(12):                    # CH1: maj7 chain + II7 (TMF + Killing Time)
    ...
    supersaw(pad,T(b),[root+12,ts[0]+12,ts[1]+12,ts[2]+12],2.0,0.040,atk=0.12,seed=i)
    riff(b,ts,0.085,seed=i)
    chug(b,ts,0.055,seed=i)
    horn(brs,T(b+1),ts[2]+12,1.2,0.045,rough=1.2)
    if i%4==3: bone(brs,T(b),ts[0]+12,1.6,0.05,growl=0.5)
for i in range(4):                     # POST: "la la la" refrain (TMF DNA)
    b=bar_at('POST',i); ch=LCH[i]; root,ts=CH[ch]
    fuzzbass(bs,T(b),root,2.0,0.15)
    supersaw(pad,T(b),[root+12]+[t+12 for t in ts[:2]],2.0,0.035,atk=0.3,seed=i)
    riff(b,ts,0.10,seed=i)
    chug(b,ts,0.05,seed=i)
for i in range(16):                    # V2: vocal nguoc (memory-style)
    b=bar_at('V2',i); ch=VCH[i%4]; root,ts=CH[ch]
    popbass(bs,T(b),root,2.0,0.30); popbass(bs,T(b+1.5),root+12,1.2,0.22)
    popbass(bs,T(b+2.5),root+7,1.2,0.24); popbass(bs,T(b+3.5),root,1.0,0.26)
    if i%2==0: riff(b,ts,0.075,seed=i)
    harp_arp(b,ts,0.035,seed=i)
    if i%4==3: clav(kbd,T(b+1),ts[1]+12,0.5,0.05,seed=i)
for i in range(8):                     # PRE2
    b=bar_at('PRE2',i); ch=PCH[i%4]; root,ts=CH[ch]
    popbass(bs,T(b),root,1.0,0.24); popbass(bs,T(b+1),root+12,1.0,0.18)
    popbass(bs,T(b+2),root,1.0,0.22); popbass(bs,T(b+3),root+7,1.0,0.20)
    supersaw(pad,T(b),[root+12]+[t+12 for t in ts[:2]],2.0,0.035,atk=0.4,seed=i)
    clav(kbd,T(b),ts[1]+12,0.9,0.05,seed=i)
for i in range(16):                    # CH2: to hon, brass thay phien
    ...
    supersaw(pad,T(b),[root+12,ts[0]+12,ts[1]+12,ts[2]+12],2.0,0.048,atk=0.10,seed=i)
    riff(b,ts,0.09,seed=i)
    chug(b,ts,0.06,seed=i)
    if i%2==0:
        bone(brs,T(b),ts[0]+12,1.6,0.045,growl=0.4)
        horn(brs,T(b+2),ts[1]+12,1.6,0.05,rough=1.3)
for i in range(8):                     # BRIDGE: Dmaj7-F#maj7b5-Cmaj7-G (modulation)
    b=bar_at('BRIDGE',i); ch=BCH[i%4]; root,ts=CH[ch]
    subbass(bs,T(b),root,T(b+2.0)-T(b),0.26)
    supersaw(pad,T(b),[root+12]+[t+12 for t in ts[:3]],2.0,
             0.040 if i<4 else 0.050,atk=1.2 if i<4 else 0.5,seed=i)
    mellotron(kbd,T(b),ts[0]+12,2.0,0.045,kind='choir',seed=i)
    bone(brs,T(b),ts[0]+12,T(b+1.5)-T(b),0.045,growl=0.25)
for i in range(12):                    # CH3: cao trao cuoi
    ...
    supersaw(pad,T(b),[root+12,ts[0]+12,ts[1]+12,ts[2]+12],2.0,0.055,atk=0.08,seed=i)
    riff(b,ts,0.10,seed=i)
    chug(b,ts,0.065,seed=i)
    if i%2==0:
        bone(brs,T(b),ts[0]+12,1.8,0.05,growl=0.6)
        horn(brs,T(b+2),ts[1]+12,1.8,0.055,rough=1.4)
for i in range(8):                     # OUT: arp warp + stutter (Fear Sex)
    b=bar_at('OUT',i); ch=OCH[i%4]; root,ts=CH[ch]
    subbass(bs,T(b),root,T(b+1.5)-T(b),0.22)
    harp_arp(b,ts,0.05,seed=i)
    siren(fx,T(b),nn('C#4'),nn('F#5'),2.5,0.030+0.008*i)
    if i==7: noise_sw(fx,T(b),4.0,0.05,False,100,6000)   # noise collapse
for i in range(4):                     # TAIL: arp mo dan, "still becoming"
    b=bar_at('TAIL',i); root,ts=CH['F#m7']
    harp_arp(b,ts,0.035*(1-i/4),seed=i)
    subbass(bs,T(b),root,T(b+2.0)-T(b),0.18*(1-i/4))
    if i<2: riff(b,ts,0.05*(1-i/4),seed=i)
# ================= GIONG HAT =================
# theme: imaginal disk - metamorphosis. "cut me open, find the wings"
V1A=[
 # "cut me open, find the wings"
 [(0,.75,'F#4','a','t'),(0.75,.25,'F#4','i','m'),(1,.5,'A4','o','n'),(1.5,.5,'A4','i','n'),
  (2,.5,'F#4','a',''),(2.5,.5,'B4','e',''),(3,.5,'C#5','i','s'),(3.5,.5,'B4','i','n')],
 # "i am not the me i knew"
 [(0,.75,'F#4','a',''),(0.75,.25,'A4','a','m'),(1,.5,'A4','a','t'),(1.5,.5,'F#4','e',''),
  (2,.5,'B4','i','m'),(2.5,.5,'A4','i',''),(3,.75,'C#5','u','')],
 # "there's a moth inside my chest"
 [(0,.5,'F#4','e','z'),(0.5,.5,'A4','a',''),(1,.5,'B4','a','t'),(1.5,.5,'F#4','i','n'),
  (2,.5,'A4','a',''),(2.5,.5,'B4','i','m'),(3,.5,'C#5','e',''),(3.5,.5,'B4','e','t')],
 # "waiting for the light to bloom"
 [(0,.75,'F#4','a','t'),(0.75,.25,'A4','i',''),(1,.5,'B4','a',''),(1.5,.5,'F#4','e',''),
  (2,.5,'A4','a',''),(2.5,.5,'B4','u',''),(3,.75,'C#5','u','m')],
 # "fold my body in the dark"
 [(0,.75,'F#4','o','d'),(0.75,.25,'A4','i',''),(1,.5,'A4','o',''),(1.5,.5,'B4','i',''),
  (2,.5,'F#4','i','n'),(2.5,.5,'B4','e',''),(3,.75,'C#5','a','k')],
 # "press the wings against my heart"
 [(0,.5,'F#4','e','s'),(0.5,.5,'A4','e',''),(1,.5,'B4','i','s'),(1.5,.5,'A4','a',''),
  (2,.5,'B4','e',''),(2.5,.5,'F#4','i','m'),(3,.5,'C#5','a',''),(3.5,.5,'B4','a','t')],
 # "growing slowly out of me"
 [(0,.75,'F#4','o',''),(0.75,.25,'A4','i',''),(1,.5,'A4','o',''),(1.5,.5,'B4','i',''),
  (2,.5,'A4','a','t'),(2.5,.5,'F#4','a',''),(3,.75,'C#5','i','m')],
 # "i am not the me i knew"
 [(0,.75,'F#4','a',''),(0.75,.25,'A4','a','m'),(1,.5,'A4','a','t'),(1.5,.5,'F#4','e',''),
  (2,.5,'B4','i','m'),(2.5,.5,'A4','i',''),(3,.75,'C#5','u','')],
]
# CHORUS HOOK: "still becoming" - melody dui chord (9th/11th), vao muon, cuoi giu dai
P1=[(0.5,.5,'A4','i','l'),(1,.25,'B4','i',''),(1.25,.25,'C5','a',''),(1.5,.5,'B4','i',''),
    (2.25,.25,'A4','a','t'),(2.5,.25,'B4','i','m'),(3,.75,'D5','o','')]
P2=[(0.5,.5,'A4','i','l'),(1,.25,'B4','i',''),(1.25,.25,'C5','a',''),(1.5,.5,'B4','i',''),
    (2.25,.25,'A4','a','t'),(2.5,.25,'B4','i','m'),(3,.75,'C5','o','')]
P3=[(0.5,.5,'A4','i','l'),(1,.25,'B4','i',''),(1.25,.25,'C5','a',''),(1.5,.5,'B4','i',''),
    (2.25,.25,'A4','e','t'),(2.5,.25,'B4','i','m'),(3,.75,'E5','o','')]
CHP=[P1,P2,P3]
CHP2=[P1,P2,P3,P2]
# POST: "la la la" (That's My Floor DNA)
LALA=[(0,.5,'A4','a','l'),(0.5,.5,'A4','a','l'),(1,.5,'B4','a','l'),(1.5,.5,'C5','a','l'),
      (2,.5,'B4','a','l'),(2.5,.5,'A4','a','l'),(3,.5,'B4','a','l'),(3.5,.75,'C5','a','l')]
# BRIDGE: croon tren modulation Dmaj7-F#maj7b5-Cmaj7-G
BR1=[(0,.75,'D4','a','t'),(0.75,.25,'F#4','e',''),(1,.5,'A4','o','d'),
     (1.5,.5,'F#4','i','m'),(2,.5,'D4','i','l'),(2.5,.5,'A4','e',''),(3,.75,'C5','a','')]
BR2=[(0,.75,'D4','a',''),(0.75,.25,'F#4','i',''),(1,.5,'A4','e',''),(1.5,.5,'F#4','a',''),
     (2,.5,'D4','i',''),(2.5,.5,'A4','e',''),(3,.75,'C5','a','')]
# OUTRO: whisper "still becoming"
OUTW=[(0,1.0,'F#4','i','l'),(1,1.0,'B4','i',''),(2,1.0,'C#5','a',''),(3,1.5,'B4','i','')]
for i in range(16):
    b=bar_at('V1',i)
    line(vx,b,V1A[i//2],g=0.20,style='deadpan',breath=0.26,seedbase=i*13,jit=0.006)
for i in range(12):
...
for i in range(4):
    b=bar_at('POST',i)
    line(vx,b,LALA,g=0.13,style='croon',breath=0.25,seedbase=800+i*13)
    chant(vx,b,LALA,g=0.10,n=5,spread=14,style='shout',seedbase=810+i)
for i in range(16):
    b=bar_at('V2',i)
    line(vx,b,V1A[(15-i)//2],g=0.21,style='croon',breath=0.30,seedbase=400+i*13,jit=0.007)
for i in range(16):
    b=bar_at('CH2',i); ph=CHP2[i//4]
...
for i in range(8):
    b=bar_at('BRIDGE',i)
    ph=[BR1,BR2][i//4]
    line(vx,b,ph,g=0.17,style='croon',breath=0.34,seedbase=1400+i*13,jit=0.014)
shriek(vx,T(S['BRIDGE']+7*BAR+3.0),nn('E5'),1.2,0.10)
for i in range(12):
    b=bar_at('CH3',i); ph=CHP[i//4]
    line(vx,b,ph,g=0.22,style='croon',oct8=0.40,breath=0.18,seedbase=1500+i*13)
    if i%2==1: chant(vx,b,ph,g=0.17,n=6,spread=18,style='shout',seedbase=1600+i)
for i in range(8):
    b=bar_at('OUT',i)
    line(vx,b,OUTW,g=0.075,style='whisper',breath=0.35,seedbase=1800+i,jit=0.008)
# ================= MIX =================
STEMS=[(pad,0.0,0.60,0.55,0.0),(kbd,0.35,0.58,0.50,0.0),(brs,-0.30,0.55,0.50,0.0),
       (hp_,-0.45,0.62,0.45,7.0),(fx,0.0,1.0,0.0,0.0)]
MAPT=[(n,a,b_,(5.0 if n in('POST','TAIL','OUT') else 6.0 if n=='BRIDGE' else
               3.4 if n.startswith('CH') else 1.0 if n in('INTRO','PRE1','PRE2') else 2.6))
      for n,a,b_ in MAP]
def _build(voc):
