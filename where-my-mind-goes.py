...
# =======================================================================
# SONG — WHERE MY MIND GOES (D major, 86 bpm, 60 bars = 2:47)
#
# Westlife device: the bridge is built on the REAL "Flying Without Wings"
# bridge harmony — Ab7 Db Db+5 Bbm7 Eb | Fm Eb Ab (I7-IV-IV+5-ii7-V |
# vi-V-I with secondary dominants and a chromatic +5), transposed to D:
#   D7 G | G+5 Em7 | A Bm | A D
# and the pre-chorus uses its first half (D7 G | Em7 A). The song that
# "teases" the modulation in its own bridge — here it just resolves home.
#
# Harmony — Pixies "Where Is My Mind?" (1988, key E): the whole song is
# the loop E - C#m - G# - A (I - vi - iii - IV, 2 beats per chord) which
# NEVER resolves (no dominant); the chorus adds E - G# - A - Am - C#m - B
# and the solo sits on a sustained B. Transposed to D:
#   verse/intro: D Bm | F# G        (the loop, 2 chords per bar)
#   chorus:      loop x2, then D F# | G Gm | Bm A, ending on B (the pedal)
#   coda:        the loop to fade (its outro motion)
# The verse bass carries a melody of its own (Kim Deal's part).
# =======================================================================
BPM = 86.0
BEAT = 60.0 / BPM
BAR = 4.0
SECS = [('INTRO',4), ('V1',8), ('PRE1',4), ('CH1',8), ('V2',6),
        ('PRE2',4), ('CH2',8), ('BRIDGE',6), ('CH3',8), ('CODA',4)]
S = {}; _b = 0.0
for _n, _c in SECS:
    S[_n] = _b; _b += _c * BAR
END_BEATS = _b
DUR = T(END_BEATS)
TOTAL = DUR + 3.5
print('WHERE MY MIND GOES  bars', int(END_BEATS / BAR), ' duration', round(DUR, 1), 's')

CH = {
 'D':   (38, (50,54,57,62), 38),   'Bm': (47, (59,62,66,71), 47),
 'F#':  (42, (54,58,61,66), 42),   'G':  (43, (55,59,62,67), 43),
 'D7':  (38, (50,54,57,60), 38),   'Em7':(40, (52,55,59,62), 40),
 'A':   (45, (57,61,64,69), 45),   'Gm': (43, (55,58,62,67), 43),
 'G+5': (43, (55,59,63,67), 43),   'B':  (47, (59,63,66,70), 47),
}
SPLIT = {'DBm': ('D','Bm'), 'F#G': ('F#','G'), 'DF#': ('D','F#'),
         'GGm': ('G','Gm'), 'BmA': ('Bm','A'), 'D7G': ('D7','G'),
         'Em7A': ('Em7','A'), 'G+5Em7': ('G+5','Em7'), 'ABm': ('A','Bm'),
         'AD': ('A','D')}
V1BAR  = ['DBm','F#G'] * 4
PREBAR = ['D7G','Em7A','Bm','A']
CHBAR  = ['DBm','F#G'] * 2 + ['DF#','GGm','BmA','B']
V2BAR  = ['DBm','F#G'] * 3
BRBAR  = ['D7G','G+5Em7','ABm','AD','D','D']
CH3BAR = ['DBm','F#G'] * 2 + ['DF#','GGm','BmA','B']
CODABAR= ['DBm','F#G','DBm','F#G']

# melodic 8th-note verse bass (the Kim Deal part)
BASSL = {
 'D':   [(0,38,.5,.30),(.5,50,.45,.24),(1,42,.5,.26),(1.5,50,.45,.24),
         (2,38,.5,.28),(2.5,45,.45,.24),(3,50,.5,.24),(3.5,45,.45,.24)],
 'Bm':  [(0,47,.5,.30),(.5,59,.45,.24),(1,50,.5,.26),(1.5,59,.45,.24),
         (2,47,.5,.28),(2.5,54,.45,.24),(3,59,.5,.24),(3.5,54,.45,.24)],
 'F#':  [(0,42,.5,.30),(.5,54,.45,.24),(1,46,.5,.26),(1.5,54,.45,.24),
         (2,42,.5,.28),(2.5,49,.45,.24),(3,54,.5,.24),(3.5,49,.45,.24)],
 'G':   [(0,43,.5,.30),(.5,55,.45,.24),(1,47,.5,.26),(1.5,55,.45,.24),
         (2,43,.5,.28),(2.5,50,.45,.24),(3,55,.5,.24),(3.5,50,.45,.24)],
 'D7':  [(0,38,1.5,.32),(1.5,45,1.3,.24)],
 'Em7': [(0,40,1.5,.30),(1.5,47,1.3,.22)],
 'A':   [(0,45,1.5,.32),(1.5,52,1.3,.24)],
 'G+5': [(0,43,1.5,.30),(1.5,55,1.3,.24)],
 'B':   [(0,47,1.7,.32),(1.7,54,1.2,.24)],
 'Gm':  [(0,43,.5,.30),(.5,55,.45,.24),(1,46,.5,.26),(1.5,55,.45,.24),
         (2,43,.5,.28),(2.5,50,.45,.24),(3,55,.5,.24),(3.5,50,.45,.24)],
}
BASSP = {
 'D7':  [(0,38,1.0,.34),(1.0,45,1.8,.26)],
 'G':   [(0,43,1.0,.34),(1.0,50,1.8,.26)],
 'Em7': [(0,40,1.0,.34),(1.0,47,1.8,.26)],
 'A':   [(0,45,1.0,.36),(1.0,52,1.8,.28)],
 'Bm':  [(0,47,1.0,.34),(1.0,54,1.8,.26)],
}
BASS8 = {
 'D':   [(0,38,.5,.32),(.5,45,.45,.24),(1,50,.5,.28),(1.5,45,.45,.24),
         (2,50,.5,.28),(2.5,45,.45,.24),(3,47,.5,.24),(3.5,45,.45,.24)],
 ...
 'F#':  [(0,42,.5,.32),(.5,49,.45,.24),(1,54,.5,.28),(1.5,49,.45,.24),
         (2,54,.5,.28),(2.5,49,.45,.24),(3,51,.5,.24),(3.5,49,.45,.24)],
...
BASSC = {
 'D':  [(0,38,1.7,.30),(1.7,45,1.2,.22)],   'Bm': [(0,47,1.7,.30),(1.7,54,1.2,.22)],
 'F#': [(0,42,1.7,.30),(1.7,49,1.2,.22)],   'G':  [(0,43,1.7,.28),(1.7,50,1.2,.20)],
}

bus = Bus(); drm = Bus(); vx = Bus()

V1 = [
 # "where does my mind go in the morning"
 [(0,.5,'D4','e','w',''),(.5,.45,'D4','u','d','z'),(1.0,.5,'F#4','ai','m',''),
  (1.5,.5,'G4','ai','n','d'),(2.0,.5,'A4','o','g',''),(2.5,.45,'A4','o','','z'),
  (2.95,.5,'G4','i','','n'),(3.45,.5,'F#4','e','th','')],
 # "when the sky begins to clear"
 [(0,.5,'F#4','e','w','n'),(.5,.45,'G4','e','th',''),(.95,.55,'A4','ai','sk',''),
  (1.5,.5,'B4','i','b',''),(2.0,.5,'A4','i','g','n'),(2.5,.45,'G4','z','',''),
  (2.95,.5,'F#4','u','t',''),(3.45,.5,'E4','i','k','l')],
 # "it keeps circling the question"
 [(0,.5,'E4','i','','t'),(.5,.5,'F#4','i','k','p'),(1.0,.55,'G4','u','s','r'),
  (1.55,.5,'A4','u','k','l'),(2.05,.5,'A4','i','','ng'),(2.55,.5,'G4','e','th',''),
  (3.05,.5,'F#4','e','k','w'),(3.55,.45,'E4','e','sh','n')],
 # "that i'm too afraid to hear"
 [(0,.5,'F#4','a','th','t'),(.5,.45,'G4','ai','','m'),(.95,.55,'A4','u','t',''),
  (1.5,.5,'A4','a','',''),(2.0,.5,'G4','ai','f','r'),(2.5,.45,'F#4','e','','d'),
  (2.95,.5,'E4','u','t',''),(3.45,.5,'D4','i','h','r')],
 # "and the answer's in the window"
 [(0,.5,'E4','a','','n'),(.5,.45,'F#4','e','th',''),(.95,.5,'G4','a','','n'),
  (1.45,.55,'A4','e','s','r'),(2.0,.5,'A4','i','','n'),(2.5,.5,'G4','e','th',''),
  (3.0,.5,'F#4','i','w',''),(3.5,.45,'E4','n','d','w')],
 # "where the curtains used to part"
 [(0,.5,'F#4','e','w','r'),(.5,.45,'G4','e','th',''),(.95,.55,'A4','u','k','r'),
  (1.5,.5,'B4','e','t','n'),(1.95,.5,'A4','u','','z'),(2.45,.45,'G4','d','',''),
  (2.9,.5,'F#4','u','t',''),(3.4,.5,'E4','a','p','rtz')],
 # "but it fades like all the others"
 [(0,.5,'E4','u','b','t'),(.5,.45,'F#4','i','','t'),(.95,.5,'G4','ay','f','d'),
  (1.45,.55,'A4','ai','l',''),(2.0,.5,'A4','o','','l'),(2.5,.5,'G4','e','th',''),
  (3.0,.5,'F#4','u','','th'),(3.5,.45,'E4','e','','r')],
 # "when the daylight breaks my heart"
 [(0,.5,'F#4','e','w','n'),(.5,.45,'G4','e','th',''),(.95,.55,'A4','ay','d',''),
  (1.5,.5,'B4','ai','l','t'),(2.0,.5,'A4','ay','br','k'),(2.5,.45,'G4','s','',''),
  (2.95,.5,'F#4','ai','m',''),(3.45,.5,'E4','a','h','rtz')],
]
PRE = [
 # "and i swear that i remember"
 [(0,.5,'B4','a','','n'),(.5,.45,'A4','ai','',''),(.95,.5,'G4','e','s','r'),
  (1.45,.55,'A4','a','th','t'),(2.0,.5,'B4','ai','',''),(2.5,.5,'A4','i','r',''),
  (3.0,.5,'G4','e','m',''),(3.5,.45,'F#4','e','b','r')],
 # "every corner of your face"
 [(0,.5,'G4','e','','v'),(.5,.45,'A4','i','r',''),(.95,.5,'B4','o','k','r'),
  (1.45,.5,'A4','e','','r'),(1.95,.5,'G4','o','','v'),(2.45,.5,'F#4','o','y','r'),
  (2.95,.5,'E4','ay','f','s')],
 # "but the memory is a river"
 [(0,.5,'E4','u','b','t'),(.5,.45,'F#4','e','th',''),(.95,.55,'G4','e','m',''),
  (1.5,.5,'A4','o','m','r'),(2.0,.5,'B4','i','','z'),(2.5,.5,'A4','a','',''),
  (3.0,.5,'G4','i','r','v'),(3.5,.45,'F#4','e','','r')],
 # "that keeps carrying me away"
 [(0,.5,'G4','a','th','t'),(.5,.5,'A4','i','k','p'),(1.0,.55,'B4','a','k','r'),
  (1.55,.5,'A4','i','r',''),(2.05,.5,'G4','i','','ng'),(2.55,.45,'F#4','m','',''),
  (3.0,.5,'E4','a','',''),(3.5,.55,'D4','ay','w','')],
]
HOOK = [
 # "where my mind goes"
 [(0,.8,'G4','e','w','r'),(.8,.6,'A4','ai','m',''),(1.4,.5,'B4','ai','n','d'),
  (1.9,.5,'D5','o','g',''),(2.4,.6,'C#5','o','','z')],
 # "when you're not here"
 [(0,.5,'B4','e','w','n'),(.5,.5,'A4','o','y','r'),(1.0,.55,'G4','o','n','t'),
  (1.55,.5,'F#4','i','h','r'),(2.05,.8,'E4','i','','')],
 # "where my mind goes" (higher landing)
 [(0,.8,'G4','e','w','r'),(.8,.6,'A4','ai','m',''),(1.4,.5,'B4','ai','n','d'),
  (1.9,.5,'E5','o','g',''),(2.4,.6,'D5','o','','z')],
 # "it disappears"
 [(0,.6,'D5','i','','t'),(.6,.55,'C#5','i','d',''),(1.15,.55,'B4','e','s','p'),
  (1.7,.6,'A4','i','','r'),(2.3,.6,'G4','e','','')],
 # "i keep circling the question"
 [(0,.5,'A4','ai','',''),(.5,.5,'B4','i','k','p'),(1.0,.55,'C#5','u','s','r'),
  (1.55,.5,'D5','u','k','l'),(2.05,.5,'C#5','i','','ng'),(2.55,.5,'B4','e','th',''),
  (3.05,.5,'A4','e','k','w'),(3.55,.45,'G4','e','sh','n')],
 # "like a satellite at night"
 [(0,.5,'G4','ai','l',''),(.5,.45,'A4','a','',''),(.95,.5,'B4','a','s','t'),
  (1.45,.55,'D5','e','l','t'),(2.0,.5,'C#5','ai','','t'),(2.5,.5,'B4','a','','t'),
  (3.0,.5,'A4','ai','n','t')],
 # "where my mind goes, mind goes"
 [(0,.55,'B4','e','w','r'),(.55,.55,'A4','ai','m',''),(1.1,.5,'G4','ai','n','d'),
  (1.6,.5,'A4','o','g',''),(2.1,.5,'B4','ai','m',''),(2.6,.5,'C#5','ai','n','d'),
  (3.1,.5,'D5','o','g',''),(3.6,.4,'E5','o','','z')],
 # "out of sight"
 [(0,.6,'D5','ow','','t'),(.6,.5,'C#5','o','','v'),(1.1,1.9,'B4','ai','s','t')],
]
V2 = [
 # "there's a room inside my memory"
 [(0,.5,'D4','e','th','r'),(.5,.45,'F#4','a','',''),(.95,.55,'G4','u','r','m'),
  (1.5,.5,'A4','i','','n'),(2.0,.5,'A4','ai','s','d'),(2.5,.5,'G4','ai','m',''),
  (3.0,.5,'F#4','e','m',''),(3.5,.45,'E4','o','m','r')],
 # "that i never locked the door"
 [(0,.5,'F#4','a','th','t'),(.5,.45,'G4','ai','',''),(.95,.55,'A4','e','n','v'),
  (1.5,.5,'A4','e','','r'),(2.0,.5,'G4','o','l','k'),(2.5,.45,'F#4','e','th',''),
  (2.95,.8,'E4','o','d','r')],
 # "and the echoes of your laughter"
 [(0,.5,'E4','a','','n'),(.5,.45,'F#4','e','th',''),(.95,.5,'G4','e','','ch'),
  (1.45,.5,'A4','o','','z'),(1.95,.5,'A4','o','','v'),(2.45,.5,'G4','o','y','r'),
  (2.95,.5,'F#4','a','l','f'),(3.45,.5,'E4','e','t','r')],
 # "keep on knocking to be sure"
 [(0,.5,'F#4','i','k','p'),(.5,.45,'G4','o','','n'),(.95,.55,'A4','o','n','k'),
  (1.5,.5,'B4','i','','ng'),(2.0,.5,'A4','u','t',''),(2.5,.45,'G4','o','b',''),
  (2.95,.5,'F#4','i','',''),(3.45,.5,'E4','o','sh','r')],
 # "so i'm leaving all the windows"
 [(0,.5,'E4','o','s',''),(.5,.5,'F#4','ai','','m'),(1.0,.55,'G4','i','l','v'),
  (1.55,.5,'A4','i','','ng'),(2.05,.5,'B4','o','','l'),(2.55,.5,'A4','e','th',''),
  (3.05,.5,'G4','i','w',''),(3.55,.45,'F#4','n','d','w')],
 # "open wide for you to find"
 [(0,.5,'G4','o','','p'),(.5,.45,'A4','e','n',''),(.95,.55,'B4','ai','w','d'),
  (1.5,.5,'A4','o','f','r'),(2.0,.5,'G4','u','y',''),(2.5,.45,'F#4','u','t',''),
  (2.95,.8,'E4','ai','f','nd')],
]
BR = [
 # "and i'd carry all the distances"
 [(0,.5,'F#4','a','','n'),(.5,.45,'G4','ai','','d'),(.95,.55,'A4','a','k','r'),
  (1.5,.5,'B4','i','r',''),(2.0,.5,'A4','o','','l'),(2.5,.5,'G4','e','th',''),
  (3.0,.5,'F#4','i','d',''),(3.5,.45,'E4','e','st','n')],
 # "if the road would lead to you"
 [(0,.5,'E4','i','f',''),(.5,.45,'F#4','e','th',''),(.95,.55,'G4','o','r','d'),
  (1.5,.5,'A4','u','w','d'),(2.0,.5,'G4','i','l','d'),(2.5,.45,'F#4','u','t',''),
  (2.95,.8,'E4','u','y','')],
 # "every question, every silence"
 [(0,.5,'G4','e','','v'),(.5,.45,'A4','i','r',''),(.95,.55,'B4','e','k','w'),
  (1.5,.5,'A4','e','sh','n'),(2.0,.5,'G4','e','','v'),(2.5,.45,'F#4','i','r',''),
  (2.95,.5,'E4','ai','s',''),(3.45,.5,'D4','e','l','n')],
 # "all the things i never knew"
 [(0,.5,'E4','o','','l'),(.5,.45,'F#4','e','th',''),(.95,.55,'G4','i','th','ng'),
  (1.5,.5,'A4','ai','',''),(2.0,.5,'G4','e','n','v'),(2.5,.45,'F#4','e','','r'),
  (2.95,.8,'E4','u','n','w')],
]
CODA_1 = [(0,.8,'G4','e','w','r'),(.8,.6,'A4','ai','m',''),(1.4,.5,'B4','ai','n','d'),
          (1.9,.5,'E5','o','g',''),(2.4,1.5,'D5','o','','z')]   # "where my mind goes"

# ---------------------------------------------------------- arrangement
# ---- INTRO: the loop with the melodic bass ----
for i, ch in enumerate(['D','Bm','F#','G']):
    b = S['INTRO'] + i * BAR
...
# ---- CH1: loop x2, then the tail ending on the B pedal ----
for i in range(8):
    b = S['CH1'] + i * BAR
...
# ---- BRIDGE: the full Flying Without Wings bridge, then build ----
for i in range(4):
    b = S['BRIDGE'] + i * BAR
...
bb = S['BRIDGE'] + 4 * BAR                # hold D
padbar(bb, 'D', gain=0.12, cut=3000, atk=0.6, seed=44)
keysarp(bb, 'D', gain=0.14)
bassbar(bb, 'D', BASSL)
for k in range(16):
...
# ---- CH3: everything. the loop that never resolves, louder ----
for i in range(8):
    b = S['CH3'] + i * BAR
    sl = bar_chords(CHBAR)[i]
...
# ---- CODA: the loop to fade (its outro), final re-sing ----
for i in range(4):
    b = S['CODA'] + i * BAR
    sl = bar_chords(CODABAR)[i]
    for ch, sp in sl:
        padbar(b, ch, gain=0.09, cut=2000, atk=1.5, seed=2100 + i)
...
b0 = S['CODA']
for ev in CODA_1:
    line(vx, b0, [ev], gain=1.0, style='croon', seedbase=2200)
    line(vx, b0, [shift([ev], -4)[0]], gain=0.5, style='croon', seedbase=2300)
    line(vx, b0, [shift([ev], -7)[0]], gain=0.4, style='croon', seedbase=2400)
    line(vx, b0, [shift([ev], 12)[0]], gain=0.30, style='croon', seedbase=2500)
    line(vx, b0, [shift([ev], -12)[0]], gain=0.34, style='croon', seedbase=2600)
    choir(vx, b0, CH['D'][1], gain=0.08, seedbase=2700)

master(bus.L, bus.R, vx.L, vx.R, 'where-my-mind-goes.wav', DUR)
master(bus.L, bus.R, None, None, 'where-my-mind-goes-instrumental.wav', DUR)
