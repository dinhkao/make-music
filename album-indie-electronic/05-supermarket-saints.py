"""05-supermarket-saints - mot file duy nhat, chi can numpy + scipy.
   Hoc tu: MGMT "Time to Pretend" (D major, chuyen sang F#m tuong doi, trong tom to,
   shaker 16th) va "Electric Feel" (vong lap thoi mien i-v-VI-VII, groove funk).
   Y do nhac ly:
   - Verse I-IV-V-vi (D G A F#m) nhung hop am DAY SOM nua phach truoc vach nhip
     (anticipation) => nghe lech, khong vuong.
   - Chorus vi-I-V-II (F#m D A E): E la II TRUONG muon tu D lydian / V cua V -
     mau "sai ma dung" sang bung, dac trung neo-psych.
   - Bridge muon bVII (C) va IV thu: G-F-C-G kieu mixolydian.
   - Trong verse KHONG co snare: tom san + shaker, backbeat chi vao o chorus =>
     chorus bung ra nho trong chu khong chi nho hoa am.
   - Ket tren E (II) khong giai quyet + tape stop.
   python3 05-supermarket-saints.py             -> ca hai ban wav
   python3 05-supermarket-saints.py --vocals    -> chi ban co giong
   python3 05-supermarket-saints.py --no-vocals -> chi ban instrumental
"""
...
# ============ HOA AM (D major, muon II truong) ============
CH={'D':  (38,[50,54,57]),   'G':  (43,[47,50,55]),
    'A':  (45,[49,52,57]),   'F#m':(42,[45,49,54]),
    'Bm': (35,[47,50,54]),   'E':  (40,[44,47,52]),
    'D/F#':(42,[50,54,57]),  'C':  (36,[48,52,55]),
    'F':  (41,[45,48,53]),   'Gmaj7':(43,[47,50,54]),
    'Asus4':(45,[50,52,57]), 'Em7':(40,[47,50,55])}
VCH=['D','G','A','F#m']*4
PCH=['Bm','G','D/F#','A']
CCH=['F#m','D','A','E']*3            # vi - I - V - II(!)
BCH=['G','F','C','G']*3              # bVII mixolydian
OCH=['F#m','D','A','E']*3
def bar_at(sec,i): return S[sec]+i*BAR

# ---- hop am day som nua phach (anticipation) ----
def push_chord(b0,ts,g=0.055,seed=0,hold=3.4,det=0.0):
    organ(kbd,T(b0-0.5),[t+12 for t in ts],T(b0+hold)-T(b0-0.5),g)
def wob_pad(b0,root,ts,g=0.05,seed=0,cut=2600):
    supersaw(pad,T(b0-0.5),[root+12]+[t+12 for t in ts],T(b0+4)-T(b0-0.5),g,
             det=14.0,lp=cut,atk=0.35,seed=seed)
# ---- riff lead nhao (MGMT: lead lech tone, wobbly) ----
RIFF=[(0.0,0,0.9),(1.0,2,0.5),(1.75,1,0.5),(2.5,0,0.5),(3.0,2,1.0)]
def lead_riff(b0,ts,g=0.075,seed=0,oct_=12):
    for k,(off,ix,d) in enumerate(RIFF):
        analead(kbd,T(b0+off),ts[ix%len(ts)]+oct_,T(b0+off+d)-T(b0+off),g,
                det=16,cut=2400,vibd=0.009,seed=seed*6+k)
# ---- piano tack gang (MGMT hay dung piano cung nhip guitar) ----
def tack_stab(b0,ts,g=0.055,seed=0):
    for off in (0.0,1.5,2.5):
        for j,m in enumerate(ts):
            tackpiano(kbd,T(b0+off)+j*0.010,m+12,0.6,g*(0.8+0.2*(j==0)),
                      tack=0.8,seed=seed*7+j)
# ---- bass: root + quang 5 nay, kieu funk cham ----
def bass_bounce(b0,root,g=0.26,seed=0,alt=True):
    fingerbass(bs,T(b0),root,0.8,g,seed=seed)
    fingerbass(bs,T(b0+1.5),root+(12 if alt else 0),0.5,g*0.70,seed=seed+1)
    fingerbass(bs,T(b0+2.5),root+7,0.5,g*0.78,seed=seed+2)
    fingerbass(bs,T(b0+3.5),root,0.45,g*0.72,dead=True,seed=seed+3)
def bass_drive(b0,root,g=0.28,seed=0):
    for k,off in enumerate(np.arange(0,4,0.5)):
        fuzzbass(bs,T(b0+float(off)),root+(12 if k==5 else 0),0.42,
                 g*(1.0 if k%2==0 else 0.74),seed=seed*8+k)

# ================= TRONG =================
K=Kit(seed=55); P=Performer(K,T,SPB,TOTAL,seed=29,style='indie')
def dr_toms(b0,lvl=1.0,arc=1.0,sh=True):        # VERSE: khong snare, tom san
    P.K(b0,0,1.0*lvl,arc,tune=44); P.K(b0+0.75,3,0.55*lvl,arc,tune=44)
    P.K(b0+2.5,10,0.85*lvl,arc,tune=44)
    P.TM(b0+2,8,0.85*lvl,tune=96,arc=arc); P.TM(b0+4,0,0.85*lvl,tune=96,arc=arc)
    P.TM(b0+3.5,14,0.42*lvl,tune=130,arc=arc)
    if sh:
        p=0.0
        while p<4.0-1e-6:
            P.SH(b0+p,int(p*4)%16,(0.36 if p%0.5 else 0.50)*lvl,arc); p+=0.25
def dr_pre(b0,lvl=0.95,arc=1.0):
    P.K(b0,0,0.95*lvl,arc,tune=45); P.K(b0+2.5,10,0.8*lvl,arc,tune=45)
    P.TM(b0+2,8,0.7*lvl,tune=110,arc=arc); P.S(b0+4,0,0.75*lvl,'center',arc)
    p=0.0
    while p<4.0-1e-6: P.SH(b0+p,int(p*4)%16,0.45*lvl,arc); p+=0.25
    P.TB(b0+1,4,0.4*lvl,arc); P.TB(b0+3,12,0.4*lvl,arc)
def dr_chorus(b0,lvl=1.05,arc=1.0,ride=False,brk=False):   # snare VAO = bung
    P.K(b0,0,1.0*lvl,arc,tune=45); P.K(b0+1.5,6,0.6*lvl,arc)
    P.K(b0+2.5,10,0.9*lvl,arc); P.K(b0+3.75,15,0.5*lvl,arc)
    P.S(b0+2,8,1.0*lvl,'center',arc); P.S(b0+4,0,1.0*lvl,'center',arc)
    P.CL(b0+2-0.004,8,0.7*lvl,arc); P.CL(b0+4-0.004,0,0.7*lvl,arc)
    p=0.0
    while p<4.0-1e-6:
        if ride: P.RD(b0+p,int(p*4)%16,0.42*lvl,bell=(p%2==0),arc=arc)
        else:
            P.H(b0+p,int(p*4)%16,(0.62 if p%1 else 0.85)*lvl,o=0.0,
                art='edge' if p%1==0 else 'tip',arc=arc)
        p+=0.5
    p=0.0
    while p<4.0-1e-6: P.SH(b0+p,int(p*4)%16,0.34*lvl,arc); p+=0.25
    P.TM(b0+3.5,14,0.45*lvl,tune=100,arc=arc)
    if brk: P.TM(b0+3.75,15,0.6*lvl,tune=84,arc=arc)
    P.CR(b0,0,0.75*lvl,size=1.05)
def dr_bridge(b0,lvl=0.8,arc=1.0,i=0):          # chi tom + shaker, dan len
    P.TM(b0,0,0.85*lvl,tune=92,arc=arc); P.TM(b0+1.5,6,0.55*lvl,tune=118,arc=arc)
    P.TM(b0+2.5,10,0.7*lvl,tune=100,arc=arc)
    if i>=4: P.K(b0,0,0.8*lvl,arc,tune=43); P.K(b0+2.5,10,0.7*lvl,arc,tune=43)
    if i>=8: P.S(b0+4,0,0.8*lvl,'center',arc)
    p=0.0
    while p<4.0-1e-6: P.SH(b0+p,int(p*4)%16,(0.30+0.03*i)*lvl,arc); p+=0.25
def dr_out(b0,lvl=1.1,arc=1.0,i=0):
    dr_chorus(b0,lvl,arc,ride=(i%4>=2))
    if i%2==1: P.TB(b0+0.5,2,0.42*lvl,arc); P.TB(b0+2.5,10,0.42*lvl,arc)

for i in range(8):
    b=bar_at('INTRO',i); a=[1.0,0.98,1.0,1.02][i%4]
    if i>=2: dr_toms(b,0.45+0.08*i,a,sh=(i>=4))
    if i==7: P.fill(b+2,2.0,'tom',0.85,next_crash_beat=S['V1'])
for i in range(16):
    b=bar_at('V1',i); a=[1.0,0.97,1.0,1.02][i%4]
    dr_toms(b,0.95,a)
    if i in (7,15): P.fill(b+3,1.0,'tom',0.9)
for i in range(4):
    b=bar_at('PRE1',i); a=[1.0,0.98,1.0,1.02][i%4]
    dr_pre(b,0.95,a)
    if i==3: P.fill(b+2,2.0,'roll',1.3,next_crash_beat=S['CH1'])
for i in range(8):
    b=bar_at('CH1',i); a=[1.0,0.97,1.01,1.03][i%4]
    dr_chorus(b,1.02,a,brk=(i%4==3))
    if i==7: P.fill(b+3,1.0,'tom',0.95,next_crash_beat=S['V2'])
for i in range(12):
    b=bar_at('V2',i); a=[1.0,0.97,1.0,1.02][i%4]
    dr_toms(b,1.0,a)
    if i>=6: P.TB(b+2,8,0.38,a); P.TB(b+4,0,0.38,a)
    if i in (5,11): P.fill(b+3,1.0,'tom',0.95)
for i in range(4):
    b=bar_at('PRE2',i); a=[1.0,0.98,1.0,1.02][i%4]
    dr_pre(b,1.0,a)
    if i==3: P.fill(b+2,2.0,'roll',1.4,next_crash_beat=S['CH2'])
for i in range(8):
    b=bar_at('CH2',i); a=[1.0,0.97,1.01,1.03][i%4]
    dr_chorus(b,1.06,a,ride=(i>=4),brk=(i%4==3))
    if i==7: P.fill(b+2,2.0,'tom',1.1)
for i in range(12):
    b=bar_at('BRIDGE',i); a=[1.0,0.99,1.0,1.01][i%4]
    dr_bridge(b,0.85,a,i)
    if i==11: P.fill(b,4.0,'trib',1.5,next_crash_beat=S['CH3'])
for i in range(8):
    b=bar_at('CH3',i); a=[1.0,0.98,1.01,1.04][i%4]
    dr_chorus(b,1.12,a,ride=(i>=4),brk=(i%4==3))
    if i%4==0: P.CR(b,0,0.85)
for i in range(12):
    b=bar_at('OUT',i); a=[1.0,0.98,1.0,1.03][i%4]
    dr_out(b,1.10-0.045*i,a,i)
    if i==11: P.fill(b+2,2.0,'roll',0.9)
for i in range(4):
    b=bar_at('TAIL',i)
    if i==0: P.CR(b,0,0.6,size=1.3); P.K(b,0,0.7,tune=44)
P.apply_chokes()
DRUMS=mix_kit(P.bus,room_amount=0.30,oh_amount=1.00,lofi=0.05,lpf=10500)

# ================= NHAC CU =================
crackle(fx,0,T(END),0.022,dens=0.04)
for i in range(8):                                   # INTRO
    b=bar_at('INTRO',i); ch=VCH[i%4]; root,ts=CH[ch]
    wob_pad(b,root,ts,0.026+0.004*i,seed=i,cut=1800+180*i)
    if i>=2: mellotron(kbd,T(b),ts[0]+12,T(b+4)-T(b),0.038,kind='flute',seed=i)
    if i>=4: bass_bounce(b,root,0.18+0.02*i,seed=i)
    if i>=6: lead_riff(b,ts,0.050,seed=i)
for i in range(16):                                  # V1
    b=bar_at('V1',i); ch=VCH[i%4]; root,ts=CH[ch]
    push_chord(b,ts,0.052,seed=i)
    bass_bounce(b,root,0.255,seed=i)
    wob_pad(b,root,ts,0.024,seed=i,cut=2200)
    if i%4==3: lead_riff(b,ts,0.055,seed=i)
    if i>=8: chime12(hp_,T(b+0.5),ts[2]+12,1.6,0.030,seed=i)
for i in range(4):                                   # PRE1
    b=bar_at('PRE1',i); ch=PCH[i%4]; root,ts=CH[ch]
    tack_stab(b,ts,0.050,seed=i)
    bass_bounce(b,root,0.26,seed=i,alt=False)
    wob_pad(b,root,ts,0.030+0.006*i,seed=i,cut=2600+300*i)
    mellotron(kbd,T(b),ts[1]+12,T(b+4)-T(b),0.032,kind='strings',seed=i)
    if i==3: riser(fx,T(b),T(b+4)-T(b),0.050,300,7000,seed=2)
for i in range(8):                                   # CH1
    b=bar_at('CH1',i); ch=CCH[i%4]; root,ts=CH[ch]
    push_chord(b,ts,0.062,seed=i)
    bass_drive(b,root,0.255,seed=i)
    wob_pad(b,root,ts,0.040,seed=i,cut=3400)
    tack_stab(b,ts,0.045,seed=i)
    if i%2==0: chime12(hp_,T(b),ts[2]+12,2.0,0.034,seed=i)
    if i%4==3: lead_riff(b,ts,0.060,seed=i)
for i in range(12):                                  # V2
    b=bar_at('V2',i); ch=VCH[i%4]; root,ts=CH[ch]
    push_chord(b,ts,0.055,seed=i)
    bass_bounce(b,root,0.26,seed=i+20)
    wob_pad(b,root,ts,0.026,seed=i,cut=2400)
    chime12(hp_,T(b+0.5),ts[2]+12,1.6,0.030,seed=i)
    if i%4==3: lead_riff(b,ts,0.058,seed=i)
    if i>=8: horn(brs,T(b+2),ts[0]+12,1.2,0.035,rough=1.1)
for i in range(4):                                   # PRE2
    b=bar_at('PRE2',i); ch=PCH[i%4]; root,ts=CH[ch]
    tack_stab(b,ts,0.055,seed=i)
    bass_bounce(b,root,0.27,seed=i,alt=False)
    wob_pad(b,root,ts,0.034+0.006*i,seed=i,cut=2800+300*i)
    mellotron(kbd,T(b),ts[1]+12,T(b+4)-T(b),0.034,kind='strings',seed=i)
    if i==3: riser(fx,T(b),T(b+4)-T(b),0.058,300,8000,seed=4)
for i in range(8):                                   # CH2
    b=bar_at('CH2',i); ch=CCH[i%4]; root,ts=CH[ch]
    push_chord(b,ts,0.066,seed=i)
    bass_drive(b,root,0.265,seed=i)
    wob_pad(b,root,ts,0.045,seed=i,cut=3800)
    tack_stab(b,ts,0.050,seed=i)
    chime12(hp_,T(b),ts[2]+12,2.0,0.036,seed=i)
    if i%2==1: horn(brs,T(b+1),ts[1]+12,1.4,0.040,rough=1.2)
for i in range(12):                                  # BRIDGE: G-F-C-G mixolydian
    b=bar_at('BRIDGE',i); ch=BCH[i%4]; root,ts=CH[ch]
    subbass(bs,T(b),root-12,T(b+3.2)-T(b),0.20)
    if i<4:
        mellotron(kbd,T(b),ts[0]+12,T(b+4)-T(b),0.042,kind='choir',seed=i)
        marimba(hp_,T(b+1),ts[1]+12,1.2,0.045,metal=True,seed=i)
    elif i<8:
        organ(kbd,T(b),[t+12 for t in ts],T(b+4)-T(b),0.055)
        wob_pad(b,root,ts,0.030,seed=i,cut=2000+200*i)
        marimba(hp_,T(b+1),ts[1]+12,1.2,0.040,metal=True,seed=i)
        bass_bounce(b,root,0.20,seed=i)
    else:
        push_chord(b,ts,0.060,seed=i)
        wob_pad(b,root,ts,0.042,seed=i,cut=2600+250*i)
        bass_drive(b,root,0.24,seed=i)
        lead_riff(b,ts,0.058,seed=i)
        horn(brs,T(b),ts[0]+12,1.8,0.045,rough=1.3)
    if i==11: riser(fx,T(b),T(b+4)-T(b),0.075,260,9000,seed=8)
for i in range(8):                                   # CH3
    b=bar_at('CH3',i); ch=CCH[i%4]; root,ts=CH[ch]
    push_chord(b,ts,0.070,seed=i)
    bass_drive(b,root,0.275,seed=i)
    wob_pad(b,root,ts,0.050,seed=i,cut=4200)
    tack_stab(b,ts,0.055,seed=i)
    chime12(hp_,T(b),ts[2]+12,2.0,0.038,seed=i)
    lead_riff(b,ts,0.062,seed=i)
    if i%2==0: horn(brs,T(b),ts[0]+12,1.8,0.045,rough=1.3)
for i in range(12):                                  # OUT: solo analead lang thang
    b=bar_at('OUT',i); ch=OCH[i%4]; root,ts=CH[ch]
    f=1.0-0.045*i
    push_chord(b,ts,0.060*f,seed=i)
    bass_drive(b,root,0.25*f,seed=i)
    wob_pad(b,root,ts,0.042*f,seed=i,cut=3600)
    # solo: di bo ngau nhien tren pentatonic D, nghe nhu nguoi choi
    _R=np.random.default_rng(900+i)
    _pent=[50,52,54,57,59,62,64,66,69]
    _ix=4
    for off in np.arange(0,4,0.5):
        _ix=int(np.clip(_ix+_R.integers(-2,3),0,len(_pent)-1))
        if _R.random()<0.22: continue
        analead(kbd,T(b+float(off))+float(_R.normal(0,0.012)),_pent[_ix]+12,
                float(_R.uniform(0.3,0.9)),0.052*f,det=15,cut=2800,
                gl=float(_R.choice([0,0,0,2])),seed=i*11+int(off*2))
for i in range(4):                                   # TAIL: dung tren E (II)
    b=bar_at('TAIL',i); root,ts=CH['E']
    if i==0:
        organ(kbd,T(b),[t+12 for t in ts],T(b+7)-T(b),0.060)
        supersaw(pad,T(b),[root+12]+[t+12 for t in ts],T(b+7)-T(b),0.045,
                 det=16,lp=3000,atk=0.8,seed=1)
        subbass(bs,T(b),root-12,T(b+6)-T(b),0.18)
        mellotron(kbd,T(b),ts[0]+12,T(b+7)-T(b),0.040,kind='choir',seed=1)
tapestop(kbd,T(S['TAIL']+2*BAR),2.2,0.85)
tapestop(pad,T(S['TAIL']+2*BAR),2.2,0.85)

# ================= GIONG HAT =================
V1=[
 # "we found a supermarket saint asleep behind the bread"
 [(0,.5,'D4','i','w'),(0.5,.5,'F#4','o','f'),(1,.5,'A4','a',''),(1.5,.5,'A4','u','s'),
  (2,.5,'A4','a','p'),(2.5,.5,'G4','e','m'),(3,.5,'A4','i','k'),(3.5,.5,'F#4','e','s'),
  (5,.5,'E4','a',''),(5.5,.5,'G4','i','s'),(6,.5,'F#4','e','p'),(6.5,1.0,'D4','e','b')],
 # "he sold us little heavens at a dollar for the pair"
 [(0,.5,'D4','i','h'),(0.5,.5,'F#4','o','s'),(1,.5,'A4','u',''),(1.5,.5,'A4','i','l'),
  (2,.5,'B4','e','h'),(2.5,.5,'A4','a','v'),(3,.75,'F#4','a','t'),
  (5,.5,'G4','a','d'),(5.5,.5,'A4','o','l'),(6,.5,'F#4','a','f'),(6.5,1.0,'E4','e','p')],
 # "nobody told us that the batteries were never in the box"
 [(0,.5,'D4','o','n'),(0.5,.5,'D4','o','b'),(1,.5,'F#4','o','t'),(1.5,.5,'A4','u','l'),
  (2,.5,'A4','a','th'),(2.5,.5,'B4','a','b'),(3,.5,'A4','i','r'),(3.5,.5,'F#4','e','z'),
  (5,.5,'G4','e','w'),(5.5,.5,'A4','e','n'),(6,.5,'F#4','i',''),(6.5,1.0,'D4','o','b')],
 # "so we prayed to the light above the frozen aisle"
 [(0,.75,'D4','o','s'),(0.75,.25,'E4','i','w'),(1,.5,'F#4','e','p'),(1.5,.5,'A4','e','d'),
  (2,.5,'A4','u','t'),(2.5,.5,'B4','a','l'),(3,.75,'A4','a','t'),
  (5,.5,'G4','a','b'),(5.5,.5,'F#4','o','v'),(6,.5,'E4','o','f'),(6.5,1.25,'D4','a','')],
]
V2L=[
 # "the saint had plastic wings and a name tag with no name"
 [(0,.5,'D4','a','th'),(0.5,.5,'F#4','e','s'),(1,.5,'A4','a','h'),(1.5,.5,'A4','a','p'),
  (2,.5,'A4','i','t'),(2.5,.5,'B4','i','w'),(3,.75,'A4','a',''),
  (5,.5,'G4','a','n'),(5.5,.5,'A4','a','t'),(6,.5,'F#4','i','w'),(6.5,1.0,'D4','o','n')],
 # "he said the trick is looking like you meant to end up here"
 [(0,.5,'D4','i','h'),(0.5,.5,'F#4','e','s'),(1,.5,'A4','a','th'),(1.5,.5,'B4','i','t'),
  (2,.5,'A4','i',''),(2.5,.5,'A4','u','l'),(3,.75,'F#4','a','l'),
  (5,.5,'G4','u','y'),(5.5,.5,'A4','e','m'),(6,.5,'F#4','e','t'),(6.5,1.0,'E4','i','h')],
 # "and every angel in the freezer had a barcode on the wing"
 [(0,.5,'D4','a',''),(0.5,.5,'F#4','e','v'),(1,.5,'A4','e','n'),(1.5,.5,'A4','i',''),
  (2,.5,'B4','i','f'),(2.5,.5,'A4','a','h'),(3,.75,'F#4','a','b'),
  (5,.5,'G4','o','k'),(5.5,.5,'A4','o','n'),(6,.5,'F#4','a','th'),(6.5,1.0,'D4','i','w')],
]
# CHORUS: cu nhay quang 8 - hat theo duoc (F#m D A E)
CH_A=[(0.5,.5,'D5','o','w'),(1,.75,'D5','o',''),(2,.5,'A4','o','h'),(2.5,.75,'F#4','a',''),
      (4.5,.5,'D5','o','w'),(5,.75,'E5','o',''),(6,1.5,'C#5','a','h')]
CH_B=[(0.5,.5,'D5','o','w'),(1,.75,'D5','o',''),(2,.5,'A4','o','h'),(2.5,.75,'F#4','a',''),
      (4.5,.5,'E5','o','w'),(5,.75,'F#5','o',''),(6,1.75,'E5','a','h')]
PREL=[(0,.5,'F#4','i','w'),(0.5,.5,'A4','o','d'),(1,.5,'B4','a','n'),(1.5,.5,'B4','e','v'),
      (2,.5,'A4','a','p'),(2.5,.5,'B4','e','y'),(3,1.0,'C#5','a','g'),
      (4.5,.5,'B4','a','b'),(5,.5,'A4','i','t'),(5.5,.5,'B4','e','w'),(6,1.5,'A4','o','n')]
BRL=[(0,.75,'G4','a','w'),(1,.5,'A4','i','d'),(1.5,.5,'B4','o','n'),(2,.5,'A4','a','t'),
     (2.5,1.0,'G4','e','k'),
     (4.5,.5,'F4','o','n'),(5,.5,'G4','a',''),(5.5,.5,'A4','i','l'),(6,1.5,'C5','a','m')]
OUTC=[(0.5,.5,'D5','o','w'),(1,.75,'D5','o',''),(2,.5,'A4','o','h'),(2.5,.75,'F#4','a','')]

for i in range(0,16,2):
    line(vx,bar_at('V1',i),V1[(i//2)%4],g=0.20,style='croon',breath=0.28,
         seedbase=i*13,jit=0.011,drag=0.014)
for i in range(0,4,2):
    line(vx,bar_at('PRE1',i),PREL,g=0.185,style='croon',breath=0.24,seedbase=200+i*13,jit=0.010)
for i in range(0,8,2):
    b=bar_at('CH1',i); ph=[CH_A,CH_B][(i//2)%2]
    line(vx,b,ph,g=0.215,style='croon',oct8=0.34,breath=0.22,seedbase=300+i*13,jit=0.012)
    chant(vx,b,ph,g=0.135,n=6,spread=20,style='shout',seedbase=340+i)
for i in range(0,12,2):
    line(vx,bar_at('V2',i),V2L[(i//2)%3],g=0.205,style='croon',breath=0.30,
         seedbase=500+i*13,jit=0.012,drag=0.016)
for i in range(0,4,2):
    line(vx,bar_at('PRE2',i),PREL,g=0.195,style='croon',breath=0.22,seedbase=600+i*13,jit=0.010)
for i in range(0,8,2):
    b=bar_at('CH2',i); ph=[CH_A,CH_B][(i//2)%2]
    line(vx,b,ph,g=0.22,style='croon',oct8=0.40,breath=0.20,seedbase=700+i*13,jit=0.012)
    chant(vx,b,ph,g=0.15,n=6,spread=20,style='shout',seedbase=740+i)
for i in range(0,8,2):
    line(vx,bar_at('BRIDGE',i),BRL,g=0.165,style='croon',breath=0.34,
         seedbase=900+i*13,jit=0.016)
for i in range(8,12,2):
    b=bar_at('BRIDGE',i)
    chant(vx,b,BRL,g=0.13,n=6,spread=22,style='shout',seedbase=960+i)
for i in range(0,8,2):
    b=bar_at('CH3',i); ph=[CH_A,CH_B][(i//2)%2]
    line(vx,b,ph,g=0.23,style='croon',oct8=0.44,breath=0.19,seedbase=1100+i*13,jit=0.012)
    chant(vx,b,ph,g=0.165,n=7,spread=22,style='shout',seedbase=1140+i)
for i in range(0,12,2):
    b=bar_at('OUT',i)
    chant(vx,b,OUTC,g=0.115*(1-i/16),n=5,spread=20,style='shout',seedbase=1300+i)

# ================= MIX =================
STEMS=[(pad,0.0,0.60,0.55,0.0),(kbd,0.28,0.64,0.50,0.0),(brs,-0.34,0.58,0.50,0.0),
       (hp_,-0.40,0.60,0.45,8.0),(fx,0.0,1.0,0.0,0.0)]
MAPT=[(n,a,b_,(5.0 if n in('OUT','TAIL') else 4.6 if n=='BRIDGE' else
               3.2 if n.startswith('CH') else 1.2 if n=='INTRO' else 2.5))
      for n,a,b_ in MAP]
def _build(voc):
    st=mixdown(NAME,vx,STEMS,DRUMS,bs,MAPT,vocals=voc,
        wet=0.26,decay=1.7,wide=1.5,drum_gain=0.80,bass_gain=0.90,crush_amt=0.26,
        rms_target=0.176,vox_gain=1.0,boost_inst=1.12,duck=0.32,tape=0.0045)
