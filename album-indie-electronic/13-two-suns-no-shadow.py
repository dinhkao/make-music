"""13-two-suns-no-shadow - mot file duy nhat, chi can numpy + scipy.
   Bai ket cua album: DOI NHIP + DOI GIONG giua verse va chorus.
   Y do nhac ly:
   - Verse o 5/4 (nhom 3+2), giong Do# thu: C#m - A - B - F#m (i - VI - VII - iv).
     Nhip le lam cau hat luon "hut" mot phach => bon chon.
   - Chorus BUNG sang 4/4 va sang Mi truong (giong song song): E - B - C#m - A.
     Cung bo not, doi trung tam => "mat troi thu hai" moc len, khong doi hoa am la.
   - Bridge o 6/4 (nhom 3+3) tren A - B - C#m: nhip thu ba, nghe nhu lo lung.
   - Coda: gang vocal + ken, roi DUNG TREN B (V cua Mi) - khong giai quyet,
     de ca album treo lai (giong tag cua "The Great Indoors").
   - Trong doi theo nhip: 5/4 danh tom-heavy 3+2; 4/4 danh backbeat day.
   python3 13-two-suns-no-shadow.py             -> ca hai ban wav
   python3 13-two-suns-no-shadow.py --vocals    -> chi ban co giong
   python3 13-two-suns-no-shadow.py --no-vocals -> chi ban instrumental
"""
...
S={}; BARS={}; BLEN={}; _b=0.0
for _n,_L in SECS:
    S[_n]=_b; BARS[_n]=[]; BLEN[_n]=list(_L)
    for _l in _L: BARS[_n].append(_b); _b+=_l
END=_b
configure(100,101,END)
CUT=T(END)
MAP=[(n,S[n],S[n]+sum(L)) for n,L in SECS]
NB={n:len(L) for n,L in SECS}
def bar_at(sec,i): return BARS[sec][i]
def blen(sec,i):   return BLEN[sec][i]
print(NAME,"bars",sum(NB.values()),"beats",END,"->",round(TOTAL,1),"s")

pad=buf(); kbd=buf(); brs=buf(); hp_=buf(); vx=buf(); bs=buf(); fx=buf()

# ============ HOA AM (C#m 5/4 -> E major 4/4) ============
CH={'C#m':(49,[52,56,61]),  'A':(45,[52,57,61]),
    'B':(47,[54,59,63]),    'F#m':(42,[54,57,61]),
    'E':(40,[52,56,59]),    'G#m':(44,[52,56,59]),
    'Amaj7':(45,[52,56,61]),'Bsus4':(47,[54,59,64]),
    'C#m7':(49,[52,56,59]), 'E/G#':(44,[52,56,59]),
...
BCH=['A','B','C#m','A','B','E']
DCH=['E','B','C#m','A']*2
def sec_ch(sec,i,lst): return CH[lst[i%len(lst)]]

# ---- dan keo: nen chinh, chuyen dong cham ----
def strings(b0,ts,L,g=0.045,seed=0,hi=True):
    ns=[ts[1],ts[2]] if hi else [ts[0],ts[1]]
    for j,m in enumerate(ns):
        bowed(pad,T(b0),m+12,T(b0+L)-T(b0),g*(1.0-0.10*j),rough=0.40,
              det=-7+11*j,atk=0.30,seed=seed*3+j)
# ---- organ gospel qua Leslie ----
def orgn(b0,ts,L,g=0.048):
    gospelorgan(kbd,T(b0),[t+12 for t in ts],T(b0+L-0.2)-T(b0),g,drive=1.7)
# ---- piano tack rai ----
def tack_roll(b0,ts,g=0.048,seed=0,off=0.0,spread=0.048):
    for j,m in enumerate(ts):
        tackpiano(kbd,T(b0+off)+j*spread,m,T(b0+off+3.0)-T(b0+off),g*(1.0-0.08*j),
                  tack=0.50,seed=seed*5+j)
# ---- bass 5/4: nhom 3+2 ----
def bass5(b0,root,g=0.26,seed=0):
    for k,(off,iv,vv) in enumerate([(0.0,0,1.0),(1.5,0,0.62),(2.0,7,0.72),
                                    (3.0,0,0.92),(4.0,-5,0.66)]):
        fingerbass(bs,T(b0+off),root+iv,0.75,g*vv,seed=seed*7+k)
def bass4(b0,root,g=0.27,seed=0,eighths=False):
    if eighths:
        for k,off in enumerate(np.arange(0,4,0.5)):
            fingerbass(bs,T(b0+float(off)),root+(12 if k==7 else 0),0.36,
                       g*(1.0 if k%2==0 else 0.76),seed=seed*8+k)
    else:
        for k,(off,iv,vv) in enumerate([(0.0,0,1.0),(1.5,0,0.62),(2.5,7,0.78),(3.5,0,0.70)]):
            fingerbass(bs,T(b0+off),root+iv,0.7,g*vv,seed=seed*7+k)
def bass6(b0,root,g=0.26,seed=0):
    for k,(off,iv,vv) in enumerate([(0.0,0,1.0),(1.5,0,0.6),(3.0,7,0.85),(4.5,0,0.65)]):
        fingerbass(bs,T(b0+off),root+iv,0.9,g*vv,seed=seed*7+k)
# ---- chime + guitar ----
def chimes(b0,ts,g=0.030,seed=0,offs=(1.0,3.0)):
    for k,off in enumerate(offs):
        chime12(hp_,T(b0+off),ts[(k+1)%len(ts)]+12,1.8,g*(1.0-0.12*k),seed=seed*5+k)

# ================= TRONG (doi theo nhip) =================
K=Kit(seed=523); P=Performer(K,T,SPB,TOTAL,seed=113,style='indie')
def dr5(b0,lvl=1.0,arc=1.0,full=True,toms=True):     # 5/4 nhom 3+2
    P.K(b0,0,1.0*lvl,arc,tune=44)
    P.K(b0+3,12,0.92*lvl,arc,tune=44)
    P.S(b0+2,8,0.90*lvl,'center',arc); P.S(b0+4,0,0.85*lvl,'center',arc)
    if toms: P.TM(b0+1.5,6,0.42*lvl,tune=104,arc=arc)
    if full:
        p=0.0
        while p<5.0-1e-6:
            P.H(b0+p,int(p*4)%16,(0.42 if p%1 else 0.66)*lvl,o=0.0,
                art='edge' if p%1==0 else 'tip',arc=arc); p+=0.5
    else:
        for p in (0.5,2.5,4.5): P.SH(b0+p,int(p*4)%16,0.28*lvl,arc)
    P.S(b0+4.5,2,0.20*lvl,'ghost',arc)
def dr5_light(b0,lvl=0.6,arc=1.0):
    P.K(b0,0,0.75*lvl,arc,tune=43); P.K(b0+3,12,0.65*lvl,arc,tune=43)
    P.S(b0+2,8,0.5*lvl,'cross',arc)
    for p in (0.5,1.5,2.5,3.5,4.5): P.SH(b0+p,int(p*4)%16,0.26*lvl,arc)
def dr4(b0,lvl=1.06,arc=1.0,ride=False,brk=False):
    ...
    P.CL(b0+2-0.004,8,0.70*lvl,arc); P.CL(b0+4-0.004,0,0.70*lvl,arc)
    p=0.0
    while p<4.0-1e-6:
        if ride: P.RD(b0+p,int(p*4)%16,0.46*lvl,bell=(p%1==0),arc=arc)
        else:
            op=(abs(p-3.5)<1e-6)
            P.H(b0+p,int(p*4)%16,(0.60 if p%1 else 0.86)*lvl,o=0.5 if op else 0.0,
    ...
    P.CR(b0,0,0.75*lvl,size=1.05)
def dr6(b0,lvl=0.8,arc=1.0,i=0):                    # 6/4 nhom 3+3
    P.K(b0,0,0.95*lvl,arc,tune=43); P.K(b0+3,12,0.88*lvl,arc,tune=43)
    P.S(b0+1.5,6,0.6*lvl,'rim',arc); P.S(b0+4.5,2,0.6*lvl,'rim',arc)
    p=0.0
    while p<6.0-1e-6: P.RD(b0+p,int(p*4)%16,0.40*lvl,bell=(p in (0.0,3.0)),arc=arc); p+=0.5
    if i>=3:
        P.TM(b0+2.5,10,0.5*lvl,tune=118,arc=arc); P.TM(b0+5.5,6,0.5*lvl,tune=96,arc=arc)

for i in range(NB['INTRO']):
    b=bar_at('INTRO',i); a=[1.0,0.99,1.0,1.01][i%4]
    if i>=1: dr5_light(b,0.55+0.12*i,a)
    if i==3: P.fill(b+3,2.0,'tom',0.9,next_crash_beat=S['V1'])
for i in range(NB['V1']):
    b=bar_at('V1',i); a=[1.0,0.98,1.0,1.01][i%4]
    dr5(b,0.92,a,full=(i>=2),toms=True)
    if i==7: P.fill(b+3,2.0,'tom',1.0)
for i in range(NB['PRE1']):
    b=bar_at('PRE1',i); a=[1.0,0.98,1.0,1.01][i%4]
    dr5(b,0.98,a)
    if i==3: P.fill(b+3,2.0,'roll',1.35,next_crash_beat=S['CH1'])
for i in range(NB['CH1']):
    b=bar_at('CH1',i); a=[1.0,0.98,1.01,1.03][i%4]
    dr4(b,1.05,a,brk=(i%4==3))
    if i==7: P.fill(b+2,2.0,'tom',1.0,next_crash_beat=S['V2'])
for i in range(NB['V2']):
    b=bar_at('V2',i); a=[1.0,0.98,1.0,1.01][i%4]
    dr5(b,0.96,a)
    if i==7: P.fill(b+3,2.0,'tom',1.05)
for i in range(NB['PRE2']):
    b=bar_at('PRE2',i); a=[1.0,0.98,1.0,1.01][i%4]
    dr5(b,1.02,a)
    if i==3: P.fill(b+3,2.0,'roll',1.45,next_crash_beat=S['CH2'])
for i in range(NB['CH2']):
    b=bar_at('CH2',i); a=[1.0,0.98,1.01,1.03][i%4]
    dr4(b,1.09,a,ride=(i>=4),brk=(i%4==3))
for i in range(NB['BRIDGE']):
    b=bar_at('BRIDGE',i); a=[1.0,0.99,1.0,1.01][i%4]
    dr6(b,0.82,a,i)
    if i==5: P.fill(b+4,2.0,'roll',1.55,next_crash_beat=S['CH3'])
for i in range(NB['CH3']):
    b=bar_at('CH3',i); a=[1.0,0.98,1.01,1.04][i%4]
    dr4(b,1.13,a,ride=(i>=6),brk=(i%4==3))
    if i%4==0: P.CR(b,0,0.85)
for i in range(NB['CODA']):
    b=bar_at('CODA',i); a=[1.0,0.98,1.0,1.02][i%4]
    dr4(b,1.10-0.10*i,a,ride=(i%2==1))
for i in range(NB['TAIL']):
    b=bar_at('TAIL',i)
    if i==0: P.CR(b,0,0.6,size=1.35); P.K(b,0,0.7,tune=44)
P.apply_chokes()
DRUMS=mix_kit(P.bus,room_amount=0.34,oh_amount=1.00,lofi=0.05,lpf=10200)

# ================= NHAC CU =================
crackle(fx,0,T(END),0.022,dens=0.04)
for i in range(NB['INTRO']):                          # INTRO 5/4
    b=bar_at('INTRO',i); L=blen('INTRO',i); root,ts=sec_ch('INTRO',i,VCH)
    strings(b,ts,L,0.030+0.008*i,seed=i)
    tack_roll(b,ts,0.036+0.005*i,seed=i)
    if i>=1: bass5(b,root,0.18+0.03*i,seed=i)
    if i>=2: chimes(b,ts,0.024,seed=i,offs=(2.0,))
for i in range(NB['V1']):                             # V1 5/4
    b=bar_at('V1',i); L=blen('V1',i); root,ts=sec_ch('V1',i,VCH)
    bass5(b,root,0.255,seed=i)
    strings(b,ts,L,0.038,seed=i)
    tack_roll(b,ts,0.046,seed=i)
    if i>=4: orgn(b,ts,L,0.030)
    if i%2==1: chimes(b,ts,0.026,seed=i)
for i in range(NB['PRE1']):                           # PRE1 5/4
    b=bar_at('PRE1',i); L=blen('PRE1',i); root,ts=sec_ch('PRE1',i,PCH)
    bass5(b,root,0.26,seed=i)
    strings(b,ts,L,0.042+0.006*i,seed=i)
    orgn(b,ts,L,0.038+0.005*i)
    tack_roll(b,ts,0.050,seed=i)
    bone(brs,T(b),ts[0]+12,T(b+3)-T(b),0.034,growl=0.25)
    if i==3: riser(fx,T(b),T(b+L)-T(b),0.060,300,8000,seed=2)
for i in range(NB['CH1']):                            # CH1 4/4 - E major
    b=bar_at('CH1',i); L=blen('CH1',i); root,ts=sec_ch('CH1',i,CCH)
    bass4(b,root,0.275,seed=i,eighths=True)
    strings(b,ts,L,0.048,seed=i)
    orgn(b,ts,L,0.048)
    tack_roll(b,ts,0.052,seed=i)
    chimes(b,ts,0.032,seed=i,offs=(0.0,2.0))
    supersaw(pad,T(b),[root+12]+[t+12 for t in ts],T(b+L)-T(b),0.030,det=9,lp=4200,
             atk=0.08,seed=i)
    if i%4==3: horn(brs,T(b),ts[1]+12,1.6,0.036,rough=1.1)
for i in range(NB['V2']):                             # V2 5/4
    b=bar_at('V2',i); L=blen('V2',i); root,ts=sec_ch('V2',i,VCH)
    bass5(b,root,0.265,seed=i+20)
    strings(b,ts,L,0.042,seed=i)
    tack_roll(b,ts,0.048,seed=i)
    orgn(b,ts,L,0.034)
    chimes(b,ts,0.028,seed=i)
    if i>=4: marimba(hp_,T(b+3.5),ts[2]+12,1.2,0.032,metal=True,seed=i)
for i in range(NB['PRE2']):                           # PRE2 5/4
    b=bar_at('PRE2',i); L=blen('PRE2',i); root,ts=sec_ch('PRE2',i,PCH)
    bass5(b,root,0.27,seed=i)
    strings(b,ts,L,0.046+0.006*i,seed=i)
    orgn(b,ts,L,0.042+0.005*i)
    tack_roll(b,ts,0.054,seed=i)
    bone(brs,T(b),ts[0]+12,T(b+3)-T(b),0.038,growl=0.30)
    if i==3: riser(fx,T(b),T(b+L)-T(b),0.070,300,9000,seed=5)
for i in range(NB['CH2']):                            # CH2
    b=bar_at('CH2',i); L=blen('CH2',i); root,ts=sec_ch('CH2',i,CCH)
    bass4(b,root,0.285,seed=i,eighths=True)
    strings(b,ts,L,0.052,seed=i)
    orgn(b,ts,L,0.054)
    tack_roll(b,ts,0.056,seed=i)
    chimes(b,ts,0.036,seed=i,offs=(0.0,2.0))
    supersaw(pad,T(b),[root+12]+[t+12 for t in ts],T(b+L)-T(b),0.036,det=10,lp=4600,
             atk=0.06,seed=i)
    if i%2==1: horn(brs,T(b),ts[1]+12,1.8,0.038,rough=1.2)
for i in range(NB['BRIDGE']):                         # BRIDGE 6/4
    b=bar_at('BRIDGE',i); L=blen('BRIDGE',i); root,ts=sec_ch('BRIDGE',i,BCH)
    bass6(b,root,0.24,seed=i)
    strings(b,ts,L,0.050,seed=i,hi=(i%2==0))
    mellotron(kbd,T(b),ts[0]+12,T(b+L)-T(b),0.038,kind='choir',seed=i)
    marimba(hp_,T(b+1.5),ts[2]+12,1.6,0.038,metal=True,seed=i)
    if i>=2: orgn(b,ts,L,0.040)
    if i>=4:
        bone(brs,T(b),ts[0]+12,T(b+4)-T(b),0.042,growl=0.4)
        tack_roll(b,ts,0.046,seed=i)
    if i==5: riser(fx,T(b),T(b+L)-T(b),0.085,260,9500,seed=11)
for i in range(NB['CH3']):                            # CH3 dinh
    b=bar_at('CH3',i); L=blen('CH3',i); root,ts=sec_ch('CH3',i,CCH)
    bass4(b,root,0.295,seed=i,eighths=True)
    strings(b,ts,L,0.058,seed=i)
    orgn(b,ts,L,0.060)
    tack_roll(b,ts,0.060,seed=i)
    chimes(b,ts,0.040,seed=i,offs=(0.0,1.5,3.0))
    supersaw(pad,T(b),[root+12]+[t+12 for t in ts],T(b+L)-T(b),0.044,det=11,lp=5000,
             atk=0.05,seed=i)
    horn(brs,T(b),ts[1]+12,1.8,0.040,rough=1.2)
    if i%4==3: bone(brs,T(b+2),ts[0]+12,1.6,0.040,growl=0.5)
for i in range(NB['CODA']):                           # CODA: ken + gang, tan dan
    b=bar_at('CODA',i); L=blen('CODA',i); root,ts=sec_ch('CODA',i,DCH)
    f=1-i/11.0
    bass4(b,root,0.27*f,seed=i,eighths=(i<4))
    strings(b,ts,L,0.052*f,seed=i)
    orgn(b,ts,L,0.052*f)
    chimes(b,ts,0.034*f,seed=i)
    if i<5: horn(brs,T(b),ts[1]+12,1.8,0.042*f,rough=1.2)
    if i<5: bone(brs,T(b+2),ts[0]+12,1.4,0.038*f,growl=0.45)
    if i>=5: tack_roll(b,ts,0.042*f,seed=i,spread=0.075)
for i in range(NB['TAIL']):                           # TAIL: dung tren B (V) - treo
    b=bar_at('TAIL',i); root,ts=CH['Bsus4']
    if i==0:
        strings(b,ts,9.0,0.052,seed=1)
        orgn(b,ts,9.0,0.050)
        subbass(bs,T(b),root-12,T(b+6)-T(b),0.17)
        tack_roll(b,ts,0.044,seed=1,spread=0.10)
        chime12(hp_,T(b+0.5),ts[2]+12,3.0,0.030,seed=1)
        bone(brs,T(b),ts[0]+12,3.0,0.036,growl=0.3)

# ================= GIONG HAT =================
# VERSE 5/4: cau hat luon thieu mot phach => bon chon
V1=[
 # "there are two suns in the window and neither one is warm"
 [(0,.5,'C#5','e','th'),(0.5,.5,'C#5','a','r'),(1,.5,'E5','u','t'),(1.5,.5,'C#5','u','s'),
  (2,.5,'B4','i','n'),(2.5,.5,'C#5','i','w'),(3,1.0,'A4','o','d'),
  (5,.5,'B4','a',''),(5.5,.5,'C#5','i','n'),(6,.5,'A4','u','w'),(6.5,1.0,'G#4','o','w'),
  (8,.75,'F#4','a','m')],
 # "i have been counting shadows but i cannot find my own"
 [(0,.5,'C#5','i',''),(0.5,.5,'E5','a','h'),(1,.5,'E5','i','b'),(1.5,.5,'C#5','a','k'),
  (2,.5,'B4','i','n'),(2.5,.5,'C#5','a','sh'),(3,1.0,'A4','o','d'),
  (5,.5,'B4','u','b'),(5.5,.5,'C#5','i',''),(6,.5,'A4','a','k'),(6.5,.75,'G#4','o','f'),
  (7.5,1.0,'F#4','o','n')],
 # "the light is coming from a place that never learned to end"
 [(0,.5,'C#5','a','th'),(0.5,.5,'C#5','i','l'),(1,.5,'E5','i','z'),(1.5,.5,'F#5','u','k'),
  (2,.5,'E5','i','m'),(2.5,.5,'C#5','o','f'),(3,1.0,'B4','a','p'),
  (5,.5,'A4','a','th'),(5.5,.5,'B4','e','n'),(6,.5,'G#4','e','l'),(6.5,1.25,'F#4','e','n')],
 # "and i am not afraid of it i only want to know"
 [(0,.5,'C#5','a',''),(0.5,.5,'C#5','i',''),(1,.5,'E5','a',''),(1.5,.5,'C#5','o','n'),
  (2,.5,'B4','a','f'),(2.5,1.0,'A4','o','v'),
  (5,.5,'B4','i',''),(5.5,.5,'C#5','o','n'),(6,.5,'A4','a','w'),(6.5,1.25,'G#4','o','n')],
]
V2L=[
 # "somebody told me shadows are a kind of privacy"
 [(0,.5,'C#5','u','s'),(0.5,.5,'C#5','o','m'),(1,.5,'E5','o','b'),(1.5,.5,'C#5','o','t'),
  (2,.5,'B4','i','m'),(2.5,.5,'C#5','a','sh'),(3,1.0,'A4','o','d'),
  (5,.5,'B4','a','r'),(5.5,.5,'C#5','a','k'),(6,.5,'A4','i',''),(6.5,1.25,'G#4','a','p')],
 # "so i stood in the doorway till the morning gave me one"
 [(0,.5,'C#5','o','s'),(0.5,.5,'E5','u','t'),(1,.5,'E5','i','n'),(1.5,.5,'C#5','o','d'),
  (2,.5,'B4','e','w'),(2.5,.5,'C#5','i','t'),(3,1.0,'A4','o','m'),
  (5,.5,'B4','i','n'),(5.5,.5,'C#5','a','g'),(6,.5,'A4','i','m'),(6.5,1.25,'F#4','u','w')],
 # "and every window in the town was burning like a hand"
 [(0,.5,'C#5','a',''),(0.5,.5,'C#5','e','v'),(1,.5,'E5','i','r'),(1.5,.5,'F#5','i','w'),
  (2,.5,'E5','o','d'),(2.5,.5,'C#5','i','n'),(3,1.0,'B4','a','t'),
  (5,.5,'A4','o','w'),(5.5,.5,'B4','e','b'),(6,.5,'G#4','i','l'),(6.5,1.25,'F#4','a','h')],
 # "if there is no shadow then there is nowhere left to stand"
 [(0,.5,'C#5','i',''),(0.5,.5,'C#5','e','th'),(1,.5,'E5','i','z'),(1.5,.5,'C#5','o','n'),
  (2,.5,'B4','a','sh'),(2.5,1.0,'A4','o','d'),
  (5,.5,'B4','e','th'),(5.5,.5,'C#5','o','n'),(6,.5,'A4','e','l'),(6.5,1.25,'G#4','a','s')],
]
# CHORUS 4/4 (E major): cau hat mo ra, giu not dai
CH_A=[(0.5,.75,'B4','o','t'),(1.5,.5,'C#5','u','s'),(2,1.25,'E5','o','n'),
      (4.5,.75,'B4','o','n'),(5.5,.5,'A4','o','sh'),(6,1.75,'G#4','a','d')]
CH_B=[(0.5,.75,'B4','o','t'),(1.5,.5,'C#5','u','s'),(2,1.25,'E5','o','n'),
      (4.5,.75,'F#5','o','n'),(5.5,.5,'E5','o','sh'),(6,1.75,'B4','a','d')]
PREL=[(0,.5,'A4','a','w'),(0.5,.5,'B4','e','n'),(1,.5,'C#5','i','l'),(1.5,.5,'E5','a','m'),
      (2,1.5,'C#5','o','n'),
      (5,.5,'B4','a','w'),(5.5,.5,'C#5','e','n'),(6,.5,'E5','i','l'),(6.5,1.25,'F#5','o','m')]
BRL=[(0,1.0,'E5','o','h'),(1.5,.5,'C#5','a','n'),(2,1.0,'B4','o','m'),
     (3,1.0,'A4','o','w'),(4.5,.5,'B4','a','n'),(5,1.25,'C#5','o','m')]
CODA=[(0.5,.75,'B4','o','t'),(1.5,.5,'C#5','u','s'),(2,1.5,'E5','o','n')]

for i in range(0,NB['V1'],2):
    line(vx,bar_at('V1',i),V1[(i//2)%4],g=0.20,style='croon',breath=0.30,
         seedbase=i*13,jit=0.012,drag=0.014)
for i in range(0,NB['PRE1'],2):
    line(vx,bar_at('PRE1',i),PREL,g=0.19,style='croon',breath=0.26,seedbase=200+i*13,jit=0.011)
for i in range(0,NB['CH1'],2):
    b=bar_at('CH1',i); ph=[CH_A,CH_B][(i//2)%2]
    line(vx,b,ph,g=0.215,style='croon',oct8=0.30,breath=0.24,seedbase=300+i*13,jit=0.011)
for i in range(0,NB['V2'],2):
    line(vx,bar_at('V2',i),V2L[(i//2)%4],g=0.205,style='croon',breath=0.32,
         seedbase=500+i*13,jit=0.013,drag=0.016)
for i in range(0,NB['PRE2'],2):
    line(vx,bar_at('PRE2',i),PREL,g=0.20,style='croon',breath=0.24,seedbase=600+i*13,jit=0.011)
for i in range(0,NB['CH2'],2):
    b=bar_at('CH2',i); ph=[CH_A,CH_B][(i//2)%2]
    line(vx,b,ph,g=0.22,style='croon',oct8=0.36,breath=0.22,seedbase=700+i*13,jit=0.011)
    chant(vx,b,ph,g=0.115,n=5,spread=17,style='croon',seedbase=740+i)
for i in range(0,NB['BRIDGE'],2):
    b=bar_at('BRIDGE',i)
    line(vx,b,BRL,g=0.17,style='croon',breath=0.36,seedbase=900+i*13,jit=0.016)
    if i>=2: chant(vx,b,BRL,g=0.10,n=5,spread=20,style='croon',seedbase=940+i)
for i in range(0,NB['CH3'],2):
    b=bar_at('CH3',i); ph=[CH_A,CH_B][(i//2)%2]
    line(vx,b,ph,g=0.225,style='croon',oct8=0.42,breath=0.20,seedbase=1100+i*13,jit=0.011)
    chant(vx,b,ph,g=0.16,n=7,spread=20,style='shout',seedbase=1140+i)
for i in range(0,NB['CODA'],2):
    b=bar_at('CODA',i)
    f=1-i/10.0
    chant(vx,b,CODA,g=0.15*f,n=8,spread=22,style='shout',seedbase=1300+i)
    line(vx,b,CODA,g=0.14*f,style='croon',oct8=0.30,breath=0.24,seedbase=1340+i*13,jit=0.012)

# ================= MIX =================
STEMS=[(pad,0.0,0.62,0.55,0.0),(kbd,0.28,0.64,0.50,0.0),(brs,-0.36,0.60,0.50,0.0),
       (hp_,-0.44,0.60,0.45,8.0),(fx,0.0,1.0,0.0,0.0)]
MAPT=[(n,a,b_,(4.6 if n in('CODA','TAIL') else 5.0 if n=='BRIDGE' else
               3.2 if n.startswith('CH') else 1.4 if n=='INTRO' else 2.6))
      for n,a,b_ in MAP]
def _build(voc):
    st=mixdown(NAME,vx,STEMS,DRUMS,bs,MAPT,vocals=voc,
        wet=0.30,decay=2.0,wide=1.55,drum_gain=0.78,bass_gain=0.90,crush_amt=0.22,
        rms_target=0.176,vox_gain=1.0,boost_inst=1.12,duck=0.30,tape=0.004)
