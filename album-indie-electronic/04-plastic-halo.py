"""04-plastic-halo - mot file duy nhat, chi can numpy + scipy.
   Hoc tu: Ladytron "Destroy Everything You Touch" (E minor, 120bpm, Em-D-C-B),
   "Seventeen" (giong deadpan mot cao do, hoa am chuyen ben duoi).
   Y do nhac ly:
   - Verse: i-VII-VI-V (Em D C B) = vong Andalusia di xuong. Giai dieu GIU NGUYEN
     not B4 trong khi hop am truot xuong => B4 lan luot la b5-6-M7-1 (cang - chung).
   - Motif nhac cu (E5 B4 E5 F#5) lap Y NGUYEN moi o nhip, khong doi theo hop am:
     cung mot chieu "not dung im, hoa am di".
   - Pre: Am - B7 - Em/D - C (iv - V7 - i6 - VI) keo cang, bo kick.
   - Chorus: xoay vong thanh VI-III-VII-i (C G D Em) => giai quyet ve i o cuoi.
   - Breakdown: A major muon tu E dorian (IV truong) - mau "sai ma dung".
   - Outro dung tren B (V) khong giai quyet.
   python3 04-plastic-halo.py             -> ca hai ban wav (co giong + instrumental)
   python3 04-plastic-halo.py --vocals    -> chi ban co giong
   python3 04-plastic-halo.py --no-vocals -> chi ban instrumental
"""
...
# ============ HOA AM ============
CH={'Em': (40,[52,55,59]),      'D':  (38,[50,54,57]),
    'C':  (36,[48,52,55]),      'B':  (35,[47,51,54]),
    'B7': (35,[47,51,54,57]),   'Am': (33,[45,48,52]),
    'Em/D':(38,[52,55,59]),     'G':  (43,[47,50,55]),
    'A':  (33,[45,49,52]),      'Cmaj7':(36,[48,52,55,59]),
    'Am7':(33,[45,48,52,55]),   'F#m7b5':(42,[45,48,52,54])}
VCH=['Em','D','C','B']*4                 # Andalusia
PCH=['Am','B7','Em/D','C']*2             # iv - V7 - i6 - VI
CCH=['C','G','D','Em']*4                 # VI-III-VII-i (giai quyet ve i)
BCH=['A','A','Cmaj7','Cmaj7','Am7','Am7','B7','B7']   # IV truong muon (E dorian)
OCH=['Em','C','Am7','B']*2
def bar_at(sec,i): return S[sec]+i*BAR

# ---- MOTIF: 4 not lap y nguyen ca bai (earworm) ----
MOT=[(0.0,'E5'),(0.75,'B4'),(1.5,'E5'),(2.5,'F#5')]
def motif(b0,g=0.075,inst='pluck',seed=0,oct_=0):
    for k,(off,nm) in enumerate(MOT):
        t0=T(b0+off); d=T(b0+off+0.6)-t0
        m=nn(nm)+oct_
        if inst=='pluck':  synpluck(kbd,t0,m,d,g,cut=3200,decay=0.30,sub=0.10,seed=seed*4+k)
        elif inst=='bell': bell(hp_,t0,m,d*1.8,g,idx=2.4,seed=seed*4+k)
        else:              analead(kbd,t0,m,d,g,det=11,cut=2600,seed=seed*4+k)
# ---- doi ap: not thu 3 cua hop am, di nguoc chieu motif ----
def counter(b0,ts,g=0.05,seed=0):
    for k,off in enumerate((0.5,1.75,3.0,3.5)):
        m=ts[(2-k)%len(ts)]+12
        bell(hp_,T(b0+off),m,0.9,g*(1.0 if k%2==0 else 0.7),idx=2.0,seed=seed*5+k)
# ---- bass: mach 8th lanh (electro) ----
def bass8(b0,root,g=0.24,oct_jump=False,seed=0):
    for k,off in enumerate(np.arange(0,4,0.5)):
        m=root+(12 if (oct_jump and k in (3,7)) else 0)
        v=g*(1.0 if k%2==0 else 0.78)
        popbass(bs,T(b0+float(off)),m,0.42,v,seed=seed*8+k)
def bass_walk(b0,root,g=0.24,seed=0):
    for off,iv,vv in [(0,0,1.0),(1.5,0,0.7),(2,7,0.8),(2.5,0,0.7),(3.5,-2,0.75)]:
        popbass(bs,T(b0+off),root+iv,0.5,g*vv,seed=seed*9+int(off*2))

# ================= TRONG =================
K=Kit(seed=41); P=Performer(K,T,SPB,TOTAL,seed=17,style='indie')
def dr_machine(b0,lvl=0.55,arc=1.0):        # intro: hop trong may, kho, deu
    for k in range(4): P.K(b0+k,0,0.62*lvl,arc,mode='elec',tune=44)
    P.S(b0+2,8,0.42*lvl,'rim',arc); P.S(b0+4,0,0.42*lvl,'rim',arc)
    for p in (0.5,1.5,2.5,3.5): P.H(b0+p,int(p*4)%16,0.28*lvl,o=0.0,art='tip',arc=arc)
def dr_verse(b0,lvl=1.0,arc=1.0,tamb=False):
    for k in range(4): P.K(b0+k,0,(0.95 if k%2==0 else 0.82)*lvl,arc,tune=45)
    P.S(b0+2,8,0.55*lvl,'rim',arc); P.S(b0+4,0,0.55*lvl,'rim',arc)
    P.CL(b0+2-0.004,8,0.42*lvl,arc); P.CL(b0+4-0.004,0,0.42*lvl,arc)
    for p in (0.5,1.5,2.5,3.5):                       # hat lech phach (offbeat)
        op=(abs(p-3.5)<1e-6)
        P.H(b0+p,int(p*4)%16,0.46*lvl,o=0.42 if op else 0.0,art='tip',arc=arc,
            choke_beat=(b0+4) if op else None)
    P.S(b0+3.25,13,0.20*lvl,'ghost',arc)
    if tamb:
        for p in (1.0,3.0): P.TB(b0+p,int(p*4)%16,0.30*lvl,arc)
def dr_pre(b0,lvl=0.85,arc=1.0,i=0):        # BO KICK: cang, treo
    P.S(b0+2,8,0.62*lvl,'cross',arc); P.S(b0+4,0,0.62*lvl,'cross',arc)
    p=0.0
    while p<4.0-1e-6:
        P.H(b0+p,int(p*4)%16,(0.34 if p%0.5 else 0.50)*lvl,o=0.0,art='tip',arc=arc); p+=0.25
    if i>=4: P.K(b0,0,0.55*lvl,arc,tune=44)
    for p in (0.75,2.75): P.SH(b0+p,int(p*4)%16,0.28*lvl,arc)
def dr_chorus(b0,lvl=1.05,arc=1.0,fast=False,brk=False):
    for k in range(4): P.K(b0+k,0,(1.0 if k%2==0 else 0.86)*lvl,arc,tune=46)
    P.K(b0+2.75,11,0.5*lvl,arc)
    P.S(b0+2,8,0.95*lvl,'center',arc); P.S(b0+4,0,0.95*lvl,'center',arc)
    P.CL(b0+2-0.003,8,0.80*lvl,arc); P.CL(b0+4-0.003,0,0.80*lvl,arc)
    step=0.25 if fast else 0.5
    p=0.0
    while p<4.0-1e-6:
        op=(abs(p-3.5)<1e-6)
        P.H(b0+p,int(p*4)%16,(0.72 if p%1 else 0.88)*lvl,o=0.55 if op else 0.0,
            art='edge' if p%1==0 else 'tip',arc=arc,choke_beat=(b0+4) if op else None)
        p+=step
    for p in (0.5,1.5,2.5,3.5): P.TB(b0+p,int(p*4)%16,0.40*lvl,arc)
    if brk: P.S(b0+3.75,15,0.72*lvl,'ghost',arc)
    P.CR(b0,0,0.72*lvl,size=1.0)
def dr_brk(b0,lvl=0.7,arc=1.0,i=0):         # breakdown: khong kick, chi hat + rim
    P.S(b0+2,8,0.5*lvl,'rim',arc); P.S(b0+4,0,0.5*lvl,'rim',arc)
    p=0.0
    while p<4.0-1e-6:
        P.H(b0+p,int(p*4)%16,0.30*lvl*(0.6+0.1*i),o=0.0,art='foot',arc=arc); p+=0.5
    if i>=4:
        P.K(b0,0,0.60*lvl,arc,tune=43); P.K(b0+2.5,10,0.48*lvl,arc,tune=43)
def dr_out(b0,lvl=1.0,arc=1.0):
    for k in range(4): P.K(b0+k,0,0.90*lvl,arc,tune=46)
    P.S(b0+2,8,0.85*lvl,'center',arc); P.S(b0+4,0,0.85*lvl,'center',arc)
    p=0.0
    while p<4.0-1e-6:
        P.H(b0+p,int(p*4)%16,0.55*lvl,o=0.0,art='tip',arc=arc); p+=0.25
    P.CL(b0+4,0,0.5*lvl,arc)

for i in range(8):
    b=bar_at('INTRO',i); a=[1.0,0.98,1.0,1.02][i%4]
    if i>=4: dr_machine(b,0.5+0.1*i,a)
    if i==7: P.fill(b+3,1.0,'snare',0.6,next_crash_beat=S['V1'])
for i in range(16):
    b=bar_at('V1',i); a=[1.0,0.97,1.0,1.02][i%4]
    dr_verse(b,0.92,a,tamb=(i>=8))
    if i in (7,15): P.fill(b+3.0,1.0,'stutter',0.75)
for i in range(8):
    b=bar_at('PRE1',i); a=[1.0,0.98,1.0,1.02][i%4]
    dr_pre(b,0.85,a,i)
    if i==7: P.fill(b+2.0,2.0,'roll',1.25,next_crash_beat=S['CH1'])
for i in range(8):
    b=bar_at('CH1',i); a=[1.0,0.97,1.01,1.03][i%4]
    dr_chorus(b,1.02,a,brk=(i%4==3))
    if i==7: P.fill(b+3.0,1.0,'snare',0.9,next_crash_beat=S['V2'])
for i in range(12):
    b=bar_at('V2',i); a=[1.0,0.97,1.0,1.02][i%4]
    dr_verse(b,0.96,a,tamb=True)
    if i in (7,11): P.fill(b+3.0,1.0,'stutter',0.85)
for i in range(8):
    b=bar_at('PRE2',i); a=[1.0,0.98,1.0,1.02][i%4]
    dr_pre(b,0.90,a,i)
    if i==7: P.fill(b+2.0,2.0,'roll',1.35,next_crash_beat=S['CH2'])
for i in range(8):
    b=bar_at('CH2',i); a=[1.0,0.97,1.01,1.03][i%4]
    dr_chorus(b,1.06,a,brk=(i%4==3))
    if i==7: P.fill(b+2.0,2.0,'tom',1.1)
for i in range(8):
    b=bar_at('BRK',i); a=[1.0,0.99,1.0,1.01][i%4]
    dr_brk(b,0.75,a,i)
for i in range(4):
    b=bar_at('LIFT',i); a=[1.0,0.98,1.0,1.02][i%4]
    dr_pre(b,0.95,a,i+4)
    if i==3: P.fill(b,4.0,'roll',1.5,next_crash_beat=S['CH3'])
for i in range(12):
    b=bar_at('CH3',i); a=[1.0,0.98,1.01,1.04][i%4]
    dr_chorus(b,1.10,a,fast=(i>=4),brk=(i%4==3))
    if i%4==0: P.CR(b,0,0.8)
    if i==11: P.fill(b+2.0,2.0,'tom',1.2,next_crash_beat=S['OUT'])
for i in range(8):
    b=bar_at('OUT',i); a=[1.0,0.98,1.0,1.02][i%4]
    dr_out(b,1.0-0.07*i,a)
for i in range(4):
    b=bar_at('TAIL',i)
    if i<2: P.H(b,0,0.22*(1-i*0.4),o=0.0,art='foot')
P.apply_chokes()
DRUMS=mix_kit(P.bus,room_amount=0.20,oh_amount=0.80,lofi=0.08,lpf=11000)

# ================= NHAC CU =================
crackle(fx,0,T(END),0.030,dens=0.05)
for i in range(8):                                   # INTRO
    b=bar_at('INTRO',i); ch=VCH[i%4]; root,ts=CH[ch]
    gatedpad(pad,T(b),[t+12 for t in ts],T(b+4)-T(b),0.030+0.004*i,
             rate=8.0,duty=0.44,bpm=122,seed=i)
    if i>=2: motif(b,0.055+0.006*i,'bell',seed=i)
    if i>=4: bass8(b,root,0.16+0.02*i,seed=i)
    if i>=6: motif(b,0.055,'pluck',seed=i+9)
for i in range(16):                                  # V1
    b=bar_at('V1',i); ch=VCH[i%4]; root,ts=CH[ch]
    bass8(b,root,0.235,oct_jump=(i%4==3),seed=i)
    motif(b,0.062,'pluck',seed=i)
    gatedpad(pad,T(b),[t+12 for t in ts],T(b+4)-T(b),0.026,rate=8.0,duty=0.42,bpm=122,seed=i)
    if i>=8: counter(b,ts,0.030,seed=i)
    if i%8==7: bell(hp_,T(b+3.5),ts[1]+24,1.2,0.035,seed=i)
for i in range(8):                                   # PRE1
    b=bar_at('PRE1',i); ch=PCH[i%4]; root,ts=CH[ch]
    supersaw(pad,T(b),[root+12]+[t+12 for t in ts],T(b+4)-T(b),
             0.024+0.004*i,det=8.0,lp=3000+180*i,atk=0.5,seed=i)
    bass_walk(b,root,0.20+0.012*i,seed=i)
    for off in (0.0,1.0,2.0,3.0):
        synpluck(kbd,T(b+off),ts[i%len(ts)]+12,0.5,0.040,cut=2400,decay=0.22,seed=i*3+int(off))
    if i>=4: motif(b,0.050,'pluck',seed=i+20)
    if i==7: riser(fx,T(b),T(b+4)-T(b),0.055,300,7000,seed=3)
for i in range(8):                                   # CH1
    b=bar_at('CH1',i); ch=CCH[i%4]; root,ts=CH[ch]
    supersaw(pad,T(b),[root+12,ts[0]+12,ts[1]+12,ts[2]+12],T(b+4)-T(b),0.040,
             det=9.0,lp=4400,atk=0.06,seed=i)
    bass8(b,root,0.265,oct_jump=True,seed=i)
    motif(b,0.070,'pluck',seed=i)
    counter(b,ts,0.036,seed=i)
    gatedpad(pad,T(b),[t+24 for t in ts[:2]],T(b+4)-T(b),0.018,rate=16.0,duty=0.40,bpm=122,seed=i)
for i in range(12):                                  # V2
    b=bar_at('V2',i); ch=VCH[i%4]; root,ts=CH[ch]
    bass8(b,root,0.245,oct_jump=(i%4==3),seed=i+30)
    motif(b,0.062,'pluck',seed=i)
    if i%4==1: motif(b,0.030,'bell',seed=i+5,oct_=12)
    gatedpad(pad,T(b),[t+12 for t in ts],T(b+4)-T(b),0.028,rate=8.0,duty=0.42,bpm=122,seed=i)
    counter(b,ts,0.034,seed=i)
    if i>=8: analead(kbd,T(b+2.5),ts[2]+12,1.1,0.045,det=12,cut=2200,seed=i)
for i in range(8):                                   # PRE2
    b=bar_at('PRE2',i); ch=PCH[i%4]; root,ts=CH[ch]
    supersaw(pad,T(b),[root+12]+[t+12 for t in ts],T(b+4)-T(b),
             0.026+0.005*i,det=8.0,lp=3200+200*i,atk=0.45,seed=i)
    bass_walk(b,root,0.22+0.012*i,seed=i)
    for off in (0.0,1.0,2.0,3.0):
        synpluck(kbd,T(b+off),ts[i%len(ts)]+12,0.5,0.044,cut=2600,decay=0.22,seed=i*3+int(off))
    if i==7: riser(fx,T(b),T(b+4)-T(b),0.060,300,8000,seed=5)
for i in range(8):                                   # CH2
    b=bar_at('CH2',i); ch=CCH[i%4]; root,ts=CH[ch]
    supersaw(pad,T(b),[root+12,ts[0]+12,ts[1]+12,ts[2]+12],T(b+4)-T(b),0.046,
             det=10.0,lp=4800,atk=0.05,seed=i)
    bass8(b,root,0.275,oct_jump=True,seed=i)
    motif(b,0.074,'pluck',seed=i)
    motif(b,0.030,'bell',seed=i+40,oct_=12)
    counter(b,ts,0.038,seed=i)
    gatedpad(pad,T(b),[t+24 for t in ts[:2]],T(b+4)-T(b),0.020,rate=16.0,duty=0.40,bpm=122,seed=i)
for i in range(8):                                   # BRK: A truong muon (E dorian)
    b=bar_at('BRK',i); ch=BCH[i%8]; root,ts=CH[ch]
    gatedpad(pad,T(b),[t+12 for t in ts],T(b+4)-T(b),0.034,rate=4.0,duty=0.55,bpm=122,
             cut=2400+150*i,seed=i)
    mellotron(kbd,T(b),ts[0]+12,T(b+4)-T(b),0.030,kind='choir',seed=i)
    subbass(bs,T(b),root,T(b+3.0)-T(b),0.20)
    if i>=2: bell(hp_,T(b+2.0),ts[1]+24,1.6,0.032,idx=2.8,seed=i)
    if i==5: subdrop(fx,T(b+2),nn('E2'),1.8,0.10)
for i in range(4):                                   # LIFT
    b=bar_at('LIFT',i); ch=['Am7','Am7','B7','B7'][i]; root,ts=CH[ch]
    supersaw(pad,T(b),[root+12]+[t+12 for t in ts],T(b+4)-T(b),0.030+0.008*i,
             det=9.0,lp=3000+400*i,atk=0.3,seed=i)
    bass_walk(b,root,0.22+0.02*i,seed=i)
    motif(b,0.055+0.008*i,'pluck',seed=i)
    if i==3: riser(fx,T(b),T(b+4)-T(b),0.085,260,9000,seed=11)
for i in range(12):                                  # CH3: dinh
    b=bar_at('CH3',i); ch=CCH[i%4]; root,ts=CH[ch]
    supersaw(pad,T(b),[root+12,ts[0]+12,ts[1]+12,ts[2]+12],T(b+4)-T(b),0.052,
             det=11.0,lp=5200,atk=0.04,seed=i)
    bass8(b,root,0.285,oct_jump=True,seed=i)
    motif(b,0.078,'pluck',seed=i)
    motif(b,0.034,'bell',seed=i+60,oct_=12)
    counter(b,ts,0.040,seed=i)
    gatedpad(pad,T(b),[t+24 for t in ts[:2]],T(b+4)-T(b),0.022,rate=16.0,duty=0.38,bpm=122,seed=i)
    if i>=4: analead(kbd,T(b),ts[2]+24,T(b+2)-T(b),0.036,det=13,cut=3400,seed=i)
for i in range(8):                                   # OUT: tan ra
    b=bar_at('OUT',i); ch=OCH[i%4]; root,ts=CH[ch]
    f=1-i/9.0
    bass8(b,root,0.22*f,seed=i)
    motif(b,0.062*f,'pluck',seed=i)
    gatedpad(pad,T(b),[t+12 for t in ts],T(b+4)-T(b),0.026*f,rate=8.0,duty=0.42,bpm=122,seed=i)
    if i%2==1: counter(b,ts,0.030*f,seed=i)
for i in range(4):                                   # TAIL: dung tren B (V)
    b=bar_at('TAIL',i); root,ts=CH['B']
    if i==0:
        bell(hp_,T(b),nn('B4'),3.0,0.055,idx=2.2,seed=1)
        bell(hp_,T(b+0.75),nn('F#5'),3.0,0.040,idx=2.0,seed=2)
        subbass(bs,T(b),root-12,T(b+3.5)-T(b),0.16)
        gatedpad(pad,T(b),[t+12 for t in ts],T(b+8)-T(b),0.022,rate=2.0,duty=0.7,bpm=122,seed=9)
    if i==2: bell(hp_,T(b),nn('E5'),3.0,0.028,idx=1.8,seed=3)
# lofi cuoi bai
_o=int(T(S['OUT'])*SR); _e=int(T(END)*SR)
kbd[_o:_e]=bitcrush(kbd[_o:_e],bits=7,srdiv=3,mix=0.55)
tapestop(kbd,T(S['TAIL']+2*BAR),1.4,0.8)

# ================= GIONG HAT =================
# Deadpan Ladytron: not gan nhu dung im, hoa am truot ben duoi.
V1=[
 # "i wear a plastic halo when the lights go on"
 [(0,.5,'B4','i',''),(0.5,.5,'B4','e','w'),(1,.5,'B4','a',''),(1.75,.5,'B4','a','p'),
  (2.25,.5,'B4','i','t'),(2.75,.5,'A4','a','h'),(3.25,1.0,'B4','o','l'),
  (5,.5,'B4','e','w'),(5.5,.5,'B4','e','n'),(6,.5,'A4','a','l'),(6.5,.5,'B4','o','g'),
  (7,.75,'G4','o','')],
 # "nobody has to know that nothing is wrong"
 [(0,.5,'B4','o','n'),(0.5,.5,'B4','o','b'),(1,.5,'B4','i','d'),(1.5,.5,'B4','a','h'),
  (2,.5,'A4','u','t'),(2.5,.75,'B4','o','n'),
  (4.5,.5,'B4','a','th'),(5,.5,'B4','o','n'),(5.5,.5,'B4','i','th'),(6,.5,'A4','i','z'),
  (6.5,1.0,'G4','o','r')],
 # "the wire in my collarbone is humming like a phone"
 [(0,.5,'B4','a','th'),(0.5,.5,'B4','i','w'),(1,.5,'B4','e','r'),(1.5,.5,'B4','i','n'),
  (2,.5,'B4','a','m'),(2.5,.5,'A4','o','k'),(3,.5,'B4','a','l'),(3.5,.5,'B4','o','b'),
  (5,.5,'B4','i','z'),(5.5,.5,'B4','a','h'),(6,.5,'A4','i','m'),(6.5,1.0,'E5','o','f')],
 # "i keep it on so no one has to see me on my own"
 [(0,.5,'B4','i',''),(0.5,.5,'B4','i','k'),(1,.5,'B4','i','t'),(1.5,.5,'B4','o','n'),
  (2,.5,'A4','o','s'),(2.5,.5,'B4','o','n'),(3,.75,'B4','a','w'),
  (5,.5,'B4','a','h'),(5.5,.5,'B4','u','t'),(6,.5,'A4','i','s'),(6.5,1.25,'F#4','o','m')],
]
V2L=[
 # "the mirror keeps a copy of the face i left behind"
 [(0,.5,'B4','a','th'),(0.5,.5,'B4','i','m'),(1,.5,'B4','o','r'),(1.5,.5,'B4','i','k'),
  (2,.5,'B4','a','p'),(2.5,.5,'A4','o','k'),(3,.75,'B4','i','p'),
  (5,.5,'B4','a','th'),(5.5,.5,'B4','e','f'),(6,.5,'A4','e','l'),(6.5,1.0,'G4','a','b')],
 # "every time i smile it takes a little longer to arrive"
 [(0,.5,'B4','e',''),(0.5,.5,'B4','i','v'),(1,.5,'B4','a','t'),(1.5,.5,'B4','i',''),
  (2,.5,'A4','a','s'),(2.5,.75,'B4','i','l'),
  (4.5,.5,'B4','i','t'),(5,.5,'B4','e','k'),(5.5,.5,'B4','a',''),(6,.5,'A4','i','l'),
  (6.5,1.0,'E5','o','l')],
 # "a plastic halo doesn't rust it only gets a little dull"
 [(0,.5,'B4','a',''),(0.5,.5,'B4','a','p'),(1,.5,'B4','i','t'),(1.5,.5,'B4','a','h'),
  (2,.5,'B4','o','l'),(2.5,.5,'A4','u','d'),(3,.75,'B4','u','r'),
  (5,.5,'B4','i','t'),(5.5,.5,'B4','o','n'),(6,.5,'A4','i','l'),(6.5,1.0,'G4','u','d')],
]
# CHORUS: cuoi cung giai dieu MOI DI - di xuong roi giu dai
CH_A=[(0.5,.5,'G5','a','p'),(1,.5,'F#5','i','l'),(1.5,.5,'E5','i','t'),(2,.75,'B4','a','h'),
      (3,.5,'B4','o','l'),
      (4.5,.5,'G5','a','b'),(5,.5,'F#5','i','n'),(5.5,.5,'E5','o',''),(6,1.25,'D5','o','s')]
CH_B=[(0.5,.5,'G5','a','p'),(1,.5,'F#5','i','l'),(1.5,.5,'E5','i','t'),(2,.75,'B4','a','h'),
      (3,.5,'B4','o','l'),
      (4.5,.5,'A5','a','b'),(5,.5,'G5','i','n'),(5.5,.5,'F#5','o',''),(6,1.5,'E5','o','s')]
# BRK: whisper tren A truong muon
BRW=[(0,1.0,'E5','i','w'),(1.5,1.0,'C#5','a',''),(3,1.0,'A4','o','n'),
     (5,1.0,'C#5','e','r'),(6.5,1.0,'E5','a','')]
LIFTL=[(0,.5,'A4','a','t'),(0.5,.5,'B4','e','k'),(1,.5,'C5','i','m'),(1.5,.5,'D5','a','b'),
       (2,1.5,'E5','a','k'),
       (4,.5,'A4','a','t'),(4.5,.5,'B4','e','k'),(5,.5,'C5','i','m'),(5.5,.5,'D5','a','b'),
       (6,2.0,'F#5','o','')]
OUTW=[(0,1.0,'B4','a','p'),(1.5,1.0,'B4','i','l'),(3,1.0,'A4','o','h'),(5.5,1.5,'B4','o','')]

for i in range(0,16,2):
    line(vx,bar_at('V1',i),V1[(i//2)%4],g=0.195,style='deadpan',breath=0.30,
         seedbase=i*13,jit=0.005,drag=0.010)
for i in range(0,8,2):
    b=bar_at('PRE1',i)
    line(vx,b,LIFTL,g=0.16,style='croon',breath=0.26,seedbase=200+i*13,jit=0.009)
for i in range(0,8,2):
    b=bar_at('CH1',i); ph=[CH_A,CH_B][(i//2)%2]
    line(vx,b,ph,g=0.215,style='croon',oct8=0.30,breath=0.20,seedbase=300+i*13,jit=0.008)
    chant(vx,b,ph,g=0.12,n=5,spread=16,style='shout',seedbase=350+i)
for i in range(0,12,2):
    line(vx,bar_at('V2',i),V2L[(i//2)%3],g=0.20,style='deadpan',breath=0.32,
         seedbase=500+i*13,jit=0.006,drag=0.012)
for i in range(0,8,2):
    b=bar_at('PRE2',i)
    line(vx,b,LIFTL,g=0.17,style='croon',breath=0.24,seedbase=600+i*13,jit=0.009)
for i in range(0,8,2):
    b=bar_at('CH2',i); ph=[CH_A,CH_B][(i//2)%2]
    line(vx,b,ph,g=0.22,style='croon',oct8=0.38,breath=0.19,seedbase=700+i*13,jit=0.008)
    chant(vx,b,ph,g=0.14,n=6,spread=18,style='shout',seedbase=750+i)
for i in range(0,8,2):
    line(vx,bar_at('BRK',i),BRW,g=0.085,style='whisper',breath=0.36,
         seedbase=900+i*13,jit=0.013)
for i in range(0,4,2):
    line(vx,bar_at('LIFT',i),LIFTL,g=0.185,style='croon',breath=0.22,oct8=0.25,
         seedbase=1000+i*13,jit=0.008)
for i in range(0,12,2):
    b=bar_at('CH3',i); ph=[CH_A,CH_B][(i//2)%2]
    line(vx,b,ph,g=0.225,style='croon',oct8=0.42,breath=0.18,seedbase=1100+i*13,jit=0.008)
    chant(vx,b,ph,g=0.155,n=6,spread=19,style='shout',seedbase=1150+i)
for i in range(0,8,2):
    line(vx,bar_at('OUT',i),OUTW,g=0.085*(1-i/12),style='whisper',breath=0.38,
         seedbase=1300+i*13,jit=0.010)

# ================= MIX =================
STEMS=[(pad,0.0,0.62,0.55,0.0),(kbd,0.30,0.62,0.50,0.0),(brs,-0.30,0.55,0.50,0.0),
       (hp_,-0.42,0.60,0.45,6.0),(fx,0.0,1.0,0.0,0.0)]
MAPT=[(n,a,b_,(5.5 if n in('OUT','TAIL') else 6.5 if n=='BRK' else
               3.2 if n.startswith('CH') else 1.2 if n=='INTRO' else 2.4))
      for n,a,b_ in MAP]
def _build(voc):
    st=mixdown(NAME,vx,STEMS,DRUMS,bs,MAPT,vocals=voc,
        wet=0.23,decay=1.5,wide=1.6,drum_gain=0.76,bass_gain=0.92,crush_amt=0.22,
        rms_target=0.176,vox_gain=1.0,boost_inst=1.12,duck=0.30,tape=0.0025)
