"""07-motorik-lavender - mot file duy nhat, chi can numpy + scipy.
   Hoc tu: Stereolab (nhip motorik krautrock + hoa am lounge/jazz maj7-min9,
   organ Farfisa, giong deadpan hat doi be 3, va NHIP LE - o 7/8 chen vao).
   Y do nhac ly:
   - Verse Imaj7 - III7 - vi9 - ii7 (Amaj7 C#7 F#m9 Bm7): C#7 la V cua vi,
     mot hop am "sang loa" giua toan maj7 => mau lounge nam 60.
   - MOI 8 O NHIP CO MOT O 7/8 (3.5 phach): vong lap bi hut ngan, tai nghe giat
     mot cai roi lai troi - dac san Stereolab.
   - Bridge: chuoi maj7 truot nua cung (Cmaj7-Bmaj7-Bbmaj7-Amaj7) = planing.
   - Trong motorik: kick 1 va 3, snare 2 va 4, hat 8th deu, KHONG fill.
     Bai tien trien bang cach CHONG LOP (bass -> organ -> vibes -> dan keo -> fuzz)
     chu khong bang doi beat => thoi mien.
   - Giong hat: nguyen am thuan ("ba la la"), hat doi be 3 phia tren, deadpan.
   python3 07-motorik-lavender.py             -> ca hai ban wav
   python3 07-motorik-lavender.py --vocals    -> chi ban co giong
   python3 07-motorik-lavender.py --no-vocals -> chi ban instrumental
"""
...
# ================================================================= BAI 07
NAME="07-motorik-lavender"
# moi doan la mot danh sach DO DAI O NHIP (phach) - co o 7/8 = 3.5
def grp(n_full=7, odd=3.5, reps=1): return ([4.0]*n_full+[odd])*reps
SECS=[('INTRO', [4.0]*8),
      ('V1',    grp(7,3.5,2)),
      ('BUILD', [4.0]*4),
      ('CH1',   grp(7,3.5,1)+[4.0]*4),
      ('V2',    grp(7,3.5,1)+[4.0]*4),
      ('CH2',   grp(7,3.5,1)+[4.0]*4),
      ('BRIDGE',[4.0]*8),
      ('SOLO',  grp(7,3.5,1)),
      ('CH3',   grp(7,3.5,2)),
      ('OUT',   [4.0]*10),
...
# ============ HOA AM (A major, lounge maj7 / min9) ============
CH={'Amaj7':(45,[57,61,64,68]), 'C#7':(49,[61,65,68,71]),
    'F#m9':(42,[57,61,64,68]),  'Bm7':(47,[59,62,66,69]),
    'Dmaj7':(50,[54,57,61,66]), 'E7sus':(40,[52,57,59,64]),
    'Cmaj7':(48,[55,59,64,67]), 'Bmaj7':(47,[54,58,63,66]),
    'Bbmaj7':(46,[53,57,62,65]),'G#m7b5':(44,[54,59,62,66]),
    'E9':(40,[56,59,62,66]),    'F#m7':(42,[57,61,64,66])}
VCH=['Amaj7','C#7','F#m9','Bm7']
CCH=['Dmaj7','E7sus','Amaj7','F#m9']
BCH=['Cmaj7','Bmaj7','Bbmaj7','Amaj7']
SCH=['Amaj7','C#7','F#m9','E9']

# ---- bass motorik: 8th deu, chi doi not o vach nhip ----
def bass_mot(b0,root,L=4.0,g=0.24,seed=0,walk=None):
    p=0.0; k=0
    while p<L-1e-6:
        iv=0
        if walk and k in walk: iv=walk[k]
        fingerbass(bs,T(b0+p),root+iv,0.30,g*(1.0 if k%2==0 else 0.78),
                   dead=(k%4==3),seed=seed*8+k)
        p+=0.5; k+=1
# ---- organ Farfisa giu hop am ca o nhip ----
def farf(b0,ts,L=4.0,g=0.055,oct_=12):
    organ(kbd,T(b0),[t+oct_ for t in ts],T(b0+L-0.15)-T(b0),g)
# ---- vibes: motif 3 not lech phach ----
VIB=[(0.5,2),(1.5,0),(2.75,3)]
def vibes(b0,ts,g=0.045,seed=0,L=4.0):
    for k,(off,ix) in enumerate(VIB):
        if off>=L-0.2: continue
        marimba(hp_,T(b0+off),ts[ix%len(ts)]+12,1.1,g,metal=True,seed=seed*4+k)
# ---- dan keo: not giu, len xuong cham ----
def bows(b0,ts,L=4.0,g=0.036,seed=0,hi=True):
    for j,m in enumerate(ts[1:3] if hi else ts[:2]):
        bowed(pad,T(b0),m+12,T(b0+L)-T(b0),g*(1.0-0.12*j),rough=0.35,
              det=-6+9*j,atk=0.30,seed=seed*3+j)
# ---- guitar nylon gay diu ----
def nyl(b0,ts,g=0.048,seed=0,L=4.0):
    for k,off in enumerate((0.0,1.0,2.0,3.0)):
        if off>=L-0.1: continue
        nylon(kbd,T(b0+off),ts[k%len(ts)],1.0,g*(1.0 if k%2==0 else 0.72),seed=seed*5+k)

# ================= TRONG: MOTORIK (khong fill) =================
K=Kit(seed=101); P=Performer(K,T,SPB,TOTAL,seed=43,style='tight')
P.laid={'kick':0.0,'snare':0.003,'hat':-0.001,'tom':0.005,'cym':0.0,'perc':0.002}
def dr_mot(b0,L=4.0,lvl=1.0,arc=1.0,hat16=False,ride=False,tamb=False):
    P.K(b0,0,1.0*lvl,arc,tune=46)
    if L>2.5: P.K(b0+2,8,0.92*lvl,arc,tune=46)
    if L>3.5: P.K(b0+3.5,14,0.50*lvl,arc,tune=46)
    if L>1.5: P.S(b0+1,4,0.90*lvl,'center',arc)
    if L>3.5: P.S(b0+3,12,0.90*lvl,'center',arc)
    else:     P.S(b0+L-0.5,12,0.82*lvl,'center',arc)
    p=0.0; step=0.25 if hat16 else 0.5
    while p<L-1e-6:
        if ride: P.RD(b0+p,int(p*4)%16,0.36*lvl,bell=(p%2==0),arc=arc)
        else:    P.H(b0+p,int(p*4)%16,(0.42 if p%1 else 0.62)*lvl,o=0.0,
                     art='tip' if p%1 else 'edge',arc=arc)
        p+=step
    if tamb:
        p=0.5
        while p<L-1e-6: P.TB(b0+p,int(p*4)%16,0.32*lvl,arc); p+=1.0
def dr_light(b0,L=4.0,lvl=0.6,arc=1.0):
    P.K(b0,0,0.72*lvl,arc,tune=45)
    if L>2.5: P.K(b0+2,8,0.62*lvl,arc,tune=45)
    p=0.0
    while p<L-1e-6:
        P.H(b0+p,int(p*4)%16,0.30*lvl,o=0.0,art='foot',arc=arc); p+=0.5

for i in range(NB['INTRO']):
    b=bar_at('INTRO',i); L=blen('INTRO',i); a=[1.0,0.99,1.0,1.01][i%4]
    if i>=2: dr_light(b,L,0.55+0.08*i,a)
    if i>=6: dr_mot(b,L,0.62,a)
for sec in ('V1','V2'):
    for i in range(NB[sec]):
        b=bar_at(sec,i); L=blen(sec,i); a=[1.0,0.99,1.0,1.01][i%4]
        dr_mot(b,L,0.95,a,tamb=(sec=='V2' and i>=4))
for i in range(NB['BUILD']):
    b=bar_at('BUILD',i); L=blen('BUILD',i); a=[1.0,0.99][i%2]
    dr_mot(b,L,0.98,a,hat16=(i>=2))
for sec,lv in (('CH1',1.04),('CH2',1.08),('CH3',1.12)):
    for i in range(NB[sec]):
        b=bar_at(sec,i); L=blen(sec,i); a=[1.0,0.98,1.01,1.02][i%4]
        dr_mot(b,L,lv,a,hat16=True,tamb=True,ride=(sec=='CH3' and i>=6))
        if i==0: P.CR(b,0,0.68)
for i in range(NB['BRIDGE']):
    b=bar_at('BRIDGE',i); L=blen('BRIDGE',i); a=[1.0,0.99,1.0,1.01][i%4]
    if i<4: dr_light(b,L,0.7,a)
    else:   dr_mot(b,L,0.85,a)
for i in range(NB['SOLO']):
    b=bar_at('SOLO',i); L=blen('SOLO',i); a=[1.0,0.99,1.0,1.01][i%4]
    dr_mot(b,L,1.05,a,hat16=True,ride=True)
for i in range(NB['OUT']):
    b=bar_at('OUT',i); L=blen('OUT',i); a=[1.0,0.99,1.0,1.01][i%4]
    dr_mot(b,L,1.05-0.09*i,a,hat16=(i<4))
for i in range(NB['TAIL']):
    b=bar_at('TAIL',i)
    if i==0: P.CR(b,0,0.5,size=1.2)
P.apply_chokes()
DRUMS=mix_kit(P.bus,room_amount=0.24,oh_amount=0.88,lofi=0.06,lpf=10000)

# ================= NHAC CU (chong lop dan) =================
crackle(fx,0,T(END),0.020,dens=0.035)
for i in range(NB['INTRO']):
    b=bar_at('INTRO',i); L=blen('INTRO',i); root,ts=CH[VCH[i%4]]
    if i>=1: bass_mot(b,root,L,0.16+0.02*i,seed=i)
    if i>=4: farf(b,ts,L,0.032+0.006*(i-4))
    if i>=6: vibes(b,ts,0.038,seed=i,L=L)
for i in range(NB['V1']):
    b=bar_at('V1',i); L=blen('V1',i); root,ts=CH[VCH[i%4]]
    bass_mot(b,root,L,0.235,seed=i,walk={5:7} if i%4==3 else None)
    farf(b,ts,L,0.052)
    vibes(b,ts,0.042,seed=i,L=L)
    if i>=8: nyl(b,ts,0.040,seed=i,L=L)
    if i>=12: bows(b,ts,L,0.026,seed=i)
for i in range(NB['BUILD']):
    b=bar_at('BUILD',i); L=blen('BUILD',i); root,ts=CH[['Bm7','C#7','Dmaj7','E7sus'][i]]
    bass_mot(b,root,L,0.25+0.008*i,seed=i)
    farf(b,ts,L,0.056+0.004*i)
    bows(b,ts,L,0.032+0.006*i,seed=i)
    nyl(b,ts,0.046,seed=i,L=L)
    if i==3: riser(fx,T(b),T(b+L)-T(b),0.045,300,7000,seed=2)
for sec,gm in (('CH1',1.0),('CH2',1.08),('CH3',1.15)):
    for i in range(NB[sec]):
        b=bar_at(sec,i); L=blen(sec,i); root,ts=CH[CCH[i%4]]
        bass_mot(b,root,L,0.26*gm,seed=i,walk={3:-2,7:7})
        farf(b,ts,L,0.060*gm)
        vibes(b,ts,0.048*gm,seed=i,L=L)
        bows(b,ts,L,0.036*gm,seed=i)
        nyl(b,ts,0.046*gm,seed=i,L=L)
        supersaw(pad,T(b),[root+12,ts[0]+12,ts[2]+12],T(b+L)-T(b),0.024*gm,
                 det=7,lp=3600,atk=0.15,seed=i)
        if gm>1.0 and i%2==0: horn(brs,T(b),ts[1]+12,1.4,0.036*gm,rough=1.1)
        if gm>1.1: chime12(hp_,T(b),ts[3]+12,2.0,0.030,seed=i)
for i in range(NB['BRIDGE']):                        # planing maj7 xuong nua cung
    b=bar_at('BRIDGE',i); L=blen('BRIDGE',i); root,ts=CH[BCH[i%4]]
    subbass(bs,T(b),root-12,T(b+L*0.8)-T(b),0.20)
    bows(b,ts,L,0.042,seed=i,hi=(i%2==0))
    mellotron(kbd,T(b),ts[0]+12,T(b+L)-T(b),0.038,kind='flute',seed=i)
    marimba(hp_,T(b+1.5),ts[2]+12,1.4,0.042,metal=True,seed=i)
    if i>=4:
        farf(b,ts,L,0.048)
        bass_mot(b,root,L,0.20,seed=i)
    if i==7: riser(fx,T(b),T(b+L)-T(b),0.060,280,8500,seed=5)
for i in range(NB['SOLO']):                          # solo organ + vibes doi dap
    b=bar_at('SOLO',i); L=blen('SOLO',i); root,ts=CH[SCH[i%4]]
    bass_mot(b,root,L,0.26,seed=i,walk={5:7})
    farf(b,ts,L,0.050)
    bows(b,ts,L,0.034,seed=i)
    _R=np.random.default_rng(700+i)
    _sc=[57,59,61,64,66,68,69,71,73,76]
    _ix=4
    p=0.0
    while p<L-0.3:
        _ix=int(np.clip(_ix+_R.integers(-2,3),0,len(_sc)-1))
        if _R.random()<0.18: p+=0.5; continue
        marimba(hp_,T(b+p)+float(_R.normal(0,0.010)),_sc[_ix]+12,
                float(_R.uniform(0.4,1.0)),0.052,metal=True,seed=i*11+int(p*4))
        if _R.random()<0.35:
            clav(kbd,T(b+p)+0.02,_sc[_ix],0.35,0.038,seed=i*7+int(p*2))
        p+=0.5
for i in range(NB['OUT']):
    b=bar_at('OUT',i); L=blen('OUT',i); root,ts=CH[VCH[i%4]]
    f=1-i/12.0
    bass_mot(b,root,L,0.24*f,seed=i)
    farf(b,ts,L,0.052*f)
    vibes(b,ts,0.042*f,seed=i,L=L)
    if i<6: nyl(b,ts,0.040*f,seed=i,L=L)
    if i>=4: bows(b,ts,L,0.030*f,seed=i)
for i in range(NB['TAIL']):
    b=bar_at('TAIL',i); root,ts=CH['Amaj7']
    if i==0:
        farf(b,ts,10.0,0.050)
        bows(b,ts,10.0,0.038,seed=1)
        subbass(bs,T(b),root-12,T(b+7)-T(b),0.16)
        marimba(hp_,T(b+0.5),ts[3]+12,2.4,0.038,metal=True,seed=1)
tapestop(kbd,T(bar_at('TAIL',1)),2.0,0.75)

# ================= GIONG HAT (deadpan, hat doi be 3) =================
# nguyen am thuan kieu Stereolab: "ba da la la"
VA=[(0,.5,'A4','a','b'),(0.5,.5,'A4','a','d'),(1,.5,'C#5','a','l'),(1.5,.5,'B4','a','l'),
    (2,.75,'A4','o','n'),(3,.5,'F#4','a','m'),
    (4,.5,'B4','a','b'),(4.5,.5,'C#5','a','d'),(5,.5,'E5','a','l'),(5.5,.5,'C#5','a','l'),
    (6,1.0,'B4','o','n')]
VB=[(0,.5,'A4','a','b'),(0.5,.5,'B4','a','d'),(1,.5,'C#5','a','l'),(1.5,.5,'E5','a','l'),
    (2,.75,'C#5','o','m'),(3,.5,'B4','a','n'),
    (4,.5,'A4','a','b'),(4.5,.5,'F#4','a','d'),(5,.5,'A4','a','l'),(5.5,.5,'B4','a','l'),
    (6,1.0,'C#5','o','n')]
CHV=[(0,.5,'D5','a','l'),(0.5,.5,'C#5','a','l'),(1,.75,'B4','a','l'),
     (2,.5,'C#5','a','m'),(2.5,.5,'D5','a','l'),(3,.75,'E5','o','n'),
     (4,.5,'C#5','a','l'),(4.5,.5,'B4','a','l'),(5,.75,'A4','a','l'),
     (6,1.25,'F#4','o','m')]
CHV2=[(0,.5,'D5','a','l'),(0.5,.5,'C#5','a','l'),(1,.75,'B4','a','l'),
      (2,.5,'C#5','a','m'),(2.5,.5,'E5','a','l'),(3,.75,'F#5','o','n'),
      (4,.5,'E5','a','l'),(4.5,.5,'C#5','a','l'),(5,.75,'B4','a','l'),
      (6,1.25,'A4','o','m')]
BRV=[(0,1.0,'E5','o','m'),(1.5,1.0,'D#5','a','n'),(3,1.0,'D5','o','l'),
     (4.5,1.0,'C#5','a','m'),(6,1.5,'B4','o','n')]
def third_up(cells,semi=4):
    return [(c[0],c[1],nn(c[2])+semi,c[3],c[4] if len(c)>4 else '') for c in cells]

for i in range(0,NB['V1']-1,2):
    b=bar_at('V1',i); ph=[VA,VB][(i//2)%2]
    line(vx,b,ph,g=0.18,style='deadpan',breath=0.26,seedbase=i*13,jit=0.007,drag=0.008)
    line(vx,b,third_up(ph,4 if (i//2)%2==0 else 3),g=0.105,style='deadpan',breath=0.30,
         seedbase=90+i*13,jit=0.011,drag=0.013)
for i in range(0,NB['BUILD'],2):
    b=bar_at('BUILD',i)
    line(vx,b,CHV,g=0.155,style='deadpan',breath=0.28,seedbase=200+i*13,jit=0.009)
for sec,g0,base in (('CH1',0.205,300),('CH2',0.215,700),('CH3',0.225,1100)):
    for i in range(0,NB[sec]-1,2):
        b=bar_at(sec,i); ph=[CHV,CHV2][(i//2)%2]
        line(vx,b,ph,g=g0,style='croon',oct8=0.30,breath=0.22,seedbase=base+i*13,jit=0.009)
        line(vx,b,third_up(ph,3),g=g0*0.55,style='deadpan',breath=0.26,
             seedbase=base+50+i*13,jit=0.012,drag=0.012)
        if sec!='CH1': chant(vx,b,ph,g=0.10,n=5,spread=15,style='croon',seedbase=base+80+i)
for i in range(0,NB['V2']-1,2):
    b=bar_at('V2',i); ph=[VB,VA][(i//2)%2]
    line(vx,b,ph,g=0.19,style='deadpan',breath=0.28,seedbase=500+i*13,jit=0.008,drag=0.010)
    line(vx,b,third_up(ph,3),g=0.11,style='deadpan',breath=0.32,
         seedbase=560+i*13,jit=0.012,drag=0.015)
for i in range(0,NB['BRIDGE'],2):
    b=bar_at('BRIDGE',i)
    line(vx,b,BRV,g=0.13,style='whisper',breath=0.38,seedbase=900+i*13,jit=0.013)
    if i>=4: line(vx,b,third_up(BRV,4),g=0.075,style='deadpan',breath=0.34,
                  seedbase=950+i*13,jit=0.014)
for i in range(0,NB['OUT']-1,2):
    b=bar_at('OUT',i); ph=[CHV,VA][(i//2)%2]
    f=1-i/13.0
    line(vx,b,ph,g=0.16*f,style='deadpan',breath=0.30,seedbase=1300+i*13,jit=0.010)
    line(vx,b,third_up(ph,3),g=0.09*f,style='deadpan',breath=0.34,
         seedbase=1360+i*13,jit=0.013)

# ================= MIX =================
STEMS=[(pad,0.0,0.60,0.55,0.0),(kbd,0.32,0.66,0.50,0.0),(brs,-0.34,0.56,0.50,0.0),
       (hp_,-0.44,0.62,0.45,7.0),(fx,0.0,1.0,0.0,0.0)]
MAPT=[(n,a,b_,(4.6 if n in('OUT','TAIL') else 5.4 if n=='BRIDGE' else
               -60.0 if n in('SOLO','INTRO') else
               3.0 if n.startswith('CH') else 2.4))
      for n,a,b_ in MAP]
def _build(voc):
    st=mixdown(NAME,vx,STEMS,DRUMS,bs,MAPT,vocals=voc,
        wet=0.24,decay=1.5,wide=1.55,drum_gain=0.78,bass_gain=0.88,crush_amt=0.20,
        rms_target=0.176,vox_gain=1.0,boost_inst=1.10,duck=0.26,tape=0.0035)
