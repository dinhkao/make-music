...
STRINGS = {"pre": (0.07, 1100, False), "chorus": (0.09, 1400, False),
           "chorus2": (0.10, 1400, True), "bridge": (0.13, 1500, True),
           "solo": (0.08, 1400, False)}
...
def bass_events(style, bar, root):
    return [(b + jit(), note_freq(root + off), d, 0.28)
...
def drum_events(style, bar):
    ...
    for q in [0, 0.5, 1, 1.5, 2, 2.5, 3, 3.5]:
        if style in ("lounge", "outro"):
            if q % 1 == 0.5:
                ev.append((q + jit(), hat, 0.08))
            else:
                ev.append((q + jit(), shaker, 0.05))
        else:
            ev.append((q + jit(), hat, 0.09))
    if p["tamb"]:
        for q in [0, 0.5, 1, 1.5, 2, 2.5, 3, 3.5]:
            ev.append((q + jit(), tamb, 0.07))
    if style == "bridge" and bar == 64:
        for i in range(16):
            ev.append((i / 4.0, snare, 0.12 + 0.22 * i / 15.0))
    return ev
...
def render_section(section, buf, send, offset):
    ...
    mkey = {"verse": "verse", "pre": "pre", "chorus": "chorus",
            "chorus2": "chorus", "verseb": "verseb", "bridge": "bridge"}.get(style)
    for bar in range(start, end + 1):
        ...
        # EP chords
        for m in voicing:
            place(buf, ep(note_freq(m), BAR * 1.1, 0.13, SR), t0 - offset, 0.35)
        # Bass
        ...
        if mkey:
            for bar_m, beat, m, d in N.MELODY[mkey]:
                ...
                sig = lead(note_freq(m), d * N.BEAT, 0.30, SR)
                place(buf, delay(sig, SR), t - offset, 0.55)
                send += to_send(sig * 0.35, t, send)
                if style == "chorus2":
                    place(buf, lead(note_freq(m + 7), d * N.BEAT, 0.10, SR),
                          t - offset + 0.01, 0.65)
...
def main():
    ...
    for bar in range(65, 73):
        for i in range(8):
            ...
            sig = solo(note_freq(m), d * N.BEAT, 0.32, SR)
            place(L, sig, t, 0.5)
    ...
    for bar in range(73, 89):
        ...
        sig = choir(freqs, BAR * 1.2, 0.10, SR)
        place(L, sig, t0, 0.5)
        send += to_send(sig * 0.5, t0, send)
    ...
    for tb, text in N.SPOKEN:
        ...
        sp = make_spoken(text)
        sp = filt(sp, SR, "lp", 3000)
        sp = reverb(sp, SR, ir0, wet=1.0) - sp
        sp = delay(sp, SR, 0.5, 0.35)
        place(L, sp * 0.5, t, 0.5)
        send += to_send(sp * 0.7, t, send)
...
def make_spoken(text, i=0):
    ...
    x = np.interp(np.linspace(0, len(x) - 1, len(x) * 2), np.arange(len(x)), x)
    x = np.interp(np.linspace(0, len(x) - 1, len(x) // 2), np.arange(len(x)), x)
    return x * 0.9
