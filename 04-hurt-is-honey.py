"""04-hurt-is-honey - mot file duy nhat, chi can numpy + scipy.
   ...
   - theme love & pain ("someone call the coroner / i loved you too much")
   python3 04-hurt-is-honey.py              -> ca hai ban wav (co giong + instrumental)
   python3 04-hurt-is-honey.py --vocals     -> chi ban co giong
   python3 04-hurt-is-honey.py --no-vocals  -> chi ban instrumental
"""
...
# ================================================================= NHAC CU BO SUNG (tu palette album indie-electronic)
# --- synth pluck analog: saw qua bo loc dong ---
...
# --- lead analog 2 osc detune (high-resonance lead, panned trong mix) ---
...
# --- piano (tackpiano dong bo) ---
def piano_hit(b0,ts,g=0.055,off=0.0,seed=0,spread=0.014):
    for j,m in enumerate(ts):
        tackpiano(kbd,T(b0+off)+j*spread,m,T(b0+off+3.0)-T(b0+off),g*(1.0-0.09*j),
                  tack=0.55,seed=seed*5+j)

# ================================================================= BAI 04
# HURT IS HONEY - 4/4, 115bpm, E major, dream-pop slow-build
# Cam hung: "Vampire in the Corner" - Magdalena Bay (Imaginal Disk, 2024):
#   - Key E major, 115 BPM, 4/4. Slow-build crescendo ca bai.
#   - Verse airy/hushed (whisper), chorus sing-songy co piano "beautifully produced".
#   - Filtered chords mo dan -> panned high-resonance leads (payoff cho su kien nhan).
#   - Nua sau full band + Mica rip nhung not cao; cuoi: vocal het, instrumental
#     loop 4 lan roi chord giai quyet -> CAT NGOT (sang "Watching T.V.").
#   - Theme: love & pain - "someone call the coroner / 'cause you're breaking my
#     heart / my god, i think i mighta loved you too much" - yeu vi dau.
# Y TUONG: "hurt is honey" - ngot va dau la mot. Nguoi nguoi tinh ma o lai vi
# gioi. Whisper -> sing-song -> full band -> rip not cao -> loop roi cat ngot.
NAME="04-hurt-is-honey"
BAR=4.0                                    # 4/4
SECS=[('INTRO',8),('V1',12),('PRE1',8),('CH1',12),('POST',4),
      ('V2',12),('PRE2',8),('CH2',16),('BRIDGE',8),('CH3',12),('OUT',8)]
S={}; _b=0.0
for _n,_c in SECS: S[_n]=_b; _b+=_c*BAR
END=_b
configure(115,115,END)
CUT=T(END)                                 # ket bai cat ngot (nhu bai goc)
MAP=[(n,S[n],S[n]+c*BAR) for n,c in SECS]
print(NAME,"bars",int(END/BAR),"beats",END,"->",round(TOTAL,1),"s (cut",round(CUT,1),"s)")

pad=buf(); kbd=buf(); brs=buf(); hp_=buf(); vx=buf(); bs=buf(); fx=buf()

# ============ NHAC CU BO SUNG (dream-pop) ============
# --- supersaw pad (7 saw detune, filtered chord DNA) ---
def supersaw(b_,t0,notes,dur,g=0.08,det=7.0,lp=4600,atk=0.3,seed=0,
    ...
    if pump_>0:
        ...
        out*=pe
    put(b_,t0,_fade(out*e),g)
# --- bell arp (glockenspiel) 8th tren hop am ---
def bell_arp(b0,ts,g=0.05,seed=0):
    p=[0,1,2,1,0,2,1,2]
    for k,off in enumerate(np.arange(0,4,0.5)):
        bell(hp_,T(b0+float(off)),ts[p[k%8]]+12,1.4,g,seed=k+seed*8)
...
def popbass(b_,t0,m,dur,g=0.30,oct_=0,seed=0):
    ...
    put(b_,t0,_fade(x),g)
# --- dan cua keo (bowed drone, cho bridge) ---
def bowed(b_,t0,m,dur,g=0.09,rough=0.5,det=0.0,atk=0.16,seed=0):
    L=int(dur*SR)+int(0.12*SR); t=np.arange(L)/SR
    R=np.random.default_rng(700+seed+int(m))
    f=hz(m)*2**(det/1200)
...
K=Kit(seed=91); P=Performer(K,T,SPB,TOTAL,seed=41,style='indie')
def dr_sparse(b0,lvl=0.6,arc=1.0):
    P.K(b0,0,0.75*lvl,arc,tune=45); P.K(b0+2.5,10,0.55*lvl,arc,tune=45)
    P.S(b0+3,12,0.55*lvl,'cross',arc)
    for p in (0.5,1.5,2.5,3.5): P.H(b0+p,int(p*4)%16,0.22*lvl,o=0.0,art='tip',arc=arc)
    P.SH(b0+1.5,6,0.35*lvl,arc); P.SH(b0+3.5,14,0.35*lvl,arc)
def dr_verse(b0,lvl=0.9,arc=1.0):
    P.K(b0,0,1.0*lvl,arc,tune=46); P.K(b0+2.5,10,0.6*lvl,arc)
    P.S(b0+3,12,0.9*lvl,'center',arc)
    p=0.0
    while p<4.0-1e-6:
        P.H(b0+p,int(p*4)%16,(0.4 if p%1 else 0.7)*lvl,o=0.0,art='tip',arc=arc); p+=0.5
    for gp in (1.5,3.5): P.S(b0+gp,int(gp*4)%16,0.3*lvl,'ghost',arc)
    P.SH(b0+0.5,2,0.5*lvl,arc); P.SH(b0+2.5,10,0.5*lvl,arc)
def dr_pre(b0,lvl=0.8,arc=1.0):
    P.K(b0,0,0.9*lvl,arc,tune=46); P.K(b0+3,12,0.8*lvl,arc)
    P.S(b0+3,12,0.85*lvl,'center',arc)
    p=0.0
    while p<4.0-1e-6:
        P.H(b0+p,int(p*4)%16,0.55*lvl,o=0.0,art='tip',arc=arc); p+=0.25
    P.TB(b0+2,8,0.5*lvl,arc); P.TB(b0+4,0,0.5*lvl,arc)
def dr_chorus(b0,lvl=1.0,arc=1.0):
    for k in range(4): P.K(b0+k,0,(0.95 if k%2==0 else 0.8)*lvl,arc,tune=47)
    P.S(b0+2,8,1.0*lvl,'center',arc); P.S(b0+4,0,1.0*lvl,'center',arc)
    P.CL(b0+2-0.003,8,0.7*lvl,arc); P.CL(b0+4-0.003,0,0.7*lvl,arc)
    p=0.0
    while p<4.0-1e-6:
        op=abs(p-3.5)<1e-6
        P.H(b0+p,int(p*4)%16,(0.75 if p%1 else 0.95)*lvl,o=0.5 if op else 0.0,
            art='edge' if p%1==0 else 'tip',arc=arc,choke_beat=(b0+4) if op else None)
        p+=0.5
    for p in (0.5,1.5,2.5,3.5): P.TB(b0+p,int(p*4)%16,0.4*lvl,arc)
    P.CR(b0,0,0.8*lvl,size=1.0)
def dr_post(b0,lvl=0.6,arc=1.0):
    P.K(b0,0,0.85*lvl,arc,tune=45); P.K(b0+2.5,10,0.6*lvl,arc)
    P.CL(b0+3,12,0.65*lvl,arc)
    for p in (0.5,1.5,2.5,3.5): P.H(b0+p,int(p*4)%16,0.25*lvl,o=0.0,art='tip',arc=arc)
def dr_bridge(b0,lvl=0.75,arc=1.0):
    P.K(b0,0,0.85*lvl,arc,tune=44); P.K(b0+3.5,14,0.7*lvl,arc,tune=44)
    P.S(b0+3,12,0.55*lvl,'rim',arc)
    p=0.0
    while p<4.0-1e-6:
        P.RD(b0+p,int(p*4)%16,0.4*lvl,bell=(p==0),arc=arc); p+=0.5
    P.TM(b0+1.5,6,0.5*lvl,tune=150,arc=arc); P.TM(b0+3.0,12,0.5*lvl,tune=112,arc=arc)
def dr_out(b0,lvl=1.1,arc=1.0):
    for k in range(4):
        P.K(b0+k,0,0.95*lvl,arc,tune=48)
        if k%2==0: P.CR(b0+k,0,0.6*lvl,size=0.85)
    P.S(b0+2,8,1.0*lvl,'center',arc); P.S(b0+4,0,1.0*lvl,'center',arc)
    p=0.0
    while p<4.0-1e-6:
        P.H(b0+p,int(p*4)%16,0.7*lvl,o=0.25 if p%1 else 0.0,
            art='edge' if p%1==0 else 'tip',arc=arc); p+=0.5
    for p in (0.5,1.5,2.5,3.5): P.TB(b0+p,int(p*4)%16,0.45*lvl,arc)

# ================= TRONG: sap xep =================
for i in range(8):
    b=bar_at('INTRO',i); a=[1.0,0.97,1.0,1.02][i%4]
    if i>=4: dr_sparse(b,0.5+0.05*i,a)
    if i==7: P.fill(b+2.0,2.0,'snare',0.6,next_crash_beat=S['V1'])
for i in range(12):
    b=bar_at('V1',i); a=[1.0,0.97,1.0,1.02][i%4]
    dr_verse(b,0.9,a)
    if i in (5,11): P.fill(b+2.5,1.5,'roll',0.6)
for i in range(8):
    b=bar_at('PRE1',i); a=[1.0,0.98,1.0,1.02][i%4]
    dr_pre(b,0.85,a)
    if i==7: P.fill(b,2.0,'roll',1.2,next_crash_beat=S['CH1'])
for i in range(12):
    b=bar_at('CH1',i); a=[1.0,0.97,1.01,1.03][i%4]
    dr_chorus(b,0.98,a)
    if i==11: P.fill(b+2.0,2.0,'tom',0.9,next_crash_beat=S['POST'])
for i in range(4):
    b=bar_at('POST',i); a=[1.0,0.98,1.0,1.02][i%4]
    dr_post(b,0.6,a)
    if i==3: P.fill(b+2.0,2.0,'snare',0.7,next_crash_beat=S['V2'])
for i in range(12):
    b=bar_at('V2',i); a=[1.0,0.97,1.0,1.02][i%4]
    dr_verse(b,0.95,a)
    if i in (5,11): P.fill(b+2.5,1.5,'roll',0.7)
for i in range(8):
    b=bar_at('PRE2',i); a=[1.0,0.98,1.0,1.02][i%4]
    dr_pre(b,0.9,a)
    if i==7: P.fill(b,2.0,'roll',1.4,next_crash_beat=S['CH2'])
for i in range(16):
    b=bar_at('CH2',i); a=[1.0,0.97,1.01,1.03][i%4]
    dr_chorus(b,1.05,a)
    if i==15: P.fill(b+2.0,2.0,'tom',1.0,next_crash_beat=S['BRIDGE'])
for i in range(8):
    b=bar_at('BRIDGE',i); a=[1.0,0.98,1.0,1.02][i%4]
    dr_bridge(b,0.8,a)
    if i==7: P.fill(b,2.0,'roll',1.5,next_crash_beat=S['CH3'])
for i in range(12):
    b=bar_at('CH3',i); a=[1.0,0.99,1.0,1.04][i%4]
    dr_chorus(b,1.1,a)
    P.CR(b,0,0.9,size=1.05)
    if i==11: P.fill(b+2.0,2.0,'roll',1.3,next_crash_beat=S['OUT'])
for i in range(8):
    b=bar_at('OUT',i); a=[1.0,0.98,1.0,1.03][i%4]
    dr_out(b,1.05,a)
    if i%2==1: P.fill(b,1.5,'roll',0.8)
P.apply_chokes()
DRUMS=mix_kit(P.bus,room_amount=0.24,oh_amount=0.90,lofi=0.0,lpf=10000)

# ================= SAP XEP NHAC CU =================
crackle(fx,0,T(END),0.045,0.03,0)         # bui bang tu xuyen bai (film grain)
for i in range(8):                         # INTRO: bell arp + filtered pad mo dan
    b=bar_at('INTRO',i); ch=VCH[i%4]; root,ts=CH[ch]
    bell_arp(b,ts,0.045,seed=i)
    supersaw(pad,T(b),[root+12]+[t+12 for t in ts[:2]],2.0,0.030,lp=1500+250*i,atk=1.0,seed=i)
    if i>=4:
        subbass(bs,T(b),root,T(b+1.5)-T(b),0.16)
        synpluck(kbd,T(b+1),ts[1]+24,1.0,0.035,cut=3800,seed=i)
for i in range(12):                        # V1: giong thi tham (airy & hushed)
    b=bar_at('V1',i); ch=VCH[i%4]; root,ts=CH[ch]
    supersaw(pad,T(b),[root+12]+[t+12 for t in ts[:2]],2.0,0.042,lp=2600,atk=0.5,seed=i)
    subbass(bs,T(b),root,T(b+2.0)-T(b),0.20)
    fingerbass(bs,T(b+0.5),root+12,0.5,0.12,seed=i)
    fingerbass(bs,T(b+2.5),root+12,0.5,0.14,seed=i+3)
    if i%2==0: synpluck(kbd,T(b+0.5),ts[1]+12,0.9,0.040,cut=3400,seed=i)
    if i%4==3: bell_arp(b,ts,0.028,seed=i)
for i in range(8):                         # PRE1: piano vao, pad mo rong dan
    b=bar_at('PRE1',i); ch=PCH[i%4]; root,ts=CH[ch]
    supersaw(pad,T(b),[root+12]+[t+12 for t in ts[:2]],2.0,0.045,lp=3400+300*i,atk=0.35,seed=i)
    piano_hit(b,ts,0.055,seed=i)
    popbass(bs,T(b),root,1.0,0.22); popbass(bs,T(b+2),root+12,1.0,0.18)
    if i==7: riser(fx,T(b),3.5,0.06,400,7500,seed=i)
    if i>=6: noise_sw(fx,T(b+2),2.0,0.030,True,200,6000)
for i in range(12):                        # CH1: full band + piano chorus
    ...
    for off,iv,vv in [(0,0,1.0),(0.5,0,0.8),(1,12,0.65),(1.5,0,0.6),
        ...
        fuzzbass(bs,T(b+off),root+iv,0.55,0.16*vv)
    supersaw(pad,T(b),[root+12,ts[0]+12,ts[1]+12,ts[2]+12],2.0,0.042,lp=5200,atk=0.12,seed=i)
    piano_hit(b,ts,0.045,seed=i+50)
    if i>=8:                                # lead vang high-resonance vao cuoi
        for off,m in [(0,'B5'),(1,'A5'),(2,'G#5'),(3,'A5'),(4,'B5'),(5,'C#6'),(6,'D6'),(7,'C#6')]:
            analead(brs,T(b+off),nn(m),0.9,0.05,det=9,cut=4200,vibd=0.008,seed=i*7+off)
for i in range(4):                         # POST: drop - piano + thi tham
    b=bar_at('POST',i); ch=PCH2[i]; root,ts=CH[ch]
    piano_hit(b,ts,0.055,seed=i)
    supersaw(pad,T(b),[root+12]+[t+12 for t in ts[:2]],2.0,0.030,lp=3000,atk=0.5,seed=i)
    bell_arp(b,ts,0.030,seed=i)
    subbass(bs,T(b),root,T(b+2.0)-T(b),0.15)
for i in range(12):                        # V2: croon, bells + analead vong lai
    b=bar_at('V2',i); ch=VCH[i%4]; root,ts=CH[ch]
    supersaw(pad,T(b),[root+12]+[t+12 for t in ts[:2]],2.0,0.046,lp=3200,atk=0.4,seed=i)
    subbass(bs,T(b),root,T(b+2.0)-T(b),0.22)
    fingerbass(bs,T(b+0.5),root+12,0.5,0.14,seed=i+3)
    fingerbass(bs,T(b+2.5),root+12,0.5,0.14,seed=i+9)
    if i%2==0: synpluck(kbd,T(b+0.5),ts[1]+12,0.9,0.040,cut=3800,seed=i)
    bell_arp(b,ts,0.032,seed=i)
    analead(brs,T(b+0.5),ts[0]+24,1.2,0.032,det=7,cut=4200,seed=i)
for i in range(8):                         # PRE2: to hon
    b=bar_at('PRE2',i); ch=PCH[i%4]; root,ts=CH[ch]
    supersaw(pad,T(b),[root+12]+[t+12 for t in ts[:2]],2.0,0.050,lp=4200+300*i,atk=0.25,seed=i)
    piano_hit(b,ts,0.060,seed=i+7)
    popbass(bs,T(b),root,1.0,0.24); popbass(bs,T(b+1.5),root+12,0.8,0.20)
    popbass(bs,T(b+3),root+7,1.0,0.22)
    if i==7: riser(fx,T(b),3.5,0.075,500,9000,seed=i)
    if i>=6: noise_sw(fx,T(b+2),2.0,0.035,True,200,7000)
for i in range(16):                        # CH2: panned high-resonance leads
    ...
    supersaw(pad,T(b),[root+12,ts[0]+12,ts[1]+12,ts[2]+12],2.0,0.050,lp=6000,atk=0.10,seed=i)
    piano_hit(b,ts,0.050,seed=i+99)
    for off,m in [(0,'B5'),(1,'A5'),(2,'G#5'),(3,'B5'),(4,'C#6'),(5,'B5'),(6,'D6'),(7,'C#6')]:
        analead(brs,T(b+off),nn(m),0.9,0.055,det=10,cut=5000,vibd=0.009,seed=i*7+off)
    if i==0: subdrop(fx,T(b),36,2.0,0.20,16)
for i in range(8):                         # BRIDGE: boc truong (piano + bowed)
    b=bar_at('BRIDGE',i); ch=BCH[i%4]; root,ts=CH[ch]
    supersaw(pad,T(b),[root+12]+[t+12 for t in ts[:3]],2.0,0.040,lp=2400,atk=1.0,seed=i)
    tackpiano(kbd,T(b),ts[0]+12,T(b+2.0)-T(b),0.05,tack=0.5,seed=i)
    bowed(brs,T(b),ts[0]+12,2.0,0.05,rough=0.4,seed=i)
    subbass(bs,T(b),root,T(b+1.5)-T(b),0.20)
    if i>=5: riser(fx,T(b),3.5,0.05+0.015*(i-5),500,8000,seed=i)
for i in range(12):                        # CH3: cao trao cuoi - moi thu
    b=bar_at('CH3',i); ch=CCH[i%4]; root,ts=CH[ch]
    for off,iv,vv in [(0,0,1.0),(0.5,0,0.9),(1,12,0.7),(1.5,0,0.65),
                      (2,0,0.95),(2.5,0,0.65),(3,12,0.8),(3.5,0,0.65)]:
        fuzzbass(bs,T(b+off),root+iv,0.55,0.19*vv)
    supersaw(pad,T(b),[root+12,ts[0]+12,ts[1]+12,ts[2]+12],2.0,0.055,lp=6800,atk=0.08,seed=i)
    mellotron(pad,T(b),ts[0]+24,2.0,0.030,kind='choir',seed=i)
    piano_hit(b,ts,0.045,seed=i+31)
...
for i in range(8):                         # OUT: instrumental loop (het giong)
    ...
    for off,iv,vv in [(0,0,1.0),(0.5,0,0.85),(1,12,0.7),(1.5,0,0.65),
                      (2,0,0.95),(2.5,0,0.65),(3,12,0.8),(3.5,0,0.65)]:
        fuzzbass(bs,T(b+off),root+iv,0.55,0.18*vv)
    supersaw(pad,T(b),[root+12,ts[0]+12,ts[1]+12,ts[2]+12],2.0,0.050,lp=6200,atk=0.10,seed=i)
    piano_hit(b,ts,0.050,seed=i+77)
    if i%2==0:
        for off,m in [(0,'B5'),(1,'A5'),(2,'G#5'),(3,'B5'),(4,'C#6'),(5,'B5'),(6,'D6'),(7,'C#6')]:
            analead(brs,T(b+off),nn(m),0.9,0.05,det=10,cut=5000,seed=i*7+off)
# bar cuoi OUT: giai quyet ve Emaj7 giu dai -> cat ngot
bb=bar_at('OUT',7)
supersaw(pad,T(bb),[52+12,56+12,59+12,63+12],4.0,0.060,lp=6200,atk=0.3,seed=999)
piano_hit(bb,CH['Emaj7'][1],0.060,seed=998)
bell(hp_,T(bb+3.5),nn('E6'),3.0,0.06,seed=42)

# ================= GIONG HAT =================
# theme: love & pain - "hurt is honey" (Vampire in the Corner: yeu vi dau)
V1A=[
 # "you come around at midnight"
 [(0,1,'E4','u','y'),(1,1,'E4','o','k'),(2,1,'G#4','a',''),(3,1,'F#4','a','r'),(4,1,'E4','a','t'),
  (5,1,'F#4','i','m'),(6.5,1.5,'G#4','a','n')],
 # "and i let you in"
 [(0,1,'F#4','a','n'),(1,1,'E4','a',''),(2,1,'E4','e','l'),(3,1,'G#4','u','y'),(4,1,'F#4','u',''),
  (6,2,'E4','i','n')],
 # "you drink the moonlight slow"
 [(0,1,'E4','u','y'),(1,1,'G#4','i','r'),(2,1,'F#4','i','n'),(3,1,'E4','e','th'),(4,1,'G#4','u','m'),
  (5,1,'A4','a','l'),(6.5,1.5,'G#4','o','s')],
 # "right out of my skin"
 [(0,1,'G#4','a','r'),(1,1,'F#4','a',''),(2,1,'E4','u','t'),(3,1,'F#4','o',''),(4,1,'E4','a','m'),
  (6,2,'G#4','i','s')],
 # "sweet little vampire"
 [(0,1,'E4','i','s'),(1,1,'F#4','i','l'),(2,1,'G#4','o','l'),(3,1,'F#4','a','v'),(4,1,'G#4','a','m'),
  (6,2,'E4','a','r')],
 # "in my kitchen light"
 [(0,1,'E4','i','n'),(1,1,'G#4','a','m'),(2,1,'F#4','i','k'),(3,1,'A4','i',''),(4,1,'G#4','a','l'),
  (6,2,'A4','a','t')],
]
# PRE: cau len (croon)
P1=[(0,1,'A4','a',''),(1,1,'A4','o','l'),(2,1,'B4','e','th'),(3,1,'A4','e','p'),(4,1,'G#4','u','y'),
    (5,1,'A4','i','r'),(6.5,1.5,'B4','i','n')]
P2=[(0,1,'A4','a',''),(1,1,'A4','o','l'),(2,1,'B4','e','th'),(3,1,'A4','e','w'),(4,1,'G#4','i',''),
    (5,1,'A4','i','s'),(6.5,1.5,'B4','i','n')]
P3=[(0,1,'B4','a',''),(1,1,'A4','i','k'),(2,1,'B4','i','s'),(3,1,'C#5','a','n'),(4,1,'B4','e','th'),
    (5,1,'A4','u','m'),(6.5,1.5,'B4','u','n')]
P4=[(0,1,'A4','a','n'),(1,1,'B4','e','th'),(2,1,'A4','i','n'),(3,1,'B4','i','r'),(4,1,'C#5','u','y'),
    (5,1,'B4','u',''),(6.5,1.5,'A4','u','n')]
PREP=[P1,P2,P3,P4]
# CHORUS hook: "sweet venom..." sing-song (chorus co piano, kieu bai goc)
H1=[(0,1,'B4','i','s'),(1,1,'C#5','i',''),(2,1,'B4','e','v'),(3,1,'A4','o','n'),(4,1,'B4','i','s'),
    (5,1,'C#5','i',''),(6,1,'B4','e','v'),(7,1,'A4','o','n'),(8,1,'G#4','u','p'),(9,1,'A4','o','r'),
    (10,2,'B4','o','s')]
H2=[(0,1,'A4','a',''),(1,1,'A4','o','l'),(2,1,'C#5','u','y'),(3,1,'B4','o',''),(4,1,'A4','o','k'),
    (5,1,'G#4','i','z'),(6,1,'A4','i','t'),(7,1,'B4','e','h'),(8,1,'C#5','u',''),(9,1,'B4','u','y'),
    (10,2,'A4','o','n')]
H3=[(0,1,'C#5','o','s'),(1,1,'B4','a','m'),(2,1,'A4','o',''),(3,1,'B4','o','k'),(4,1,'C#5','e','th'),
    (5,1,'B4','o','r'),(6,1,'A4','o','n'),(7,1,'G#4','a',''),(8,1,'A4','a','m'),(9,1,'B4','o','g'),
    (10,2,'A4','o','n')]
H4=[(0,1,'A4','a',''),(1,1,'B4','o','l'),(2,1,'C#5','u','y'),(3,1,'B4','e','w'),(4,1,'A4','u','t'),
    (5,1,'B4','o','m'),(6,1,'C#5','o',''),(7,1,'D5','e','w'),(8,1,'B4','u','t'),(9,2,'A4','o','m')]
CHP=[H1,H2,H3,H4]
# POST: whisper "hurt is honey"
...
BR3=[(0,1,'E4','e','th'),(1,1,'G#4','a','z'),(2,1,'F#4','u','w'),(3,1,'E4','o','n'),(4,1,'G#4','i','d'),
     (5,1,'F#4','o','r'),(6.5,1.5,'A4','o','s')]
BR4=[(0,1,'A4','a','n'),(1,1,'G#4','i',''),(2,1,'F#4','i','s'),(3,1,'E4','i','t'),(4,1,'F#4','a','l'),
     (6.5,1.5,'G#4','a','t')]
BRP=[BR1,BR2,BR3,BR4]
for i in range(12):                        # V1: whisper (airy & hushed)
    b=bar_at('V1',i)
    line(vx,b,V1A[i//2],g=0.16,style='whisper',breath=0.30,seedbase=i*13,jit=0.007)
for i in range(8):                         # PRE1: croon len
    b=bar_at('PRE1',i)
    line(vx,b,PREP[i//2],g=0.19,style='croon',breath=0.22,seedbase=300+i*13)
    if i%2==1: chant(vx,b,PREP[i//2],g=0.12,n=5,spread=16,style='shout',seedbase=350+i)
for i in range(12):                        # CH1: hook + oct8 + chant
    b=bar_at('CH1',i); ph=CHP[i//3]
    line(vx,b,ph,g=0.20,style='croon',oct8=0.35,breath=0.18,seedbase=600+i*13)
    if i%2==1: chant(vx,b,ph,g=0.14,n=6,spread=18,style='shout',seedbase=700+i)
for i in range(1):                         # POST: whisper
    line(vx,S['POST'],POSTW,g=0.09,style='whisper',breath=0.32,seedbase=800,jit=0.008)
for i in range(12):                        # V2: croon (len mot bac so V1)
    b=bar_at('V2',i)
    line(vx,b,V1A[i//2],g=0.20,style='croon',breath=0.26,seedbase=400+i*13,jit=0.006)
for i in range(8):                         # PRE2
    b=bar_at('PRE2',i)
    line(vx,b,PREP[i//2],g=0.20,style='croon',oct8=0.3,breath=0.20,seedbase=900+i*13)
    if i%2==1: chant(vx,b,PREP[i//2],g=0.13,n=5,spread=16,style='shout',seedbase=950+i)
for i in range(16):                        # CH2: to hon
    b=bar_at('CH2',i); ph=CHP[i//4]
    line(vx,b,ph,g=0.21,style='croon',oct8=0.40,breath=0.16,seedbase=1100+i*13)
    if i%2==0: chant(vx,b,ph,g=0.15,n=6,spread=18,style='shout',seedbase=1200+i)
for i in range(8):                         # BRIDGE: theater + shriek vao CH3
    b=bar_at('BRIDGE',i)
    line(vx,b,BRP[i//2],g=0.17,style='croon',breath=0.30,seedbase=1400+i*13,jit=0.012)
shriek(vx,T(S['BRIDGE']+7*BAR+3.0),nn('E5'),1.2,0.10)
for i in range(12):                        # CH3: rip not cao tren dinh
    b=bar_at('CH3',i); ph=CHP[i//3]
    line(vx,b,ph,g=0.22,style='croon',oct8=0.45,breath=0.16,seedbase=1500+i*13)
    if i%2==1: chant(vx,b,ph,g=0.16,n=6,spread=18,style='shout',seedbase=1600+i)
say(vx,T(S['CH3']+11*BAR+1.0),nn('E5'),2.2,'o','',0.13,'falsetto',breath=0.30,seed=77)
shriek(vx,T(S['CH3']+11*BAR+3.2),nn('G5'),1.6,0.09)
# ================= MIX =================
def hard_cut(st,t_cut):
    n=int(t_cut*SR); f=min(int(0.03*SR),n)
    st[:,n-f:n]*=np.linspace(1,0,f)
    return st[:,:n]
STEMS=[(pad,0.0,0.62,0.55,0.0),(kbd,0.30,0.55,0.55,0.0),(brs,-0.38,0.55,0.50,6.0),
       (hp_,-0.45,0.60,0.45,7.0),(fx,0.0,1.0,0.0,0.0)]
MAPT=[(n,a,b_,(6.0 if n in('POST','OUT') else 5.0 if n=='BRIDGE' else
               3.2 if n.startswith('CH') else 2.6 if n=='INTRO' else 3.6))
      for n,a,b_ in MAP]
def _build(voc):
    st=mixdown(NAME,vx,STEMS,DRUMS,bs,MAPT,vocals=voc,
        wet=0.26,decay=1.6,wide=1.55,drum_gain=0.75,bass_gain=0.88,crush_amt=0.22,
        rms_target=0.176,vox_gain=1.0,boost_inst=1.10,duck=0.30,tape=0.003)
