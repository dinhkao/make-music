"""02-double-vision - mot file duy nhat, chi can numpy + scipy.
   Cam hung: 01-thirteen-doors (engine) + "Tunnel Vision" - Magdalena Bay (key Db, 120bpm,
   chorus I-bVI-IV-v, post-chorus chant, bridge giu 1 hop am, outro chroma cat ngot).
   python3 02-double-vision.py             -> ca hai ban wav (co giong + instrumental)
   python3 02-double-vision.py --vocals    -> chi ban co giong
   python3 02-double-vision.py --no-vocals -> chi ban instrumental
"""
...
NAME="02-double-vision"
BAR=4.0                                    # 4/4
SECS=[('INTRO',8),('V1',16),('INSTR',4),('V2',16),('CH1',12),('POST',4),
      ('V3',12),('CH2',12),('BRIDGE',12),('OUTRO',16),('TAIL',4)]
S={}; _b=0.0
...
# ============ HOA AM (Db major, muon hop am cua bai goc) ============
CH={'Db':  (37,[49,53,58]), 'Dbmaj7':(37,[49,53,58,60]), 'Fm':(41,[53,58,60]),
   'Gb':   (44,[56,58,61]), 'Ab':(46,[58,60,63]), 'Bbm7':(46,[58,61,65,68]),
   'B':    (47,[63,66,71]), 'Abm7':(46,[59,66,70]), 'Dbm':(37,[49,52,58]),
   'A':    (45,[57,61,64]), 'C':(48,[52,55]), 'E':(40,[52,56,59]), 'D':(50,[54,57]),
   'Gbm':  (44,[56,57,61]), 'Abm':(46,[58,61,63])}
V1CH =['Db','Dbmaj7','Db','Dbmaj7','Db','Dbmaj7','Gb','Db']
V2CH =['Db','Fm','Db','Fm','Gb','Ab','Bbm7','Ab']
CH1CH=['Db','B','Gb','Abm7']*2+['Db','B','Gb','Db']
CH2CH=['Db','B','Db','Abm7','Db','B','Db','Ab']+['Db','B','Db','Ab']
V3CH =['Db','Fm']*4+['Db','Db','Dbm','Ab']
OUTCH=['Dbm','Db','Dbm','Db','Dbm','Abm','A','Dbm',
       'Dbm','D','Dbm','A','Dbm','E','Gbm','A']
TAGCH=['C','Dbm','C','Dbm']
...
for i in range(16):
    ...
    dr_verse(b,0.95,a)
    if i==15: P.fill(b+2.0,2.0,'tom',0.9,next_crash_beat=S['INSTR'])
for i in range(4):
    b=bar_at('INSTR',i); a=[1.0,0.98,1.0,1.03][i%4]
    dr_instr(b,1.0,a)
    if i==3: P.fill(b+2.0,2.0,'snare',0.95,next_crash_beat=S['V2'])
for i in range(16):
    b=bar_at('V2',i); a=[1.0,0.97,1.0,1.02][i%4]
    dr_verse(b,0.98,a,tamb=(i>=8))
    if i==15: P.fill(b+2.0,2.0,'roll',0.9,next_crash_beat=S['CH1'])
for i in range(12):
    b=bar_at('CH1',i); a=[1.0,0.97,1.01,1.03][i%4]
    dr_chorus(b,1.02,a,brk=(i in (3,7)))
    if i==11: P.fill(b+2.0,2.0,'tom',1.0,next_crash_beat=S['POST'])
for i in range(4):
    b=bar_at('POST',i); a=[1.0,0.98,1.0,1.02][i%4]
    dr_post(b,0.75,a)
    if i==3: P.fill(b+2.0,2.0,'snare',0.8,next_crash_beat=S['V3'])
for i in range(12):
    b=bar_at('V3',i); a=[1.0,0.97,1.0,1.03][i%4]
    dr_verse(b,1.0,a,tamb=True)
    if i==11: P.fill(b+2.0,2.0,'roll',1.0,next_crash_beat=S['CH2'])
for i in range(12):
    b=bar_at('CH2',i); a=[1.0,0.97,1.01,1.03][i%4]
    dr_chorus(b,1.08,a,brk=(i in (3,7)))
    if i==11: P.fill(b+2.0,2.0,'tom',1.1,next_crash_beat=S['BRIDGE'])
for i in range(12):
    b=bar_at('BRIDGE',i); a=[1.0,0.98,1.0,1.02][i%4]
    dr_bridge(b,0.8 if i<8 else 0.95,a)
    if i>=8:
        P.fill(b,1.5,'tom',0.85); P.fill(b+2.5,1.5,'roll',0.7)
    if i==11: P.fill(b+2.0,2.0,'roll',1.2,next_crash_beat=S['OUTRO'])
for i in range(16):
    b=bar_at('OUTRO',i); a=[1.0,0.98,1.0,1.03][i%4]
    if i==15:                                # o cuoi: co dong nhung ro hon
        P.K(b,0,1.0,a,tune=48); P.S(b,0,1.0,'center',a); P.CR(b,0,1.0,size=1.3)
        P.K(b+2,8,0.8,a); P.S(b+3,12,0.9,'center',a)
    else:
        dr_outro(b,1.12,a)
        if i%2==0: P.fill(b,1.5,'tom',0.85)
        if i%2==1: P.fill(b+2.5,1.5,'roll',0.7)
P.apply_chokes()
DRUMS=mix_kit(P.bus,room_amount=0.22,oh_amount=0.90,lofi=0.0,lpf=10000)

# ================= SAP XEP NHAC CU =================
noise_sw(fx,0,T(END),0.006,True,60,900)
GLIDE=[73,72,70,68,66,65,66,68]            # "harp gliding" mo dau
for i in range(8):
...
for i in range(4):
    ...
    wurli(kbd,T(b+1),ts[1]+12,1.4,0.06)
for i in range(16):
    b=bar_at('V2',i); ch=V2CH[i%8]; root,ts=CH[ch]
...
for i in range(4):
    ...
    supersaw(pad,T(b),[49,53,58,60],2.0,0.05,atk=0.5,seed=i,pump_=1.0)
for i in range(12):
...
for i in range(12):
    ...
    if i>=8: siren(fx,T(b),nn('Ab4'),nn('Db5'),3.5,0.04+0.01*(i-8))
for i in range(16):
...
V1A=[
 ...
 [(0,.5,'F4','u','t'),(0.5,.5,'Ab4','o','d'),(1,.5,'Gb4','e','z'),(1.5,.5,'F4','u','l'),
  (2,.5,'Ab4','e','s'),(2.5,.5,'F4','e','m'),(3,.5,'Gb4','u','t'),(3.5,.5,'Ab4','i','m')],
 # "i count them once, i count them twice"
 [(0,.5,'F4','a',''),(0.5,.5,'Ab4','a','k'),(1,.5,'Gb4','e','z'),(1.5,.5,'F4','a','w'),
  (2,.5,'F4','a',''),(2.5,.5,'Ab4','a','k'),(3,.5,'Gb4','e','z'),(3.5,.5,'F4','a','t')],
 # "the hall is turning into me"
 [(0,.5,'Gb4','e','z'),(0.5,.5,'F4','o','h'),(1,.5,'Ab4','i',''),(1.5,.5,'Ab4','u','t'),
  (2,.5,'Gb4','i','n'),(2.5,.5,'F4','i','n'),(3,.5,'Ab4','u','t'),(3.5,.5,'Gb4','i','m')],
 # "the double in the glass, i know him"
 [(0,.5,'F4','e','z'),(0.5,.5,'Ab4','a','d'),(1,.5,'Gb4','e','b'),(1.5,.5,'F4','i','n'),
  (2,.5,'Gb4','e','z'),(2.5,.5,'Ab4','a','g'),(3,.5,'F4','a',''),(3.5,.5,'Ab4','o','n')],
 # "he knows the way i'll never be"
 [(0,.5,'F4','i','h'),(0.5,.5,'Ab4','o','n'),(1,.5,'Gb4','e','z'),(1.5,.5,'F4','e','w'),
  (2,.5,'Ab4','e','l'),(2.5,.5,'Gb4','e','n'),(3,.5,'F4','e','v'),(3.5,.5,'Ab4','i','b')],
 # "i walk the line between the two"
 [(0,.5,'F4','a',''),(0.5,.5,'Ab4','o','w'),(1,.5,'Gb4','e','z'),(1.5,.5,'Ab4','a','l'),
  (2,.5,'Gb4','i','b'),(2.5,.5,'F4','i','t'),(3,.5,'Ab4','e','z'),(3.5,.5,'F4','u','t')],
 # "i lose the door, i find the view"
...
V3A=[
 [(0,.5,'F4','a',''),(0.5,.5,'Ab4','i','s'),(1,.5,'Gb4','e','z'),(1.5,.5,'F4','o','w'),
  (2,.5,'F4','a',''),(2.5,.5,'Ab4','i','s'),(3,.5,'Gb4','e','z'),(3.5,.5,'F4','u','m')],
 [(0,.5,'F4','u','t'),(0.5,.5,'Ab4','e','sh'),(1,.5,'Gb4','o','d'),(1.5,.5,'F4','a','d'),
  (2,.5,'Ab4','i','s'),(2.5,.5,'Gb4','o','n'),(3,.5,'F4','e','z'),(3.5,.5,'Gb4','o','w')],
 [(0,.5,'Gb4','e','z'),(0.5,.5,'Ab4','a','d'),(1,.5,'F4','e','b'),(1.5,.5,'Gb4','o','n'),
  (2,.5,'F4','e','m'),(2.5,.5,'Ab4','o','k'),(3,.5,'Gb4','a','m'),(3.5,.5,'F4','e','n')],
 [(0,.5,'F4','o','b'),(0.5,.5,'F4','o','v'),(1,.5,'Gb4','u',''),(1.5,.5,'F4','a',''),
  (2,.5,'Ab4','o','n'),(2.5,.5,'Gb4','e','z'),(3,.5,'F4','e','s'),(3.5,.5,'Gb4','e','m')],
 # "i see i see the light go dark"  (bars 8-9, co stutter tren "see")
 [(0,.5,'Ab4','a',''),(0.5,.5,'Ab4','i','s'),(1,.5,'Ab4','a',''),(1.5,.5,'Ab4','i','s'),
  (2,.5,'Bb4','e','z'),(2.5,.5,'Bb4','a','l'),(3,.5,'Ab4','o','g'),(3.5,.5,'Ab4','a','d')],
 # "i see the dark in me, the dark"  (bars 10-11, Dbm/Ab)
 [(0,.5,'Ab4','a',''),(0.5,.5,'Ab4','i','s'),(1,.5,'Gb4','e','z'),(1.5,.5,'F4','a','d'),
  (2,.5,'Ab4','i','k'),(2.5,.5,'Gb4','i','n'),(3,.5,'F4','i','m'),(3.5,.5,'Gb4','e','z')],
]
BR1=[(0,1.5,'Db5','a',''),(0.5,1.5,'Db5','u','d'),(1,1.5,'C5','a','r'),(1.5,1.5,'C5','a','l'),
     (2,1.5,'Bb4','a','f'),(2.5,1.5,'Bb4','o','r'),(3,1.5,'Ab4','i','m'),
     (3.25,0.75,'Ab4','u','t'),(3.5,0.5,'F4','e','m')]
BR2=[(0,1.5,'F4','a',''),(0.5,1.5,'F4','o','k'),(1,1.5,'Ab4','e','s'),(1.5,1.5,'Ab4','i','t'),
     (2,1.5,'Bb4','a','t'),(2.5,1.5,'Bb4','u','w'),(3,1.5,'Ab4','a','n'),
     (3.5,1.0,'Ab4','a','k'),(3.75,0.5,'F4','i','s')]
BR3=[(0,1.2,'Ab4','u','t'),(0.5,1.2,'Ab4','o','d'),(1,1.2,'Bb4','u','t'),(1.5,1.2,'Bb4','i','k'),
     (2,1.2,'C5','a','w'),(2.5,1.2,'C5','a','l'),(3,1.2,'Bb4','i','f'),(3.5,1.0,'Ab4','i','m')]
for i in range(16):
    b=bar_at('V1',i)
...
for i in range(12):
    b=bar_at('V3',i)
    ph=V3A[i//2]
    line(vx,b,ph,g=0.20,style='deadpan',breath=0.30,seedbase=900+i*13)
for i in range(12):
...
for i in range(12):
    b=bar_at('BRIDGE',i)
    ph=[BR1,BR2,BR3][i//4]
    line(vx,b,ph,g=0.17+0.004*i,style='croon',breath=0.34,seedbase=1400+i*13,jit=0.014)
shriek(vx,T(S['BRIDGE']+11*BAR+3.0),nn('Bb5'),1.2,0.10)
for i in range(16):
    b=bar_at('OUTRO',i)
    if i%4==1: chant(vx,b,LE_ME,g=0.07,n=4,spread=10,style='whisper',seedbase=1600+i)
    if i%4==3: shriek(vx,T(b+2.5),nn('Bb5'),0.6,0.07,seed=i)
for i in range(4):
    b=bar_at('TAIL',i)
    ph=[(0,1.0,'Db4','a',''),(1,1.0,'Db4','i','s'),(2,1.0,'Db4','a','d'),(3,1.5,'Db4','e','b'),
        (0,1.0,'Db4','u','t'),(1,1.0,'Db4','o','d'),(2,1.0,'Db4','a','s'),(3,1.5,'Db4','i','k')]
    line(vx,b,ph[4*i:4*i+4],g=0.075,style='whisper',breath=0.35,seedbase=1800+i,jit=0.008)
# ---- fractured: stutter tren "see" (bars 8-9 cua V3) ----
V3b=S['V3']+8*BAR
for off in (0.5,1.5):
    stutter_buf(vx,T(V3b+off),0.22,n=4,step_ms=85)

# ================= MIX =================
STEMS=[(pad,0.0,0.60,0.55,0.0),(kbd,0.35,0.58,0.50,0.0),(brs,-0.30,0.55,0.50,0.0),
       (hp_,-0.45,0.62,0.45,7.0),(fx,0.0,1.0,0.0,0.0)]
MAPT=[(n,a,b_,(5.0 if n in('POST','TAIL') else 6.0 if n=='BRIDGE' else
               3.4 if n.startswith('CH') else 1.0 if n in('INTRO','INSTR','OUTRO') else 2.6))
      for n,a,b_ in MAP]
def _build(voc):
