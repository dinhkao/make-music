...
# =======================================================================
# SONG — STAY ANYWAY (D major, 84 bpm, 62 bars = 2:57)
#
# Westlife "Swear It Again" trick: the final chorus is NOT a key change —
# it peaks through arrangement (bridge built to a dominant tease, bigger
# drums, string stabs, fuller stacks). Verified: Swear It Again teases a
# modulation in the bridge but never modulates; climax comes from the build.
#
# Harmony — Radiohead "Creep" (1992): G - B - C - Cm (I - III - IV - iv),
# same cycle for intro/verse/chorus, 2 bars per chord. The sound is the
# chromatic voice-leading D-D#-E-Eb-D and the borrowed minor iv. Transposed
# to D:  D - F# - G - Gm.
#   intro/verse/chorus: D(2) F#(2) G(2) Gm(2)      = 8-bar cycle
#   pre-chorus (My Love, +2): Em G | A D | G Em | G A
#   bridge (My Love, +2):     A Bm E A F#m Bm | G A   -> ends on V (tease)
#   coda: D F# G Gm — fade on the iv, the Creep fingerprint.
# =======================================================================
BPM = 84.0
BEAT = 60.0 / BPM
BAR = 4.0
...
DUR = T(END_BEATS)
TOTAL = DUR + 3.5
print('STAY ANYWAY  bars', int(END_BEATS / BAR), ' duration', round(DUR, 1), 's')

CH = {
 'D':   (38, (50,54,57,62), 38),   'F#':  (42, (54,58,61,66), 42),
 'G':   (43, (55,59,62,67), 43),   'Gm':  (43, (55,58,62,67), 43),
 'Em':  (40, (52,55,59,64), 40),   'Em7': (40, (52,55,59,62), 40),
 'A':   (45, (57,61,64,69), 45),   'Bm':  (47, (59,62,66,71), 47),
 'E':   (40, (52,56,59,64), 40),   'F#m': (42, (54,57,61,66), 42),
 'A7':  (45, (57,61,64,67), 45),
}
SPLIT = {'EmG': ('Em','G'), 'AD': ('A','D'), 'GEm': ('G','Em'),
         'GA': ('G','A'), 'F#mBm': ('F#m','Bm')}
V1BAR  = ['D','D','F#','F#','G','G','Gm','Gm']
V2BAR  = ['D','D','F#','F#','G','G','Gm','Gm']
PREBAR = ['EmG','AD','GEm','GA']
CHBAR  = ['D','D','F#','F#','G','G','Gm','Gm']
BRBAR  = ['A','Bm','E','A','F#mBm','GA']
CH3BAR = ['D','D','F#','F#','G','G','Gm','Gm']     # NO key change
CODABAR= ['D','F#','G','Gm']

BASSL = {
 'D':   [(0,38,1.7,.30),(1.7,45,1.2,.22)],
 'F#':  [(0,42,1.7,.30),(1.7,49,1.2,.24)],
 'G':   [(0,43,1.7,.30),(1.7,50,1.2,.22)],
 'Gm':  [(0,43,1.7,.30),(1.7,50,1.2,.22)],
 'Em':  [(0,40,1.5,.30),(1.5,43,.6,.22),(2.1,47,.8,.20)],
 'Em7': [(0,40,1.5,.30),(1.5,43,.6,.22),(2.1,47,.8,.20)],
 'A':   [(0,45,1.6,.32),(1.6,52,1.3,.24)],
...
BASSC = {
 'D':  [(0,38,2.9,.30)],  'F#': [(0,42,2.9,.30)],
 'G':  [(0,43,2.9,.30)],  'Gm': [(0,43,2.9,.30)],
}

bus = Bus(); drm = Bus(); vx = Bus()

V1 = [
 # "counting cracks along the ceiling"
 [(0,.5,'D4','ow','k','nt'),(.5,.5,'D4','i','','ng'),(1.0,.45,'E4','a','k','k'),
  (1.45,.55,'F#4','a','',''),(2.0,.5,'F#4','o','l','ng'),(2.5,.45,'A4','e','th',''),
  (2.95,.5,'A4','i','s',''),(3.45,.55,'B4','i','l','ng')],
 # "like they're stars i never knew"
 [(0,.5,'F#4','ai','l',''),(.5,.45,'A4','e','th',''),(.95,.5,'A4','a','st','rs'),
  (1.45,.45,'B4','ai','',''),(1.9,.5,'B4','e','n','v'),(2.4,.45,'A4','e','','r'),
  (2.85,.6,'B4','u','n','w')],
 # "half the man i could be feeling"
 [(0,.5,'A4','a','h','lf'),(.5,.45,'B4','e','th',''),(.95,.5,'A4','a','m','n'),
  (1.45,.4,'A4','ai','',''),(1.85,.45,'G4','u','k','d'),(2.3,.4,'A4','i','b',''),
  (2.7,.5,'B4','i','f','l'),(3.2,.7,'B4','i','','ng')],
 # "half the person that is you"
 [(0,.5,'A4','a','h','lf'),(.5,.45,'B4','e','th',''),(.95,.5,'G4','u','p','r'),
  (1.45,.5,'G4','u','s','n'),(1.95,.45,'A4','a','th','t'),(2.4,.4,'F#4','i','','z'),
  (2.8,.8,'E4','u','y','')],
 # "but when the room goes quiet"
 [(0,.5,'E4','u','b','t'),(.5,.5,'F#4','e','w','n'),(1.0,.45,'G4','e','th',''),
  (1.45,.55,'A4','u','r','m'),(2.0,.5,'B4','o','g','z'),(2.5,.5,'B4','ai','k','w'),
  (3.0,.7,'A4','e','','t')],
 # "and the dark begins to fall"
 [(0,.5,'A4','a','','n'),(.5,.45,'B4','e','th',''),(.95,.5,'A4','a','d','r'),
  (1.45,.5,'B4','i','b',''),(1.95,.5,'A4','i','g','n'),(2.45,.45,'G4','u','t',''),
  (2.9,.8,'F#4','o','f','l')],
 # "you're the only light i know"
 [(0,.5,'F#4','o','y','r'),(.5,.45,'G4','e','th',''),(.95,.5,'A4','o','','n'),
  (1.45,.5,'B4','i','l',''),(1.95,.5,'B4','ai','l','t'),(2.45,.45,'A4','ai','',''),
  (2.9,.8,'B4','o','n','w')],
 # "the only one who hears me call"
 [(0,.5,'A4','e','th',''),(.5,.45,'B4','o','','n'),(.95,.45,'A4','i','l',''),
  (1.4,.5,'G4','u','w','n'),(1.9,.5,'F#4','u','h',''),(2.4,.5,'E4','i','h','r'),
  (2.9,.45,'D4','i','m',''),(3.35,.6,'E4','o','k','l')],
]
PRE = [
 # "and i'm not what you deserve"
 [(0,.55,'B4','a','','n'),(.55,.5,'B4','ai','','m'),(1.05,.5,'A4','o','n','t'),
  (1.55,.5,'A4','u','w','t'),(2.05,.5,'G4','u','y',''),(2.55,.5,'A4','i','d',''),
  (3.05,.7,'B4','u','s','r')],
 # "i'm a storm without a shore"
 [(0,.5,'C5','ai','','m'),(.5,.45,'C5','a','',''),(.95,.55,'D5','o','st','rm'),
  (1.5,.5,'C5','i','w','th'),(2.0,.5,'B4','ow','','t'),(2.5,.45,'A4','a','',''),
  (2.95,.7,'B4','o','sh','r')],
 # "but you pull the thunder in"
 [(0,.55,'C5','u','b','t'),(.55,.5,'D5','u','y',''),(1.05,.55,'C5','u','p','l'),
  (1.6,.5,'B4','e','th',''),(2.1,.5,'A4','u','th','n'),(2.6,.5,'G4','e','d','r'),
  (3.1,.7,'A4','i','','n')],
 # "and you love me anyway"
 [(0,.5,'B4','a','','n'),(.5,.5,'C5','u','y',''),(1.0,.55,'D5','u','l','v'),
  (1.55,.5,'C5','i','m',''),(2.05,.5,'B4','e','','n'),(2.55,.45,'B4','i','',''),
  (3.0,.8,'D5','ay','w','')],
]
HOOK = [
 # "you stay anyway"
 [(0,1.0,'A4','u','y',''),(1.0,.8,'D5','ay','st',''),(1.8,.5,'C#5','e','','n'),
  (2.3,1.6,'B4','ay','w','')],
 # "when the walls come down"
 [(0,.6,'B4','e','w','n'),(.6,.5,'D5','e','th',''),(1.1,.55,'C#5','o','w','l'),
  (1.65,.55,'A4','u','k','m'),(2.2,1.7,'B4','ow','d','n')],
 # "you stay anyway" (higher landing)
 [(0,1.0,'A4','u','y',''),(1.0,.8,'D5','ay','st',''),(1.8,.5,'C#5','e','','n'),
  (2.3,1.6,'E5','ay','w','')],
 # "when i'm lost not found"
 [(0,.6,'D5','e','w','n'),(.6,.5,'C#5','ai','','m'),(1.1,.55,'B4','o','l','st'),
  (1.65,.55,'A4','o','n','t'),(2.2,1.7,'B4','ow','f','nd')],
 # "i don't know what i did right"
 [(0,.5,'A4','ai','',''),(.5,.5,'D5','o','d','nt'),(1.0,.5,'D5','o','n','w'),
  (1.5,.45,'C#5','u','w','t'),(1.95,.45,'B4','ai','',''),(2.4,.5,'A4','i','d',''),
  (2.9,.6,'B4','ai','r','t')],
 # "to keep you by my side"
 [(0,.5,'B4','u','t',''),(.5,.5,'D5','i','k','p'),(1.0,.45,'C#5','u','y',''),
  (1.45,.5,'B4','ai','b',''),(1.95,.5,'A4','ai','m',''),(2.45,.9,'B4','ai','s','d')],
 # "but you stay, you stay"
 [(0,.55,'G4','u','b','t'),(.55,.5,'A4','u','y',''),(1.05,.9,'B4','ay','st',''),
  (1.95,.5,'A4','u','y',''),(2.45,1.4,'C#5','ay','st','')],
 # "anyway"
 [(0,.6,'B4','e','','n'),(.6,.5,'D5','ay','w',''),(1.1,1.8,'B4','o','','')],
]
V2 = [
 # "the mirror tells a different truth"
 [(0,.5,'D4','e','th',''),(.5,.5,'D4','i','m','r'),(1.0,.45,'E4','e','r',''),
  (1.45,.55,'F#4','e','t','l'),(2.0,.45,'A4','a','',''),(2.45,.5,'A4','i','d',''),
  (2.95,.5,'G4','e','f','r'),(3.45,.55,'A4','u','tr','th')],
 # "than the one you see in me"
 [(0,.5,'A4','a','th','n'),(.5,.45,'B4','e','th',''),(.95,.5,'A4','u','w','n'),
  (1.45,.5,'G4','u','y',''),(1.95,.5,'F#4','i','s',''),(2.45,.45,'E4','i','','n'),
  (2.9,.8,'D4','i','m','')],
 # "i hand you all my broken parts"
 [(0,.5,'F#4','ai','',''),(.5,.5,'G4','a','h','nd'),(1.0,.45,'A4','u','y',''),
  (1.45,.5,'B4','o','','l'),(1.95,.5,'A4','ai','m',''),(2.45,.5,'G4','o','b','r'),
  (2.95,.5,'F#4','e','k','n'),(3.45,.6,'E4','a','p','rtz')],
 # "and you hand me back the key"
 [(0,.5,'E4','a','','n'),(.5,.5,'F#4','u','y',''),(1.0,.55,'G4','a','h','nd'),
  (1.55,.5,'A4','i','m',''),(2.05,.5,'B4','a','b','k'),(2.55,.45,'A4','e','th',''),
  (3.0,.7,'B4','i','k','')],
 # "and when the thunder rolls"
 [(0,.5,'A4','a','','n'),(.5,.5,'B4','e','w','n'),(1.0,.45,'A4','e','th',''),
  (1.45,.55,'G4','u','th','n'),(2.0,.5,'F#4','e','d','r'),(2.5,.9,'E4','o','r','l')],
 # "and the floor begins to shake"
 [(0,.5,'F#4','a','','n'),(.5,.45,'G4','e','th',''),(.95,.5,'A4','o','f','l'),
  (1.45,.5,'B4','i','b',''),(1.95,.5,'A4','i','g','n'),(2.45,.45,'G4','u','t',''),
  (2.9,.7,'F#4','ay','sh','k')],
 # "you're the ground i'm standing on"
 [(0,.5,'F#4','o','y','r'),(.5,.45,'G4','e','th',''),(.95,.55,'A4','ow','gr','nd'),
  (1.5,.5,'B4','ai','','m'),(2.0,.5,'B4','a','st',''),(2.5,.45,'A4','i','','ng'),
  (2.95,.6,'B4','o','','n')],
 # "the calm inside the earthquake"
 [(0,.5,'A4','e','th',''),(.5,.5,'B4','a','k','l'),(1.0,.45,'A4','i','','n'),
  (1.45,.5,'G4','ai','s','d'),(1.95,.45,'B4','e','th',''),(2.4,.5,'A4','u','','r'),
  (2.9,.6,'B4','ay','k','w')],
]
BR = [
 # "i've tried to be the person"
 [(0,.6,'D4','ai','','v'),(.6,.55,'D4','ai','tr','d'),(1.15,.5,'E4','u','t',''),
  (1.65,.5,'F#4','i','b',''),(2.15,.5,'G4','e','th',''),(2.65,.5,'A4','u','p','r'),
  (3.15,.55,'A4','u','s','n')],
 # "that i see inside your eyes"
 [(0,.5,'A4','a','th','t'),(.5,.45,'B4','ai','',''),(.95,.5,'C5','i','s',''),
  (1.45,.5,'B4','i','','n'),(1.95,.5,'A4','ai','s','d'),(2.45,.5,'G4','o','y','r'),
  (2.95,.6,'F#4','ai','','z')],
 # "but every time i stumble"
 [(0,.55,'F#4','u','b','t'),(.55,.5,'G4','e','','v'),(1.05,.5,'A4','i','r',''),
  (1.55,.55,'B4','ai','t','m'),(2.1,.5,'A4','ai','',''),(2.6,.5,'G4','u','st','m'),
  (3.1,.5,'A4','u','b','l')],
 # "you catch me before the floor"
 [(0,.5,'B4','u','y',''),(.5,.55,'C5','a','k','ch'),(1.05,.5,'B4','i','m',''),
  (1.55,.5,'A4','i','b',''),(2.05,.5,'G4','o','f','r'),(2.55,.5,'F#4','e','th',''),
  (3.05,.6,'E4','o','f','l')],
 # "carry me across the sea"
 [(0,.5,'G4','a','k','r'),(.5,.45,'A4','i','r',''),(.95,.5,'B4','i','m',''),
  (1.45,.4,'A4','a','',''),(1.85,.5,'G4','o','k','r'),(2.35,.45,'A4','e','th',''),
  (2.8,.7,'B4','i','s','')],
 # "put your faith in what we are"
 [(0,.55,'B4','u','p','t'),(.55,.5,'D5','o','y','r'),(1.05,.55,'C5','ay','f','th'),
  (1.6,.5,'B4','i','','n'),(2.1,.5,'A4','u','w','t'),(2.6,.5,'B4','i','w',''),
  (3.1,.6,'C5','a','','r')],
]
CODA_1 = [(0,.9,'B4','u','y',''),(.9,1.5,'D5','ay','st','')]      # "you stay"
CODA_2 = [(0,.8,'D5','e','','n'),(.8,2.0,'E5','ay','w','')]       # "anyway" (over Gm)

# ---------------------------------------------------------- arrangement
# ---- INTRO: keys arp the Creep cycle; the maj3/iv color notes land by chord
for i in range(4):
    b = S['INTRO'] + i * BAR
    ch = ['D','F#','G','Gm'][i]
    ...
    if i == 2:
        bus.add(T(b + 0.5), pluck(mhz(CH[ch][1][1] + 12), 1.1, 0.14), pan=0.25)
...
# ---- BRIDGE: stripped; ends on A (V of D) — the tease that never lands ----
for i in range(6):
    b = S['BRIDGE'] + i * BAR
...
# ---- CH3: no key change — peak by arrangement (Swear It Again) ----
for i in range(8):
    b = S['CH3'] + i * BAR
    ch = CHBAR[i]
...
# ---- CODA: re-sing the hook over the cycle, fade on the iv ----
for i in range(4):
    b = S['CODA'] + i * BAR
    ch = CODABAR[i]
    padbar(b, ch, gain=0.10, cut=2000, atk=1.5, seed=2100 + i)
...
b2 = S['CODA'] + 2 * BAR          # "you stay" over G
for ev in CODA_1:
    ...
    choir(vx, b2, CH['G'][1], gain=0.08, seedbase=2700)
b3 = S['CODA'] + 3 * BAR          # "anyway" over Gm — the iv moment
for ev in CODA_2:
    line(vx, b3, [ev], gain=1.0, style='croon', seedbase=2800)
    ...
    choir(vx, b3, CH['Gm'][1], gain=0.09, seedbase=3300)

master(bus.L, bus.R, vx.L, vx.R, 'stay-anyway.wav', DUR)
master(bus.L, bus.R, None, None, 'stay-anyway-instrumental.wav', DUR)
