...
BAR = 4.0
DUR = 60 * BAR * BEAT      # 60 bars = 180.0 s (3:00 max)
...
# =======================================================================
# SONG — AIRBORNE (G major, 80 bpm, 60 bars = 3:00)
# =======================================================================
...
END_BEATS = _b
DUR = T(END_BEATS)
print('AIRBORNE  bars', int(END_BEATS / BAR), ' duration', round(DUR, 1), 's')

# chords: (root midi, voicing midi, bass root midi)
CH = {
 'G':     (43, (55,59,62,67), 43),   'D':     (38, (50,54,57,62), 38),
 'Dmaj7': (38, (50,54,57,61), 38),   'Em7':   (40, (52,55,59,62), 40),
 'A':     (45, (57,61,64,69), 45),   'Gmaj7': (43, (55,59,62,66), 43),
 'E':     (40, (52,56,59,64), 40),   'Emaj7': (40, (52,56,59,63), 40),
 'Amaj7': (45, (57,61,64,68), 45),   'F#m7':  (42, (54,57,61,64), 42),
 'B':     (47, (59,63,66,70), 47),   'Bmaj7': (47, (59,63,66,70), 47),
 'Em':    (40, (52,55,59,64), 40),
}
# two-chord bars: (chord at beat 0, chord at beat 2) — the 1979 2-chord beats
SPLIT = {'Dmaj7D': ('Dmaj7','D'), 'Em7A': ('Em7','A'), 'Dmaj7Gmaj7': ('Dmaj7','Gmaj7'),
         'EmA': ('Em','A'), 'EmG': ('Em','G'), 'Bmaj7B': ('Bmaj7','B'),
         'Emaj7Amaj7': ('Emaj7','Amaj7'), 'F#m7B': ('F#m7','B')}
# the 1979 progression in G: verse I-V7-V, chorus a fifth down, bridge vi-IV-V
V1BAR  = ['G','Dmaj7D','G','Em7A'] * 2
V2BAR  = ['G','Dmaj7D','G','Em7A','G','Em7A']
PREBAR = ['Em7','G','A','A']
CHBAR  = ['D','Dmaj7Gmaj7','D','Em7A'] * 2
BRBAR  = ['Em','G','A','EmA','EmG','Em7A']
CH3BAR = ['E','Emaj7Amaj7','E','F#m7B'] * 2      # whole step up (My Love lift)
CODABAR= ['E','Bmaj7B','E','E']

def bar_chords(bardef):
    """per-bar list of (chord, split_beat) pairs"""
    out = []
    for c in bardef:
        if c in SPLIT:
            ch1, ch2 = SPLIT[c]
...
# ================================================================ vocals
V1 = [
 # "the rain falls on the same old street"
 [(0,.55,'D4','e','th',''),(.55,.5,'D4','ai','r',''),(1.05,.55,'E4','o','f',''),
  (1.6,.5,'E4','o','','n'),(2.1,.5,'G4','e','th',''),(2.6,.55,'G4','ay','s','m'),
  (3.15,.45,'A4','ow','','l'),(3.6,.4,'A4','i','st','t')],
 # "i walk it every day"
 [(0,.5,'F#4','ai','',''),(.5,.6,'G4','o','w','k'),(1.1,.4,'G4','i','','t'),
  (1.5,.5,'A4','e','','v'),(2.0,.45,'A4','i','r',''),(2.45,.6,'B4','ay','d','')],
 # "my head is full of dreams i keep"
 [(0,.5,'B4','ai','m',''),(.5,.5,'G4','e','h','d'),(1.0,.45,'G4','i','','z'),
  (1.45,.5,'F#4','u','f','l'),(1.95,.4,'E4','o','','v'),(2.35,.55,'D4','i','dr','m'),
  (2.9,.45,'G4','ai','',''),(3.35,.55,'D4','i','k','p')],
 # "that always slip away"
 [(0,.5,'E4','a','th','t'),(.5,.5,'G4','o','','l'),(1.0,.55,'A4','ay','w','z'),
  (1.55,.5,'G4','i','sl','p'),(2.05,.5,'A4','a','',''),(2.55,.9,'B4','ay','w','')],
 # "but tonight the sky has changed"
 [(0,.5,'A4','u','b','t'),(.5,.5,'B4','u','t',''),(1.0,.55,'A4','ai','n','t'),
  (1.55,.5,'G4','e','th',''),(2.05,.55,'G4','ai','sk',''),(2.6,.5,'A4','a','h','z'),
  (3.1,.8,'B4','ay','ch','n')],
 # "it's leaning in to hear"
 [(0,.5,'F#4','i','','ts'),(.5,.55,'A4','i','l','n'),(1.05,.5,'A4','i','','ng'),
  (1.55,.45,'B4','i','','n'),(2.0,.45,'G4','u','t',''),(2.45,1.4,'A4','i','h','r')],
 # "and every step i take is light"
 [(0,.5,'A4','a','','n'),(.5,.5,'B4','e','','v'),(1.0,.45,'A4','i','r',''),
  (1.45,.55,'G4','e','st','p'),(2.0,.45,'A4','ai','',''),(2.45,.45,'B4','ay','t','k'),
  (2.9,.4,'A4','i','','z'),(3.3,.6,'B4','ai','l','t')],
 # "now that i know you're near"
 [(0,.55,'A4','ow','n',''),(.55,.5,'G4','a','th','t'),(1.05,.5,'A4','ai','',''),
  (1.55,.55,'G4','o','n','w'),(2.1,.5,'F#4','o','y','r'),(2.6,1.3,'E4','i','n','r')],
]
PRE = [
 # "and if the sky should fall tonight"
 [(0,.55,'B4','a','','n'),(.55,.45,'B4','i','f',''),(1.0,.5,'A4','e','th',''),
  (1.5,.5,'A4','ai','sk',''),(2.0,.5,'G4','u','sh','d'),(2.5,.45,'G4','o','f','l'),
  (2.95,.4,'F#4','u','t',''),(3.35,.6,'A4','ai','n','t')],
 # "i would still be standing"
 [(0,.5,'A4','ai','',''),(.5,.55,'B4','u','w','d'),(1.05,.55,'C5','i','st','l'),
  (1.6,.5,'B4','i','b',''),(2.1,.55,'A4','a','st',''),(2.65,1.3,'B4','i','','ng')],
 # "if the world forgets my name"
 [(0,.5,'C5','i','f',''),(.5,.45,'D5','e','th',''),(.95,.5,'C5','uh','w','r'),
  (1.45,.5,'B4','o','f','r'),(1.95,.5,'A4','e','g','ts'),(2.45,.5,'G4','ai','m',''),
  (2.95,.95,'B4','ay','n','m')],
 # "i'll remember yours"
 [(0,.5,'C5','ai','','l'),(.5,.55,'D5','i','r',''),(1.05,.55,'E5','e','m',''),
  (1.6,.55,'E5','e','b','r'),(2.15,1.7,'E5','o','y','r')],
]
HOOK = [
 # "i'm airborne, airborne"
 [(0,.8,'A4','ai','','m'),(.8,.5,'A4','ai','',''),(1.3,.7,'C5','o','b','r'),
  (2.0,.4,'D5','ai','',''),(2.4,1.5,'D5','o','b','r')],
 # "the ground is far below"
 [(0,.5,'C5','e','th',''),(.5,.55,'D5','ow','gr','nd'),(1.05,.45,'B4','i','','z'),
  (1.5,.5,'A4','a','f','r'),(2.0,.5,'G4','i','b',''),(2.5,1.4,'A4','ow','l','')],
 # "i'm airborne, airborne" (higher landing)
 [(0,.8,'A4','ai','','m'),(.8,.5,'A4','ai','',''),(1.3,.7,'C5','o','b','r'),
  (2.0,.4,'D5','ai','',''),(2.4,1.5,'E5','o','b','r')],
 # "and it's you that makes me go"
 [(0,.5,'D5','a','','n'),(.5,.4,'C5','i','','ts'),(.9,.45,'B4','u','y',''),
  (1.35,.45,'A4','a','th','t'),(1.8,.5,'G4','ay','m','k'),(2.3,.45,'A4','i','m',''),
  (2.75,.7,'B4','o','g','')],
 # "i'm not afraid of falling"
 [(0,.55,'A4','ai','','m'),(.55,.45,'C5','o','n','t'),(1.0,.5,'D5','a','',''),
  (1.5,.5,'D5','ay','f','r'),(2.0,.45,'C5','o','','v'),(2.45,.5,'B4','o','f',''),
  (2.95,1.0,'C5','i','','ng')],
 # "i'm afraid of staying down"
 [(0,.55,'C5','ai','','m'),(.55,.45,'D5','a','',''),(1.0,.5,'D5','ay','f','r'),
  (1.5,.45,'E5','o','','v'),(1.95,.5,'C5','ay','st',''),(2.45,.5,'B4','i','','ng'),
  (2.95,1.0,'A4','ow','d','n')],
 # "so hold me like the morning"
 [(0,.55,'G4','o','s',''),(.55,.55,'A4','o','h','l'),(1.1,.5,'B4','i','m',''),
  (1.6,.5,'C5','ai','l',''),(2.1,.45,'B4','e','th',''),(2.55,.55,'A4','o','m','r'),
  (3.1,1.0,'G4','i','','ng')],
 # "till i touch the clouds"
 [(0,.5,'G4','i','t','l'),(.5,.45,'A4','ai','',''),(.95,.5,'B4','u','t','ch'),
  (1.45,.5,'A4','e','th',''),(1.95,.6,'G4','ow','k','l'),(2.55,1.4,'B4','o','','')],
]
V2 = [
 # "people talk about the future"
 [(0,.5,'D4','i','p',''),(.5,.45,'D4','u','p','l'),(.95,.5,'E4','o','t','k'),
  (1.45,.45,'G4','a','',''),(1.9,.5,'G4','ow','b','t'),(2.4,.45,'A4','e','th',''),
  (2.85,.5,'G4','u','f',''),(3.35,.5,'A4','u','t','r')],
 # "like it's written in stone"
 [(0,.5,'A4','ai','l',''),(.5,.45,'B4','i','','ts'),(.95,.5,'A4','i','r',''),
  (1.45,.5,'G4','e','t','n'),(1.95,.45,'A4','i','','n'),(2.4,1.5,'G4','o','st','n')],
 # "but i just need tomorrow"
 [(0,.5,'A4','u','b','t'),(.5,.45,'B4','ai','',''),(.95,.5,'A4','u','j','st'),
  (1.45,.55,'G4','i','n','d'),(2.0,.5,'A4','u','t',''),(2.5,.5,'G4','o','m','r'),
  (3.0,1.0,'B4','ow','r','')],
 # "if tomorrow you're at home"
 [(0,.5,'B4','i','f',''),(.5,.5,'A4','u','t',''),(1.0,.5,'G4','o','m','r'),
  (1.5,.55,'F#4','ow','r',''),(2.05,.5,'E4','o','y','r'),(2.55,.45,'D4','a','','t'),
  (3.0,.9,'E4','o','h','m')],
 # "the moon is just a flashlight"
 [(0,.5,'A4','e','th',''),(.5,.5,'B4','u','m','n'),(1.0,.45,'A4','i','','z'),
  (1.45,.5,'G4','u','j','st'),(1.95,.4,'A4','a','',''),(2.35,.55,'B4','a','f','sh'),
  (2.9,.6,'A4','ai','l','t')],
 # "and every road leads back to you"
 [(0,.5,'A4','a','','n'),(.5,.45,'B4','e','','v'),(.95,.45,'A4','i','r',''),
  (1.4,.5,'G4','o','r','d'),(1.9,.5,'F#4','i','l','d'),(2.4,.45,'E4','a','b','k'),
  (2.85,.4,'D4','u','t',''),(3.25,.6,'E4','u','y','')],
]
BR = [
 # "when the dark comes calling"
 [(0,.6,'D4','e','w','n'),(.6,.5,'D4','e','th',''),(1.1,.55,'E4','a','d','r'),
  (1.65,.55,'G4','u','k','m'),(2.2,.5,'G4','o','k','l'),(2.7,1.2,'A4','i','','ng')],
 # "and my knees give way"
 [(0,.5,'A4','a','','n'),(.5,.5,'B4','ai','m',''),(1.0,.55,'C5','i','n','z'),
  (1.55,.5,'B4','i','g','v'),(2.05,1.8,'G4','ay','w','')],
 # "you're the only answer"
 [(0,.6,'F#4','o','y','r'),(.6,.5,'G4','e','th',''),(1.1,.55,'A4','o','','n'),
  (1.65,.5,'B4','i','l',''),(2.15,.5,'C5','a','','n'),(2.65,1.3,'B4','e','s','r')],
 # "to the questions i fake"
 [(0,.5,'B4','u','t',''),(.5,.45,'C5','e','th',''),(.95,.5,'B4','e','k','w'),
  (1.45,.5,'A4','e','sh','n'),(1.95,.45,'G4','ai','',''),(2.4,1.5,'F#4','ay','f','k')],
 # "carry me across the distance"
 [(0,.5,'G4','a','k','r'),(.5,.45,'A4','i','r',''),(.95,.5,'B4','i','m',''),
  (1.45,.4,'A4','a','',''),(1.85,.5,'G4','o','k','r'),(2.35,.45,'A4','e','th',''),
  (2.8,.5,'B4','i','d',''),(3.3,.6,'C5','e','st','n')],
 # "put your heartbeat under mine"
 [(0,.55,'B4','u','p','t'),(.55,.5,'D5','o','y','r'),(1.05,.55,'C5','a','h','r'),
  (1.6,.5,'B4','i','b','t'),(2.1,.5,'A4','u','','n'),(2.6,.5,'B4','e','d','r'),
  (3.1,1.3,'C5','ai','m','n')],
]
CH3 = [shift(l, 2) for l in HOOK]      # the whole hook, up a whole step
CODA_1 = [(0,1.0,'E5','ai','','m'),(1.0,.7,'E5','ai','',''),(1.7,1.8,'F#5','o','b','r')]
CODA_2 = [(0,.5,'E5','i','t','l'),(.5,.45,'F#5','ai','',''),(.95,.5,'G#5','u','t','ch'),
          (1.45,.5,'F#5','e','th',''),(1.95,2.1,'G#5','ow','k','l')]

# ---------------------------------------------------------- arrangement
# ---- INTRO: a cappella 4-part block (G / Dmaj7), keys+pad join in
INTOP = [  # (voice, bar0 events, bar1 events)
 ('top',   [(0,1.4,'G4','a','',''),(1.5,1.4,'B4','a','','')],
           [(0,1.4,'A4','a','',''),(1.5,1.4,'C#5','a','','')]),
 ('mid',   [(0,1.4,'D4','a','',''),(1.5,1.4,'G4','a','','')],
           [(0,1.4,'F#4','a','',''),(1.5,1.4,'A4','a','','')]),
 ('tenor', [(0,1.4,'B3','a','',''),(1.5,1.4,'D4','a','','')],
           [(0,1.4,'D4','a','',''),(1.5,1.4,'F#4','a','','')]),
 ('low',   [(0,1.4,'G3','a','',''),(1.5,1.4,'G3','a','','')],
           [(0,1.4,'D3','a','',''),(1.5,1.4,'A3','a','','')]),
]
for (vn, ev0, ev1) in INTOP:
    line(vx, S['INTRO'], ev0, gain=0.55, style='croon', seedbase=10)
    line(vx, S['INTRO'] + BAR, ev1, gain=0.55, style='croon', seedbase=20)
for i in (2, 3):
    b = S['INTRO'] + i * BAR
    ch = 'G' if i == 2 else 'Dmaj7'
    padbar(b, ch, gain=0.10, cut=2000, atk=1.6, seed=i)
    keysarp(b, ch, gain=0.13)
    choir(vx, b, CH[ch][1], gain=0.07, seedbase=30 + i)

# ---- V1: voice + keys, bass from bar 2, Villa groove from bar 5 ----
for i in range(8):
    b = S['V1'] + i * BAR
    sl = bar_chords(V1BAR)[i]
    keysbar_split(b, sl, gain=0.14)
    padbar(b, chord_at(sl, 0), gain=0.09, cut=2100, atk=1.2, seed=100 + i)
    if i >= 1:
        bassbar_split(b, sl, BASSL)
    if i >= 4:
        vverse(b, vel=0.55)
    line(vx, b, V1[i], gain=0.80, style='croon', seedbase=200 + i * 17)

# ---- PRE1: build, keys + pluck, kick creeping to 4-floor ----
for i in range(4):
    b = S['PRE1'] + i * BAR
    ch = PREBAR[i]
    padbar(b, ch, gain=0.12 + 0.02 * i, cut=2600 + 400 * i, atk=0.7, seed=300 + i)
    keysarp(b, ch, gain=0.16)
    bus.add(T(b + 0.5), pluck(mhz(CH[ch][1][1] + 12), 1.1, 0.15), pan=0.25)
    bassbar(b, ch, BASSP)
    vpre(b, i, vel=0.7)
    line(vx, b, PRE[i], gain=0.95, style='croon', seedbase=400 + i * 17)

# ---- CH1: the lift. full Villa disco + Westlife stack ----
for i in range(8):
    b = S['CH1'] + i * BAR
    sl = bar_chords(CHBAR)[i]
    padbar(b, chord_at(sl, 0), gain=0.16, cut=4200, atk=0.35, seed=500 + i)
    keysbar_split(b, sl, gain=0.16)
    bassbar_split(b, sl, BASS8)
...
# ---- V2: fuller voice, Villa groove in from bar 1 ----
for i in range(6):
    b = S['V2'] + i * BAR
    sl = bar_chords(V2BAR)[i]
    keysbar_split(b, sl, gain=0.14)
    padbar(b, chord_at(sl, 0), gain=0.10, cut=2300, atk=1.0, seed=800 + i)
    bassbar_split(b, sl, BASSL)
    vverse(b, vel=0.6)
...
for i in range(8):
    ...
    drm.add(T(b), crash(0.75))                       # crash every bar
    if i % 2 == 0:                                   # string stabs at the lift
...
# ---- CODA: re-sing the hook a cappella-style, fade ----
for i in range(4):
    b = S['CODA'] + i * BAR
    sl = bar_chords(CODABAR)[i]
    padbar(b, chord_at(sl, 0), gain=0.10, cut=2000, atk=1.5, seed=2100 + i)
    keysbar_split(b, sl, gain=0.12)
    bassbar_split(b, sl, BASSC)
    if i < 2:
        drm.add(T(b + 0), kick(0.38)); drm.add(T(b + 2), kick(0.32))
        drm.add(T(b + 2), rim(0.3))
        for p in range(8):
            drm.add(T(b + p * 0.5), hat(0.10))
# final re-sing with the 4-part stack
b2 = S['CODA'] + 2 * BAR
for ev in CODA_1:
    nm2 = ev[2]
    line(vx, b2, [ev], gain=1.0, style='croon', seedbase=2200)
    line(vx, b2, [shift([ev], -4)[0]], gain=0.5, style='croon', seedbase=2300)
    line(vx, b2, [shift([ev], -7)[0]], gain=0.4, style='croon', seedbase=2400)
    line(vx, b2, [shift([ev], 12)[0]], gain=0.30, style='croon', seedbase=2500)
    line(vx, b2, [shift([ev], -12)[0]], gain=0.34, style='croon', seedbase=2600)
    choir(vx, b2, CH['E'][1], gain=0.08, seedbase=2700)
b3 = S['CODA'] + 3 * BAR
for ev in CODA_2:
    line(vx, b3, [ev], gain=1.1, style='croon', seedbase=2800)
    line(vx, b3, [shift([ev], -4)[0]], gain=0.5, style='croon', seedbase=2900)
    line(vx, b3, [shift([ev], -7)[0]], gain=0.42, style='croon', seedbase=3000)
    line(vx, b3, [shift([ev], 12)[0]], gain=0.30, style='croon', seedbase=3100)
    line(vx, b3, [shift([ev], -12)[0]], gain=0.36, style='croon', seedbase=3200)
    choir(vx, b3, CH['E'][1], gain=0.09, seedbase=3300)
bus.add(T(b3 + 2.2), bell(hz('G#5'), 3.0, 0.10))

# ================================================================= mix
drmL = reverb(drm.L, 0.22); drmR = reverb(drm.R, 0.22)
vxL = reverb(vx.L, 0.42); vxR = reverb(vx.R, 0.42)

def master(instL, instR, vL=None, vR=None, out='airborne.wav'):
    L = instL * 0.42 + drmL * 0.30 + (vL * 0.50 if vL is not None else 0)
    R = instR * 0.42 + drmR * 0.30 + (vR * 0.50 if vR is not None else 0)
...
master(bus.L, bus.R, vx.L, vx.R, 'airborne.wav')
master(bus.L, bus.R, None, None, 'airborne-instrumental.wav')
