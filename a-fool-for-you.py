...
# =======================================================================
# SONG — A FOOL FOR YOU (E minor, 78 bpm, 58 bars = 2:59)
#
# Westlife "elevating modulation": the key change is the trademark — it is
# usually a tone up, but often a semitone (Dai Griffiths: "key change up a
# tone or semitone ... carried strong association with the Irish boy-band
# Westlife"). Here the final chorus lifts Em -> Fm (one semitone).
#
# Harmony — R.E.M. "Losing My Religion" (1991, A minor): the cycle is
# Am - Em - Dm - G (i - v - iv - VII), verses run Am(2) Em(2) closing
# Dm - G, the "I thought that I heard you laughing" section is
# F - Dm - G - Am, the bridge C - D7sus2, and the outro sits on a
# descending bass. Transposed to E minor:
#   verse/chorus: Em(2) Bm(2) Cm(2) D(2)        = 8-bar cycle
#   pre-chorus:   A - Cm - D - Em
#   bridge:       G(2) A7(2)
#   coda:         Em with descending bass E2 D2 C2 B2 (the original's
#                 Am/G-Fmaj7-F6-D7sus2 outro motion)
# The 8th-note mandolin arpeggio (pluck synth) is the signature texture.
# =======================================================================
BPM = 78.0
BEAT = 60.0 / BPM
BAR = 4.0
...
DUR = T(END_BEATS)
TOTAL = DUR + 3.5
print('A FOOL FOR YOU  bars', int(END_BEATS / BAR), ' duration', round(DUR, 1), 's')

CH = {
 'Em':  (40, (52,55,59,64), 40),   'Bm':  (47, (59,62,66,71), 47),
 'Cm':  (36, (48,51,55,60), 36),   'D':   (38, (50,54,57,62), 38),
 'A':   (45, (57,61,64,69), 45),   'G':   (43, (55,59,62,67), 43),
 'A7':  (45, (57,61,64,67), 45),
 'Fm':  (41, (53,56,60,65), 41),   'Bbm': (46, (58,61,65,70), 46),
 'Eb':  (39, (51,55,58,63), 39),   'Ab':  (44, (56,60,63,68), 44),
}
V1BAR  = ['Em','Em','Bm','Bm','Cm','Cm','D','D']
PREBAR = ['A','Cm','D','Em']
CHBAR  = ['Em','Em','Bm','Bm','Cm','Cm','D','D']
V2BAR  = ['Em','Em','Bm','Bm','Cm','D']
BRBAR  = ['G','G','A7','A7']
CH3BAR = ['Fm','Fm','Cm','Cm','Bbm','Bbm','Eb','Eb']   # semitone up
CODABAR= ['Em','Em','Em','Em']

BASSL = {
 'Em': [(0,40,1.7,.32),(1.7,47,1.2,.24)],
 'Bm': [(0,47,1.7,.30),(1.7,54,1.2,.22)],
 'Cm': [(0,36,1.7,.30),(1.7,43,1.2,.22)],
 'D':  [(0,38,1.7,.32),(1.7,45,1.2,.24)],
 'A':  [(0,45,1.7,.32),(1.7,52,1.2,.24)],
 'G':  [(0,43,1.7,.30),(1.7,50,1.2,.22)],
 'A7': [(0,45,1.7,.32),(1.7,52,1.2,.24)],
}
BASSP = {
 'A':  [(0,45,1.0,.36),(1.0,52,1.8,.28)],
 'Cm': [(0,36,1.0,.34),(1.0,43,1.8,.26)],
 'D':  [(0,38,1.0,.34),(1.0,45,1.8,.26)],
 'Em': [(0,40,1.0,.36),(1.0,47,1.8,.28)],
}
BASS8 = {
 'Em': [(0,40,.5,.32),(.5,47,.45,.24),(1,52,.5,.28),(1.5,47,.45,.24),
        (2,52,.5,.28),(2.5,47,.45,.24),(3,54,.5,.24),(3.5,47,.45,.24)],
 'Bm': [(0,47,.5,.32),(.5,54,.45,.24),(1,59,.5,.28),(1.5,54,.45,.24),
        (2,59,.5,.28),(2.5,54,.45,.24),(3,57,.5,.24),(3.5,54,.45,.24)],
 'Cm': [(0,36,.5,.32),(.5,43,.45,.24),(1,48,.5,.28),(1.5,43,.45,.24),
        (2,48,.5,.28),(2.5,43,.45,.24),(3,46,.5,.24),(3.5,43,.45,.24)],
 'D':  [(0,38,.5,.32),(.5,45,.45,.24),(1,50,.5,.28),(1.5,45,.45,.24),
        (2,50,.5,.28),(2.5,45,.45,.24),(3,47,.5,.24),(3.5,45,.45,.24)],
 'Fm': [(0,41,.5,.32),(.5,48,.45,.24),(1,53,.5,.28),(1.5,48,.45,.24),
        (2,53,.5,.28),(2.5,48,.45,.24),(3,55,.5,.24),(3.5,48,.45,.24)],
 'Bbm':[(0,46,.5,.32),(.5,53,.45,.24),(1,58,.5,.28),(1.5,53,.45,.24),
        (2,58,.5,.28),(2.5,53,.45,.24),(3,56,.5,.24),(3.5,53,.45,.24)],
 'Eb': [(0,39,.5,.32),(.5,46,.45,.24),(1,51,.5,.28),(1.5,46,.45,.24),
        (2,51,.5,.28),(2.5,46,.45,.24),(3,48,.5,.24),(3.5,46,.45,.24)],
}

bus = Bus(); drm = Bus(); vx = Bus()

def mando(b, chord, gain=0.10):
    """the mandolin 8th-note arpeggio (pluck synth) — the song's identity"""
    _, voic, _ = CH[chord]
    seq = [voic[0] + 12, voic[2] + 12, voic[1] + 12, voic[0] + 24,
           voic[2] + 12, voic[1] + 12, voic[0] + 12, voic[2] + 12]
    for p, m in enumerate(seq):
        bus.add(T(b + p * 0.5), pluck(mhz(m), 0.9, gain),
                pan=0.3 if p % 2 else -0.3)

V1 = [
 # "every whisper down the hallway"
 [(0,.5,'E4','e','','v'),(.5,.5,'E4','i','r',''),(1.0,.5,'G4','i','w',''),
  (1.5,.45,'G4','e','p','r'),(1.95,.5,'A4','ow','d','n'),(2.45,.45,'A4','e','th',''),
  (2.9,.5,'B4','o','h','l'),(3.4,.5,'B4','ay','w','')],
 # "every echo in the hall"
 [(0,.5,'E4','e','','v'),(.5,.45,'G4','i','r',''),(.95,.55,'A4','e','','ch'),
  (1.5,.45,'A4','o','',''),(1.95,.5,'B4','i','','n'),(2.45,.5,'G4','e','th',''),
  (2.95,.8,'A4','o','h','l')],
 # "i keep thinking that you called me"
 [(0,.5,'A4','ai','',''),(.5,.5,'B4','i','k','p'),(1.0,.55,'B4','i','th','ng'),
  (1.55,.45,'A4','i','','ng'),(2.0,.5,'G4','a','th','t'),(2.5,.5,'A4','u','y',''),
  (3.0,.5,'G4','o','k','l'),(3.5,.55,'F#4','i','m','')],
 # "but the silence says it all"
 [(0,.5,'G4','u','b','t'),(.5,.45,'A4','e','th',''),(.95,.5,'B4','ai','s',''),
  (1.45,.55,'B4','e','l','n'),(2.0,.5,'A4','e','s','z'),(2.5,.45,'G4','i','','t'),
  (2.95,.8,'E4','o','','l')],
 # "i've been counting every hour"
 [(0,.5,'G4','ai','','v'),(.5,.5,'A4','i','b','n'),(1.0,.55,'B4','ow','k','nt'),
  (1.55,.5,'B4','i','','ng'),(2.05,.5,'A4','e','','v'),(2.55,.45,'G4','i','r',''),
  (3.0,.8,'F#4','ow','','r')],
 # "like a church counts every sin"
 [(0,.5,'F#4','ai','l',''),(.5,.45,'G4','a','',''),(.95,.55,'A4','u','ch','r'),
  (1.5,.5,'B4','ow','k','nt'),(2.0,.5,'A4','e','','v'),(2.5,.45,'G4','i','r',''),
  (2.95,.8,'E4','i','s','n')],
 # "and the corner where you stood still"
 [(0,.5,'G4','a','','n'),(.5,.45,'A4','e','th',''),(.95,.5,'B4','o','k','r'),
  (1.45,.5,'B4','e','n',''),(1.95,.5,'A4','e','w','r'),(2.45,.5,'G4','u','y',''),
  (2.95,.5,'F#4','u','st','d'),(3.45,.5,'E4','i','st','l')],
 # "keeps the shape that you were in"
 [(0,.5,'F#4','i','k','p'),(.5,.45,'G4','e','th',''),(.95,.55,'A4','ay','sh','p'),
  (1.5,.5,'B4','a','th','t'),(2.0,.5,'A4','u','y',''),(2.5,.45,'G4','u','w','r'),
  (2.95,.8,'F#4','i','','n')],
]
PRE = [
 # "and i swear i heard you laughing"
 [(0,.5,'B4','a','','n'),(.5,.45,'B4','ai','',''),(.95,.5,'A4','e','s','r'),
  (1.45,.45,'A4','ai','',''),(1.9,.5,'G4','u','h','r'),(2.4,.45,'G4','u','y',''),
  (2.85,.5,'A4','a','l','f'),(3.35,.55,'B4','i','','ng')],
 # "but it was only the rain"
 [(0,.5,'A4','u','b','t'),(.5,.45,'C5','i','','t'),(.95,.55,'B4','o','w','z'),
  (1.5,.5,'A4','o','','n'),(2.0,.5,'G4','i','l',''),(2.5,.45,'A4','e','th',''),
  (2.95,.8,'B4','ay','r','n')],
 # "so i followed the sound"
 [(0,.55,'C5','o','s',''),(.55,.5,'C5','ai','',''),(1.05,.55,'D5','o','f','l'),
  (1.6,.5,'D5','o','l',''),(2.1,.5,'C5','e','th',''),(2.6,.55,'B4','ow','s','nd')],
 # "and i called out your name"
 [(0,.55,'B4','a','','n'),(.55,.5,'D5','ai','',''),(1.05,.55,'E5','o','k','l'),
  (1.6,.55,'E5','ow','','t'),(2.15,.5,'D5','o','y','r'),(2.65,1.3,'E5','ay','n','m')],
]
HOOK = [
 # "i'm a fool, a fool for you"
 [(0,.8,'B4','ai','','m'),(.8,.4,'B4','a','',''),(1.2,.7,'D5','u','f','l'),
  (1.9,.4,'D5','a','',''),(2.3,.55,'E5','u','f','l'),(2.85,.45,'D5','o','f','r'),
  (3.3,.6,'B4','u','y','')],
 # "every time i swear i'm through"
 [(0,.5,'D5','e','','v'),(.5,.45,'D5','i','r',''),(.95,.55,'E5','ai','t','m'),
  (1.5,.45,'E5','ai','',''),(1.95,.5,'D5','e','s','r'),(2.45,.5,'B4','ai','','m'),
  (2.95,.8,'B4','u','th','r')],
 # "i'm a fool, a fool for you" (higher landing)
 [(0,.8,'B4','ai','','m'),(.8,.4,'B4','a','',''),(1.2,.7,'D5','u','f','l'),
  (1.9,.4,'D5','a','',''),(2.3,.55,'E5','u','f','l'),(2.85,.45,'E5','o','f','r'),
  (3.3,.6,'E5','u','y','')],
 # "i keep coming back to you"
 [(0,.5,'D5','ai','',''),(.5,.5,'E5','i','k','p'),(1.0,.55,'E5','u','k','m'),
  (1.55,.5,'D5','i','','ng'),(2.05,.5,'B4','a','b','k'),(2.55,.45,'A4','u','t',''),
  (3.0,.8,'B4','u','y','')],
 # "and i don't know what i'm chasing"
 [(0,.5,'B4','a','','n'),(.5,.45,'C5','ai','',''),(.95,.5,'D5','o','d','nt'),
  (1.45,.5,'E5','o','n','w'),(1.95,.5,'D5','u','w','t'),(2.45,.5,'B4','ai','','m'),
  (2.95,.5,'C5','ay','ch','s'),(3.45,.5,'B4','i','','ng')],
 # "or the reason that i stay"
 [(0,.5,'B4','o','','r'),(.5,.45,'A4','e','th',''),(.95,.5,'B4','i','r',''),
  (1.45,.5,'C5','u','s','n'),(1.95,.5,'B4','a','th','t'),(2.45,.45,'A4','ai','',''),
  (2.9,.8,'B4','ay','st','')],
 # "but i'm a fool, i'm a fool"
 [(0,.55,'A4','u','b','t'),(.55,.55,'B4','ai','','m'),(1.1,.4,'B4','a','',''),
  (1.5,.6,'D5','u','f','l'),(2.1,.45,'D5','ai','','m'),(2.55,.4,'C5','a','',''),
  (2.95,.8,'E5','u','f','l')],
 # "for the fool's way"
 [(0,.5,'D5','o','f','r'),(.5,.45,'E5','e','th',''),(.95,.55,'F#5','u','f','l'),
  (1.5,1.5,'E5','ay','w',''),(3.0,.7,'B4','o','','')],
]
V2 = [
 # "every morning that i wake up"
 [(0,.5,'E4','e','','v'),(.5,.5,'E4','i','r',''),(1.0,.55,'G4','o','m','r'),
  (1.55,.5,'G4','i','','ng'),(2.05,.5,'A4','a','th','t'),(2.55,.5,'B4','ai','',''),
  (3.05,.5,'B4','ay','w','k'),(3.55,.4,'A4','u','','p')],
 # "with your name on my tongue"
 [(0,.5,'A4','i','w','th'),(.5,.5,'B4','o','y','r'),(1.0,.55,'B4','ay','n','m'),
  (1.55,.5,'A4','o','','n'),(2.05,.5,'G4','ai','m',''),(2.55,.8,'F#4','u','t','ng')],
 # "every night i lie down low"
 [(0,.5,'G4','e','','v'),(.5,.45,'A4','i','r',''),(.95,.55,'B4','ai','n','t'),
  (1.5,.45,'A4','ai','',''),(1.95,.5,'G4','ai','l',''),(2.45,.55,'F#4','ow','d','n'),
  (3.0,.7,'E4','o','l','')],
 # "where the last light used to glow"
 [(0,.5,'E4','e','w','r'),(.5,.45,'G4','e','th',''),(.95,.5,'A4','a','l','st'),
  (1.45,.55,'B4','ai','l','t'),(2.0,.5,'A4','u','','z'),(2.5,.45,'G4','u','t',''),
  (2.95,.8,'F#4','o','g','l')],
 # "i paint your face on the ceiling"
 [(0,.5,'A4','ai','',''),(.5,.55,'B4','ay','p','nt'),(1.05,.5,'B4','o','y','r'),
  (1.55,.5,'A4','ay','f','s'),(2.05,.5,'G4','o','','n'),(2.55,.5,'A4','e','th',''),
  (3.05,.5,'B4','i','s',''),(3.55,.45,'A4','i','l','ng')],
 # "till the paint begins to run"
 [(0,.5,'A4','i','t','l'),(.5,.45,'B4','e','th',''),(.95,.55,'B4','ay','p','nt'),
  (1.5,.5,'A4','i','b',''),(2.0,.5,'G4','i','g','n'),(2.5,.45,'F#4','u','t',''),
  (2.95,.8,'E4','u','r','n')],
]
BR = [
 # "and i built you up in daylight"
 [(0,.5,'D4','a','','n'),(.5,.45,'E4','ai','',''),(.95,.55,'G4','i','b','l'),
  (1.5,.5,'A4','u','y',''),(2.0,.45,'B4','u','','p'),(2.45,.5,'B4','i','','n'),
  (2.95,.5,'A4','ay','d',''),(3.45,.45,'G4','ai','l','t')],
 # "till the daylight had to go"
 [(0,.5,'G4','i','t','l'),(.5,.45,'A4','e','th',''),(.95,.5,'B4','ay','d',''),
  (1.45,.5,'B4','ai','l','t'),(1.95,.5,'A4','a','h','d'),(2.45,.45,'G4','u','t',''),
  (2.9,.8,'F#4','o','g','')],
 # "now i'm learning to believe in"
 [(0,.5,'G4','ow','n',''),(.5,.5,'A4','ai','','m'),(1.0,.55,'B4','u','l','r'),
  (1.55,.5,'B4','i','','ng'),(2.05,.5,'A4','u','t',''),(2.55,.5,'G4','i','b',''),
  (3.05,.5,'F#4','i','l','v'),(3.55,.45,'E4','i','','n')],
 # "what the darkness cannot know"
 [(0,.5,'F#4','u','w','t'),(.5,.45,'G4','e','th',''),(.95,.55,'A4','a','d','r'),
  (1.5,.5,'A4','e','n',''),(2.0,.5,'G4','a','k','n'),(2.5,.45,'F#4','o','n','t'),
  (2.95,.8,'E4','o','n','w')],
]
CH3 = [shift(l, 1) for l in HOOK]           # semitone lift (F minor)
CODA_1 = [(0,.9,'B4','ai','','m'),(.9,.4,'B4','a','',''),(1.3,.7,'D5','u','f','l'),
          (2.0,.4,'D5','a','',''),(2.4,1.5,'E5','u','f','l')]   # "i'm a fool"
CODA_2 = [(0,.5,'E5','o','f','r'),(.5,1.8,'D5','u','y','')]     # "for you"

# ---------------------------------------------------------- arrangement
# ---- INTRO: mandolin arps over the cycle A | Cm D | Em ----
for i, ch in enumerate(['A','Cm','D','Em']):
    b = S['INTRO'] + i * BAR
    padbar(b, ch, gain=0.10, cut=2100, atk=1.5, seed=i)
    keysarp(b, ch, gain=0.13)
    mando(b, ch, gain=0.11)

# ---- V1: voice + mandolin, bass in, Villa groove late ----
for i in range(8):
    b = S['V1'] + i * BAR
    ch = V1BAR[i]
    keysarp(b, ch, gain=0.12)
    mando(b, ch, gain=0.09)
...
for i in range(4):
    b = S['PRE1'] + i * BAR
    ch = PREBAR[i]
    padbar(b, ch, gain=0.12 + 0.02 * i, cut=2500 + 400 * i, atk=0.8, seed=300 + i)
    keysarp(b, ch, gain=0.15)
    mando(b, ch, gain=0.10)
    bassbar(b, ch, BASSP)
    vpre(b, i, vel=0.7)
...
for i in range(4):
    b = S['PRE2'] + i * BAR
    ch = PREBAR[i]
    padbar(b, ch, gain=0.15 + 0.02 * i, cut=3100 + 500 * i, atk=0.5, seed=1000 + i)
...
# ---- BRIDGE: stripped, G-A7; the suspension that lifts into the change ----
for i in range(4):
    b = S['BRIDGE'] + i * BAR
    ch = BRBAR[i]
    padbar(b, ch, gain=0.08, cut=1900, atk=1.6, seed=1500 + i)
    keysbar_split(b, [(ch, 0)], gain=0.10)
    bassbar(b, ch, BASSL)
    vbridge(b, i)
    line(vx, b, BR[i], gain=1.0, style='croon', seedbase=1600 + i * 17)

# ---- CH3: THE KEY CHANGE. semitone up, everything opens ----
for i in range(8):
    b = S['CH3'] + i * BAR
    ch = CH3BAR[i]
    padbar(b, ch, gain=0.20, cut=5000, atk=0.25, seed=1700 + i)
    mando(b, ch, gain=0.13)
    keysarp(b, ch, gain=0.17)
    bassbar(b, ch, BASS8)
...
# ---- CODA: descending bass over Em (the original's outro motion), fade ----
desc = [40, 38, 36, 35]                    # E2 D2 C2 B1
for i in range(4):
    b = S['CODA'] + i * BAR
    padbar(b, 'Em', gain=0.10, cut=2000, atk=1.5, seed=2100 + i)
    keysarp(b, 'Em', gain=0.11)
    mando(b, 'Em', gain=0.08)
    bassat(bus, T(b), desc[i], 3.4, 0.30)
    if i < 2:
        drm.add(T(b + 0), kick(0.38)); drm.add(T(b + 2), kick(0.32))
        drm.add(T(b + 2), rim(0.3))
b2 = S['CODA'] + 0 * BAR                   # "i'm a fool"
for ev in CODA_1:
...
b3 = S['CODA'] + 1 * BAR                   # "for you"
for ev in CODA_2:
    line(vx, b3, [ev], gain=1.05, style='croon', seedbase=2800)
    ...
    choir(vx, b3, CH['Em'][1], gain=0.08, seedbase=3300)

master(bus.L, bus.R, vx.L, vx.R, 'a-fool-for-you.wav', DUR)
master(bus.L, bus.R, None, None, 'a-fool-for-you-instrumental.wav', DUR)
