"""08-the-glitter-is-a-lie - mot file duy nhat, chi can numpy + scipy.
   Hoc tu: Grimes "Kill V. Maim" / "Oblivion" (Art Angels / Visions):
   vong i-III-VI-iv, "chord-melody tension" cao (giai dieu dam tren not ngoai hop am),
   chant kieu doi co vu, lop giong falsetto cao, hat 16/32 doi khi giat.
   Y do nhac ly:
   - Verse F#m - A - D - Bm (i - III - VI - iv). Giai dieu co tinh dam vao bac 9
     va bac 11 (G#, B tren F#m) => "tension" cao dung nhu Hooktheory do o Grimes.
   - Pre-chorus bo hop am xuong con hai (D - E) va nhoi mot cao do => don nen.
   - Chorus la CHANT (nhieu giong cung ho) tren i-III-VI-iv, tung cau 2 o nhip,
     ket bang mot not giu cao (F#5) - hat theo duoc ngay lan dau.
   - DROP: sau chorus 2, tat het chi con kick + chant + tieng ret => khoang lang gay soc.
   - Trong: bo trong that + hat 16/32 kieu may, chuyen dan mat do theo doan.
   python3 08-the-glitter-is-a-lie.py             -> ca hai ban wav
   python3 08-the-glitter-is-a-lie.py --vocals    -> chi ban co giong
   python3 08-the-glitter-is-a-lie.py --no-vocals -> chi ban instrumental
"""
...
# ================================================================= BAI 08
NAME="08-the-glitter-is-a-lie"
BAR=4.0
SECS=[('INTRO',8),('V1',16),('PRE1',8),('CH1',8),('POST',4),('V2',12),('PRE2',8),
      ('CH2',8),('DROP',8),('BR',8),('CH3',12),('OUT',8),('TAIL',4)]
S={}; _b=0.0
for _n,_c in SECS: S[_n]=_b; _b+=_c*BAR
END=_b
configure(134,135,END)
CUT=T(END)
...
# ============ HOA AM (F# minor) ============
CH={'F#m':(42,[54,57,61]),  'A':(45,[57,61,64]),
    'D':(50,[54,57,62]),    'Bm':(47,[54,59,62]),
    'E':(40,[56,59,64]),    'C#m':(49,[56,61,64]),
    'Dmaj7':(50,[54,57,61]),'Bm7':(47,[54,57,62]),
    'G':(43,[55,59,62]),    'F#m11':(42,[54,59,61,64]),
    'Emaj':(40,[56,59,64]), 'C#7':(49,[56,61,65])}
VCH=['F#m','A','D','Bm']*4
PCH=['D','D','E','E']*2
CCH=['F#m','A','D','Bm']*3
BCH=['G','G','C#7','C#7','Bm7','Bm7','E','E']
OCH=['F#m','A','D','E']*2
def bar_at(sec,i): return S[sec]+i*BAR

# ---- arp synth 16th (Oblivion DNA: arpeggio la xuong song) ----
APAT=[0,2,1,2,0,3,2,1]
def arp16(b0,ts,g=0.055,seed=0,oct_=12,step=0.5):
    k=0; p=0.0
    while p<4.0-1e-6:
        m=ts[APAT[k%8]%len(ts)]+oct_
        synpluck(kbd,T(b0+p),m,step*1.4,g*(1.0 if k%2==0 else 0.78),
                 cut=3400,decay=0.22,sub=0.06,seed=seed*9+k)
        p+=step; k+=1
# ---- bass: root 8th + nhay quang 8 cuoi o ----
def bass_hard(b0,root,g=0.26,seed=0,drive=True):
    fn=fuzzbass if drive else popbass
    for k,off in enumerate(np.arange(0,4,0.5)):
        m=root+(12 if k==7 else 0)
        fn(bs,T(b0+float(off)),m,0.40,g*(1.0 if k%2==0 else 0.74),seed=seed*8+k)
# ---- stab supersaw ngan (nhat vao dau o) ----
def stab(b0,root,ts,g=0.05,seed=0,n=2):
    for off in (0.0,2.0)[:n]:
        supersaw(pad,T(b0+off),[root+12]+[t+12 for t in ts],0.55,g,det=12,lp=5200,
                 atk=0.01,seed=seed*3+int(off))
# ---- chuong cao lap lai (glitter) ----
def glit(b0,ts,g=0.030,seed=0):
    for k,off in enumerate((0.25,1.25,2.75,3.25)):
        bell(hp_,T(b0+off),ts[k%len(ts)]+24,0.7,g*(1.0-0.12*k),idx=3.4,seed=seed*5+k)

# ================= TRONG =================
K=Kit(seed=131); P=Performer(K,T,SPB,TOTAL,seed=53,style='indie')
def hats(b0,lvl,arc,step=0.25,o_at=None):
    p=0.0
    while p<4.0-1e-6:
        op=(o_at is not None and abs(p-o_at)<1e-6)
        P.H(b0+p,int(p*4)%16,(0.40 if p%0.5 else 0.66)*lvl,o=0.5 if op else 0.0,
            art='tip' if p%1 else 'edge',arc=arc,choke_beat=(b0+4) if op else None)
        p+=step
def roll32(b0,start,beats,lvl,arc):
    p=start
    while p<start+beats-1e-9:
        P.H(b0+p,int(p*4)%16,(0.32+0.5*(p-start)/max(beats,1e-6))*lvl,o=0.0,art='tip',arc=arc)
        p+=0.125
def dr_intro(b0,lvl=0.6,arc=1.0):
    P.K(b0,0,0.75*lvl,arc,tune=44); P.K(b0+2.5,10,0.6*lvl,arc,tune=44)
    P.S(b0+2,8,0.5*lvl,'cross',arc); P.S(b0+4,0,0.5*lvl,'cross',arc)
    hats(b0,0.55*lvl,arc,0.5)
def dr_verse(b0,lvl=1.0,arc=1.0,i=0):
    P.K(b0,0,1.0*lvl,arc,tune=45); P.K(b0+1.75,7,0.62*lvl,arc)
    P.K(b0+2.5,10,0.85*lvl,arc)
    P.S(b0+2,8,0.95*lvl,'center',arc); P.S(b0+4,0,0.95*lvl,'center',arc)
    P.CL(b0+2-0.004,8,0.5*lvl,arc); P.CL(b0+4-0.004,0,0.5*lvl,arc)
    hats(b0,lvl*0.9,arc,0.25)
    P.S(b0+1.25,5,0.22*lvl,'ghost',arc); P.S(b0+3.25,13,0.24*lvl,'ghost',arc)
    if i%4==3: roll32(b0,3.5,0.5,lvl,arc)
def dr_pre(b0,lvl=0.9,arc=1.0,i=0):
    P.K(b0,0,0.9*lvl,arc,tune=44)
    if i>=4: P.K(b0+2,8,0.85*lvl,arc,tune=44)
    P.S(b0+4,0,0.85*lvl,'center',arc)
    P.CL(b0+4-0.004,0,0.7*lvl,arc)
    hats(b0,lvl*(0.7+0.06*i),arc,0.25 if i<6 else 0.125)
    P.TB(b0+1,4,0.35*lvl,arc); P.TB(b0+3,12,0.35*lvl,arc)
def dr_chorus(b0,lvl=1.08,arc=1.0,brk=False,extra=False):
    for k in range(4): P.K(b0+k,0,(1.0 if k%2==0 else 0.85)*lvl,arc,tune=46)
    P.K(b0+2.75,11,0.55*lvl,arc); P.K(b0+3.75,15,0.5*lvl,arc)
    P.S(b0+2,8,1.0*lvl,'center',arc); P.S(b0+4,0,1.0*lvl,'center',arc)
    P.CL(b0+2-0.003,8,0.85*lvl,arc); P.CL(b0+4-0.003,0,0.85*lvl,arc)
    hats(b0,lvl,arc,0.25,o_at=3.5)
    for p in (0.5,1.5,2.5,3.5): P.TB(b0+p,int(p*4)%16,0.40*lvl,arc)
    if extra: roll32(b0,3.0,1.0,lvl*0.9,arc)
    if brk: P.S(b0+3.75,15,0.7*lvl,'flam',arc)
    P.CR(b0,0,0.80*lvl,size=1.05)
def dr_drop(b0,lvl=0.9,arc=1.0,i=0):           # DROP: gan nhu chi kick
    P.K(b0,0,1.05*lvl,arc,tune=44)
    P.K(b0+2.5,10,0.85*lvl,arc,tune=44)
    if i>=4: P.CL(b0+4-0.004,0,0.6*lvl,arc)
    if i>=6: hats(b0,0.5*lvl,arc,0.5)
    if i==7: roll32(b0,2.0,2.0,lvl,arc)
def dr_br(b0,lvl=0.75,arc=1.0,i=0):
    P.K(b0,0,0.8*lvl,arc,tune=43)
    ...
    if i>=4: P.TM(b0+3.5,14,0.5*lvl,tune=108,arc=arc)

for i in range(8):
    b=bar_at('INTRO',i); a=[1.0,0.98,1.0,1.02][i%4]
    if i>=3: dr_intro(b,0.55+0.07*i,a)
    if i==7: P.fill(b+2,2.0,'roll',0.9,next_crash_beat=S['V1'])
for i in range(16):
    b=bar_at('V1',i); a=[1.0,0.97,1.0,1.02][i%4]
    dr_verse(b,0.94,a,i)
    if i==15: P.fill(b+3,1.0,'stutter',0.9)
for i in range(8):
    b=bar_at('PRE1',i); a=[1.0,0.98,1.0,1.02][i%4]
    dr_pre(b,0.92,a,i)
    if i==7: P.fill(b+2,2.0,'roll',1.35,next_crash_beat=S['CH1'])
for i in range(8):
    b=bar_at('CH1',i); a=[1.0,0.97,1.01,1.03][i%4]
    dr_chorus(b,1.05,a,brk=(i%4==3),extra=(i>=4))
for i in range(4):
    b=bar_at('POST',i); a=[1.0,0.98,1.0,1.02][i%4]
    dr_chorus(b,0.95,a)
    if i==3: P.fill(b+3,1.0,'stutter',1.0,next_crash_beat=S['V2'])
for i in range(12):
    b=bar_at('V2',i); a=[1.0,0.97,1.0,1.02][i%4]
    dr_verse(b,0.98,a,i)
    if i==11: P.fill(b+3,1.0,'stutter',1.0)
for i in range(8):
    b=bar_at('PRE2',i); a=[1.0,0.98,1.0,1.02][i%4]
    dr_pre(b,0.98,a,i)
    if i==7: P.fill(b+2,2.0,'roll',1.45,next_crash_beat=S['CH2'])
for i in range(8):
    b=bar_at('CH2',i); a=[1.0,0.97,1.01,1.03][i%4]
    dr_chorus(b,1.10,a,brk=(i%4==3),extra=True)
for i in range(8):
    b=bar_at('DROP',i); a=[1.0,0.99,1.0,1.01][i%4]
    dr_drop(b,0.92,a,i)
for i in range(8):
    b=bar_at('BR',i); a=[1.0,0.99,1.0,1.01][i%4]
    dr_br(b,0.8,a,i)
    if i==7: P.fill(b,4.0,'roll',1.6,next_crash_beat=S['CH3'])
for i in range(12):
    b=bar_at('CH3',i); a=[1.0,0.98,1.01,1.04][i%4]
    dr_chorus(b,1.14,a,brk=(i%4==3),extra=True)
    if i%4==0: P.CR(b,0,0.85)
    if i==11: P.fill(b+2,2.0,'tom',1.2,next_crash_beat=S['OUT'])
for i in range(8):
    b=bar_at('OUT',i); a=[1.0,0.98,1.0,1.02][i%4]
    dr_verse(b,1.0-0.09*i,a,i)
for i in range(4):
    b=bar_at('TAIL',i)
    if i==0: P.CR(b,0,0.55,size=1.25); P.K(b,0,0.7,tune=44)
P.apply_chokes()
DRUMS=mix_kit(P.bus,room_amount=0.20,oh_amount=0.85,lofi=0.04,lpf=11500)

# ================= NHAC CU =================
crackle(fx,0,T(END),0.016,dens=0.03)
for i in range(8):                                   # INTRO
    b=bar_at('INTRO',i); root,ts=CH[VCH[i%4]]
    arp16(b,ts,0.030+0.006*i,seed=i,oct_=12 if i<4 else 24)
    if i>=4: bass_hard(b,root,0.16+0.02*i,seed=i,drive=False)
    if i>=6: stab(b,root,ts,0.030,seed=i,n=1)
for i in range(16):                                  # V1
    b=bar_at('V1',i); root,ts=CH[VCH[i%4]]
    arp16(b,ts,0.052,seed=i)
    bass_hard(b,root,0.245,seed=i,drive=False)
    if i%4==0: stab(b,root,ts,0.034,seed=i,n=1)
    if i>=8: glit(b,ts,0.024,seed=i)
    if i%8==7: shriek(fx,T(b+3.5),nn('F#5'),0.5,0.035,seed=i)
for i in range(8):                                   # PRE1
    b=bar_at('PRE1',i); root,ts=CH[PCH[i%4]]
    arp16(b,ts,0.050+0.004*i,seed=i,step=0.25 if i>=4 else 0.5)
    bass_hard(b,root,0.22+0.008*i,seed=i,drive=False)
    supersaw(pad,T(b),[root+12]+[t+12 for t in ts],T(b+4)-T(b),0.022+0.005*i,
             det=10,lp=2800+220*i,atk=0.3,seed=i)
    if i==7: riser(fx,T(b),T(b+4)-T(b),0.075,300,9000,seed=3)
for i in range(8):                                   # CH1
    b=bar_at('CH1',i); root,ts=CH[CCH[i%4]]
    arp16(b,ts,0.062,seed=i,step=0.25)
    bass_hard(b,root,0.265,seed=i)
    stab(b,root,ts,0.048,seed=i)
    supersaw(pad,T(b),[root+12,ts[0]+12,ts[1]+12,ts[2]+12],T(b+4)-T(b),0.040,
             det=12,lp=5000,atk=0.05,seed=i)
    glit(b,ts,0.030,seed=i)
for i in range(4):                                   # POST: hook nhac cu khong loi
    b=bar_at('POST',i); root,ts=CH[CCH[i%4]]
    arp16(b,ts,0.066,seed=i,step=0.25,oct_=24)
    bass_hard(b,root,0.26,seed=i)
    supersaw(pad,T(b),[root+12]+[t+12 for t in ts],T(b+4)-T(b),0.036,det=12,lp=4600,
             atk=0.05,seed=i)
    analead(kbd,T(b),ts[2]+24,T(b+2)-T(b),0.048,det=14,cut=3800,seed=i)
for i in range(12):                                  # V2
    b=bar_at('V2',i); root,ts=CH[VCH[i%4]]
    arp16(b,ts,0.054,seed=i+20)
    bass_hard(b,root,0.25,seed=i,drive=False)
    glit(b,ts,0.026,seed=i)
    if i%4==0: stab(b,root,ts,0.036,seed=i,n=1)
    if i>=8: supersaw(pad,T(b),[root+12]+[t+12 for t in ts[:2]],T(b+4)-T(b),0.022,
                      det=9,lp=3000,atk=0.4,seed=i)
for i in range(8):                                   # PRE2
    b=bar_at('PRE2',i); root,ts=CH[PCH[i%4]]
    arp16(b,ts,0.054+0.004*i,seed=i,step=0.25 if i>=2 else 0.5)
    bass_hard(b,root,0.23+0.008*i,seed=i,drive=False)
    supersaw(pad,T(b),[root+12]+[t+12 for t in ts],T(b+4)-T(b),0.024+0.006*i,
             det=10,lp=3000+240*i,atk=0.28,seed=i)
    if i==7: riser(fx,T(b),T(b+4)-T(b),0.085,300,10000,seed=7)
for i in range(8):                                   # CH2
    b=bar_at('CH2',i); root,ts=CH[CCH[i%4]]
    arp16(b,ts,0.068,seed=i,step=0.25)
    bass_hard(b,root,0.275,seed=i)
    stab(b,root,ts,0.052,seed=i)
    supersaw(pad,T(b),[root+12,ts[0]+12,ts[1]+12,ts[2]+12],T(b+4)-T(b),0.046,
             det=13,lp=5400,atk=0.04,seed=i)
    glit(b,ts,0.032,seed=i)
    analead(kbd,T(b+2),ts[1]+24,T(b+4)-T(b+2),0.038,det=15,cut=4000,seed=i)
for i in range(8):                                   # DROP: rong hoac
    b=bar_at('DROP',i); root,ts=CH[['F#m','F#m','Bm','Bm','D','D','E','E'][i]]
    subbass(bs,T(b),root-12,T(b+3.4)-T(b),0.24)
    if i>=2: glit(b,ts,0.022,seed=i)
    if i>=4:
        arp16(b,ts,0.026+0.006*(i-4),seed=i,oct_=24)
        supersaw(pad,T(b),[t+12 for t in ts],T(b+4)-T(b),0.018+0.005*(i-4),
                 det=9,lp=2400+250*i,atk=0.5,seed=i)
    if i==7: riser(fx,T(b),T(b+4)-T(b),0.070,260,9500,seed=13)
for i in range(8):                                   # BR: G - C#7 - Bm7 - E
    b=bar_at('BR',i); root,ts=CH[BCH[i%8]]
    subbass(bs,T(b),root-12,T(b+3.4)-T(b),0.20)
    mellotron(kbd,T(b),ts[0]+12,T(b+4)-T(b),0.038,kind='choir',seed=i)
    supersaw(pad,T(b),[root+12]+[t+12 for t in ts],T(b+4)-T(b),0.030+0.004*i,
             det=11,lp=2600+220*i,atk=0.5,seed=i)
    bell(hp_,T(b+2),ts[2]+24,1.4,0.030,idx=3.0,seed=i)
    if i>=4: arp16(b,ts,0.034,seed=i,step=0.5)
for i in range(12):                                  # CH3
    b=bar_at('CH3',i); root,ts=CH[CCH[i%4]]
    arp16(b,ts,0.072,seed=i,step=0.25)
    bass_hard(b,root,0.285,seed=i)
    stab(b,root,ts,0.056,seed=i)
    supersaw(pad,T(b),[root+12,ts[0]+12,ts[1]+12,ts[2]+12],T(b+4)-T(b),0.052,
             det=14,lp=5800,atk=0.03,seed=i)
    glit(b,ts,0.034,seed=i)
    analead(kbd,T(b),ts[2]+24,T(b+2)-T(b),0.044,det=16,cut=4200,seed=i)
for i in range(8):                                   # OUT
    b=bar_at('OUT',i); root,ts=CH[OCH[i%4]]
    f=1-i/10.0
    arp16(b,ts,0.058*f,seed=i,step=0.25 if i<4 else 0.5)
    bass_hard(b,root,0.25*f,seed=i,drive=False)
    glit(b,ts,0.028*f,seed=i)
    if i%2==0: stab(b,root,ts,0.030*f,seed=i,n=1)
for i in range(4):                                   # TAIL
    b=bar_at('TAIL',i); root,ts=CH['F#m11']
    if i==0:
        supersaw(pad,T(b),[root+12]+[t+12 for t in ts],T(b+7)-T(b),0.040,det=13,
                 lp=3200,atk=0.6,seed=1)
        subbass(bs,T(b),root-12,T(b+6)-T(b),0.18)
        bell(hp_,T(b),nn('F#5'),3.0,0.038,idx=2.6,seed=1)
    if i==2: bell(hp_,T(b),nn('C#6'),3.0,0.026,idx=2.2,seed=2)
_o=int(T(S['OUT'])*SR); _e=int(T(END)*SR)
kbd[_o:_e]=bitcrush(kbd[_o:_e],bits=6,srdiv=4,mix=0.5)
stutter_buf(kbd,T(bar_at('OUT',6)),0.30,n=5,step_ms=70)

# ================= GIONG HAT =================
V1=[
 # "they sewed the glitter to my teeth and told me it was mine"
 [(0,.5,'C#5','e','th'),(0.5,.5,'C#5','o','s'),(1,.5,'E5','a','th'),(1.5,.5,'C#5','i','g'),
  (2,.5,'B4','u','t'),(2.5,.5,'C#5','i','m'),(3,.75,'A4','e','t'),
  (5,.5,'B4','o','t'),(5.5,.5,'C#5','i','m'),(6,.5,'A4','i',''),(6.5,1.0,'F#4','a','m')],
 # "i wore it like a weapon in a room that had no light"
 [(0,.5,'C#5','i',''),(0.5,.5,'E5','o','w'),(1,.5,'E5','i','t'),(1.5,.5,'C#5','a','l'),
  (2,.5,'B4','e','w'),(2.5,.5,'C#5','o','p'),(3,.75,'G#4','i','n'),
  (5,.5,'B4','u','r'),(5.5,.5,'C#5','a','th'),(6,.5,'A4','a','h'),(6.5,1.0,'F#4','o','n')],
 # "a hundred little mirrors and not one of them is me"
 [(0,.5,'A4','a',''),(0.5,.5,'C#5','u','h'),(1,.5,'C#5','i','d'),(1.5,.5,'E5','i','l'),
  (2,.5,'C#5','i','m'),(2.5,.5,'B4','o','r'),(3,.75,'A4','a','n'),
  (5,.5,'B4','o','n'),(5.5,.5,'C#5','a','w'),(6,.5,'A4','e','th'),(6.5,1.0,'G#4','i','m')],
 # "so keep your pretty ending i will write the rest myself"
 [(0,.75,'C#5','o','s'),(0.75,.25,'E5','i','k'),(1,.5,'E5','o','y'),(1.5,.5,'F#5','e','p'),
  (2,.5,'E5','i','t'),(2.5,.5,'C#5','e','n'),(3,.75,'B4','i','d'),
  (5,.5,'C#5','i',''),(5.5,.5,'B4','a','r'),(6,.5,'A4','e','th'),(6.5,1.25,'F#4','e','s')],
]
V2L=[
 # "the camera loves a girl who knows the angle of the knife"
 [(0,.5,'C#5','a','th'),(0.5,.5,'C#5','a','k'),(1,.5,'E5','u','l'),(1.5,.5,'C#5','a','g'),
  (2,.5,'B4','u','h'),(2.5,.5,'C#5','o','n'),(3,.75,'A4','a',''),
  (5,.5,'B4','a','n'),(5.5,.5,'C#5','o',''),(6,.5,'A4','a','th'),(6.5,1.0,'F#4','i','n')],
 # "i am not the villain i am only very tired"
 [(0,.5,'C#5','i',''),(0.5,.5,'E5','a',''),(1,.5,'E5','o','n'),(1.5,.5,'F#5','i','v'),
  (2,.5,'E5','a','l'),(2.5,.75,'C#5','i',''),
  (4.5,.5,'B4','a',''),(5,.5,'C#5','o','n'),(5.5,.5,'A4','i','l'),(6,.5,'G#4','e','v'),
  (6.5,1.0,'F#4','a','t')],
 # "every crown i ever wore was made of borrowed light"
 [(0,.5,'A4','e',''),(0.5,.5,'C#5','i','v'),(1,.5,'C#5','o','k'),(1.5,.5,'E5','a','r'),
  (2,.5,'C#5','e','v'),(2.5,.5,'B4','o','w'),(3,.75,'A4','a','w'),
  (5,.5,'B4','e','m'),(5.5,.5,'C#5','o','b'),(6,.5,'A4','o','r'),(6.5,1.0,'F#4','a','l')],
]
# CHORUS = CHANT (doi co vu). Hook: "the glit-ter is a lie"
CH_A=[(0,.5,'F#5','a','th'),(0.5,.5,'F#5','i','g'),(1,.5,'E5','a','t'),(1.5,.5,'C#5','i','z'),
      (2,.5,'B4','a',''),(2.5,1.25,'C#5','a','l'),
      (4,.5,'F#5','a','th'),(4.5,.5,'F#5','i','g'),(5,.5,'E5','a','t'),(5.5,.5,'A5','i','z'),
      (6,1.75,'F#5','a','l')]
CH_B=[(0,.5,'F#5','a','th'),(0.5,.5,'F#5','i','g'),(1,.5,'E5','a','t'),(1.5,.5,'C#5','i','z'),
      (2,.5,'B4','a',''),(2.5,1.25,'A4','a','l'),
      (4,.5,'B4','o','n'),(4.5,.5,'C#5','a','b'),(5,.5,'E5','i','l'),(5.5,.5,'F#5','i','v'),
      (6,1.75,'E5','a','t')]
PREL=[(0,.25,'D5','a','b'),(0.25,.25,'D5','a','t'),(0.5,.5,'D5','a','m'),
      (1.5,.25,'D5','a','b'),(1.75,.25,'D5','a','t'),(2,.5,'D5','a','m'),
      (3,1.0,'E5','o','n'),
      (4,.25,'E5','a','b'),(4.25,.25,'E5','a','t'),(4.5,.5,'E5','a','m'),
      (5.5,.25,'E5','a','b'),(5.75,.25,'E5','a','t'),(6,.5,'F#5','a','m'),
      (6.75,1.0,'G#5','i','')]
DROPW=[(0,.5,'F#4','a','th'),(0.5,.5,'F#4','i','g'),(1,.5,'F#4','a','t'),(1.5,.75,'F#4','i','z'),
       (4,.5,'F#4','a','l'),(5,1.0,'F#4','a','')]
BRL=[(0,.75,'B4','a','w'),(1,.5,'D5','i','l'),(1.5,.5,'C#5','a','m'),(2,1.0,'B4','o','n'),
     (4,.75,'C#5','a','b'),(5,.5,'E5','i','l'),(5.5,.5,'D5','a','v'),(6,1.5,'C#5','o','m')]
OUTW=[(0,1.0,'F#4','a','th'),(1.5,1.0,'A4','i','g'),(3,1.0,'F#4','a','l'),
      (5.5,1.5,'C#4','a','')]

for i in range(0,16,2):
    line(vx,bar_at('V1',i),V1[(i//2)%4],g=0.20,style='croon',breath=0.24,
         seedbase=i*13,jit=0.008,drag=0.004)
for i in range(0,8,2):
    b=bar_at('PRE1',i)
    line(vx,b,PREL,g=0.19,style='shout',breath=0.22,seedbase=200+i*13,jit=0.007)
for i in range(0,8,2):
    b=bar_at('CH1',i); ph=[CH_A,CH_B][(i//2)%2]
    line(vx,b,ph,g=0.20,style='shout',oct8=0.34,breath=0.20,seedbase=300+i*13,jit=0.008)
    chant(vx,b,ph,g=0.175,n=7,spread=22,style='shout',seedbase=340+i)
for i in range(0,4,2):
    b=bar_at('POST',i)
    chant(vx,b,CH_A,g=0.115,n=5,spread=20,style='shout',seedbase=430+i)
for i in range(0,12,2):
    line(vx,bar_at('V2',i),V2L[(i//2)%3],g=0.205,style='croon',breath=0.26,
         seedbase=500+i*13,jit=0.009,drag=0.006)
for i in range(0,8,2):
    b=bar_at('PRE2',i)
    line(vx,b,PREL,g=0.20,style='shout',breath=0.20,seedbase=600+i*13,jit=0.007)
for i in range(0,8,2):
    b=bar_at('CH2',i); ph=[CH_A,CH_B][(i//2)%2]
    line(vx,b,ph,g=0.21,style='shout',oct8=0.40,breath=0.19,seedbase=700+i*13,jit=0.008)
    chant(vx,b,ph,g=0.19,n=7,spread=22,style='shout',seedbase=740+i)
for i in range(0,8,2):
    b=bar_at('DROP',i)
    chant(vx,b,DROPW,g=0.145,n=6,spread=24,style='shout',seedbase=900+i)
    if i>=4: line(vx,b,DROPW,g=0.10,style='falsetto',breath=0.28,seedbase=940+i,jit=0.010)
shriek(vx,T(S['DROP']+7*BAR+2.0),nn('F#5'),1.0,0.085,seed=3)
for i in range(0,8,2):
    b=bar_at('BR',i)
    line(vx,b,BRL,g=0.175,style='croon',breath=0.32,seedbase=1000+i*13,jit=0.013)
    if i>=4: line(vx,b,BRL,g=0.075,style='falsetto',breath=0.30,seedbase=1050+i*13,jit=0.014)
for i in range(0,12,2):
    b=bar_at('CH3',i); ph=[CH_A,CH_B][(i//2)%2]
    line(vx,b,ph,g=0.215,style='shout',oct8=0.46,breath=0.18,seedbase=1100+i*13,jit=0.008)
    chant(vx,b,ph,g=0.20,n=8,spread=24,style='shout',seedbase=1140+i)
for i in range(0,8,2):
    line(vx,bar_at('OUT',i),OUTW,g=0.11*(1-i/12),style='falsetto',breath=0.30,
         seedbase=1300+i*13,jit=0.010)

# ================= MIX =================
STEMS=[(pad,0.0,0.60,0.55,0.0),(kbd,0.30,0.64,0.50,0.0),(brs,-0.32,0.55,0.50,0.0),
       (hp_,-0.44,0.62,0.45,6.0),(fx,0.0,1.0,0.0,0.0)]
MAPT=[(n,a,b_,(5.0 if n in('OUT','TAIL') else 6.0 if n=='DROP' else 4.6 if n=='BR' else
               3.4 if n.startswith('CH') else 3.0 if n=='POST' else
               1.2 if n=='INTRO' else 2.6))
      for n,a,b_ in MAP]
def _build(voc):
    st=mixdown(NAME,vx,STEMS,DRUMS,bs,MAPT,vocals=voc,
        wet=0.20,decay=1.35,wide=1.6,drum_gain=0.80,bass_gain=0.92,crush_amt=0.28,
        rms_target=0.178,vox_gain=1.0,boost_inst=1.12,duck=0.34,tape=0.002)
