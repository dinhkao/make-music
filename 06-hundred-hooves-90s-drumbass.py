"""06-hundred-hooves-90s-drumbass - BAN THAY THE cua 06 (DRUM + BASS)
   XOA het hop am: khong ken trombone, khong ken dam, khong kim loai.
   Chi con: trong + fuzz bass mot not E + bo go ngoai vi + giong hat hook.
   BPM 155-157, drop nua-nhip (A3), tease (A5), final crash moi o (A6).
   python3 06-hundred-hooves-90s-drumbass.py             -> ca hai ban wav (co giong + instrumental)
   python3 06-hundred-hooves-90s-drumbass.py --vocals    -> chi ban co giong
   python3 06-hundred-hooves-90s-drumbass.py --no-vocals -> chi ban instrumental
"""
# ================================================================= ENGINE (inlined)
import struct
import sys
...
# ================================================================= BAI 06 DRUM+BASS (BAN 1'30 - CHI TRONG + BASS)
# HUNDRED HOOVES DRUM+BASS - ban thay the: XOA het hop am, khong con ken nao.
# Chi con NAM thu: trong - fuzz bass 1 not E - bo go ngoai vi - giong hat hook - nhieu.
# Y tuong: mot not bass duy nhat 90 giay, trong day 6 cap do khong quay lai,
# mot cai DROP nua-nhip that (A3), mot doan TEASE boc het (A5) de cau hook
# dinh tai, roi FINAL (A6) crash moi o nhieu fill.
NAME="06-hundred-hooves-90s-drumbass"
BAR=4.0
SECS=[('INTRO',8),('A1',6),('A2',8),('A3',6),('A4',6),('A5',8),('A6',8),('TAG',4)]
S={}; _b=0.0
for _n,_c in SECS: S[_n]=_b; _b+=_c*BAR
END=_b
configure(155,157,END+2)
MAP=[(n,S[n],S[n]+c*BAR) for n,c in SECS]
def bar_at(sec,i): return S[sec]+i*BAR
print(NAME,"bars",int(END/BAR),"->",round(TOTAL,1),"s")

vx=buf(); bs=buf(); fx=buf()

# --------- FUZZ BASS: 1 not E duy nhat, 4 mau tiet tau ---------
def fzbass(b_,t0,m,dur,g=0.24,gl=0,seed=0,bite=1.0):
    ...
    x=_lp(x,155,6)                                          # chi con E1,E2,B2,E3
...
def bass_motor(b0,g=0.27,octv=True,dens=16):   # mau 1: motorik 16 day dac
    r=nn('E1')
    for s in range(0,16,16//max(dens,1)):
        p=s*0.25; iv=12 if (octv and s in (6,7,14,15)) else 0
        fzbass(bs,T(b0+p),r+iv,SPB(b0)*0.21,
               g*(1.0 if s%4==0 else (0.62 if s%2==0 else 0.46)),seed=s)
    subbass(bs,T(b0),r-12,SPB(b0)*3.7,g*0.5)
def bass_gallop(b0,g=0.30):                    # mau 2: gallop (3 chay 1 nghi)
    r=nn('E1')
    for s in (0,1,2,4,5,6,8,9,10,12,13,14):
        p=s*0.25; iv=12 if s in (2,6,10,14) else 0
        fzbass(bs,T(b0+p),r+iv,SPB(b0)*0.20,
               g*(1.0 if s%4==0 else (0.70 if s%2==0 else 0.52)),seed=s+3)
    subbass(bs,T(b0),r-12,SPB(b0)*3.7,g*0.5)
    subbass(bs,T(b0+2.0),r-12,SPB(b0)*3.0,g*0.4)
def bass_half(b0,g=0.32):                      # mau 3: nua-nhip drop, phat 1&3
    r=nn('E1')
    for s in (0,2,4,6,8,10,12,14):
        p=s*0.25; iv=12 if s in (4,12) else 0
        fzbass(bs,T(b0+p),r+iv,SPB(b0)*0.28,
               g*(1.0 if s%4==0 else 0.66),seed=s+7)
    subbass(bs,T(b0),r-12,SPB(b0)*4.6,g*0.6)
    subbass(bs,T(b0+2.0),r-12,SPB(b0)*4.6,g*0.6)
def bass_sparse(b0,g=0.26):                    # mau 4: thuong, co khoang trong
    r=nn('E1')
    for s in (0,3,4,7,8,11,12,15):
        p=s*0.25; iv=12 if s in (7,15) else 0
        fzbass(bs,T(b0+p),r+iv,SPB(b0)*0.22,
               g*(1.0 if s%4==0 else 0.55),seed=s+11)
    subbass(bs,T(b0),r-12,SPB(b0)*3.0,g*0.45)

# --------- CHUONG AGOGO (kim loai ca nhan) ---------
...
# --------- GIONG HAT: cau HOOK (earworm) + tieng "HO!" rave ---------
...
ANS =[(0,.25,'B4','a','h'),(.25,.25,'B4','o','n'),(.5,.5,'A4','a','d'),
      ...
      (3,1.0,'E4','a','y')]
def ho(b,at=0.0,g=0.22):
    chant(vx,b,[(at,0.30,'E5','o','h')],g=g,n=4,spread=14,style='shout',
          seedbase=3100+int(at*4)%997)

# --------- SAU CAP DO TRONG, MOI CAP THEM MOT BUOC ---------
def dk1(b0,lvl,arc):      # cap 1: 4 kick + backbeat + hat 8
...
def dk2(b0,lvl,arc):      # cap 2: hat 16 + ghost + shaker
    for q in range(4): P.K(b0+q,q*4,(1.05 if q%2==0 else 0.85)*lvl,arc,tune=49)
    P.S(b0+1,4,1.0*lvl,'center',arc); P.S(b0+3,12,1.0*lvl,'center',arc)
    for gp in (1.75,2.25,3.5,3.75): P.S(b0+gp,int(gp*4)%16,0.42*lvl,'ghost',arc)
...
def dk3(b0,lvl,arc):      # cap 3: kick doi + hat mo cuoi o + conga + clap + tamb
    for q in range(4): P.K(b0+q,q*4,(1.05 if q%2==0 else 0.85)*lvl,arc,tune=49)
    ...
    for s in range(16): P.SH(b0+s*0.25,s,(0.34 if s%4==0 else 0.18)*lvl,arc)
    for s in range(16): P.TB(b0+s*0.25,s,(0.30 if s%4==0 else 0.15)*lvl,arc)
...
def dk4(b0,lvl,arc,i=0):  # cap 4: + agogo + tom + ride xen ke
    dk3(b0,lvl,arc)
    ...
    for p in (0.5,2.0,3.25): P.TM(b0+p,int(p*4)%16,0.55*lvl,
                                  tune=[112,92,76][int(p)%3],arc=arc)
    if i%2==0:
        for s in range(8): P.RD(b0+s*0.5,s*2,0.34*lvl,bell=(s==0),arc=arc)
def dk5(b0,lvl,arc):      # cap 5 = DROP nua-nhip: kick 1&3 that, hat 32, snare cuon
    P.K(b0,0,1.18*lvl,arc,tune=49); P.K(b0+1.75,7,0.95*lvl,arc,tune=49)
    P.K(b0+3.0,12,1.22*lvl,arc,tune=49); P.K(b0+3.75,15,0.90*lvl,arc,tune=49)
    P.S(b0+3.0,12,1.15*lvl,'center',arc)
    for gp in (0.5,1.25,2.5): P.S(b0+gp,int(gp*4)%16,0.40*lvl,'ghost',arc)
    for s in range(32):
        p=s*0.125
        P.H(b0+p,int(p*4)%16,(0.70 if s%8==0 else (0.38 if s%2==0 else 0.24))*lvl,o=0.0,
            art='edge' if s%8==0 else 'tip',arc=arc)
    P.H(b0+3.5,14,0.62*lvl,o=0.7,art='edge',arc=arc,choke_beat=b0+4.0)
    for s in range(16): P.TB(b0+s*0.25,s,(0.40 if s%4==0 else 0.20)*lvl,arc)
    for s in range(16): P.SH(b0+s*0.25,s,(0.30 if s%4==0 else 0.16)*lvl,arc)
    P.CG(b0+2.5,10,0.55*lvl,tune=175,art='slap',arc=arc)
    P.CR(b0,0,0.72*lvl,size=1.0)
def dk6(b0,lvl,arc):      # cap 6 = FULL: kick 8, snare cuon, hat 32, agogo, conga
    ...
    for p,tu,ar in [(0.25,235,'open'),(1.5,235,'open'),(2.75,175,'slap')]:
        P.CG(b0+p,int(p*4)%16,0.50*lvl,tune=tu,art=ar,arc=arc)

# ================= TRONG =================
K=Kit(seed=606); P=Performer(K,T,SPB,TOTAL,seed=113,style='indie')

# --------- INTRO: woodblock dem nguoc, kick + hat len dan ---------
for i in range(8):
    ...
    if i<1:
        ...
        for p in (0.5,1.0,1.75,2.5,3.0,3.75):
            P.WD(b+p,int(p*4)%16,0.45 if p in (0.5,2.5) else 0.30,
                 tune=1150 if p in (0.5,2.5) else 820)
    if i>=3:
        for q in range(4): P.K(b+q,q*4,0.55+0.05*(i-3),a,tune=49)
    if i>=5:
        P.S(b+1,4,0.55,'cross',a); P.S(b+3,12,0.58,'cross',a)
        for s in range(8): P.H(b+s*0.5,s*2,0.42+0.04*(i-5),o=0.0,art='tip',arc=a)
    if i>=6:
        for s in range(8): P.SH(b+s*0.5,s*2,0.24+0.04*(i-6),a)
        for s in range(8): P.TB(b+s*0.5,s*2,0.20+0.03*(i-6),a)
    if i==7:
        P.CR(b,0,0.60,size=0.9)
        P.fill(b+2.5,1.5,'roll',0.95,next_crash_beat=S['A1'])
        bass_motor(b,0.16,dens=8)

# --------- A1: cap 1->2, bass motorik, CHUA CO GIONG ---------
for i in range(6):
    b=bar_at('A1',i); a=[1.0,0.97,1.01,1.03][i%4]
    (dk1 if i<3 else dk2)(b,0.94+0.02*i,a)
    bass_motor(b,0.26+0.01*i,dens=8 if i<3 else 16)
    if i==5: P.fill(b+3.0,1.0,'stutter',0.85,next_crash_beat=S['A2'])

# --------- A2: cap 3, conga/clap/hat mo, GIONG HOOK vao ---------
for i in range(8):
    b=bar_at('A2',i); a=[1.0,0.97,1.01,1.03][i%4]
    dk3(b,0.96+0.01*i,a)
    bass_motor(b,0.27,octv=(i>=4))
    if i==7: P.fill(b+3.0,1.0,'roll',0.95,next_crash_beat=S['A3'])

# --------- A3: DROP nua-nhip - that nhat bai, "HO!" moi o ---------
for i in range(6):
    b=bar_at('A3',i); a=[1.0,0.97,1.01,1.03][i%4]
    dk5(b,1.12+0.02*i,a)
    bass_half(b,0.34+0.02*i)
    ho(b,0.0); ho(b,2.0)
    if i==5: P.fill(b+3.0,1.0,'trib',1.1,next_crash_beat=S['A4'])

# --------- A4: cap 4 full double-time ---------
for i in range(6):
    b=bar_at('A4',i); a=[1.0,0.97,1.01,1.03][i%4]
    dk4(b,1.00+0.02*i,a,i)
    bass_motor(b,0.28)
    if i==5: P.fill(b+3.0,1.0,'stutter',1.05,next_crash_beat=S['A5'])

# --------- A5: TEASE - boc het, chi kick+snare+clap, giong noi ro ---------
for i in range(8):
    b=bar_at('A5',i); a=[1.0,0.97,1.01,1.03][i%4]
    for q in range(4): P.K(b+q,q*4,1.00 if q%2==0 else 0.80,a,tune=49)
    P.S(b+1,4,0.95,'center',a); P.S(b+3,12,0.98,'center',a)
    P.CL(b+1+0.005,4,0.70,a); P.CL(b+3+0.005,12,0.74,a)
    if i>=4:
        for s in range(8): P.H(b+s*0.5,s*2,0.42,0.0,'tip',a)
    if i>=6:
        for s in range(16): P.SH(b+s*0.25,s,0.26,a)
        for s in range(16): P.TB(b+s*0.25,s,0.22,a)
    bass_sparse(b,0.25)
    if i==2: P.S(b+2.5,10,0.60,'ghost',a)
    if i==7: P.fill(b+2.5,1.5,'roll',1.15,next_crash_beat=S['A6'])

# --------- A6: FINAL - crash moi o, fill xen ke, gallop xen motorik ---------
for i in range(8):
    b=bar_at('A6',i); a=[1.0,0.97,1.01,1.03][i%4]
    dk6(b,1.06+0.015*i,a)
    (bass_gallop if i%2==0 else bass_motor)(b,0.30)
    P.CR(b,0,0.62,size=0.95)
    if i%2==1: P.fill(b+3.0,1.0,'trib' if i==7 else 'stutter',1.1)
    if i==7: P.fill(b+2.5,1.5,'roll',1.2,next_crash_beat=S['TAG'])

# --------- TAG: 2 o full, 1 o cuon, 1 o dam chan ---------
for i in range(4):
    b=bar_at('TAG',i)
    if i<2:
        dk6(b,1.15-0.08*i,1.0)
        (bass_gallop if i==0 else bass_half)(b,0.32)
        P.CR(b,0,0.70,size=1.1)
        P.fill(b+3.0,1.0,'stutter',1.15)
    elif i==2:
        for s in range(8): P.K(b+s*0.5,s*2,1.0,tune=49)
        P.S(b+1,4,1.0,'center'); P.S(b+3,12,1.05,'center')
        for s in range(16): P.H(b+s*0.25,s,0.45,o=0.0,art='tip')
        bass_half(b,0.30)
        P.fill(b+3.0,1.0,'roll',1.3)
    else:
        P.K(b,0,1.25); P.CR(b,0,0.90,size=1.4); P.S(b,0,1.08,'center')
        P.CG(b,0,0.85,tune=150,art='slap'); AG(b,0,1.0,830,seed=1); AG(b,0,0.85,554,seed=2)
        fzbass(bs,T(b),nn('E1'),SPB(b)*3.0,0.40,bite=1.2)
        subbass(bs,T(b),nn('E1')-12,SPB(b)*3.0,0.6)
P.apply_chokes()
...
for sec,L in LAY.items():
    ...
    for i in range(nb):
        b=bar_at(sec,i)
        if sec=='A1': continue                  # chi trong + bass, chua co giong
        if sec=='A5':                           # tease: giong noi ro giua tieng trong
            line(vx,b,ANS,g=0.20,style='shout',oct8=0.20,breath=0.20,seedbase=L*100+i*29)
            if i>=4: chant(vx,b,ANS,g=0.12,n=6,spread=20,style='shout',seedbase=L*300+i)
            continue
        if sec=='A3':                           # drop: cau hook lau lai + ho
            line(vx,b,HOOK,g=0.22,style='shout',oct8=0.35,breath=0.20,seedbase=L*100+i*29)
            chant(vx,b,HOOK,g=0.15,n=6,spread=14,style='shout',seedbase=L*300+i)
            continue
...
shriek(vx,T(bar_at('A6',5)+2.0),nn('B5'),0.5,0.09)
for i in range(4):
    b=bar_at('TAG',i)
    if i<3:
        line(vx,b,HOOK if i<2 else ANS,g=0.21,style='shout',oct8=0.44,breath=0.18,
             seedbase=2500+i*29)
        chant(vx,b,HOOK if i<2 else ANS,g=0.15,n=6,spread=20,style='shout',seedbase=2600+i)
    else:
        chant(vx,b,[(0,1.5,'B4','a','h')],g=0.16,n=6,spread=20,style='shout',seedbase=2700)

# ================= MIX =================
STEMS=[(fx,0.0,1.0,0.0,0.0)]
STEM_NAMES=["NOISE-FX"]
MAPT=[(n,a,b_,(3.8 if n in ('A3','A6') else (3.2 if n=='A5' else 2.8))) for n,a,b_ in MAP]
KITMIX=dict(room_amount=0.20,oh_amount=0.86,lofi=0.0,lpf=10500)
MIXARGS=dict(wet=0.12,decay=1.0,wide=1.35,drum_gain=0.92,bass_gain=0.95,
             crush_amt=0.28,rms_target=0.185,boost_inst=1.12)
if (sys.argv[1] if len(sys.argv)>1 else "")=="--stems":
    stemdown(NAME,vx,STEMS,DRUMS,bs,MAPT,stem_names=STEM_NAMES,
             kitbus=P.bus,kitmix=KITMIX,outdir="stems-90s-drumbass",**MIXARGS)
else:
