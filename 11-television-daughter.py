"""11-television-daughter - mot file duy nhat, chi can numpy + scipy.
   Hoc tu: Broadcast / Stereolab huong "hauntology" - nhac lofi tu bang tu cu:
   mellotron, piano tack, trong nho danh nhe, tieng dia than, giong noi nhieu hoi.
   Y do nhac ly:
   - Fm - Dbmaj7 - Ab - Eb/G (i - VI - III - VII6) o toc do cham 96bpm:
     hoa am doi 2 o mot lan => cham, de tho.
   - Pre-chorus chen viio: Edim7 (E G Bb Db) dan ve Fm - vi chua cay dac trung
     nhac phim thap nien 60-70.
   - Chorus khong to hon bang am luong ma bang MAT DO: them mellotron + chuong,
     giong hat len quang 8; trong van nho => "to trong dau, khong to trong loa".
   - Doan HOLLOW: tat het chi con mellotron + giong tho, KHONG trong, 8 o nhip.
   - Ca bai co crackle + bitcrush nhe dan len o cuoi nhu bang tu mon.
   python3 11-television-daughter.py             -> ca hai ban wav
   python3 11-television-daughter.py --vocals    -> chi ban co giong
   python3 11-television-daughter.py --no-vocals -> chi ban instrumental
"""
...
# ================================================================= BAI 11
NAME="11-television-daughter"
BAR=4.0
SECS=[('INTRO',6),('V1',12),('PRE1',6),('CH1',8),('V2',10),('PRE2',6),('CH2',8),
      ('HOLLOW',8),('CH3',10),('OUT',8),('TAIL',4)]
S={}; _b=0.0
for _n,_c in SECS: S[_n]=_b; _b+=_c*BAR
END=_b
configure(96,96.5,END)
CUT=T(END)
...
# ============ HOA AM (F minor, mau nhac phim cu) ============
CH={'Fm':(41,[53,56,60]),      'Dbmaj7':(37,[53,56,60,65]),
    'Ab':(44,[51,56,60]),      'Eb/G':(43,[51,55,58]),
    'Edim7':(40,[51,55,58]),   'Bbm7':(46,[53,58,61,65]),
    'Cm':(48,[51,55,60]),      'Fm9':(41,[53,56,60,67]),
    'Absus2':(44,[51,56,58]),  'Db6':(37,[53,56,58,65]),
    'C7b9':(48,[52,55,58,61]), 'Fm7':(41,[53,56,60,63])}
VCH=['Fm','Fm','Dbmaj7','Dbmaj7','Ab','Ab','Eb/G','Eb/G']
PCH=['Bbm7','Bbm7','Edim7','Edim7','Cm','C7b9']
CCH=['Dbmaj7','Ab','Eb/G','Fm9']*3
HCH=['Fm9','Db6','Absus2','Eb/G']*2
OCH=['Fm','Dbmaj7','Ab','Eb/G']*2
def bar_at(sec,i): return S[sec]+i*BAR

# ---- mellotron chum: nen chinh cua bai ----
def mel_bed(b0,ts,g=0.042,kind='strings',seed=0,L=4.0):
    for j,m in enumerate(ts[:3]):
        mellotron(kbd,T(b0)+j*0.030,m+12,T(b0+L)-T(b0),g*(1.0-0.10*j),kind=kind,
                  seed=seed*4+j)
# ---- piano tack: hop am roi tung not, mem ----
def tack_roll(b0,ts,g=0.050,seed=0,off=0.0,spread=0.055):
    for j,m in enumerate(ts):
        tackpiano(kbd,T(b0+off)+j*spread,m,T(b0+off+3.4)-T(b0+off),g*(1.0-0.08*j),
                  tack=0.42,seed=seed*5+j)
# ---- chuong nho, thua ----
def bells(b0,ts,g=0.028,seed=0,offs=(1.5,3.0)):
    for k,off in enumerate(offs):
        bell(hp_,T(b0+off),ts[(k+1)%len(ts)]+12,1.8,g*(1.0-0.15*k),idx=2.2,seed=seed*3+k)
# ---- bass: nguoi choi, hai not mot o ----
def bass_soft(b0,root,g=0.24,seed=0,walk=False):
    fingerbass(bs,T(b0),root,1.4,g,seed=seed)
    if walk: fingerbass(bs,T(b0+2.5),root+7,1.0,g*0.66,seed=seed+1)
    else:    fingerbass(bs,T(b0+3),root+(12 if seed%2 else 0),0.7,g*0.60,seed=seed+1)

# ================= TRONG: nho, nhe, ngoi phia sau =================
K=Kit(seed=311); P=Performer(K,T,SPB,TOTAL,seed=83,style='indie')
P.laid={'kick':0.006,'snare':0.020,'hat':0.004,'tom':0.010,'cym':0.004,'perc':0.008}
def dr_soft(b0,lvl=0.85,arc=1.0,cross=True,shak=True):
    P.K(b0,0,0.85*lvl,arc,tune=42)
    P.K(b0+2.5,10,0.60*lvl,arc,tune=42)
    if cross: P.S(b0+2,8,0.55*lvl,'cross',arc); P.S(b0+4,0,0.55*lvl,'cross',arc)
    else:     P.S(b0+2,8,0.70*lvl,'edge',arc);  P.S(b0+4,0,0.70*lvl,'edge',arc)
    if shak:
        for p in (0.5,1.5,2.5,3.5): P.SH(b0+p,int(p*4)%16,0.26*lvl,arc)
def dr_verse(b0,lvl=0.9,arc=1.0,hat=True):
    P.K(b0,0,0.92*lvl,arc,tune=42); P.K(b0+2.5,10,0.62*lvl,arc,tune=42)
    P.S(b0+2,8,0.72*lvl,'edge',arc); P.S(b0+4,0,0.72*lvl,'edge',arc)
    if hat:
        for p in (0.5,1.5,2.5,3.5): P.H(b0+p,int(p*4)%16,0.26*lvl,o=0.0,art='foot',arc=arc)
    for p in (1.0,3.0): P.SH(b0+p,int(p*4)%16,0.24*lvl,arc)
    P.S(b0+3.5,14,0.14*lvl,'ghost',arc)
def dr_pre(b0,lvl=0.9,arc=1.0,i=0):
    P.K(b0,0,0.9*lvl,arc,tune=42)
    P.S(b0+2,8,0.70*lvl,'edge',arc); P.S(b0+4,0,0.78*lvl,'center',arc,tune=190)
    p=0.0
    while p<4.0-1e-6: P.SH(b0+p,int(p*4)%16,0.28*lvl,arc); p+=0.25
    if i>=3: P.TM(b0+3.5,14,0.36*lvl,tune=120,arc=arc)
def dr_chorus(b0,lvl=0.95,arc=1.0,tamb=True):
    P.K(b0,0,0.95*lvl,arc,tune=43); P.K(b0+1.5,6,0.50*lvl,arc)
    P.K(b0+2.5,10,0.72*lvl,arc)
    P.S(b0+2,8,0.85*lvl,'center',arc,tune=190); P.S(b0+4,0,0.85*lvl,'center',arc,tune=190)
    p=0.0
    while p<4.0-1e-6:
        P.H(b0+p,int(p*4)%16,(0.30 if p%1 else 0.46)*lvl,o=0.0,art='tip',arc=arc); p+=0.5
    if tamb:
        for p in (2.0,4.0): P.TB(b0+p-0.004,int(p*4)%16,0.34*lvl,arc)
    for p in (0.75,2.75): P.SH(b0+p,int(p*4)%16,0.24*lvl,arc)
    P.S(b0+1.25,5,0.14*lvl,'ghost',arc)

for i in range(6):
    b=bar_at('INTRO',i); a=[1.0,0.99,1.0,1.01][i%4]
    if i>=2: dr_soft(b,0.5+0.08*i,a,shak=(i>=3))
    if i==5: P.fill(b+3,1.0,'negative',0.6,next_crash_beat=S['V1'])
for i in range(12):
    b=bar_at('V1',i); a=[1.0,0.98,1.0,1.01][i%4]
    dr_verse(b,0.88,a,hat=(i>=4))
    if i==11: P.fill(b+3,1.0,'snare',0.6)
for i in range(6):
    b=bar_at('PRE1',i); a=[1.0,0.98,1.0,1.01][i%4]
    dr_pre(b,0.9,a,i)
    if i==5: P.fill(b+2,2.0,'roll',0.95,next_crash_beat=S['CH1'])
for i in range(8):
    b=bar_at('CH1',i); a=[1.0,0.98,1.01,1.02][i%4]
    dr_chorus(b,0.95,a)
    if i==0: P.CR(b,0,0.45,size=1.2)
    if i==7: P.fill(b+3,1.0,'snare',0.7,next_crash_beat=S['V2'])
for i in range(10):
    b=bar_at('V2',i); a=[1.0,0.98,1.0,1.01][i%4]
    dr_verse(b,0.92,a)
    if i>=6: P.TB(b+3,12,0.26,a)
    if i==9: P.fill(b+3,1.0,'snare',0.7)
for i in range(6):
    b=bar_at('PRE2',i); a=[1.0,0.98,1.0,1.01][i%4]
    dr_pre(b,0.95,a,i)
    if i==5: P.fill(b+2,2.0,'roll',1.05,next_crash_beat=S['CH2'])
for i in range(8):
    b=bar_at('CH2',i); a=[1.0,0.98,1.01,1.02][i%4]
    dr_chorus(b,1.0,a)
    if i==0: P.CR(b,0,0.5,size=1.2)
# HOLLOW: KHONG trong
for i in range(10):
    b=bar_at('CH3',i); a=[1.0,0.98,1.01,1.02][i%4]
    dr_chorus(b,1.04,a)
    if i==0: P.CR(b,0,0.55,size=1.25)
    if i==9: P.fill(b+2,2.0,'roll',0.9,next_crash_beat=S['OUT'])
for i in range(8):
    b=bar_at('OUT',i); a=[1.0,0.99,1.0,1.01][i%4]
    dr_soft(b,0.85-0.09*i,a,cross=(i>=3))
for i in range(4):
    b=bar_at('TAIL',i)
    if i==0: P.S(b,0,0.28,'cross')
P.apply_chokes()
DRUMS=mix_kit(P.bus,room_amount=0.42,oh_amount=1.05,lofi=0.30,lpf=6800)

# ================= NHAC CU =================
crackle(fx,0,T(END),0.055,dens=0.11)
for i in range(6):                                   # INTRO
    b=bar_at('INTRO',i); root,ts=CH[VCH[i%8]]
    mel_bed(b,ts,0.030+0.005*i,kind='flute',seed=i)
    if i>=1: tack_roll(b,ts,0.038,seed=i)
    if i>=3: bass_soft(b,root,0.18+0.02*i,seed=i)
    if i>=4: bells(b,ts,0.022,seed=i,offs=(2.5,))
for i in range(12):                                  # V1
    b=bar_at('V1',i); root,ts=CH[VCH[i%8]]
    bass_soft(b,root,0.235,seed=i,walk=(i%4==3))
    mel_bed(b,ts,0.036,kind='strings',seed=i)
    if i%2==0: tack_roll(b,ts,0.048,seed=i)
    else:      tack_roll(b,ts,0.030,seed=i,off=1.5,spread=0.030)
    if i>=6: bells(b,ts,0.024,seed=i)
    if i%4==3: nylon(kbd,T(b+2),ts[2],1.6,0.036,seed=i)
for i in range(6):                                   # PRE1: viio Edim7
    b=bar_at('PRE1',i); root,ts=CH[PCH[i%6]]
    bass_soft(b,root,0.24,seed=i,walk=True)
    mel_bed(b,ts,0.040+0.004*i,kind='choir',seed=i)
    tack_roll(b,ts,0.052,seed=i)
    bowed(pad,T(b),ts[1]+12,T(b+4)-T(b),0.030+0.005*i,rough=0.5,atk=0.5,seed=i)
    if i==5: noise_sw(fx,T(b+2),T(b+4)-T(b+2),0.030,True,300,4000)
for i in range(8):                                   # CH1: day hon nhung KHONG to hon
    b=bar_at('CH1',i); root,ts=CH[CCH[i%4]]
    bass_soft(b,root,0.25,seed=i,walk=(i%2==1))
    mel_bed(b,ts,0.050,kind='strings',seed=i)
    mel_bed(b,ts,0.026,kind='choir',seed=i+40)
    tack_roll(b,ts,0.054,seed=i)
    bells(b,ts,0.030,seed=i,offs=(0.5,2.0,3.5))
    bowed(pad,T(b),ts[2]+12,T(b+4)-T(b),0.030,rough=0.4,atk=0.4,seed=i)
for i in range(10):                                  # V2
    b=bar_at('V2',i); root,ts=CH[VCH[i%8]]
    bass_soft(b,root,0.24,seed=i+20,walk=(i%3==2))
    mel_bed(b,ts,0.038,kind='strings',seed=i)
    tack_roll(b,ts,0.046,seed=i,off=(0.0 if i%2==0 else 1.0))
    bells(b,ts,0.026,seed=i)
    if i%4==2: marimba(hp_,T(b+2.5),ts[1]+12,1.4,0.030,seed=i)
    if i>=6: nylon(kbd,T(b+1),ts[2],1.8,0.034,seed=i)
for i in range(6):                                   # PRE2
    b=bar_at('PRE2',i); root,ts=CH[PCH[i%6]]
    bass_soft(b,root,0.25,seed=i,walk=True)
    mel_bed(b,ts,0.044+0.004*i,kind='choir',seed=i)
    tack_roll(b,ts,0.056,seed=i)
    bowed(pad,T(b),ts[1]+12,T(b+4)-T(b),0.034+0.005*i,rough=0.5,atk=0.45,seed=i)
    if i==5: noise_sw(fx,T(b+2),T(b+4)-T(b+2),0.036,True,300,4500)
for i in range(8):                                   # CH2
    b=bar_at('CH2',i); root,ts=CH[CCH[i%4]]
    bass_soft(b,root,0.26,seed=i,walk=(i%2==1))
    mel_bed(b,ts,0.054,kind='strings',seed=i)
    mel_bed(b,ts,0.030,kind='choir',seed=i+40)
    tack_roll(b,ts,0.058,seed=i)
    bells(b,ts,0.032,seed=i,offs=(0.5,2.0,3.5))
    bowed(pad,T(b),ts[2]+12,T(b+4)-T(b),0.034,rough=0.4,atk=0.4,seed=i)
    if i%4==3: marimba(hp_,T(b+3),ts[0]+24,1.2,0.028,metal=True,seed=i)
for i in range(8):                                   # HOLLOW: khong trong
    b=bar_at('HOLLOW',i); root,ts=CH[HCH[i%4]]
    mel_bed(b,ts,0.048 if i<4 else 0.040,kind='choir' if i<4 else 'flute',seed=i)
    if i>=2: subbass(bs,T(b),root-12,T(b+3.4)-T(b),0.16)
    if i>=4:
        tack_roll(b,ts,0.040,seed=i,spread=0.090)
        bells(b,ts,0.026,seed=i,offs=(2.0,))
    if i==7: noise_sw(fx,T(b),T(b+4)-T(b),0.045,True,200,5000)
for i in range(10):                                  # CH3
    b=bar_at('CH3',i); root,ts=CH[CCH[i%4]]
    bass_soft(b,root,0.265,seed=i,walk=(i%2==1))
    mel_bed(b,ts,0.058,kind='strings',seed=i)
    mel_bed(b,ts,0.034,kind='choir',seed=i+40)
    tack_roll(b,ts,0.062,seed=i)
    bells(b,ts,0.034,seed=i,offs=(0.5,2.0,3.5))
    bowed(pad,T(b),ts[2]+12,T(b+4)-T(b),0.038,rough=0.4,atk=0.35,seed=i)
    nylon(kbd,T(b+1.5),ts[1],1.4,0.030,seed=i)
for i in range(8):                                   # OUT: bang tu mon dan
    b=bar_at('OUT',i); root,ts=CH[OCH[i%4]]
    f=1-i/10.0
    bass_soft(b,root,0.23*f,seed=i)
    mel_bed(b,ts,0.042*f,kind='strings',seed=i)
    if i%2==0: tack_roll(b,ts,0.044*f,seed=i)
    if i<5: bells(b,ts,0.026*f,seed=i)
for i in range(4):                                   # TAIL
    b=bar_at('TAIL',i); root,ts=CH['Fm9']
    if i==0:
        mel_bed(b,ts,0.044,kind='choir',seed=1,L=9.0)
        tack_roll(b,ts,0.040,seed=1,spread=0.11)
        subbass(bs,T(b),root-12,T(b+6)-T(b),0.15)
    if i==2: bell(hp_,T(b),ts[3]+12,3.0,0.024,idx=1.9,seed=2)
# bang tu mon dan: bitcrush tang len ve cuoi
_a=int(T(S['CH3'])*SR); _e=int(T(END)*SR)
for _bl in (kbd,hp_,pad):
    _bl[_a:_e]=bitcrush(_bl[_a:_e],bits=8,srdiv=2,mix=0.35)
_o=int(T(S['OUT'])*SR)
for _bl in (kbd,hp_,pad):
    _bl[_o:_e]=bitcrush(_bl[_o:_e],bits=6,srdiv=3,mix=0.55)
tapestop(kbd,T(bar_at('TAIL',2)),2.4,0.9)
tapestop(pad,T(bar_at('TAIL',2)),2.4,0.9)

# ================= GIONG HAT (rat gan mic, nhieu hoi) =================
V1=[
 # "the television had a daughter and she never learned to sleep"
 [(0,.5,'F4','a','th'),(0.5,.5,'F4','e','t'),(1,.5,'Ab4','i','l'),(1.5,.5,'F4','i','v'),
  (2,.5,'Eb4','a','h'),(2.5,.5,'F4','a','d'),(3,1.0,'Db4','o','t'),
  (5,.5,'Eb4','a',''),(5.5,.5,'F4','i','sh'),(6,.5,'Db4','e','n'),(6.5,1.0,'C4','e','s')],
 # "she watched the little people go to bed inside the glass"
 [(0,.5,'F4','i','sh'),(0.5,.5,'Ab4','o','w'),(1,.5,'Ab4','a','th'),(1.5,.5,'F4','i','l'),
  (2,.5,'Eb4','i','p'),(2.5,.5,'F4','o','g'),(3,1.0,'Db4','e','b'),
  (5,.5,'Eb4','i','n'),(5.5,.5,'F4','a','th'),(6,1.25,'C4','a','g')],
 # "and every night she memorized a face that wasn't hers"
 [(0,.5,'F4','a',''),(0.5,.5,'F4','e','v'),(1,.5,'Ab4','i','n'),(1.5,.5,'Bb4','i','sh'),
  (2,.5,'Ab4','e','m'),(2.5,.5,'F4','a','r'),(3,1.0,'Eb4','a','f'),
  (5,.5,'Db4','a','th'),(5.5,.5,'Eb4','o','w'),(6,1.25,'C4','e','h')],
 # "now she only speaks in weather and in commercial breaks"
 [(0,.5,'F4','a','n'),(0.5,.5,'F4','i','sh'),(1,.5,'Ab4','o','n'),(1.5,.5,'F4','i','s'),
  (2,.5,'Eb4','i','n'),(2.5,.5,'F4','e','w'),(3,1.0,'Db4','a','th'),
  (5,.5,'Eb4','a',''),(5.5,.5,'F4','e','k'),(6,1.25,'C4','e','b')],
]
V2L=[
 # "i found her in the static with her hands around the sound"
 [(0,.5,'F4','i',''),(0.5,.5,'Ab4','a','f'),(1,.5,'Ab4','e','h'),(1.5,.5,'F4','i','n'),
  (2,.5,'Eb4','a','s'),(2.5,.5,'F4','i','t'),(3,1.0,'Db4','e','w'),
  (5,.5,'Eb4','a','h'),(5.5,.5,'F4','a','r'),(6,1.25,'C4','o','s')],
 # "she said the world is only what the antenna will allow"
 [(0,.5,'F4','i','sh'),(0.5,.5,'F4','e','s'),(1,.5,'Ab4','a','th'),(1.5,.5,'Bb4','o','w'),
  (2,.5,'Ab4','i','z'),(2.5,.5,'F4','o','n'),(3,1.0,'Eb4','a','w'),
  (5,.5,'Db4','a','th'),(5.5,.5,'Eb4','e','t'),(6,1.25,'C4','a','l')],
 # "so i turned the volume down and let the picture keep her warm"
 [(0,.5,'F4','o','s'),(0.5,.5,'F4','a','t'),(1,.5,'Ab4','o','v'),(1.5,.5,'F4','u','d'),
  (2,.5,'Eb4','a',''),(2.5,.5,'F4','e','l'),(3,1.0,'Db4','i','p'),
  (5,.5,'Eb4','i','k'),(5.5,.5,'F4','e','h'),(6,1.25,'C4','o','w')],
]
# CHORUS: khong to, chi day - giai dieu len quang 8 o lop hai
CH_A=[(0.5,.75,'Ab4','o','d'),(1.5,.5,'Bb4','a','n'),(2,.5,'Ab4','i','t'),
      (2.5,1.25,'F4','e','g'),
      (4.5,.75,'Ab4','o','d'),(5.5,.5,'C5','a','n'),(6,1.75,'Bb4','e','')]
CH_B=[(0.5,.75,'Ab4','o','d'),(1.5,.5,'Bb4','a','n'),(2,.5,'Ab4','i','t'),
      (2.5,1.25,'F4','e','g'),
      (4.5,.75,'C5','o','d'),(5.5,.5,'Db5','a','n'),(6,1.75,'C5','e','')]
PREL=[(0,.5,'Db4','a','w'),(0.5,.5,'F4','e','n'),(1,.5,'Ab4','i','l'),(1.5,.5,'Bb4','a','m'),
      (2,1.5,'Ab4','o','n'),
      (4,.5,'Eb4','a','w'),(4.5,.5,'G4','e','n'),(5,.5,'Bb4','i','l'),(5.5,.5,'C5','a','m'),
      (6,1.75,'Db5','o','n')]
HOLW=[(0,1.5,'F4','o','m'),(2,1.0,'Ab4','a',''),(4,1.5,'Eb4','i','l'),(6,1.5,'F4','a','n')]
OUTW=[(0,1.25,'F4','o','d'),(2,1.0,'Db4','a','n'),(4,1.25,'C4','e','h'),(6,1.5,'F4','o','')]

for i in range(0,12,2):
    line(vx,bar_at('V1',i),V1[(i//2)%4],g=0.185,style='croon',breath=0.44,
         seedbase=i*13,jit=0.014,drag=0.018)
for i in range(0,6,2):
    line(vx,bar_at('PRE1',i),PREL,g=0.175,style='croon',breath=0.38,seedbase=200+i*13,jit=0.012)
for i in range(0,8,2):
    b=bar_at('CH1',i); ph=[CH_A,CH_B][(i//2)%2]
    line(vx,b,ph,g=0.20,style='croon',oct8=0.32,breath=0.34,seedbase=300+i*13,jit=0.013)
for i in range(0,10,2):
    line(vx,bar_at('V2',i),V2L[(i//2)%3],g=0.19,style='croon',breath=0.46,
         seedbase=500+i*13,jit=0.015,drag=0.020)
for i in range(0,6,2):
    line(vx,bar_at('PRE2',i),PREL,g=0.185,style='croon',breath=0.36,seedbase=600+i*13,jit=0.012)
for i in range(0,8,2):
    b=bar_at('CH2',i); ph=[CH_A,CH_B][(i//2)%2]
    line(vx,b,ph,g=0.205,style='croon',oct8=0.38,breath=0.32,seedbase=700+i*13,jit=0.013)
    chant(vx,b,ph,g=0.075,n=4,spread=13,style='croon',seedbase=740+i)
for i in range(0,8,2):
    line(vx,bar_at('HOLLOW',i),HOLW,g=0.115,style='whisper',breath=0.46,
         seedbase=900+i*13,jit=0.016)
for i in range(0,10,2):
    b=bar_at('CH3',i); ph=[CH_A,CH_B][(i//2)%2]
    line(vx,b,ph,g=0.21,style='croon',oct8=0.42,breath=0.30,seedbase=1100+i*13,jit=0.013)
    chant(vx,b,ph,g=0.095,n=5,spread=15,style='croon',seedbase=1140+i)
for i in range(0,8,2):
    line(vx,bar_at('OUT',i),OUTW,g=0.095*(1-i/12),style='whisper',breath=0.48,
         seedbase=1300+i*13,jit=0.014)

# ================= MIX =================
STEMS=[(pad,0.0,0.58,0.55,0.0),(kbd,0.24,0.68,0.50,0.0),(brs,-0.34,0.55,0.50,0.0),
       (hp_,-0.42,0.58,0.45,10.0),(fx,0.0,1.0,0.0,0.0)]
MAPT=[(n,a,b_,(5.4 if n in('OUT','TAIL') else 6.2 if n=='HOLLOW' else
               3.6 if n.startswith('CH') else 1.4 if n=='INTRO' else 3.0))
      for n,a,b_ in MAP]
def _build(voc):
    st=mixdown(NAME,vx,STEMS,DRUMS,bs,MAPT,vocals=voc,
        wet=0.34,decay=2.1,wide=1.40,drum_gain=0.66,bass_gain=0.92,crush_amt=0.14,
        rms_target=0.170,vox_gain=1.05,boost_inst=1.14,duck=0.28,tape=0.0075)
