"""06-hundred-hooves-intro - mot file duy nhat, chi can numpy + scipy.
   python3 06-hundred-hooves-intro.py             -> ca hai ban wav (co giong + instrumental)
   python3 06-hundred-hooves-intro.py --vocals    -> chi ban co giong
   python3 06-hundred-hooves-intro.py --no-vocals -> chi ban instrumental
"""
# ================================================================= ENGINE (inlined)
import struct
import sys
...
# ================================================================= BAI 06 (BAN CAT 2 PHUT - INTRO ALBUM)
# HUNDRED HOOVES  -  funk hanh quan, mot not bass duy nhat, cong don lop
# KHONG CO ORGAN/PIANO: lop nen la kim loai bi cao (inharmonic) + chuong agogo.
# Y tuong mot cau: mot hop am va mot not bass duy nhat; cu 10 o nhip lai co them mot
# lop nhap bon va KHONG lop nao bi bo di - 2 phut de mo dia, ket bang mot cu dam.
# Khac ban dai: hook goi 3 lan lien tiep, co doan STOP-TIME o giua, trong giu nguyen.
NAME="06-hundred-hooves-intro"
BAR=4.0
SECS=[('INTRO',12),('A1',10),('A2',10),('A3',8),('STOP',4),
      ('A4',8),('A5',10),('A6',10),('TAG',4)]
S={}; _b=0.0
...
cl=buf(); brs=buf(); org=buf(); mar=buf(); vx=buf(); bs=buf(); fx=buf()
...
# --------- BASS: mot not E, 16 not khong nghi ---------
def bass_bar(b0,g=0.24,octjump=False,k='Em9',dens=16):
    r=nn('E1')
...
# --------- CLAVINET: doi DUNG nhip cua hook (day la cho bat tai) ---------
# hook = goi 3 lan: (B B A) (B B G) (E5 D5 B) roi doi (A)
CLR=[(0.0,7,1.0),(0.25,7,0.7),(0.5,5,0.95),
     (1.0,7,1.0),(1.25,7,0.7),(1.5,3,0.95),
     (2.0,12,1.05),(2.25,10,0.75),(2.5,7,0.95),
     (3.0,5,1.0),(3.5,3,0.6)]
def clav_bar(b0,k,g=0.085,seed=0,oct_=0):
    r=nn('E4')
    for off,iv,acc in CLR:
        clav(cl,T(b0+off),r+iv+oct_,SPB(b0)*0.26,g*acc,seed=(seed+int(off*4))%5)

# --------- KEN ---------
def brass_bar(b0,k,g=0.080,hi=False):
    r=min(C_(k))+12                                  # dong thanh: goc + quang 8, khong phai hop am
    ns=[r,r+12] if not hi else [r+12,r+24]
    for off,d,acc in [(0.0,0.45,1.0),(1.75,0.35,0.75),(2.5,0.55,0.95)]:
        for j,m in enumerate(ns):
            bone(brs,T(b0+off)+j*0.010,m,SPB(b0)*d,g*acc,det=-8+16*j,growl=0.65,seed=j)

# --------- LOP KIM LOAI: khong cao do, khong hoa am (thay cho organ) ---------
_METAL={}
def _metalbed(v=0):
    if v not in _METAL:
...
def metalair(b0,dur,g=0.045,seed=0,bright=1.0,rate=0.70):
    """tam kim loai bi cao bang cung - dai tan lien tuc, khong ra not nao"""
    x=_metalbed(seed%4)
    ...
    put(org,T(b0),_fade(x*e),g)
def agogo(vel=1.0,f=560.0,seed=0):
    """chuong bo/agogo: hai hinh vuong ti le 1:1.4986 -> inharmonic"""
    ...
    x=_bp(x,f*0.85,min(f*8,15000),2)
    x*=np.exp(-t/0.052)
...
def bell_bar(b0,lvl=1.0,arc=1.0):
    for p,f in [(0.0,830),(0.75,554),(1.5,830),(2.25,554),(2.75,830),(3.5,554)]:
        AG(b0+p,int(p*4)%16,(0.85 if f>700 else 0.62)*lvl,f,arc,seed=int(p*4))

def mar_bar(b0,k,g=0.055,seed=0):
    ns=top(k,'B3')
    for i,off in enumerate([0.0,0.75,1.5,2.25,3.0,3.75]):
        marimba(mar,T(b0+off),ns[i%len(ns)]+12,SPB(b0)*0.9,g*(1.0 if i%2==0 else .7),
                metal=True,seed=(seed+i)%5)

# ================= TRONG: giu nguyen hoan toan =================
K=Kit(seed=606); P=Performer(K,T,SPB,TOTAL,seed=113,style='indie')
def march(b0,lvl=1.0,arc=1.0,hats=16,toms=False,dbl=False):
    for q in range(4): P.K(b0+q,q*4,(1.05 if q%2==0 else 0.85)*lvl,arc,tune=49,mode='acoustic')
    if dbl:
    ...
    if hats:
        ...
        for s in range(n):
            p=s*(4.0/n)
            P.H(b0+p,int(p*4)%16,(0.78 if s%(n//4)==0 else 0.44)*lvl,o=0.0,
                art='edge' if s%(n//4)==0 else 'tip',arc=arc)
    if toms:
        for p in (0.5,2.0): P.TM(b0+p,int(p*4)%16,0.55*lvl,tune=112 if p<1 else 92,arc=arc)
def perc_army(b0,lvl=1.0,cg=True,cl_=True,sh=True):
    if cg:
        for p,tu,ar in [(0.25,235,'open'),(0.75,175,'slap'),(1.5,235,'open'),
                        (2.25,150,'open'),(2.75,235,'slap'),(3.5,175,'open')]:
            P.CG(b0+p,int(p*4)%16,0.52*lvl,tune=tu,art=ar)
    if cl_: P.CL(b0+1+0.005,4,0.8*lvl); P.CL(b0+3+0.005,12,0.84*lvl)
    if sh:
        for s in range(16): P.SH(b0+s*0.25,s,(0.38 if s%4==0 else 0.20)*lvl)

for i in range(12):
    b=bar_at('INTRO',i); a=[1.0,0.97,1.0,1.02][i%4]
    # o 0-1: chi hai tieng mo. o 2-3: mo day du.
    if i<2:
    ...
    # o 4: kick vao, rat nhe, day len dan
    if i>=4:
        for q in range(4): P.K(b+q,q*4,0.58+0.045*(i-4),a,tune=49)
    # o 6: snare cross-stick truoc, o 9 moi thanh snare that
    if 6<=i<9: P.S(b+1,4,0.52,'cross',a); P.S(b+3,12,0.55,'cross',a)
    if i>=9:   P.S(b+1,4,0.74,'center',a); P.S(b+3,12,0.76,'center',a)
    # o 7: hat 8, o 10: hat 16
    if 7<=i<10:
        for sx in range(8): P.H(b+sx*0.5,sx*2,0.40+0.03*(i-7),o=0.0,art='tip',arc=a)
    if i>=10:
        for sx in range(16): P.H(b+sx*0.25,sx,(0.66 if sx%4==0 else 0.38),o=0.0,
                                 art='edge' if sx%4==0 else 'tip',arc=a)
    # o 8: lac tay.  o 10: vo tay.
    if i>=8:
        for sx in range(8): P.SH(b+sx*0.5,sx*2,0.24+0.03*(i-8),a)
    if i>=10: P.CL(b+1+0.005,4,0.62); P.CL(b+3+0.005,12,0.64)
    if i==11: P.fill(b+3.0,1.0,'roll',0.95,next_crash_beat=S['A1'])
LAY={'A1':1,'A2':2,'A3':3,'A4':4,'A5':5,'A6':6}
for sec in ('A1','A2','A3','A4','A5','A6'):
    L=LAY[sec]; nb=dict(SECS)[sec]
    for i in range(nb):
        b=bar_at(sec,i); a=[1.0,0.97,1.01,1.03][i%4]
        march(b,0.90+0.035*L,a,hats=(8 if L<=1 else 16),toms=(L>=4 and i%4==2),dbl=(L>=2))
        perc_army(b,0.50+0.10*L,cg=(L>=2),cl_=True,sh=(L>=3))
        if L>=4: bell_bar(b,0.85+0.06*L,a)
        if i==0 and sec not in ('A1','A4'): P.CR(b,0,0.60+0.028*L,size=0.95)
        if i==nb-1: P.fill(b+3.0,1.0,'stutter',0.9+0.04*L)
# STOP-TIME: ca ban nhac dap 2 cu roi im, dam dong lap vao cho trong
for i in range(4):
    b=bar_at('STOP',i)
    if i<3:
        ...
        march(b,1.15,1.0,hats=16,toms=True,dbl=True)
        perc_army(b,1.1); bell_bar(b,1.1)
        P.fill(b+2.0,2.0,'roll',1.2,next_crash_beat=S['A4'])
# TAG: hai o cuoi cung tat het, chi con mot cu dam
for i in range(4):
    b=bar_at('TAG',i)
    if i<2:
        march(b,max(0.55,1.05-0.25*i),1.0,hats=16,dbl=True); perc_army(b,max(0.5,1.0-0.3*i)); bell_bar(b,max(0.5,1.05-0.3*i))
    elif i==2:
        for q in range(4): P.K(b+q,q*4,0.95,tune=49)
        P.S(b+1,4,0.95,'center'); P.S(b+3,12,1.0,'center')
        P.fill(b+3.0,1.0,'roll',1.2)
    else:
        P.K(b,0,1.2); P.CR(b,0,0.85,size=1.35); P.S(b,0,1.05,'center'); P.CG(b,0,0.8,tune=150,art='slap')
        AG(b,0,1.0,830,seed=1); AG(b,0,0.85,554,seed=2)
P.apply_chokes()
...
for i in range(12):
    ...
    if i>=8:  clav_bar(b,k,0.030+0.016*(i-8),seed=i)     # cau hook he lo o cuoi intro
    if i>=10: bass_bar(b,0.14+0.045*(i-10),k=k,dens=8)
for sec in ('A1','A2','A3','A4','A5','A6'):
    L=LAY[sec]; nb=dict(SECS)[sec]
    for i in range(nb):
        b=bar_at(sec,i); k=kof(i)
        clav_bar(b,k,0.090,seed=i,oct_=(12 if (L>=6 and i%4==3) else 0))
        bass_bar(b,0.25,octjump=(L>=4),k=k,dens=(8 if L<=1 else 16))
        if L>=3: brass_bar(b,k,0.078,hi=(sec=='A6'))
        if L>=4: metalair(b,T(b+BAR*1.05)-T(b),0.040+0.004*L,seed=i,bright=0.9+0.06*L)
        if L>=5: mar_bar(b,k,0.058,seed=i)
        if L>=6:
            metalair(b+2.0,T(b+BAR*1.1)-T(b+2.0),0.034,seed=i+2,bright=1.5,rate=1.15)
            for j,m in enumerate(top(k,'B3')):
                marimba(mar,T(b)+j*0.012,m+24,SPB(b)*0.8,0.030,metal=True,seed=(i+j)%5)
for i in range(4):
    b=bar_at('STOP',i); k=kof(i)
    if i<3:
        for off in (0.0,2.5):
            for j,m in enumerate(top(k,'B3')[:3]):
                bone(brs,T(b+off)+j*0.010,m,SPB(b)*0.42,0.105,det=-9+8*j,growl=0.7,seed=j)
            fzbass(bs,T(b+off),nn('E1'),SPB(b)*0.40,0.36,bite=1.3)
            marimba(mar,T(b+off),nn('E5'),SPB(b)*1.0,0.070,metal=True,seed=i)
        metalair(b,T(b+BAR*0.98)-T(b),0.038,seed=i,bright=1.4,rate=1.4)
    else:
        clav_bar(b,k,0.095,seed=i); bass_bar(b,0.26,octjump=True,k=k)
        brass_bar(b,k,0.085)
        metalair(b,T(b+BAR*1.1)-T(b),0.052,seed=9,bright=1.3); mar_bar(b,k,0.060,seed=i)
for i in range(4):
    b=bar_at('TAG',i); k=kof(i)
    if i<3:
        f=[1.0,0.85,0.7][i]
        clav_bar(b,k,0.090*f,seed=i); bass_bar(b,0.25*f,octjump=(i<2),k=k)
        brass_bar(b,k,0.078*f)
        metalair(b,T(b+BAR*1.05)-T(b),0.048*f,seed=i+5,bright=1.2)
        if i<2: mar_bar(b,k,0.055*f,seed=i)
    else:
        for j,m in enumerate(top('Em9','B3')):
            bone(brs,T(b)+j*0.012,m,SPB(b)*2.2,0.10,det=-9+8*j,growl=0.5,seed=j)
        fzbass(bs,T(b),nn('E1'),SPB(b)*1.6,0.36,bite=1.2)
        metalair(b,SPB(b)*3.0,0.075,seed=3,bright=1.1,rate=0.5)

# ================= GIONG HAT: hook goi 3 lan =================
# 5 cao do (G4 A4 B4 D5 E5), moi not co phu am, toan not moc kep
HOOK=[(0,.25,'B4','a','h'),(.25,.25,'B4','o','n'),(.5,.5,'A4','a','d'),
      (1,.25,'B4','a','h'),(1.25,.25,'B4','o','n'),(1.5,.5,'G4','a','d'),
      (2,.25,'E5','a','k'),(2.25,.25,'D5','o','m'),(2.5,.5,'B4','a','l'),
      (3,1.0,'A4','a','w')]
# cau tra loi: chi doi 2 not cuoi -> nghe quen ma khong chan
...
for sec in ('A1','A2','A3','A4','A5','A6'):
    ...
    for i in range(nb):
        b=bar_at(sec,i)
        if L<=1: continue                      # A1: clavinet noi cau hook mot luot, chua co giong
        cells=ANS if i%4==3 else HOOK          # 3 lan hook, 1 lan tra loi
        st='deadpan' if L<=2 else ('croon' if L<=4 else 'shout')
        line(vx,b,cells,g=0.205+0.006*L,style=st,oct8=0.06*L,breath=0.26,
             seedbase=L*100+i*29)
        if L>=3: chant(vx,b,cells,g=0.030+0.022*(L-2),n=min(6,L+1),spread=10+2*L,
                       style='shout' if L>=5 else 'croon',seedbase=L*300+i)
# STOP: chi con dam dong hat vao cho trong -> bat tai nhat bai
for i in range(4):
    b=bar_at('STOP',i)
    if i<3:
        chant(vx,b,STOPC,g=0.20+0.02*i,n=6,spread=20,style='shout',seedbase=2000+i*29)
...
# ================= MIX =================
STEMS=[(cl,-0.5,0.62,0.45,0.0),(brs,-0.28,0.58,0.50,0.0),
       (org,0.30,0.42,0.62,0.0),(mar,0.42,0.52,0.45,9.0),(fx,0.0,1.0,0.0,0.0)]
MAPT=[(n,a,b_,(4.5 if n=='STOP' else 3.0)) for n,a,b_ in MAP]
KITMIX=dict(room_amount=0.20,oh_amount=0.86,lofi=0.0,lpf=10500)
STEM_NAMES=["CLAVINET","BRASS","METAL","MARIMBA",None]   # None = bo lop nhieu nen
MIXARGS=dict(wet=0.15,decay=1.15,wide=1.30,drum_gain=0.80,bass_gain=0.86,
...
if (sys.argv[1] if len(sys.argv)>1 else "")=="--stems":
    stemdown(NAME,vx,STEMS,DRUMS,bs,MAPT,stem_names=STEM_NAMES,
             kitbus=P.bus,kitmix=KITMIX,outdir="stems",**MIXARGS)
else:
