"""06-cassette-angel - mot file duy nhat, chi can numpy + scipy.
   Hoc tu: Sky Ferreira "Everything Is Embarrassing" (C major, 115bpm, C-Am7-F-G,
   beat R&B thap nien 80 mo ao, bass gay ngon, piano le loi - Rechtshaid/Dev Hynes).
   Y do nhac ly:
   - Verse hoa am CHAM (2 o mot hop am): Cmaj9 - Am7. Trong nua nhip (half-time),
     snare gate roi vao phach 3 => khong gian rong cho giong hat noi truoc/sau phach.
   - Pre di len IV-V-iii-vi (Fmaj7 G6 Em7 Am7), day cang truoc chorus.
   - Chorus IV-V-I-vi voi giai dieu dau tren bac 9 (not D tren Cmaj7) = "ngot ma buon".
   - Chorus cuoi CHUYEN GIONG len Eb (quang 3 thu) - chieu pop thap nien 80,
     nhung giai dieu giu nguyen hinh => nghe quen ma sang han.
   - Giong hat co tinh "noi", hat TRE hon phach (drag 0.03) cho lo dang.
   python3 06-cassette-angel.py             -> ca hai ban wav
   python3 06-cassette-angel.py --vocals    -> chi ban co giong
   python3 06-cassette-angel.py --no-vocals -> chi ban instrumental
"""
...
# ============ HOA AM ============
CH={'Cmaj9':(36,[52,55,59,62]), 'Am7':(33,[48,52,55,60]),
    'Fmaj7':(41,[48,53,57,60]), 'G6':(43,[50,55,59,64]),
    'Em7':(40,[50,55,59,62]),   'Dm7':(38,[50,53,57,60]),
    'Cmaj7':(36,[52,55,59,64]), 'G/B':(35,[50,55,59,62]),
    # chuyen giong Eb cho CH3
    'Abmaj7':(44,[51,55,60,63]),'Bb6':(46,[53,58,62,65]),
    'Ebmaj9':(39,[55,58,62,65]),'Cm7':(36,[51,55,58,63]),
    'Fm7':(41,[48,53,56,60]),   'Ebmaj7':(39,[55,58,62,67])}
VCH=['Cmaj9','Cmaj9','Am7','Am7']*4
PCH=['Fmaj7','G6','Em7','Am7']*2
CCH=['Fmaj7','G6','Cmaj7','Am7']*3
MCH=['Dm7','G6','Cmaj9','Am7']*2
KCH=['Abmaj7','Bb6','Ebmaj9','Cm7']*3     # chorus cuoi: len Eb
OCH=['Abmaj7','Bb6','Ebmaj9','Cm7']*2
def bar_at(sec,i): return S[sec]+i*BAR

# ---- snare "gate": snare + clap + duoi nhieu bi cat ngan ----
def gate_tail(b0,off=2.0,g=0.030,dur=0.26):
    noise_sw(fx,T(b0+off)+0.004,dur,g,up=False,lo=700,hi=6500)
# ---- bass gay ngon, syncop (hook nho) ----
BPAT=[(0.0,0,1.00),(0.75,0,0.62),(1.5,7,0.72),(2.5,0,0.85),(3.25,-5,0.60)]
def bass_pluck(b0,root,g=0.26,seed=0,var=0):
    pat=BPAT if var==0 else [(0.0,0,1.0),(1.0,12,0.6),(1.75,7,0.7),(2.5,0,0.85),(3.5,3,0.6)]
    for k,(off,iv,vv) in enumerate(pat):
        fingerbass(bs,T(b0+off),root+iv,0.55,g*vv,seed=seed*7+k)
# ---- piano le loi: mot cu dam roi de vang ----
...
# ---- wurli "rung" giu nen ----
def wurli_bed(b0,ts,g=0.040,seed=0):
    for j,m in enumerate(ts):
        wurli(kbd,T(b0)+j*0.018,m+12,T(b0+3.5)-T(b0),g*(0.9-0.07*j),det=-5+3*j)

# ================= TRONG: half-time R&B mo ao =================
K=Kit(seed=88); P=Performer(K,T,SPB,TOTAL,seed=37,style='indie')
def dr_half(b0,lvl=1.0,arc=1.0,hat16=False,ghost=True):
    P.K(b0,0,1.0*lvl,arc,tune=43)
    P.K(b0+1.5,6,0.55*lvl,arc,tune=43)
    P.S(b0+2,8,1.0*lvl,'center',arc,tune=196)          # snare gate o phach 3
    P.CL(b0+2-0.005,8,0.55*lvl,arc)
    gate_tail(b0,2.0,0.030*lvl)
    p=0.0; step=0.25 if hat16 else 0.5
    while p<4.0-1e-6:
        P.H(b0+p,int(p*4)%16,(0.30 if p%0.5 else 0.46)*lvl,o=0.0,art='tip',arc=arc); p+=step
    if ghost:
        P.S(b0+1.25,5,0.16*lvl,'ghost',arc); P.S(b0+3.5,14,0.20*lvl,'ghost',arc)
def dr_intro(b0,lvl=0.6,arc=1.0):
    P.K(b0,0,0.7*lvl,arc,tune=43)
    P.S(b0+2,8,0.55*lvl,'rim',arc)
    for p in (0.5,1.5,2.5,3.5): P.H(b0+p,int(p*4)%16,0.24*lvl,o=0.0,art='foot',arc=arc)
def dr_pre(b0,lvl=0.95,arc=1.0,i=0):
    P.K(b0,0,0.9*lvl,arc,tune=43); P.K(b0+2.5,10,0.7*lvl,arc,tune=43)
    P.S(b0+2,8,0.9*lvl,'center',arc,tune=198); gate_tail(b0,2.0,0.028*lvl)
    P.S(b0+4,0,0.7*lvl,'center',arc,tune=198)
    p=0.0
    while p<4.0-1e-6:
        P.H(b0+p,int(p*4)%16,(0.26 if p%0.5 else 0.42)*lvl,o=0.0,art='tip',arc=arc); p+=0.25
    if i%2==1: P.TB(b0+3,12,0.32*lvl,arc)
def dr_chorus(b0,lvl=1.05,arc=1.0,brk=False):
    P.K(b0,0,1.0*lvl,arc,tune=44); P.K(b0+1.5,6,0.62*lvl,arc)
    P.K(b0+2.5,10,0.72*lvl,arc)
    P.S(b0+2,8,1.0*lvl,'center',arc,tune=200); gate_tail(b0,2.0,0.038*lvl,0.30)
    P.S(b0+4,0,1.0*lvl,'center',arc,tune=200); gate_tail(b0,4.0,0.038*lvl,0.30)
    P.CL(b0+2-0.004,8,0.72*lvl,arc); P.CL(b0+4-0.004,0,0.72*lvl,arc)
    p=0.0
    while p<4.0-1e-6:
        op=(abs(p-3.5)<1e-6)
        P.H(b0+p,int(p*4)%16,(0.42 if p%0.5 else 0.62)*lvl,o=0.45 if op else 0.0,
            art='tip',arc=arc,choke_beat=(b0+4) if op else None)
        p+=0.25
    P.S(b0+1.25,5,0.20*lvl,'ghost',arc); P.S(b0+3.25,13,0.22*lvl,'ghost',arc)
    for p in (1.0,3.0): P.TB(b0+p,int(p*4)%16,0.34*lvl,arc)
    if brk: P.S(b0+3.75,15,0.55*lvl,'ghost',arc)
def dr_mid(b0,lvl=0.78,arc=1.0,i=0):           # solo ken: ride + brush
    P.K(b0,0,0.8*lvl,arc,tune=43); P.K(b0+2.5,10,0.62*lvl,arc,tune=43)
    P.S(b0+2,8,0.75*lvl,'edge',arc); P.S(b0+4,0,0.7*lvl,'edge',arc)
    p=0.0
    while p<4.0-1e-6:
        P.RD(b0+p,int(p*4)%16,0.36*lvl,bell=(p==2.0),arc=arc); p+=0.5
    for p in (0.75,1.75,2.75,3.75): P.SH(b0+p,int(p*4)%16,0.24*lvl,arc)

for i in range(8):
    b=bar_at('INTRO',i); a=[1.0,0.98,1.0,1.02][i%4]
    if i>=3: dr_intro(b,0.5+0.08*i,a)
    if i==7: P.fill(b+3,1.0,'negative',0.8,next_crash_beat=S['V1'])
for i in range(16):
    b=bar_at('V1',i); a=[1.0,0.97,1.0,1.02][i%4]
    dr_half(b,0.92,a,hat16=(i>=8))
    if i in (7,15): P.fill(b+3.25,0.75,'snare',0.7)
for i in range(8):
    b=bar_at('PRE1',i); a=[1.0,0.98,1.0,1.02][i%4]
    dr_pre(b,0.95,a,i)
    if i==7: P.fill(b+2,2.0,'roll',1.2,next_crash_beat=S['CH1'])
for i in range(8):
    b=bar_at('CH1',i); a=[1.0,0.97,1.01,1.03][i%4]
    dr_chorus(b,1.02,a,brk=(i%4==3))
    if i==7: P.fill(b+3,1.0,'snare',0.85,next_crash_beat=S['V2'])
for i in range(12):
    b=bar_at('V2',i); a=[1.0,0.97,1.0,1.02][i%4]
    dr_half(b,0.96,a,hat16=True)
    if i>=6: P.TB(b+3,12,0.30,a)
    if i in (5,11): P.fill(b+3.25,0.75,'snare',0.8)
for i in range(8):
    b=bar_at('PRE2',i); a=[1.0,0.98,1.0,1.02][i%4]
    dr_pre(b,1.0,a,i)
    if i==7: P.fill(b+2,2.0,'roll',1.3,next_crash_beat=S['CH2'])
for i in range(8):
    b=bar_at('CH2',i); a=[1.0,0.97,1.01,1.03][i%4]
    dr_chorus(b,1.06,a,brk=(i%4==3))
for i in range(8):
    b=bar_at('MID',i); a=[1.0,0.99,1.0,1.01][i%4]
    dr_mid(b,0.72,a,i)
    if i==7: P.fill(b+2,2.0,'roll',1.4,next_crash_beat=S['CH3'])
for i in range(12):
    b=bar_at('CH3',i); a=[1.0,0.98,1.01,1.04][i%4]
    dr_chorus(b,1.10,a,brk=(i%4==3))
    if i%4==0: P.CR(b,0,0.62,size=1.1)
    if i==11: P.fill(b+2,2.0,'tom',1.1,next_crash_beat=S['OUT'])
for i in range(8):
    b=bar_at('OUT',i); a=[1.0,0.98,1.0,1.02][i%4]
    dr_half(b,0.95-0.08*i,a,hat16=(i<4))
for i in range(4):
    b=bar_at('TAIL',i)
    if i==0: P.K(b,0,0.55,tune=43); P.S(b+2,8,0.35,'rim')
P.apply_chokes()
DRUMS=mix_kit(P.bus,room_amount=0.34,oh_amount=0.95,lofi=0.14,lpf=8600)

# ================= NHAC CU =================
crackle(fx,0,T(END),0.038,dens=0.07)
for i in range(8):                                   # INTRO
    b=bar_at('INTRO',i); ch=VCH[i%4]; root,ts=CH[ch]
    if i%2==0: piano_hit(b,ts,0.048+0.005*i,seed=i)
    if i>=2: wurli_bed(b,ts,0.030,seed=i)
    if i>=4: bass_pluck(b,root,0.18+0.02*i,seed=i)
    if i>=6: bell(hp_,T(b+2.5),ts[2]+12,1.4,0.030,idx=2.2,seed=i)
for i in range(16):                                  # V1
    b=bar_at('V1',i); ch=VCH[i%4]; root,ts=CH[ch]
    bass_pluck(b,root,0.25,seed=i,var=i%4//2)
    wurli_bed(b,ts,0.036,seed=i)
    if i%4==0: piano_hit(b,ts,0.050,seed=i)
    if i%4==2: piano_hit(b,ts,0.038,off=0.5,seed=i)
    if i>=8: gatedpad(pad,T(b),[t+12 for t in ts[:3]],T(b+4)-T(b),0.016,
                      rate=8.0,duty=0.5,bpm=115,cut=2400,seed=i)
    if i%8==7: bell(hp_,T(b+3),ts[3]+12,1.6,0.028,idx=2.0,seed=i)
for i in range(8):                                   # PRE1
    b=bar_at('PRE1',i); ch=PCH[i%4]; root,ts=CH[ch]
    bass_pluck(b,root,0.26,seed=i,var=1)
    piano_hit(b,ts,0.052,seed=i)
    mellotron(kbd,T(b),ts[1]+12,T(b+4)-T(b),0.030+0.004*i,kind='strings',seed=i)
    supersaw(pad,T(b),[root+12]+[t+12 for t in ts[:3]],T(b+4)-T(b),0.020+0.004*i,
             det=7,lp=2600+150*i,atk=0.5,seed=i)
for i in range(8):                                   # CH1
    b=bar_at('CH1',i); ch=CCH[i%4]; root,ts=CH[ch]
    bass_pluck(b,root,0.27,seed=i)
    piano_hit(b,ts,0.058,seed=i)
    wurli_bed(b,ts,0.038,seed=i)
    supersaw(pad,T(b),[root+12]+[t+12 for t in ts],T(b+4)-T(b),0.032,det=8,lp=3600,
             atk=0.10,seed=i)
    mellotron(kbd,T(b),ts[2]+12,T(b+4)-T(b),0.032,kind='strings',seed=i)
    bell(hp_,T(b+1.5),ts[3]+12,1.2,0.030,idx=2.4,seed=i)
    if i%4==3: bone(brs,T(b+2),ts[0]+12,1.4,0.040,growl=0.3)
for i in range(12):                                  # V2
    b=bar_at('V2',i); ch=VCH[i%4]; root,ts=CH[ch]
    bass_pluck(b,root,0.255,seed=i+20,var=(i+1)%2)
    wurli_bed(b,ts,0.038,seed=i)
    gatedpad(pad,T(b),[t+12 for t in ts[:3]],T(b+4)-T(b),0.018,rate=8.0,duty=0.5,
             bpm=115,cut=2600,seed=i)
    if i%4==0: piano_hit(b,ts,0.052,seed=i)
    if i%4==2: bell(hp_,T(b+1),ts[2]+12,1.5,0.030,idx=2.2,seed=i)
    if i>=8: bone(brs,T(b+2.5),ts[1]+12,1.0,0.032,growl=0.25)
for i in range(8):                                   # PRE2
    b=bar_at('PRE2',i); ch=PCH[i%4]; root,ts=CH[ch]
    bass_pluck(b,root,0.27,seed=i,var=1)
    piano_hit(b,ts,0.055,seed=i)
    mellotron(kbd,T(b),ts[1]+12,T(b+4)-T(b),0.034+0.004*i,kind='strings',seed=i)
    supersaw(pad,T(b),[root+12]+[t+12 for t in ts[:3]],T(b+4)-T(b),0.022+0.005*i,
             det=7,lp=2800+160*i,atk=0.45,seed=i)
    if i==7: riser(fx,T(b),T(b+4)-T(b),0.045,300,6500,seed=6)
for i in range(8):                                   # CH2
    b=bar_at('CH2',i); ch=CCH[i%4]; root,ts=CH[ch]
    bass_pluck(b,root,0.28,seed=i)
    piano_hit(b,ts,0.062,seed=i)
    wurli_bed(b,ts,0.040,seed=i)
    supersaw(pad,T(b),[root+12]+[t+12 for t in ts],T(b+4)-T(b),0.036,det=9,lp=4000,
             atk=0.09,seed=i)
    mellotron(kbd,T(b),ts[2]+12,T(b+4)-T(b),0.034,kind='strings',seed=i)
    bell(hp_,T(b+1.5),ts[3]+12,1.2,0.032,idx=2.4,seed=i)
    if i%2==1: bone(brs,T(b),ts[0]+12,1.8,0.042,growl=0.35)
for i in range(8):                                   # MID: solo ken (bone) trai dai
    b=bar_at('MID',i); ch=MCH[i%4]; root,ts=CH[ch]
    bass_pluck(b,root,0.24,seed=i,var=1)
    wurli_bed(b,ts,0.036,seed=i)
    piano_hit(b,ts,0.042,off=0.5,seed=i)
    _R=np.random.default_rng(500+i)
    _sc=[48,50,52,53,55,57,59,60,62,64,65,67]
    _ix=6
    for off in (0.0,1.0,1.75,2.5,3.25):
        _ix=int(np.clip(_ix+_R.integers(-2,3),0,len(_sc)-1))
        if _R.random()<0.20: continue
        bone(brs,T(b+off)+float(_R.normal(0,0.014)),_sc[_ix]+12,
             float(_R.uniform(0.4,1.1)),0.036,growl=0.35,seed=i*9+int(off*4))
    if i>=4: supersaw(pad,T(b),[root+12]+[t+12 for t in ts],T(b+4)-T(b),
                      0.018+0.005*(i-4),det=8,lp=2600+200*i,atk=0.6,seed=i)
    if i==7: riser(fx,T(b),T(b+4)-T(b),0.062,280,8000,seed=9)
for i in range(12):                                  # CH3: LEN Eb
    b=bar_at('CH3',i); ch=KCH[i%4]; root,ts=CH[ch]
    bass_pluck(b,root,0.29,seed=i)
    piano_hit(b,ts,0.072,seed=i)
    wurli_bed(b,ts,0.046,seed=i)
    supersaw(pad,T(b),[root+12]+[t+12 for t in ts],T(b+4)-T(b),0.050,det=10,lp=4400,
             atk=0.07,seed=i)
    mellotron(kbd,T(b),ts[2]+12,T(b+4)-T(b),0.038,kind='strings',seed=i)
    bell(hp_,T(b+1.5),ts[3]+12,1.2,0.034,idx=2.6,seed=i)
    if i%2==0: bone(brs,T(b),ts[0]+12,1.8,0.046,growl=0.4)
for i in range(8):                                   # OUT
    b=bar_at('OUT',i); ch=OCH[i%4]; root,ts=CH[ch]
    f=1-i/10.0
    bass_pluck(b,root,0.24*f,seed=i,var=1)
    wurli_bed(b,ts,0.036*f,seed=i)
    if i%2==0: piano_hit(b,ts,0.048*f,seed=i)
    gatedpad(pad,T(b),[t+12 for t in ts[:3]],T(b+4)-T(b),0.018*f,rate=8.0,duty=0.5,
             bpm=115,cut=2200,seed=i)
for i in range(4):                                   # TAIL: Ebmaj7 tan
    b=bar_at('TAIL',i); root,ts=CH['Ebmaj7']
    if i==0:
        piano_hit(b,ts,0.050,seed=1,spread=0.030)
        mellotron(kbd,T(b),ts[2]+12,T(b+8)-T(b),0.034,kind='choir',seed=1)
        subbass(bs,T(b),root-12,T(b+6)-T(b),0.16)
    if i==2: bell(hp_,T(b),ts[3]+12,2.4,0.030,idx=2.0,seed=2)
_o=int(T(S['OUT'])*SR); _e=int(T(END)*SR)
for _bufl in (kbd,hp_): _bufl[_o:_e]=bitcrush(_bufl[_o:_e],bits=8,srdiv=2,mix=0.45)

# ================= GIONG HAT =================
V1=[
 # "you keep a cassette angel in the glovebox of the car"
 [(0,.5,'E4','u','y'),(0.5,.5,'E4','i','k'),(1,.5,'G4','a',''),(1.5,.5,'E4','a','k'),
  (2,.5,'E4','e','s'),(2.5,.5,'D4','a','t'),(3,.75,'E4','e','n'),
  (5,.5,'C4','i',''),(5.5,.5,'E4','a','g'),(6,.5,'D4','o','v'),(6.5,1.0,'C4','a','k')],
 # "you never play it you just like to know it's there"
 [(0,.5,'E4','u','y'),(0.5,.5,'G4','e','n'),(1,.5,'G4','e','v'),(1.5,.5,'E4','e','p'),
  (2,.75,'A4','i','t'),
  (4.5,.5,'E4','u','y'),(5,.5,'E4','u','j'),(5.5,.5,'G4','a','l'),(6,.5,'E4','o','n'),
  (6.5,1.0,'C4','e','th')],
 # "everything is soft and slightly out of tune tonight"
 [(0,.5,'E4','e',''),(0.5,.5,'E4','i','v'),(1,.5,'G4','i','th'),(1.5,.5,'G4','i','z'),
  (2,.5,'A4','o','s'),(2.5,.5,'G4','a','n'),(3,.75,'E4','a','s'),
  (5,.5,'D4','a','t'),(5.5,.5,'E4','u','n'),(6,.5,'C4','u','t'),(6.5,1.25,'A3','a','n')],
 # "i am not embarrassed i am only in the dark"
 [(0,.5,'E4','i',''),(0.5,.5,'E4','a',''),(1,.5,'G4','o','n'),(1.5,.5,'A4','e','b'),
  (2,.5,'G4','a','r'),(2.5,.75,'E4','e','s'),
  (4.5,.5,'E4','i',''),(5,.5,'G4','a',''),(5.5,.5,'A4','o','n'),(6,.5,'G4','i','l'),
  (6.5,1.0,'E4','a','d')],
]
V2L=[
 # "the tape is stretched the voices sound a little further down"
 [(0,.5,'E4','a','th'),(0.5,.5,'E4','e','t'),(1,.5,'G4','i','z'),(1.5,.5,'E4','e','s'),
  (2,.5,'E4','a','th'),(2.5,.5,'D4','o','v'),(3,.75,'E4','a','s'),
  (5,.5,'C4','a',''),(5.5,.5,'E4','i','l'),(6,.5,'D4','u','f'),(6.5,1.0,'C4','o','d')],
 # "i memorized the hiss before i memorized the song"
 [(0,.5,'E4','i',''),(0.5,.5,'G4','e','m'),(1,.5,'G4','o','r'),(1.5,.5,'E4','a','z'),
  (2,.75,'A4','i','h'),
  (4.5,.5,'E4','i','b'),(5,.5,'E4','o','f'),(5.5,.5,'G4','i',''),(6,.5,'E4','e','m'),
  (6.5,1.0,'C4','o','s')],
 # "and if you rewind me i will still be twenty one"
 [(0,.5,'E4','a',''),(0.5,.5,'E4','i','f'),(1,.5,'G4','u','y'),(1.5,.5,'G4','i','r'),
  (2,.5,'A4','a','w'),(2.5,.5,'G4','i','m'),(3,.75,'E4','a','w'),
  (5,.5,'D4','i','s'),(5.5,.5,'E4','e','b'),(6,.5,'C4','e','t'),(6.5,1.25,'A3','u','w')],
]
# CHORUS: dau tren bac 9 (not D tren Cmaj7) - ngot ma buon
CH_A=[(0.75,.5,'A4','o','h'),(1.25,.5,'G4','i','l'),(1.75,.75,'A4','i','m'),
      (3,.75,'E4','e','n'),
      (4.75,.5,'A4','o','h'),(5.25,.5,'C5','i','l'),(5.75,.5,'B4','a','m'),
      (6.5,1.5,'G4','o','n')]
CH_B=[(0.75,.5,'A4','o','h'),(1.25,.5,'G4','i','l'),(1.75,.75,'A4','i','m'),
      (3,.75,'E4','e','n'),
      (4.75,.5,'C5','o','h'),(5.25,.5,'D5','i','l'),(5.75,.5,'C5','a','m'),
      (6.5,1.75,'A4','o','n')]
PREL=[(0,.5,'F4','a','n'),(0.5,.5,'G4','o','b'),(1,.5,'A4','i','d'),(1.5,.5,'G4','e','t'),
      (2,1.0,'B4','e','l'),
      (4,.5,'A4','o','s'),(4.5,.5,'B4','e','m'),(5,.5,'C5','a','b'),(5.5,.5,'B4','i','l'),
      (6,1.5,'A4','o','n')]
MIDW=[(0,1.0,'E4','a','h'),(1.5,1.0,'D4','o',''),(3,1.0,'C4','i','m'),
      (5,1.0,'D4','e',''),(6.5,1.0,'E4','a','')]
# CH3 len Eb: cung hinh giai dieu, dich +3
def up3(cells): return [(c[0],c[1],nn(c[2])+3,c[3],c[4] if len(c)>4 else '') for c in cells]
CH_A3=up3(CH_A); CH_B3=up3(CH_B)
OUTL=[(0,1.0,'Bb3','o','h'),(2,1.0,'C4','i','l'),(4,1.0,'Eb4','a','m'),(6,1.5,'Bb3','o','n')]

for i in range(0,16,2):
    line(vx,bar_at('V1',i),V1[(i//2)%4],g=0.20,style='croon',breath=0.34,
         seedbase=i*13,jit=0.010,drag=0.030)
for i in range(0,8,2):
    line(vx,bar_at('PRE1',i),PREL,g=0.19,style='croon',breath=0.28,seedbase=200+i*13,
         jit=0.010,drag=0.022)
for i in range(0,8,2):
    b=bar_at('CH1',i); ph=[CH_A,CH_B][(i//2)%2]
    line(vx,b,ph,g=0.215,style='croon',oct8=0.28,breath=0.26,seedbase=300+i*13,
         jit=0.011,drag=0.026)
for i in range(0,12,2):
    line(vx,bar_at('V2',i),V2L[(i//2)%3],g=0.205,style='croon',breath=0.36,
         seedbase=500+i*13,jit=0.011,drag=0.032)
for i in range(0,8,2):
    line(vx,bar_at('PRE2',i),PREL,g=0.20,style='croon',breath=0.26,seedbase=600+i*13,
         jit=0.010,drag=0.020)
for i in range(0,8,2):
    b=bar_at('CH2',i); ph=[CH_A,CH_B][(i//2)%2]
    line(vx,b,ph,g=0.22,style='croon',oct8=0.34,breath=0.24,seedbase=700+i*13,
         jit=0.011,drag=0.024)
    chant(vx,b,ph,g=0.085,n=4,spread=14,style='croon',seedbase=740+i)
for i in range(0,8,2):
    line(vx,bar_at('MID',i),MIDW,g=0.075,style='whisper',breath=0.40,
         seedbase=900+i*13,jit=0.014)
for i in range(0,12,2):
    b=bar_at('CH3',i); ph=[CH_A3,CH_B3][(i//2)%2]
    line(vx,b,ph,g=0.225,style='croon',oct8=0.40,breath=0.22,seedbase=1100+i*13,
         jit=0.012,drag=0.020)
    chant(vx,b,ph,g=0.105,n=5,spread=16,style='croon',seedbase=1150+i)
for i in range(0,8,2):
    line(vx,bar_at('OUT',i),OUTL,g=0.10*(1-i/12),style='whisper',breath=0.42,
         seedbase=1300+i*13,jit=0.012)

# ================= MIX =================
STEMS=[(pad,0.0,0.58,0.55,0.0),(kbd,0.26,0.66,0.50,0.0),(brs,-0.36,0.60,0.50,0.0),
       (hp_,-0.42,0.58,0.45,9.0),(fx,0.0,1.0,0.0,0.0)]
MAPT=[(n,a,b_,(5.2 if n in('OUT','TAIL') else 6.0 if n=='MID' else
               3.4 if n.startswith('CH') else 1.2 if n=='INTRO' else 2.8))
      for n,a,b_ in MAP]
def _build(voc):
    st=mixdown(NAME,vx,STEMS,DRUMS,bs,MAPT,vocals=voc,
        wet=0.30,decay=1.9,wide=1.45,drum_gain=0.72,bass_gain=0.95,crush_amt=0.18,
        rms_target=0.174,vox_gain=1.02,boost_inst=1.12,duck=0.30,tape=0.006)
