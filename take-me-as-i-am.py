...
# =======================================================================
# SONG — TAKE ME AS I AM (E minor, 88 bpm, 62 bars = 2:49)
#
# Westlife final-chorus trick: the hook is re-sung and the top voices
# ad-lib over it (their five-part sound: the last chorus is where Brian/
# Mark-style falsetto runs float above the lead). No key change here —
# the lift is the ad-lib layer plus fuller stacks.
#
# Harmony — Weezer "Say It Ain't So" (1994): the whole verse AND chorus
# sit on one loop, C#m - G# - A - E (i - V - VI - III), and the bridge
# is a thick descending line B - B/A# - E - G. Transposed to E minor:
#   verse/chorus: Em - B - C - G            (1 bar each, the 4-bar loop)
#   bridge:       D - D/C# - G - B          (the descending bridge)
#   coda:         Em(2) B C G Em(2), fade
# The verse energy is hushed (Weezer's clean backstroke), the chorus is
# the start-stop eruption — the Villa ghost-note stutters carry that.
# =======================================================================
BPM = 88.0
BEAT = 60.0 / BPM
BAR = 4.0
...
DUR = T(END_BEATS)
TOTAL = DUR + 3.5
print('TAKE ME AS I AM  bars', int(END_BEATS / BAR), ' duration', round(DUR, 1), 's')

CH = {
 'Em':  (40, (52,55,59,64), 40),   'B':   (47, (59,63,66,71), 47),
 'C':   (36, (48,52,55,60), 36),   'G':   (43, (55,59,62,67), 43),
 'D':   (38, (50,54,57,62), 38),   'D/C#':(38, (50,54,57,62), 37),
}
V1BAR  = ['Em','B','C','G'] * 2
PREBAR = ['Em','B','C','G']
CHBAR  = ['Em','B','C','G'] * 2
...
BASSC = {
 'Em': [(0,40,3.4,.30)],  'B': [(0,47,3.4,.30)],
 'C':  [(0,36,3.4,.28)],  'G': [(0,43,3.4,.28)],
}

bus = Bus(); drm = Bus(); vx = Bus()

V1 = [
 # "i wrote your name on the ceiling"
 [(0,.5,'E4','ai','',''),(.5,.5,'G4','o','r','t'),(1.0,.5,'A4','o','y','r'),
  (1.5,.55,'B4','ay','n','m'),(2.05,.5,'B4','o','','n'),(2.55,.5,'A4','e','th',''),
  (3.05,.5,'G4','i','s',''),(3.55,.45,'A4','i','l','ng')],
 # "in the middle of the dark"
 [(0,.5,'A4','i','','n'),(.5,.45,'B4','e','th',''),(.95,.55,'B4','i','m','d'),
  (1.5,.5,'A4','u','d','l'),(2.0,.5,'G4','o','','v'),(2.5,.45,'F#4','e','th',''),
  (2.95,.8,'E4','a','d','r')],
 # "and i practiced how to say it"
 [(0,.5,'G4','a','','n'),(.5,.5,'A4','ai','',''),(1.0,.55,'B4','a','p','r'),
  (1.55,.5,'B4','i','t','st'),(2.05,.5,'A4','ow','h',''),(2.55,.5,'G4','u','t',''),
  (3.05,.5,'F#4','ay','s',''),(3.55,.45,'E4','i','','t')],
 # "till it felt like falling apart"
 [(0,.5,'E4','i','t','l'),(.5,.45,'G4','i','','t'),(.95,.5,'A4','e','f','l'),
  (1.45,.55,'B4','ai','l',''),(2.0,.5,'B4','o','f','l'),(2.5,.5,'A4','i','','ng'),
  (3.0,.45,'G4','a','',''),(3.45,.5,'A4','a','p','rtz')],
 # "now the moon is my witness"
 [(0,.5,'A4','ow','n',''),(.5,.45,'B4','e','th',''),(.95,.5,'B4','u','m','n'),
  (1.45,.5,'A4','i','','z'),(1.95,.5,'G4','ai','m',''),(2.45,.5,'F#4','i','w',''),
  (2.95,.8,'E4','e','n','')],
 # "and the streetlights hold their breath"
 [(0,.5,'G4','a','','n'),(.5,.45,'A4','e','th',''),(.95,.55,'B4','i','st','t'),
  (1.5,.5,'B4','ai','l','ts'),(2.0,.5,'A4','o','h','l'),(2.5,.45,'G4','e','th',''),
  (2.95,.8,'F#4','e','br','th')],
 # "i'm a boy who's learned the hard way"
 [(0,.5,'G4','ai','','m'),(.5,.45,'A4','a','',''),(.95,.5,'B4','o','b','y'),
  (1.45,.5,'A4','u','h','z'),(1.95,.5,'G4','u','l','r'),(2.45,.5,'F#4','e','th',''),
  (2.95,.5,'E4','a','h','r'),(3.45,.5,'F#4','ay','w','')],
 # "some words are so hard to confess"
 [(0,.5,'E4','u','s','m'),(.5,.45,'G4','u','w','r'),(.95,.5,'A4','a','','r'),
  (1.45,.55,'B4','a','h','r'),(2.0,.5,'A4','u','t',''),(2.5,.5,'G4','o','k','n'),
  (3.0,.8,'F#4','e','f','s')],
]
PRE = [
 # "so if i say it out loud"
 [(0,.5,'B4','o','s',''),(.5,.45,'B4','i','f',''),(.95,.5,'A4','ai','',''),
  (1.45,.55,'B4','ay','s',''),(2.0,.45,'A4','i','','t'),(2.45,.5,'G4','ow','','t'),
  (2.95,.8,'F#4','ow','l','d')],
 # "will the walls come falling down"
 [(0,.5,'A4','i','w','l'),(.5,.45,'B4','e','th',''),(.95,.55,'C5','o','w','l'),
  (1.5,.5,'B4','u','k','m'),(2.0,.5,'A4','o','f','l'),(2.5,.5,'G4','i','','ng'),
  (3.0,.7,'A4','ow','d','n')],
 # "i've been quiet for so long"
 [(0,.5,'A4','ai','','v'),(.5,.5,'B4','i','b','n'),(1.0,.55,'C5','ai','k','w'),
  (1.55,.5,'B4','e','','t'),(2.05,.5,'A4','o','f','r'),(2.55,.5,'G4','o','s',''),
  (3.05,.7,'B4','o','l','ng')],
 # "that i've forgotten how"
 [(0,.55,'B4','a','th','t'),(.55,.5,'D5','ai','','v'),(1.05,.55,'E5','o','f','r'),
  (1.6,.55,'E5','o','g','t'),(2.15,.5,'D5','e','t','n'),(2.65,1.3,'D5','ow','h','')],
]
HOOK = [
 # "so take me as i am"
 [(0,.9,'G4','o','s',''),(.9,.6,'B4','ay','t','k'),(1.5,.5,'A4','i','m',''),
  (2.0,.4,'B4','a','','z'),(2.4,.45,'C5','ai','',''),(2.85,.8,'D5','a','','m')],
 # "with all my quiet flaws"
 [(0,.55,'C5','i','w','th'),(.55,.5,'D5','o','','l'),(1.05,.55,'E5','ai','m',''),
  (1.6,.5,'D5','ai','k','w'),(2.1,.45,'C5','e','','t'),(2.55,.8,'B4','o','f','l')],
 # "take me as i am" (higher landing)
 [(0,.8,'G4','ay','t','k'),(.8,.5,'B4','i','m',''),(1.3,.45,'B4','a','','z'),
  (1.75,.45,'C5','ai','',''),(2.2,1.4,'E5','a','','m')],
 # "i'm not what i was"
 [(0,.55,'D5','ai','','m'),(.55,.5,'C5','o','n','t'),(1.05,.5,'B4','u','w','t'),
  (1.55,.45,'A4','ai','',''),(2.0,.9,'G4','o','w','z')],
 # "i'm learning how to say it"
 [(0,.5,'G4','ai','','m'),(.5,.5,'A4','u','l','r'),(1.0,.45,'B4','i','','ng'),
  (1.45,.5,'C5','ow','h',''),(1.95,.45,'B4','u','t',''),(2.4,.5,'A4','ay','s',''),
  (2.9,.6,'B4','i','','t')],
 # "with my heart on my sleeve"
 [(0,.5,'B4','i','w','th'),(.5,.45,'C5','ai','m',''),(.95,.55,'D5','a','h','r'),
  (1.5,.5,'E5','o','','n'),(2.0,.5,'D5','ai','m',''),(2.5,.9,'C5','i','sl','v')],
 # "so take me, take me"
 [(0,.55,'B4','o','s',''),(.55,.6,'D5','ay','t','k'),(1.15,.5,'C5','i','m',''),
  (1.65,.6,'D5','ay','t','k'),(2.25,1.5,'E5','i','m','')],
 # "as i am"
 [(0,.55,'D5','a','','z'),(.55,.5,'C5','ai','',''),(1.05,1.8,'B4','a','','m')],
]
V2 = [
 # "you look at me like i'm something"
 [(0,.5,'E4','u','y',''),(.5,.5,'G4','u','l','k'),(1.0,.45,'A4','a','','t'),
  (1.45,.5,'B4','i','m',''),(1.95,.5,'B4','ai','l',''),(2.45,.45,'A4','ai','','m'),
  (2.9,.5,'G4','u','s','m'),(3.4,.55,'A4','i','th','ng')],
 # "that the world forgot to break"
 [(0,.5,'A4','a','th','t'),(.5,.45,'B4','e','th',''),(.95,.55,'B4','u','w','r'),
  (1.5,.5,'A4','o','f','r'),(2.0,.5,'G4','o','g','t'),(2.5,.45,'F#4','u','t',''),
  (2.95,.8,'E4','ay','br','k')],
 # "and you stay through the storm"
 [(0,.5,'G4','a','','n'),(.5,.45,'A4','u','y',''),(.95,.55,'B4','ay','st',''),
  (1.5,.5,'A4','u','th','r'),(2.0,.5,'G4','e','th',''),(2.5,.9,'F#4','o','st','rm')],
 # "when the others leave a trace"
 [(0,.5,'E4','e','w','n'),(.5,.45,'G4','e','th',''),(.95,.5,'A4','o','','th'),
  (1.45,.5,'A4','e','','r'),(1.95,.5,'B4','i','l','v'),(2.45,.45,'A4','a','',''),
  (2.9,.8,'G4','ay','tr','s')],
 # "so i'm walking to the water"
 [(0,.5,'G4','o','s',''),(.5,.5,'A4','ai','','m'),(1.0,.55,'B4','o','w','k'),
  (1.55,.5,'B4','i','','ng'),(2.05,.5,'A4','u','t',''),(2.55,.5,'G4','e','th',''),
  (3.05,.5,'F#4','o','w',''),(3.55,.45,'E4','e','t','r')],
 # "with my hands held out to you"
 [(0,.5,'E4','i','w','th'),(.5,.45,'G4','ai','m',''),(.95,.55,'A4','a','h','nd'),
  (1.5,.5,'B4','e','h','l'),(2.0,.5,'A4','ow','','t'),(2.5,.45,'G4','u','t',''),
  (2.95,.8,'F#4','u','y','')],
 # "i've been down where the floor gives way"
 [(0,.5,'A4','ai','','v'),(.5,.5,'B4','i','b','n'),(1.0,.5,'B4','ow','d','n'),
  (1.5,.5,'A4','e','w','r'),(2.0,.5,'G4','e','th',''),(2.5,.5,'F#4','o','f','l'),
  (3.0,.5,'E4','i','g','v'),(3.5,.45,'F#4','ay','w','')],
 # "i've been under, i've been through"
 [(0,.5,'F#4','ai','','v'),(.5,.45,'G4','i','b','n'),(.95,.5,'A4','u','','n'),
  (1.45,.5,'G4','e','d','r'),(1.95,.5,'A4','ai','','v'),(2.45,.45,'G4','i','b','n'),
  (2.9,.8,'E4','u','th','r')],
]
BR = [
 # "and the truth is i've been waiting"
 [(0,.5,'G4','a','','n'),(.5,.45,'A4','e','th',''),(.95,.55,'B4','u','tr','th'),
  (1.5,.5,'A4','i','','z'),(2.0,.5,'G4','ai','','v'),(2.5,.5,'F#4','i','b','n'),
  (3.0,.5,'E4','ay','w','t'),(3.5,.45,'F#4','i','','ng')],
 # "all my life to say it to you"
 [(0,.5,'G4','o','','l'),(.5,.45,'A4','ai','m',''),(.95,.55,'B4','ai','l','f'),
  (1.5,.5,'A4','u','t',''),(2.0,.5,'G4','ay','s',''),(2.5,.45,'F#4','i','','t'),
  (2.95,.5,'E4','u','t',''),(3.45,.5,'F#4','u','y','')],
 # "every door i never opened"
 [(0,.5,'A4','e','','v'),(.5,.45,'B4','i','r',''),(.95,.5,'B4','o','d','r'),
  (1.45,.5,'A4','ai','',''),(1.95,.5,'G4','e','n','v'),(2.45,.45,'F#4','e','','r'),
  (2.9,.5,'E4','o','',''),(3.4,.55,'F#4','e','p','n')],
 # "i'm opening tonight"
 [(0,.55,'A4','ai','','m'),(.55,.5,'B4','o','',''),(1.05,.55,'C5','e','p','n'),
  (1.6,.5,'B4','i','','ng'),(2.1,.5,'A4','u','t',''),(2.6,.9,'B4','ai','n','t')],
]
# ad-lib falsetto runs over the final chorus (bars 5,6,7) — the Westlife lift
ADLIB = {
 5: [(0,.7,'E5','o','',''),(1.5,.7,'G5','o','','')],
 6: [(0,.7,'F#5','o','',''),(1.5,.7,'E5','o','','')],
 7: [(0,.7,'G5','o','',''),(1.5,1.2,'E5','o','','')],
}
CODA_1 = [(0,.9,'G4','o','s',''),(.9,.6,'B4','ay','t','k'),(1.5,.5,'A4','i','m',''),
          (2.0,.4,'B4','a','','z'),(2.4,.45,'C5','ai','',''),(2.85,1.0,'D5','a','','m')]

# ---------------------------------------------------------- arrangement
# ---- INTRO: voice enters over keys alone (Swear It Again piano feel) ----
for i, ch in enumerate(['Em','B','C','G']):
    b = S['INTRO'] + i * BAR
    padbar(b, ch, gain=0.09, cut=2000, atk=1.5, seed=i)
    keysarp(b, ch, gain=0.15)

# ---- V1: hushed backstroke feel, no drums until the tail ----
for i in range(8):
    b = S['V1'] + i * BAR
    ch = V1BAR[i]
...
# ---- CH1: start-stop eruption, Villa 4-floor ----
for i in range(8):
    b = S['CH1'] + i * BAR
    ch = CHBAR[i]
...
# ---- BRIDGE: the descending line D - D/C# - G - B ----
for i in range(4):
    b = S['BRIDGE'] + i * BAR
    ch = BRBAR[i]
...
# ---- CH3: re-sung hook + ad-lib falsetto runs over it ----
for i in range(8):
    b = S['CH3'] + i * BAR
    ch = CHBAR[i]
    ...
    if i in ADLIB:                                   # the ad-lib layer
        line(vx, b, ADLIB[i], gain=0.42, style='croon', seedbase=2500 + i * 13)
        halo(vx, b, ADLIB[i], gain=0.12, seedbase=2600 + i * 13)

# ---- CODA: final re-sing, fade ----
for i in range(6):
    b = S['CODA'] + i * BAR
    ch = CODABAR[i]
...
for ev in CODA_1:
    ...
    choir(vx, b0, CH['Em'][1], gain=0.08, seedbase=2700)

master(bus.L, bus.R, vx.L, vx.R, 'take-me-as-i-am.wav', DUR)
master(bus.L, bus.R, None, None, 'take-me-as-i-am-instrumental.wav', DUR)
